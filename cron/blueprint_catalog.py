...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``sonic://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``sonic://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"sonic://blueprint/{quote(blueprint.key)}{qs}"
