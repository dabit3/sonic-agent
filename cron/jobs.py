...
def _normalize_profile(profile: Optional[str]) -> Optional[str]:
    ...
    is canonicalized with the same rules as ``sonic -p`` and must refer to an
    ...
    # so profile directories can move with the Sonic root.
...
def create_job(
    ...
    Args:
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        profile: Optional Sonic profile name. When set, the job runs with
                that profile's SONIC_HOME so profile-specific config,
