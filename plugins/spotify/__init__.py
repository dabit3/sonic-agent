...
``_check_spotify_available()`` — when the user has not run ``lightning auth
spotify``, the tools remain registered (so they appear in ``lightning
...
The Spotify auth flow (``lightning auth spotify``), CLI plumbing, and docs
