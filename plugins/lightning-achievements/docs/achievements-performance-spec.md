# Lightning Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: lightning-achievements plugin
...
### `GET /api/plugins/lightning-achievements/achievements`
...
### `POST /api/plugins/lightning-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/lightning-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/lightning-achievements/overview`
...
- Persist plugin data under:
  - `~/.lightning/plugins/lightning-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.lightning/plugins/lightning-achievements/scan_checkpoint.json`
...
- `~/.lightning/plugins/lightning-achievements/`
