"""`lightning meet node ...` subcommand tree.

Wired into the existing ``lightning meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``lightning meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="lightning-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``lightning meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             lightning meet node approve <name> ws://<host>:{args.port} {token}")
        try:
