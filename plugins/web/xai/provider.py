...
``{title, url, description, position}`` rows every other Sonic web
provider produces.
...
prefers Sonic-managed xAI Grok OAuth (via ``sonic auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.sonic/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``sonic tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `sonic auth` to sign in with "
