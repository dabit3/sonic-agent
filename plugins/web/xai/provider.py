...
``{title, url, description, position}`` rows every other Lightning web
provider produces.
...
prefers Lightning-managed xAI Grok OAuth (via ``lightning auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.lightning/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``lightning tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `lightning auth` to sign in with "
