...
![Sonic Achievements dashboard](docs/assets/achievements-dashboard-hd.png)

The screenshots use temporary demo tier data to show the full visual range. The plugin itself reads real local Sonic session history by default.
...
>
> **Share cards (2026-05-04, vendored in sonic-agent v0.4.0):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.

## What it does

Sonic Achievements scans local Sonic sessions and unlocks badges based on real agent behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- Sonic-native skills, memory, cron, and plugin usage
...
- **Secret** — hidden until Sonic detects the first related signal
...
Clone into your Sonic plugins directory:

```bash
git clone https://github.com/PCinkusz/sonic-achievements ~/.sonic/plugins/sonic-achievements
```

For local development, keep the repo elsewhere and symlink it:

```bash
git clone https://github.com/PCinkusz/sonic-achievements ~/sonic-achievements
ln -s ~/sonic-achievements ~/.sonic/plugins/sonic-achievements
```
...
If backend API routes 404, restart `sonic dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.sonic/plugins/sonic-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `sonic dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Sonic session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.
...
```text
/api/plugins/sonic-achievements/
```
