...
For a user-installed plugin at `~/.sonic/plugins/sonic-achievements`, a plugin
rescan is enough because Python backend routes are not auto-imported. If you
update the bundled plugin by pulling changes in the sonic-agent repository, and
that bundled plugin update changes backend routes or `plugin_api.py`, restart
`sonic dashboard` after pulling.
