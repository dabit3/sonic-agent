# Sonic Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: sonic-achievements plugin
...
### `GET /api/plugins/sonic-achievements/achievements`
...
### `POST /api/plugins/sonic-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/sonic-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/sonic-achievements/overview`
...
- Persist plugin data under:
  - `~/.sonic/plugins/sonic-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.sonic/plugins/sonic-achievements/scan_checkpoint.json`
...
- `~/.sonic/plugins/sonic-achievements/`
