...
This plugin connects Sonic Agent to iMessage (and other Spectrum
...
TypeScript-only, Sonic runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Sonic-Sidecar-Token`.
...
2. **Find or create** the `Sonic Agent` project on the Photon dashboard.
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.sonic/.env` so the
...
There is no separate `login` command; like every other Sonic channel,
...
Run `sonic photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.sonic/.env` (the same place every other
...
Management metadata lives in `~/.sonic/auth.json` under `credential_pool`:
...
6. Verify end-to-end: `sonic photon status`, a DM and a group roundtrip,
