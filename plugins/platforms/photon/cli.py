"""
``sonic photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Sonic
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `sonic photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Sonic Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``sonic photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Sonic Agent" project.
    ...
    #    and persist both (runtime creds -> ~/.sonic/.env, ids -> auth.json).
    try:
    ...
    print("  Start the gateway:  sonic gateway start --platform photon")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `sonic photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`sonic photon telemetry on|off`)")
    return 0
...
#
# `sonic gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``sonic photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `sonic gateway setup` wizard."""
