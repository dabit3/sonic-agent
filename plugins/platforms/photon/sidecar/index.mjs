// Sonic Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Sonic): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Sonic -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Sonic-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `sonic photon telemetry`)
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-sonic-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Sonic supplied them so a known-good
