...
Small Node helper that bridges Sonic Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Sonic is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Sonic-Sidecar-Token`)
...
The Sonic plugin's `sonic photon setup` command runs `npm install`
here automatically.
