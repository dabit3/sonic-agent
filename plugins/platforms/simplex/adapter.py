...
Optional environment variables:
    ...
    SONIC_SIMPLEX_TEXT_BATCH_DELAY
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SONIC_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    ...
    auto-accept / home channel. Writes to ``~/.sonic/.env`` via
    ``sonic_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "sonic_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.sonic/.env"
        )
        return
