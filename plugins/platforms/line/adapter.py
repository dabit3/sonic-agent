"""
LINE Messaging API platform adapter for Sonic Agent.
...
to Sonic Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SONIC_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            sonic_home = Path.home().joinpath(".sonic").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``sonic status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``sonic status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``sonic setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.sonic/.env`` via ``sonic_cli.config``.
    """
    print()
    ...
    try:
        from sonic_cli.config import get_env_var, set_env_var
    except ImportError:
        print("sonic_cli.config not available; set LINE_* vars manually in ~/.sonic/.env")
        return
