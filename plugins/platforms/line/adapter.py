"""
LINE Messaging API platform adapter for Lightning Agent.
...
to Lightning Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``LIGHTNING_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            lightning_home = Path.home().joinpath(".lightning").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``lightning status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``lightning status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lightning setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.lightning/.env`` via ``lightning_cli.config``.
    """
    print()
    ...
    try:
        from lightning_cli.config import get_env_var, set_env_var
    except ImportError:
        print("lightning_cli.config not available; set LINE_* vars manually in ~/.lightning/.env")
        return
