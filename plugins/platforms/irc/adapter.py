"""
IRC Platform Adapter for Lightning Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Lightning agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "lightning-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Lightning Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Lightning Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: lightning_, lightning_1, lightning_2...
...
def check_requirements() -> bool:
    ...
    # lightning setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `lightning gateway setup` flow for the IRC platform.

    Lazy-imports ``lightning_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Lightning to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. lightning-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #lightning — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.lightning/.env")
    print_info("Restart the gateway for changes to take effect: lightning gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "lightning-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "lightning-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Lightning Agent (cron)")
