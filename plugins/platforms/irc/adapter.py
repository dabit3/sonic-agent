"""
IRC Platform Adapter for Sonic Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Sonic agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sonic-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Sonic Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Sonic Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: sonic_, sonic_1, sonic_2...
...
def check_requirements() -> bool:
    ...
    # sonic setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `sonic gateway setup` flow for the IRC platform.

    Lazy-imports ``sonic_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Sonic to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #sonic — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.sonic/.env")
    print_info("Restart the gateway for changes to take effect: sonic gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sonic-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "sonic-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Sonic Agent (cron)")
