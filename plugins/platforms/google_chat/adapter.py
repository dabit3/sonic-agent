...
# ``model_tools`` import time, so ``lightning status``, ``lightning chat``, etc.
...
# ``lightning_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Lightning is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a lightning MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${LIGHTNING_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _lightning_home = _get_lightning_home()
        except (ModuleNotFoundError, ImportError):
            _lightning_home = _Path.home() / ".lightning"
        self._thread_count_store = _ThreadCountStore(
            _lightning_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Lightning is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("LIGHTNING_HOME", str(_Path.home() / ".lightning"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${LIGHTNING_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a lightning MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Lightning is thinking…' marker message.
        ...
        "Lightning is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Lightning is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Lightning is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Lightning is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Lightning is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``lightning setup``.

    The setup wizard at ``lightning_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.lightning/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. lightning-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.lightning/.env")
    print_info("Restart the gateway: lightning gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lightning-agent[google_chat]'",
        ...
        # the "Lightning is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Lightning is thinking…' marker message "
