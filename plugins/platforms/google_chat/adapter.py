...
# ``model_tools`` import time, so ``sonic status``, ``sonic chat``, etc.
...
# ``sonic_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Sonic is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a sonic MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SONIC_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _sonic_home = _get_sonic_home()
        except (ModuleNotFoundError, ImportError):
            _sonic_home = _Path.home() / ".sonic"
        self._thread_count_store = _ThreadCountStore(
            _sonic_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Sonic is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SONIC_HOME", str(_Path.home() / ".sonic"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SONIC_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a sonic MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Sonic is thinking…' marker message.
        ...
        "Sonic is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Sonic is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Sonic is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Sonic is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Sonic is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``sonic setup``.

    The setup wizard at ``sonic_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.sonic/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. sonic-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.sonic/.env")
    print_info("Restart the gateway: sonic gateway restart")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'sonic-agent[google_chat]'",
        ...
        # the "Sonic is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Sonic is thinking…' marker message "
