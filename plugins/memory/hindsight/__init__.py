...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: lightning)
...
Or via $LIGHTNING_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Lightning can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $LIGHTNING_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_lightning_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "lightning": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "lightning"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Lightning config."""
    profile = config.get("profile", "lightning")
    return str(profile or "lightning")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Lightning profile name (from agent_identity)
      {workspace} — Lightning workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``lightning-{user}`` with no user becomes ``lightning``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Lightning Agent and the User"
    ...
    def save_config(self, values, lightning_home):
        """Write config to $LIGHTNING_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(lightning_home) / "hindsight"
    ...
    def post_setup(self, lightning_home: str, config: dict) -> None:
        ...
        from lightning_cli.memory_setup import _curses_select
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(lightning_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "lightning")
        ...
        self.save_config(provider_config, lightning_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(lightning_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(lightning_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "lightning"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: lightning-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Lightning Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "lightning"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "lightning"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "lightning", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "lightning")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Lightning Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See lightning-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Lightning created/used that client on the shared async loop,
