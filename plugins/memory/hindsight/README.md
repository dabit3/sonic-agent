...
Sonic spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.sonic/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p sonic ui start
```
...
Config file: `~/.sonic/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `sonic` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `sonic-{profile}` isolates memory per active Sonic profile. Empty placeholders collapse cleanly (e.g. `sonic-{user}` with no user becomes `sonic`). |
...
The LLM API key is stored in `~/.sonic/.env` as `HINDSIGHT_LLM_API_KEY`.
