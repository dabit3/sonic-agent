...
Behavioral settings live in `$SONIC_HOME/mem0.json` (set them via `sonic memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.sonic/.env`.
...
```bash
sonic memory setup mem0 --mode oss \
...
```bash
sonic memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SONIC_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.sonic/mem0_qdrant"}}
  }
}
```
...
```bash
sonic memory setup mem0 --mode platform --api-key sk-...
```
