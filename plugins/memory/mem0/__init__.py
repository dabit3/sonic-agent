...
Configuration
-------------
Secret (lives in $SONIC_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $SONIC_HOME/mem0.json, set via `sonic memory
setup`):
  ...
  agent_id           — Agent identifier (default: sonic)
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $SONIC_HOME/mem0.json) —
