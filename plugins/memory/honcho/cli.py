...
def cmd_setup(args) -> None:
    ...
    if not is_local:
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'sonic honcho setup' to retry, or choose an API key instead.\n")
                return
            sonic_host["apiKey"] = cred.access_token
            sonic_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                sonic_host["peerName"] = cred.consent_peer_name
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Sonic gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
