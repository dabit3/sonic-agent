...
``sonic://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Sonic OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "sonic-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``sonic-desktop`` / ``sonic-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Sonic.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SONIC_HOME override can't reach.
