/**
 * Sonic Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.sonic/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SONIC_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SONIC_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "sonic-kanban-dot-triage",
    todo: "sonic-kanban-dot-todo",
    ready: "sonic-kanban-dot-ready",
    running: "sonic-kanban-dot-running",
    blocked: "sonic-kanban-dot-blocked",
    done: "sonic-kanban-dot-done",
    archived: "sonic-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-sonic-task";
  ...
  const LS_BOARD_KEY = "sonic.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="sonic-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "sonic-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "sonic-kanban-md",
  ...
  // standard `drop` event and our `sonic-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("sonic-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("sonic-kanban-column--drop");
          if (target) target.classList.add("sonic-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("sonic-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("sonic-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("sonic-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SONIC_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "sonic-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "sonic-kanban-attention",
        "sonic-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "sonic-kanban-attention-bar" },
        h("span", { className: "sonic-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "sonic-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "sonic-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "sonic-kanban-attention-row",
                  "sonic-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "sonic-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "sonic-kanban-attention-row-id" }, task.id),
                h("span", { className: "sonic-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "sonic-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "sonic-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "sonic-kanban-diag-action-btn",
      action.suggested ? "sonic-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " sonic-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".sonic-kanban-drawer-comment-row input, .sonic-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "sonic-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("sonic-kanban-diag", sevClass) },
      h("div", { className: "sonic-kanban-diag-header" },
        h("span", { className: "sonic-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "sonic-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "sonic-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "sonic-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "sonic-kanban-diag-data-row" },
                  h("span", { className: "sonic-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "sonic-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "sonic-kanban-diag-data-row" },
                h("span", { className: "sonic-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "sonic-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "sonic-kanban-diag-reassign-row" },
            h("span", { className: "sonic-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "sonic-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "sonic-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "sonic-kanban-diag-msg",
              msg.ok ? "sonic-kanban-diag-msg--ok" : "sonic-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "sonic-kanban-section" },
      h("div", { className: "sonic-kanban-section-head-row" },
        h("span", { className: "sonic-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "sonic-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "sonic-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "sonic-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "sonic-kanban-docs-link",
      title: "Open Sonic Kanban docs in a new tab",
      "aria-label": "Sonic Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `sonic kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "sonic-kanban-msg-ok" : "sonic-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "sonic-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "sonic-kanban-boardswitcher" },
      h("div", { className: "sonic-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "sonic-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "sonic-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "sonic-kanban-dialog-title" },
        ...
        h("div", { className: "sonic-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Sonic profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "sonic-kanban-bulk" },
      h("span", { className: "sonic-kanban-bulk-count" },
      ...
      h("div", { className: "sonic-kanban-bulk-priority",
      ...
      h("div", { className: "sonic-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Sonic profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "sonic-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sonic-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("sonic-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "sonic-kanban-trash",
        dragOver ? "sonic-kanban-trash--drop" : "",
        props.draggingTaskId ? "sonic-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "sonic-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "sonic-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".sonic-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "sonic-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sonic-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("sonic-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "sonic-kanban-column",
        dragOver ? "sonic-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "sonic-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "sonic-kanban-col-check",
        ...
        h("span", { className: cn("sonic-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "sonic-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "sonic-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "sonic-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "sonic-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "sonic-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "sonic-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "sonic-kanban-lane" },
                  h("div", { className: "sonic-kanban-lane-head" },
                    h("span", { className: "sonic-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "sonic-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "sonic-kanban-card--stale-red";
    if (age >= tier.amber) return "sonic-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".sonic-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "sonic-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "sonic-kanban-card",
        props.selected ? "sonic-kanban-card--selected" : "",
        props.failed ? "sonic-kanban-card--failed" : "",
        props.draggingSource ? "sonic-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "sonic-kanban-card-content" },
          h("div", { className: "sonic-kanban-card-row" },
            h("label", {
              className: "sonic-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "sonic-kanban-card-check",
            ...
            h("span", { className: "sonic-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or sonic kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "sonic-kanban-warning-badge",
                    "sonic-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "sonic-kanban-needs-assignee",
          ...
          h("div", { className: "sonic-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "sonic-kanban-card-row sonic-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "sonic-kanban-assignee",
                            title: `Assigned to Sonic profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "sonic-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "sonic-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "sonic-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "sonic-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Sonic profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Sonic profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "sonic-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "sonic-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "sonic-kanban-drawer-head" },
        ...
        data ? h("div", { className: "sonic-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "sonic-kanban-drawer-body" },
      h("div", { className: "sonic-kanban-drawer-title" },
        h("span", { className: cn("sonic-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "sonic-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "sonic-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "sonic-kanban-section" },
        h("div", { className: "sonic-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "sonic-kanban-section" },
        h("div", { className: "sonic-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "sonic-kanban-comment" },
                h("div", { className: "sonic-kanban-comment-head" },
                  h("span", { className: "sonic-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "sonic-kanban-comment-ago" },
      ...
      h("div", { className: "sonic-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "sonic-kanban-event",
              isDiag ? "sonic-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "sonic-kanban-event-header" },
                  h("span", { className: "sonic-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "sonic-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "sonic-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "sonic-kanban-event-header-plain" },
                  h("span", { className: "sonic-kanban-event-kind" }, e.kind),
                  h("span", { className: "sonic-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "sonic-kanban-event-phantom-row" },
                  h("span", { className: "sonic-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "sonic-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "sonic-kanban-event-payload" },
    ...
    return h("div", { className: "sonic-kanban-section" },
      h("div", { className: "sonic-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "sonic-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `sonic-kanban-run--${r.outcome || r.status || "ended"}`
          : "sonic-kanban-run--active";
        return h("div", { key: r.id, className: cn("sonic-kanban-run", outcomeClass) },
          h("div", { className: "sonic-kanban-run-head" },
            h("span", { className: "sonic-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "sonic-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "sonic-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "sonic-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "sonic-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "sonic-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "sonic-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "sonic-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "sonic-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "sonic-kanban-pre sonic-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "sonic-kanban-meta-row" },
      h("span", { className: "sonic-kanban-meta-label" }, props.label),
      h("span", { className: "sonic-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "sonic-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sonic-kanban-meta-row" },
        h("span", { className: "sonic-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "sonic-kanban-meta-value sonic-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sonic-kanban-meta-row" },
        h("span", { className: "sonic-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "sonic-kanban-meta-value sonic-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "sonic-kanban-section" },
      h("div", { className: "sonic-kanban-section-head-row" },
        h("span", { className: "sonic-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "sonic-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "sonic-kanban-section" },
      h("div", { className: "sonic-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "sonic-kanban-deps-row" },
        h("span", { className: "sonic-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "sonic-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "sonic-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "sonic-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "sonic-kanban-dep-chip-x",
      ...
      h("div", { className: "sonic-kanban-deps-row" },
        h("span", { className: "sonic-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "sonic-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "sonic-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "sonic-kanban-section" },
      ...
      h("div", { className: "sonic-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "sonic-kanban-home-sub sonic-kanban-home-sub--on"
              : "sonic-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SONIC_PLUGINS__ && typeof window.__SONIC_PLUGINS__.register === "function") {
    window.__SONIC_PLUGINS__.register("kanban", KanbanPage);
  }
})();
