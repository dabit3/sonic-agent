/**
 * Lightning Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.lightning/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__LIGHTNING_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__LIGHTNING_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "lightning-kanban-dot-triage",
    todo: "lightning-kanban-dot-todo",
    ready: "lightning-kanban-dot-ready",
    running: "lightning-kanban-dot-running",
    blocked: "lightning-kanban-dot-blocked",
    done: "lightning-kanban-dot-done",
    archived: "lightning-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-lightning-task";
  ...
  const LS_BOARD_KEY = "lightning.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="lightning-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "lightning-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "lightning-kanban-md",
  ...
  // standard `drop` event and our `lightning-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("lightning-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("lightning-kanban-column--drop");
          if (target) target.classList.add("lightning-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("lightning-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("lightning-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("lightning-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__LIGHTNING_SESSION_TOKEN__ || "";
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "lightning-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "lightning-kanban-attention",
        "lightning-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "lightning-kanban-attention-bar" },
        h("span", { className: "lightning-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "lightning-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "lightning-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "lightning-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "lightning-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "lightning-kanban-attention-row",
                  "lightning-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "lightning-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "lightning-kanban-attention-row-id" }, task.id),
                h("span", { className: "lightning-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "lightning-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "lightning-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "lightning-kanban-diag-action-btn",
      action.suggested ? "lightning-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " lightning-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".lightning-kanban-drawer-comment-row input, .lightning-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "lightning-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("lightning-kanban-diag", sevClass) },
      h("div", { className: "lightning-kanban-diag-header" },
        h("span", { className: "lightning-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "lightning-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "lightning-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "lightning-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "lightning-kanban-diag-data-row" },
                  h("span", { className: "lightning-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "lightning-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "lightning-kanban-diag-data-row" },
                h("span", { className: "lightning-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "lightning-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "lightning-kanban-diag-reassign-row" },
            h("span", { className: "lightning-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "lightning-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "lightning-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "lightning-kanban-diag-msg",
              msg.ok ? "lightning-kanban-diag-msg--ok" : "lightning-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "lightning-kanban-section" },
      h("div", { className: "lightning-kanban-section-head-row" },
        h("span", { className: "lightning-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "lightning-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "lightning-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "lightning-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "lightning-kanban-docs-link",
      title: "Open Lightning Kanban docs in a new tab",
      "aria-label": "Lightning Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `lightning kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "lightning-kanban-msg-ok" : "lightning-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "lightning-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "lightning-kanban-boardswitcher" },
      h("div", { className: "lightning-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "lightning-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "lightning-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "lightning-kanban-dialog-title" },
        ...
        h("div", { className: "lightning-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Lightning profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "lightning-kanban-bulk" },
      h("span", { className: "lightning-kanban-bulk-count" },
      ...
      h("div", { className: "lightning-kanban-bulk-priority",
      ...
      h("div", { className: "lightning-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Lightning profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "lightning-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("lightning-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("lightning-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "lightning-kanban-trash",
        dragOver ? "lightning-kanban-trash--drop" : "",
        props.draggingTaskId ? "lightning-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "lightning-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "lightning-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".lightning-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "lightning-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("lightning-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("lightning-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "lightning-kanban-column",
        dragOver ? "lightning-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "lightning-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "lightning-kanban-col-check",
        ...
        h("span", { className: cn("lightning-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "lightning-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "lightning-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "lightning-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "lightning-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "lightning-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "lightning-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "lightning-kanban-lane" },
                  h("div", { className: "lightning-kanban-lane-head" },
                    h("span", { className: "lightning-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "lightning-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "lightning-kanban-card--stale-red";
    if (age >= tier.amber) return "lightning-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".lightning-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "lightning-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "lightning-kanban-card",
        props.selected ? "lightning-kanban-card--selected" : "",
        props.failed ? "lightning-kanban-card--failed" : "",
        props.draggingSource ? "lightning-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "lightning-kanban-card-content" },
          h("div", { className: "lightning-kanban-card-row" },
            h("label", {
              className: "lightning-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "lightning-kanban-card-check",
            ...
            h("span", { className: "lightning-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or lightning kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "lightning-kanban-warning-badge",
                    "lightning-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "lightning-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "lightning-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "lightning-kanban-progress",
                    progress.done === progress.total ? "lightning-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "lightning-kanban-needs-assignee",
          ...
          h("div", { className: "lightning-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "lightning-kanban-card-row lightning-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "lightning-kanban-assignee",
                            title: `Assigned to Lightning profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "lightning-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "lightning-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "lightning-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "lightning-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Lightning profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Lightning profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "lightning-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "lightning-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "lightning-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "lightning-kanban-drawer-close",
        ...
        data ? h("div", { className: "lightning-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "lightning-kanban-drawer-body" },
      h("div", { className: "lightning-kanban-drawer-title" },
        h("span", { className: cn("lightning-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "lightning-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "lightning-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "lightning-kanban-section" },
        h("div", { className: "lightning-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "lightning-kanban-section" },
        h("div", { className: "lightning-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "lightning-kanban-comment" },
                h("div", { className: "lightning-kanban-comment-head" },
                  h("span", { className: "lightning-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "lightning-kanban-comment-ago" },
      ...
      h("div", { className: "lightning-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "lightning-kanban-event",
              isDiag ? "lightning-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "lightning-kanban-event-header" },
                  h("span", { className: "lightning-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "lightning-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "lightning-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "lightning-kanban-event-header-plain" },
                  h("span", { className: "lightning-kanban-event-kind" }, e.kind),
                  h("span", { className: "lightning-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "lightning-kanban-event-phantom-row" },
                  h("span", { className: "lightning-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "lightning-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "lightning-kanban-event-payload" },
    ...
    return h("div", { className: "lightning-kanban-section" },
      h("div", { className: "lightning-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "lightning-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `lightning-kanban-run--${r.outcome || r.status || "ended"}`
          : "lightning-kanban-run--active";
        return h("div", { key: r.id, className: cn("lightning-kanban-run", outcomeClass) },
          h("div", { className: "lightning-kanban-run-head" },
            h("span", { className: "lightning-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "lightning-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "lightning-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "lightning-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "lightning-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "lightning-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "lightning-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "lightning-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "lightning-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "lightning-kanban-pre lightning-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "lightning-kanban-meta-row" },
      h("span", { className: "lightning-kanban-meta-label" }, props.label),
      h("span", { className: "lightning-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "lightning-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lightning-kanban-meta-row" },
        h("span", { className: "lightning-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "lightning-kanban-meta-value lightning-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "lightning-kanban-meta-row" },
        h("span", { className: "lightning-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "lightning-kanban-meta-value lightning-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "lightning-kanban-section" },
      h("div", { className: "lightning-kanban-section-head-row" },
        h("span", { className: "lightning-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "lightning-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "lightning-kanban-section" },
      h("div", { className: "lightning-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "lightning-kanban-deps-row" },
        h("span", { className: "lightning-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "lightning-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "lightning-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "lightning-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "lightning-kanban-dep-chip-x",
      ...
      h("div", { className: "lightning-kanban-deps-row" },
        h("span", { className: "lightning-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "lightning-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "lightning-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "lightning-kanban-msg-ok"
          : "lightning-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "lightning-kanban-section" },
      ...
      h("div", { className: "lightning-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "lightning-kanban-home-sub lightning-kanban-home-sub--on"
              : "lightning-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__LIGHTNING_PLUGINS__ && typeof window.__LIGHTNING_PLUGINS__.register === "function") {
    window.__LIGHTNING_PLUGINS__.register("kanban", KanbanPage);
  }
})();
