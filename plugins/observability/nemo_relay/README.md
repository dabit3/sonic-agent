...
Optional Sonic observability plugin that maps Sonic observer hooks to
...
not replace Sonic Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Sonic emit NeMo Relay lifecycle events for
the work Sonic already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Sonic Agent can:

- Preserve Sonic execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
sonic plugins enable observability/nemo_relay
```

The `SONIC_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SONIC_HOME` that the
agent run will use:

```bash
env SONIC_HOME=/tmp/sonic-nemo-relay-test \
  sonic plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SONIC_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SONIC_HOME` is the Sonic profile/config home used by both
`sonic plugins enable ...` and the later `sonic chat ...` run. If unset,
Sonic uses the user's default home, usually `~/.sonic`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SONIC_HOME=/tmp/sonic-nemo-relay-test
sonic plugins enable observability/nemo_relay
sonic chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `sonic` command you run is built from the
...
uv run sonic plugins enable observability/nemo_relay
uv run sonic chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/sonic_agent-*.whl
...
The plugin can configure exporters directly from `SONIC_NEMO_RELAY_*`
...
```bash
export SONIC_NEMO_RELAY_ATOF_ENABLED=1
export SONIC_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SONIC_NEMO_RELAY_ATIF_ENABLED=1
export SONIC_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SONIC_NEMO_RELAY_ATOF_FILENAME`
- `SONIC_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SONIC_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SONIC_NEMO_RELAY_ATIF_AGENT_NAME`
- `SONIC_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SONIC_NEMO_RELAY_ATIF_MODEL_NAME`
- `SONIC_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Sonic at it:

```bash
export SONIC_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
export SONIC_HOME=/tmp/sonic-nemo-relay-docs/sonic-home
mkdir -p "$SONIC_HOME"
...
This run starts a parent Sonic session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SONIC_NEMO_RELAY_ATOF_ENABLED=1
export SONIC_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sonic-nemo-relay-docs/subagent/atof
export SONIC_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SONIC_NEMO_RELAY_ATOF_MODE=overwrite
export SONIC_NEMO_RELAY_ATIF_ENABLED=1
export SONIC_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sonic-nemo-relay-docs/subagent/atif
export SONIC_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SONIC_NEMO_RELAY_ATIF_AGENT_NAME='Sonic Agent E2E'
export SONIC_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SONIC_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"sonic.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Sonic Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Sonic dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/sonic-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/sonic-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/sonic-nemo-relay-docs/workdir/beta.txt
cd /tmp/sonic-nemo-relay-docs/workdir

export SONIC_NEMO_RELAY_ATOF_ENABLED=1
export SONIC_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sonic-nemo-relay-docs/parallel/atof
export SONIC_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SONIC_NEMO_RELAY_ATOF_MODE=overwrite
export SONIC_NEMO_RELAY_ATIF_ENABLED=1
export SONIC_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sonic-nemo-relay-docs/parallel/atif
export SONIC_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SONIC_NEMO_RELAY_ATIF_AGENT_NAME='Sonic Agent E2E'
export SONIC_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Sonic sessions map to `agent` scopes.
- Sonic API request hooks map to `llm` scope start/end events.
- Sonic tool hooks map to `tool` scope start/end events.
...
middleware for a local Sonic run. This path requires a NeMo Relay runtime that
