...
the existing module-level functions in sonic_cli.gateway and
sonic_cli.gateway_windows directly. This protocol is a thin facade
...
``sonic gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``sonic gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged sonic
    ...
    detection always fails. Since every Sonic runtime call inside the
    container drops to sonic via ``s6-setuidgid``, that silent failure
...
# in ``sonic_cli.gateway`` (systemd/launchd) and ``sonic_cli.gateway_windows``
...
# active (set via the ``sonic -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in sonic_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from sonic_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from sonic_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from sonic_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from sonic_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in sonic_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from sonic_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from sonic_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from sonic_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from sonic_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``sonic_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `sonic profile create`
# runs inside the container (Phase 4). Static services (main-sonic, dashboard)
...
# UID/GID of the in-image ``sonic`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the sonic user.
    ...
    (the ``sonic`` user — UID 10000 — running every Sonic runtime
    ...
    is dispatched as the sonic user.
    ...
    with sonic ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           sonic:sonic, 0755 (parent must already exist)
    ``svc_dir/event/``                     sonic:sonic, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 sonic:sonic, 0755
    ``svc_dir/supervise/event/``           sonic:sonic, 03730
    ``svc_dir/supervise/control``          sonic:sonic, 0660    (FIFO)
    ...
    slot's supervise/ was sonic-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _SONIC_UID, _SONIC_GID)
        except PermissionError:
            # Running as the sonic user already — directory is sonic-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _SONIC_UID, _SONIC_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is sonic-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _SONIC_UID, _SONIC_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``sonic -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`sonic profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-sonic, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SONIC_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SONIC_HOME=/data/sonic`` is honored at run
          ...
          4. Drops to the sonic user and exec's
             ``sonic -p <profile> gateway run`` (or just ``sonic
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``sonic gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SONIC_HOME profile" (the implicit profile that exists at
        the top of $SONIC_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``sonic_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SONIC_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``SONIC_HOME``,
        ...
        allocator in ``sonic_cli.profiles._allocate_gateway_port``).
        ...
        # profile's SONIC_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``sonic gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-SONIC_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The SONIC_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own SONIC_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "sonic gateway run --replace"
        else:
            gateway_cmd = f"sonic -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid sonic {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SONIC_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SONIC_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SONIC_HOME=/data/sonic``
        gets its logs under /data/sonic/logs/..., not the build-time
        default.
        ...
        return (
            ...
            f'exec s6-setuidgid sonic s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with sonic ownership
            ...
            # the sonic user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is sonic-owned (see ``_seed_supervise_skeleton``), and on
