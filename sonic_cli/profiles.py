"""
Profile management for multiple isolated Sonic instances.

Each profile is a fully independent SONIC_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.sonic/profiles/<name>/`` by default.

The "default" profile is ``~/.sonic`` itself — backward compatible,
zero migration needed.

Usage::

    sonic profile create coder          # fresh profile + bundled skills
    sonic profile create coder --clone  # also copy config, .env, SOUL.md, skills
    sonic profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    sonic -p coder chat                 # or via flag
    sonic profile use coder             # set as sticky default
    sonic profile delete coder          # remove profile + alias + service
"""
...
# a profile's root, callers of seed_profile_skills() (fresh-create, `sonic
...
# `sonic skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_sonic_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.sonic) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sonic-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "sonic", "default", "test", "tmp", "root", "sudo",
})

# Sonic subcommands that cannot be used as profile names/aliases
_SONIC_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the sonic root, NOT to the current SONIC_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SONIC_HOME points outside
    ``~/.sonic``, profiles live under ``SONIC_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_sonic_home() / "profiles"


def _get_default_sonic_home() -> Path:
    """Return the default (pre-profile) SONIC_HOME path.

    In standard deployments this is ``~/.sonic``.
    In Docker/custom deployments where SONIC_HOME is outside ``~/.sonic``
    (e.g. ``/opt/data``), returns SONIC_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_sonic_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Sonic installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SONIC_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_sonic_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, sonic subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SONIC_SUBCOMMANDS:
        return f"'{canon}' conflicts with a sonic subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``sonic -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``sonic profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``sonic profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SONIC_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Sonic config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sonic_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, sonic_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SONIC_HOME.
    ...
    The returned ``sonic_home`` is the path to pass to
    ``set_sonic_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_sonic_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``sonic update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sonic)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sonic/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `sonic update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`sonic profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `sonic profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SONIC_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``sonic profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SONIC_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.sonic).\n"
            "To remove everything, use: sonic uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SONIC_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SONIC_HOME")
    try:
        os.environ["SONIC_HOME"] = str(profile_dir)
        from sonic_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SONIC_HOME"] = old_home
        elif "SONIC_HOME" in os.environ:
            del os.environ["SONIC_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sonic/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: sonic profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SONIC_HOME.

    Returns ``"default"`` if SONIC_HOME is not set or points to ``~/.sonic``.
    Returns the profile name if SONIC_HOME points into ``~/.sonic/profiles/<name>``.
    Returns ``"custom"`` if SONIC_HOME is set to an unrecognized path.
    """
    from sonic_constants import get_sonic_home
    sonic_home = get_sonic_home()
    resolved = sonic_home.resolve()

    default_resolved = _get_default_sonic_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.sonic itself — its parent is ~/ and its
        # directory name is ".sonic", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: sonic profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.sonic itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.sonic). "
            "Specify a different name: sonic profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="sonic_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"sonic_{old_name}"
    legacy_old_host = f"sonic.{old_name}"
    new_host = f"sonic_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_sonic_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SONIC_HOME path string.

    Called early in the CLI entry point, before any sonic modules
    are imported, to set the SONIC_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
