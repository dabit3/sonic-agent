"""``sonic logs`` — view and filter Sonic log files.
...
Usage examples::
    ...
    sonic logs                    # last 50 lines of agent.log
    ...
    sonic logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_sonic_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Sonic runs — try 'sonic chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_sonic_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sonic_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sonic_home()}/logs/")
        return

    print(f"Log files in {display_sonic_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sonic chat' to generate logs)")
