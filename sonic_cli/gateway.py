"""
Gateway subcommand for sonic CLI.

Handles: sonic gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_sonic_home is imported lazily at call sites to avoid ImportError
# when sonic_constants is cached from a pre-update version during `sonic update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "sonic CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``sonic gateway status`` from falsely counting the ``sonic`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``sonic gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "sonic_cli.main gateway",
        "sonic_cli.main --profile",
        "sonic_cli.main -p",
        "sonic_cli/main.py gateway",
        "sonic_cli/main.py --profile",
        "sonic_cli/main.py -p",
        "sonic gateway",
        ...
        # ``sonic.exe --profile`` / ``sonic.exe -p`` would also match
        # ``sonic.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "sonic.exe gateway",
        "sonic-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_sonic_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"sonic_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. SONIC_HOME
        ...
        # SONIC_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "sonic_home=" in command_lc
            and f"sonic_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``sonic update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Sonic profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Sonic profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from sonic_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "sonic_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``sonic update`` users
    # with no running gateway until they re-invoke ``sonic gateway``
...
def _sync_sonic_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SONIC_HOME``.

    Under ``sudo``, ``SONIC_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_sonic_home` falls back to ``/root/.sonic`` — the wrong
    profile. The unit file pins ``SONIC_HOME`` for the actual gateway
    ...
    unit_home = env.get("SONIC_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SONIC_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SONIC_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}sonic gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}sonic gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from sonic_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `sonic gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``sonic gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from sonic_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SONIC_HOME-scoped).
...
from sonic_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``sonic gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``SONIC_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("SONIC_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "sonic-gateway"
SERVICE_DESCRIPTION = "Sonic Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SONIC_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SONIC_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from sonic_constants import get_default_sonic_root

    home = get_sonic_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SONIC_HOME paths
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SONIC_HOME.

    Default ``~/.sonic`` returns ``sonic-gateway`` (backward compatible).
    Profile ``~/.sonic/profiles/coder`` returns ``sonic-gateway-coder``.
    Any other SONIC_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Sonic installs that predate the
# sonic-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (sonic-gateway-*.service) and unrelated third-party
# "sonic" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("sonic.service",)
...
def _find_legacy_sonic_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Sonic gateway units.

    Detects unit files installed by older Sonic versions that used a
    different service name (e.g. ``sonic.service`` before the rename to
    ``sonic-gateway.service``). When both a legacy unit and the current
    ``sonic-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``sonic-gateway-coder.service`` and unrelated third-party
      ``sonic-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``sonic.service`` running an unrelated
...
def has_legacy_sonic_units() -> bool:
    """Return True when any legacy Sonic gateway unit files exist."""
    return bool(_find_legacy_sonic_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Sonic gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_sonic_units()
    if not legacy:
        return
    print_warning("Legacy Sonic gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current sonic-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    sonic gateway migrate-legacy")


def remove_legacy_sonic_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Sonic gateway unit files.

    Iterates over whatever ``_find_legacy_sonic_units()`` returns — which is
    ...
    legacy = _find_legacy_sonic_units()
    if not legacy:
        print("No legacy Sonic gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Sonic gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: sonic gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo sonic gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    sonic gateway uninstall")
    print_info("    sudo sonic gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Sonic can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo sonic gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo sonic gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo sonic gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Sonic often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.sonic`` → ``ai.sonic.gateway.plist`` (backward compatible).
    Profile ``~/.sonic/profiles/coder`` → ``ai.sonic.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.sonic.gateway-{suffix}" if suffix else "ai.sonic.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.sonic/sonic-agent  -> /home/alice/.sonic/sonic-agent
      /opt/sonic                 -> /opt/sonic  (kept as-is)
...
def _sonic_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SONIC_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_sonic_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.sonic                    → /home/alice/.sonic
      /root/.sonic/profiles/coder     → /home/alice/.sonic/profiles/coder
      /opt/custom-sonic               → /opt/custom-sonic  (kept as-is)
    """
    current_sonic = get_sonic_home().resolve()
    current_default = (Path.home() / ".sonic").resolve()
    target_default = Path(target_home_dir) / ".sonic"

    # Default ~/.sonic → remap to target user's default
    if current_sonic == current_default:
        return str(target_default)

    # Profile or subdir of ~/.sonic → preserve the relative structure
    try:
        relative = current_sonic.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.sonic) — keep as-is
        return str(current_sonic)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    sonic_node = sonic_home / "node" / "bin"
    if _is_dir(sonic_node):
        candidates.append(str(sonic_node))
    sonic_nm = sonic_home / "node_modules" / ".bin"
    if _is_dir(sonic_nm):
        candidates.append(str(sonic_nm))
...
User={username}
Group={group_name}
ExecStart={python_path} -m sonic_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="SONIC_HOME={sonic_home}"
Restart=always
RestartSec=5
...
"""
    ...
    sonic_home = str(get_sonic_home().resolve())
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir SONIC_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved SONIC_HOME
    ...
    exported a throwaway ``SONIC_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``sonic gateway restart``
    with ``SONIC_HOME=/tmp/sonic-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="SONIC_HOME=..."`` lines and launchd
    ``<key>SONIC_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'SONIC_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>SONIC_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp SONIC_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: SONIC_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported SONIC_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only SONIC_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp SONIC_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../sonic_test`` path into
    # ``Environment="SONIC_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/sonic_test"' in new_unit
        or "/sonic_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # SONIC_HOME into the unit (manual E2E homes like /tmp/sonic-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Sonic install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo sonic gateway uninstall --system")
    print_info("         sonic gateway install")
    print_info("         sonic gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("SONIC_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (sonic.service from pre-rename installs)
    # before installing the new sonic-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}sonic gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}sonic gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}sonic gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_sonic_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `sonic gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `sonic gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}sonic gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}sonic gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}sonic gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}sonic gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.sonic.gateway-{suffix}" if suffix else "ai.sonic.gateway"
...
# (the documented `nohup sonic gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m sonic_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active SONIC_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "sonic_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup sonic gateway run --replace`
    ...
    """
    from sonic_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_sonic_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: sonic gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup sonic gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Sonic install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  sonic gateway status             # Check status")
    from sonic_constants import display_sonic_home as _dhh
...
def launchd_stop():
    ...
    # `sonic gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Sonic install")
    else:
        print("⚠ Service definition is stale relative to the current Sonic install")
    ...
    if deep:
        log_file = get_sonic_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SONIC_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Sonic gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'sonic' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/sonic/docker/entrypoint.sh before the Sonic command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SONIC_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set SONIC_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `sonic gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set SONIC_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `sonic gateway restart` which already
    ...
    # `sonic gateway start/restart` — leaving the gateway vulnerable to
    ...
    # SONIC_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SONIC_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Sonic will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @sonic:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. sonic) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Sonic delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Sonic agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Sonic will use (e.g., sonic@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Sonic connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: sonic pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Sonic will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``sonic setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run sonic under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.sonic/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_sonic_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: sonic pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'sonic gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from sonic_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from sonic_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Sonic will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Sonic will store the returned account_id/token in ~/.sonic/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `sonic gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_sonic_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `sonic pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `sonic pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "SonicAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``sonic_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from sonic_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.sonic/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.sonic/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: sonic gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo sonic gateway install --system"
                        )
                    print_info("  Or run in foreground:  sonic gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: sonic gateway run")
                print_info(
                    "  For persistence:   tmux new -s sonic 'sonic gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: sonic gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup sonic gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'sonic gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``sonic gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``sonic gateway run`` are unaffected.
      2. ``SONIC_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``sonic gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``SONIC_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("SONIC_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    ...
    # `docker logs` and ${SONIC_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if SONIC_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or SONIC_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: sonic gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: sonic gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s sonic 'sonic gateway run'"
                )
                print()
            ...
            print()
            print(
                "  sonic gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s sonic 'sonic gateway run'         # persistent via tmux"
            )
            print(
                "  nohup sonic gateway run > ~/.sonic/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: sonic gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: sonic gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: sonic gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: sonic gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_SONIC_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `sonic gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_SONIC_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `sonic gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: sonic gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sonic gateway install")
                else:
                    print("To install as a service:")
                    print("  sonic gateway install")
                    print("  sudo sonic gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  sonic gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup sonic gateway run > ~/.sonic/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s sonic 'sonic gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  sonic gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  sonic gateway install  # Install as user service")
                    print(
                        "  sudo sonic gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Sonic gateway unit files from
        # pre-rename installs (e.g. sonic.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_sonic_units(interactive=not yes, dry_run=dry_run)
