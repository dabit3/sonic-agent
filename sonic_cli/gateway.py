        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "sonic-gateway*",
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"SONIC_HOME={current_home}" in command
            )
        ...
        if "--profile " in command or " -p " in command:
            return False
        if "SONIC_HOME=" in command and f"SONIC_HOME={current_home}" not in command:
            return False
        return True
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}sonic gateway restart{scope_flag}")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Sonic can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo sonic gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo sonic gateway install --system --run-as-user <your-user>")
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Sonic install")
    return True
...
def systemd_install(
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}sonic gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}sonic gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}sonic gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}sonic gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}sonic gateway start{scope_flag}")
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Sonic install")
    return True
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print("  Set SONIC_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _setup_weixin():
    ...
    print_info("  3. Sonic will store the returned account_id/token in ~/.sonic/.env.")
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `sonic pairing approve`.")
    elif access_idx == 1:
    ...
    if any_configured:
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                print_info("  For persistence:   tmux new -s sonic 'sonic gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from sonic_constants import display_sonic_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: sonic gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup sonic gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info("  Consider running in foreground instead: sonic gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s sonic 'sonic gateway run'")
                print()
            ...
            print()
            print("  sonic gateway run                              # direct foreground")
            print("  tmux new -s sonic 'sonic gateway run'         # persistent via tmux")
            print("  nohup sonic gateway run > ~/.sonic/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup sonic gateway run > ~/.sonic/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s sonic 'sonic gateway run'         # persistent via tmux")
                    print("  nohup sonic gateway run > ~/.sonic/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  sonic gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  sonic gateway install  # Install as user service")
                    print("  sudo sonic gateway install --system  # Install as boot-time system service")
