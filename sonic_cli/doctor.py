"""
Doctor command for sonic CLI.

Diagnoses issues with Sonic Agent setup.
"""
...
from sonic_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
SONIC_HOME = get_sonic_home()
_DHH = display_sonic_home()  # user-facing display path (e.g. ~/.sonic or ~/.sonic/profiles/coder)

# Load environment variables from ~/.sonic/.env so API key checks work
_env_path = get_env_path()
load_sonic_dotenv(sonic_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from sonic_cli.colors import Colors, color
from sonic_cli.models import _SONIC_USER_AGENT
from sonic_constants import OPENROUTER_MODELS_URL
...
from sonic_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.sonic/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SONIC_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from sonic_cli.auth import get_minimax_oauth_auth_status
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from sonic_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `sonic`.
    os.environ.setdefault("SONIC_INTERACTIVE", "1")

    # Handle `sonic doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.sonic/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Sonic Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`sonic doctor --ack {hit.advisory.id}`."
                )
    ...
    managed_scope_check()
    # Check ~/.sonic/.env (primary location for user config)
    env_path = SONIC_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'sonic setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'sonic setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'sonic setup' to create one")
                issues.append("Run 'sonic setup' to create .env")
    
    # Check ~/.sonic/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from sonic_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from sonic_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'sonic config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from sonic_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.sonic/.env or run 'sonic setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'sonic setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'sonic config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from sonic_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'sonic setup' to migrate config")
                else:
                    issues.append("Run 'sonic doctor --fix' or 'sonic setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'sonic doctor --fix'")
        except Exception:
            pass

        # Detect stale SONIC_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives SONIC_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from sonic_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("SONIC_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"SONIC_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `sonic setup` run)",
                )
                if should_fix:
                    if remove_env_value("SONIC_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale SONIC_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove SONIC_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the SONIC_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale SONIC_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'sonic doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Sonic' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = sonic_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Sonic a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Sonic Agent Persona\n\n"
                "<!-- Edit this file to customize how Sonic communicates. -->\n\n"
                "You are Sonic, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = sonic_home / "memories"
    ...
    state_db_path = sonic_home / "state.db"
    ...
    wal_path = sonic_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'sonic doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "sonic"
        ...
        _cmd_link = _cmd_link_dir / "sonic"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(sonic not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/sonic → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/sonic points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/sonic → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/sonic — run 'sonic doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/sonic exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/sonic not found",
                    "(sonic command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/sonic → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/sonic symlink — run 'sonic doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `sonic update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `sonic doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable SONIC_HOME mirror
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'sonic config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``sonic doctor``.
    ...
    # never the legitimate source for `sonic doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'sonic setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = SONIC_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: sonic skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = SONIC_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: sonic memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: sonic memory setup",
                    "No Honcho API key — run 'sonic memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run sonic memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: sonic memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: sonic memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from sonic_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "sonic -p" in content:
                            _m = _re.search(r"sonic -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'sonic doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
