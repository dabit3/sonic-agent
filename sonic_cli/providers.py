"""
Single source of truth for provider identity in Sonic Agent.
...
2. **Sonic overlays** — transport type, auth patterns, aggregator flags,
...
# -- Sonic overlay ----------------------------------------------------------
# Sonic-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SonicOverlay:
    """Sonic-specific provider metadata layered on top of models.dev."""
...
SONIC_OVERLAYS: Dict[str, SonicOverlay] = {
    ...
    "qwen-oauth": SonicOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SONIC_QWEN_BASE_URL",
    ),
    "google-gemini-cli": SonicOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "google-antigravity": SonicOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="antigravity-pa://google",
    ),
    "lmstudio": SonicOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SonicOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SonicOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SonicOverlay(
    ...
    "kimi-for-coding": SonicOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SonicOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SonicOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SonicOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SonicOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SonicOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SonicOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SonicOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": SonicOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SonicOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "sonic", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Sonic overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Sonic overlay
    ...
    overlay = SONIC_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + sonic extra
    ...
    if overlay is not None:
        # Sonic-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in SONIC_OVERLAYS
    if provider == "bedrock":
