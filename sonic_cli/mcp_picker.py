"""MCP picker — interactive `sonic mcp picker` (also the default `sonic mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``sonic mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `sonic plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from sonic_cli.cli_output import prompt_yes_no
from sonic_cli.curses_ui import curses_single_select
from sonic_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Sonic session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from sonic_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `sonic mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: sonic mcp install <name>    Picker: sonic mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Sonic is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Sonic — run `sonic update` "
...
def show_catalog() -> None:
    """`sonic mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`sonic mcp picker` (and default `sonic mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`sonic mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `sonic mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
