"""Per-provider model-selection wizard flows for ``sonic setup`` / ``sonic model``.

Extracted from ``sonic_cli/main.py`` as part of the god-file decomposition
campaign (``~/.sonic/plans/god-file-decomposition.md``, Phase 2 — splitting
...
in main.py re-imports them (``from sonic_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``sonic_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from sonic_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``sonic_cli.main`` at import time -> no import cycle.
"""
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from sonic_cli.main import _prompt_api_key
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from sonic_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from sonic_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from sonic_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from sonic_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from sonic_cli.auth import _save_model_choice, deactivate_provider
    from sonic_cli.config import load_config, save_config
    from sonic_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from sonic_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from sonic_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from sonic_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from sonic_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from sonic_cli.main import _run_anthropic_oauth_flow
    from sonic_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
