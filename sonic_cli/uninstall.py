"""
Sonic Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.sonic/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Sonic PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing sonic-agent or sonic PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Sonic Agent" comment and following line
                if '# Sonic Agent' in line or '# sonic-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('sonic' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing sonic
                if 'sonic' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the sonic wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "sonic",
        Path("/usr/local/bin/sonic"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains sonic_cli reference)
                content = wrapper.read_text()
                if 'sonic_cli' in content or 'sonic-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(sonic_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``sonic`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Sonic home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (sonic_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sonic gateway run`` processes
    ...
    try:
        from sonic_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from sonic_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SONIC_HOME`` and ``SONIC_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\sonic\git\cmd``, ``%LOCALAPPDATA%\sonic\git\bin``,
#      ``%LOCALAPPDATA%\sonic\git\usr\bin``, ``%LOCALAPPDATA%\sonic\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\sonic\git\`` and Node to
#      ``%LOCALAPPDATA%\sonic\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``sonic dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\sonic\gateway-service\`` and sometimes
...
def _sonic_path_markers(sonic_home: Path) -> list[str]:
    """Path-entry substrings that identify Sonic-owned User-PATH entries."""
    root = str(sonic_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare sonic-agent install dir.
    markers = [root + "\\sonic-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SONIC_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "sonic".  We don't want to catch
    # unrelated entries like "csonic-foo" or "ephermeral", so we look for
    # backslash-sonic as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(sonic_home: Path) -> list[str]:
    """Strip Sonic-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _sonic_path_markers(sonic_home)
            kept: list[str] = []
...
def remove_sonic_env_vars_windows() -> list[str]:
    """Delete SONIC_HOME and SONIC_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SONIC_HOME", "SONIC_GIT_BASH_PATH"):
                try:
...
def _is_default_sonic_home(sonic_home: Path) -> bool:
    """Return True when ``sonic_home`` points at the default (non-profile) root."""
    try:
        from sonic_constants import get_default_sonic_root
        return sonic_home.resolve() == get_default_sonic_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SONIC_HOME directory.

    We shell out to ``sonic -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SONIC_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m sonic_cli.main` so we don't depend on a `sonic`
    #    wrapper that may be half-removed mid-uninstall.
    sonic_invocation = [_sys.executable, "-m", "sonic_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                sonic_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SONIC_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``sonic uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SONIC_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from sonic_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(sonic_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Sonic Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Sonic Chat GUI installation was found.")
        print(f"  Checked: {sonic_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Sonic agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(sonic_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Sonic agent at {sonic_home / 'sonic-agent'}")
        print(f"  • Your config, sessions, and secrets under {sonic_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Sonic agent is still installed. Run 'sonic' to use the CLI,")
    print("or 'sonic uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sonic/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.sonic/ for future reinstall
    """
    project_root = get_project_root()
    sonic_home = get_sonic_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SONIC_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_sonic_home(sonic_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.sonic data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Sonic Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {sonic_home / 'config.yaml'}")
    print(f"  Secrets: {sonic_home / '.env'}")
    print(f"  Data:    {sonic_home / 'cron/'}, {sonic_home / 'sessions/'}, {sonic_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SONIC_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Sonic data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Sonic code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``sonic`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SONIC_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in sonic_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\sonic\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(sonic_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Sonic-owned PATH entries in User environment")

        log_info("Removing SONIC_HOME / SONIC_GIT_BASH_PATH User env vars...")
        removed_env = remove_sonic_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Sonic-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing sonic command...")
    ...
    #     (only when they still point into this Sonic home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Sonic-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(sonic_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Sonic-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(sonic_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SONIC_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from sonic_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(sonic_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.sonic/, just remove the sonic-agent subdir
            if sonic_home in project_root.parents or project_root.parent == sonic_home:
    ...
    #     under SONIC_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(sonic_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(sonic_home)
    ...
    # 5. Optionally remove ~/.sonic/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SONIC_HOME dirs live under
        ...
        try:
            if sonic_home.exists():
                shutil.rmtree(sonic_home)
                log_success(f"Removed {sonic_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {sonic_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {sonic_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/dabit3/sonic-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/dabit3/sonic-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Sonic Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m sonic_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import sonic_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``sonic_constants`` + ``sonic_cli.colors``
    (and lazily ``sonic_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m sonic_cli.uninstall")
    parser.add_argument(
        "--mode",
