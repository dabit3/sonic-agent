"""
Sonic Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``sonic desktop`` / ``sonic gui``)
     Built inside the agent checkout under ``$SONIC_HOME/sonic-agent/``:
       ...
       - ``$SONIC_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Sonic.app`` or ``~/Applications/Sonic.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Sonic`` (NSIS per-user)
...
the app name ("Sonic"), separate from ``$SONIC_HOME``:
  - macOS:   ``~/Library/Application Support/Sonic``
  - Windows: ``%APPDATA%\\Sonic``
  - Linux:   ``$XDG_CONFIG_HOME/Sonic`` (default ``~/.config/Sonic``)
...
``sonic uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(sonic_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return sonic_home / "sonic-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Sonic"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Sonic"
...
def source_built_gui_artifacts(sonic_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``sonic desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``sonic-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(sonic_home)
    ...
    return [
        ...
        sonic_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Sonic" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Sonic.app"),
            home / "Applications" / "Sonic.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Sonic).
            local_base / "Programs" / "Sonic",
            # Older / alternate layout some builds used.
            local_base / "sonic-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Sonic")
    else:
        ...
        paths += [
            data_base / "applications" / "sonic.desktop",
            data_base / "applications" / "Sonic.desktop",
        ]
    return paths


def agent_is_installed(sonic_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SONIC_HOME.
    ...
    if (agent_root / "sonic_cli").is_dir():
        return True
...
def gui_is_installed(sonic_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(sonic_home):
        if p.exists():
            return True
...
def gui_install_summary(sonic_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = sonic_home if sonic_home is not None else get_sonic_home()
...
def uninstall_gui(sonic_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``sonic-agent/sonic_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SONIC_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove sonic' or "
            "'sudo dnf remove sonic'). AppImage builds are a single file you "
