"""Slash command definitions and autocomplete for the Sonic CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Sonic config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Sonic works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Sonic is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.sonic/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Sonic Agent to the latest version", "Info"),
    CommandDef("version", "Show Sonic Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`sonic_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/sonic`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Sonic
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``sonic skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``sonic skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``sonic skills list`` and
...
# unrelated command landed. These claim their slots right after /sonic,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /sonic <command>).
...
# native slot, the alias spelling stays reachable via /sonic reset).
...
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /sonic billing.
#   - moa: high-cost slash mode, available through /sonic moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /sonic debug on Slack.
_SLACK_VIA_SONIC_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/sonic <verb>`` subcommand.
    ...
    can still reach them via ``/sonic <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/sonic`` is always reserved as the first entry so the
    legacy ``/sonic <subcommand>`` form keeps working for anything that
    ...
    # Reserve /sonic as the catch-all top-level command.
    entries.append(("sonic", "Talk to Sonic or run a subcommand", "[subcommand] [args]"))
    seen.add("sonic")
    ...
    # everything except /sonic, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://sonic-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /sonic handler.

    Maps both canonical names and aliases so /sonic bg do stuff works
    the same as /sonic background do stuff.

    Plugin-registered slash commands are included so ``/sonic <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from sonic_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
