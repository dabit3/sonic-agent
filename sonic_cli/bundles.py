"""Implementation of the ``sonic bundles`` CLI subcommand.

Mirrors the structure of ``sonic_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `sonic bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  sonic bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``sonic bundles`` argparse tree.

    Called from ``sonic_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `sonic bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``sonic bundles <subcommand>`` to the right handler."""
