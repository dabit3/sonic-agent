...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            print_info(
                "Sonic can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Sonic can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'sonic setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``sonic model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'sonic-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'sonic-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Sonic delivers cron job results and notifications.")
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``sonic setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``sonic setup --portal``)
    that gets a brand-new user from zero to a fully working Sonic session
    ...
    # runs `sonic setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `sonic auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `sonic auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `sonic model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `sonic portal status` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        setup_mode = prompt_choice("How would you like to set up Sonic?", [
...
def _run_first_time_quick_setup(config: dict, sonic_home, is_existing: bool):
    ...
    the user can customize later via ``sonic setup <section>``.
    """
