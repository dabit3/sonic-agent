"""
Interactive setup wizard for Sonic Agent.
...
# display_sonic_home imported lazily at call sites (stale-module safety during sonic update)
...
from sonic_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Sonic Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Sonic using environment variables or config commands:")
    print_info("  sonic config set model.provider custom")
    print_info("  sonic config set model.base_url http://localhost:8080/v1")
    print_info("  sonic config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'sonic setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from sonic_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'sonic setup')")


def _print_setup_summary(config: dict, sonic_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'sonic setup' to configure"))
    ...
    # Video generation — opt-in via `sonic tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from sonic_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'sonic setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'sonic setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'sonic setup terminal'"))
    ...
    # Spotify (OAuth via sonic auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'sonic setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {sonic_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('sonic setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('sonic setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('sonic setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('sonic setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('sonic setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('sonic config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('sonic config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('sonic config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('sonic', Colors.GREEN)}              Start chatting")
    print(f"   {color('sonic gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('sonic doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `sonic tools` and `sonic setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``sonic model``)
    ...
    provider added to ``sonic model`` is automatically available here.
    ...
    # Delegate to the shared sonic model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from sonic_cli.main import select_provider_and_model
    try:
    ...
    # on demand via `sonic auth add`, `sonic setup` vision, and
    # `sonic setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``sonic model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from sonic_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.sonic/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Sonic auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via sonic setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'sonic setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Sonic runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SONIC_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SONIC_MAX_ITERATIONS")
    ...
    print_info("  Run `sonic setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SONIC_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_bluebubbles():
    ...
    print_info("Connects Sonic to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from sonic_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  sonic config edit")
    print_info("   Open config in your editor:  sonic config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from sonic_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Sonic from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'sonic setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     sonic config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_sonic_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("   sonic gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   sonic gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `sonic setup tools` and `sonic tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``sonic_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from sonic_cli.config
    ...
    if section_key == "model":
        ...
        from sonic_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_sonic.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_sonic migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_sonic", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Sonic.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Sonic to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Sonic at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Sonic at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Sonic at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Sonic at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Sonic equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Sonic config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Sonic.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Sonic's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(sonic_home: Path) -> bool:
    ...
    print_info("Sonic can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: sonic claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=sonic_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: sonic claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Sonic configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Sonic config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Sonic (use sonic claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``sonic setup --portal`` and ``sonic portal``. This is the
    ...
    Sonic session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``sonic setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``sonic model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``sonic portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Sonic Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("  Run `sonic portal info` to inspect routing.")
    print_info("  Run `sonic` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      sonic setup           — full or quick (auto-detected)
      sonic setup model     — just model/provider
      sonic setup tts       — just text-to-speech
      sonic setup terminal  — just terminal backend
      sonic setup gateway   — just messaging platforms
      sonic setup tools     — just tool configuration
      sonic setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Sonic Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Sonic Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Sonic Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, sonic_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Sonic configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'sonic setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(sonic_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Sonic?",
            [
    ...
    print_info(f"Data folder:  {sonic_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'sonic config edit'")
    ...
    _print_setup_summary(config, sonic_home)


def _run_first_time_quick_setup(config: dict, sonic_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'sonic setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    sonic setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  sonic setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``sonic tools`` (which
    ...
    try:
        ...
        from sonic_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``sonic setup agent`` /
    ``sonic config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, sonic_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `sonic update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        sonic tools")
        print_info("  Seed skills:         sonic skills opt-in --sync")
        print_info("  Add MCP servers:     sonic mcp add")
        print_info("  Enable plugins:      sonic plugins")
        print_info("  Tune agent settings: sonic setup agent")
        print()
        _print_setup_summary(config, sonic_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, sonic_home)


def _blank_slate_walkthrough(config: dict, sonic_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`sonic update` runs from re-injecting them. Opt back in any")
            print_info("time with `sonic skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `sonic tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `sonic plugins list` / `sonic plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `sonic plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `sonic mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `sonic mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   sonic tools")
    print_info("  Seed skills:         sonic skills opt-in --sync")
    print_info("  Add MCP servers:     sonic mcp add")
    print_info("  Tune agent settings: sonic setup agent")
    print()
...
def _run_quick_setup(config: dict, sonic_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'sonic setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Sonic to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'sonic setup gateway'.")
