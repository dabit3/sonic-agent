...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_sonic_action(["gateway", "restart"], "gateway-restart"), False
...
async def update_sonic():
    ...
    if _dashboard_hosted_agent_mode():
        message = (
            "Sonic updates are managed by the hosted agent service for this "
...
async def start_gateway():
    try:
        proc = _spawn_sonic_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_sonic_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
