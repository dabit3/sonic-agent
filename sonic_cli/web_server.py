"""
Sonic Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m sonic_cli.main web          # Start on http://127.0.0.1:9119
    python -m sonic_cli.main web --port 8080
"""
...
try:
    ...
    # running `sonic dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["SONIC_WEB_DIST"]) if "SONIC_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``sonic gateway run`` — but the
    desktop app spawns a ``sonic dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same SONIC_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire sonic_cli.gateway import into a background thread so the event
    # loop is not blocked and SONIC_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (SONIC_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `sonic
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("SONIC_DESKTOP") == "1":
...
app = FastAPI(title="Sonic Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from sonic_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# SONIC_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("SONIC_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Sonic-Session-Token"
...
#
# This list is defined in ``sonic_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from sonic_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Sonic-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``sonic-0day``
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Sonic provider slug — fine.
    ...
    1. Vendor-name → Sonic-provider mapping: when the provider string is
       not a known Sonic provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from sonic_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from sonic_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from sonic_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "SONIC_DASHBOARD_FILES_ROOT"
...
def _default_sonic_root_is_opt_data() -> bool:
    raw = os.environ.get("SONIC_HOME", "").strip()
    if not raw:
        return False
    try:
        from sonic_constants import get_default_sonic_root

        root = get_default_sonic_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    a container — e.g. the sonic-webui image sharing the Sonic source tree),
    the dashboard's ``sonic update`` button is the correct update path and
    ...
    """
    if _default_sonic_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Sonic root is actually /opt/data
    # (the container/hosted layout) or when SONIC_DASHBOARD_FILES_ROOT is set.
    if _default_sonic_root_is_opt_data():
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_sonic_home() at call time
    ...
    try:
        ...
        # Windows install the first import of sonic_cli.gateway blocks the
...
# streamed to a per-action log file under ``~/.sonic/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_sonic_home() / "logs"
...
def _spawn_sonic_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``sonic <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``sonic_cli.main`` module so the action
    ...
    cmd = [spawn_executable, "-m", "sonic_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SONIC_NONINTERACTIVE": "1", **spawn_env_overlay},
    }
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["sonic", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``sonic gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``sonic gateway restart`` children race each other on the
    ...
    return _spawn_sonic_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``sonic gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``SONIC_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/sonic/update")
async def update_sonic():
    """Kick off ``sonic update`` in the background."""
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("sonic-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "sonic-update",
    ...
    try:
        proc = _spawn_sonic_action(["update"], "sonic-update")
    except Exception as exc:
        _log.exception("Failed to spawn sonic update")
...
@app.get("/api/sonic/update/check")
async def check_sonic_update(force: bool = False):
    """Report whether a Sonic update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/sonic/update`` actually runs ``sonic update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Sonic version string
    ...
    try:
        ...
        if force:
            try:
                (get_sonic_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.sonic/config.yaml`` under ``tts.``.
    """
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Sonic supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in sonic_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    """
    try:
        from sonic_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `sonic model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `sonic model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.sonic/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (sonic_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from sonic_cli.nous_subscription import apply_nous_managed_defaults
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Sonic from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Sonic to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Sonic from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Sonic to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Sonic in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Sonic through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Sonic via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Sonic through an IMAP/SMTP mailbox.",
        "docs_url": "https://lightning-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Sonic to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Sonic inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Sonic through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Sonic to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Sonic to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Sonic as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://lightning-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://lightning-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Sonic appears
...
# the canonical ``sonic auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.sonic/.anthropic_oauth.json`` — Sonic-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_sonic_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_sonic_oauth_credentials,
            _SONIC_OAUTH_FILE,
        )
    except ImportError:
        read_sonic_oauth_credentials = None  # type: ignore
        _SONIC_OAUTH_FILE = None  # type: ignore

    sonic_creds = None
    if read_sonic_oauth_credentials:
        try:
            sonic_creds = read_sonic_oauth_credentials()
        except Exception:
            sonic_creds = None
    if sonic_creds and sonic_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "sonic_pkce",
            "source_label": f"Sonic PKCE ({_SONIC_OAUTH_FILE})",
            "token_preview": _truncate_token(sonic_creds.get("accessToken")),
            "expires_at": sonic_creds.get("expiresAt"),
            "has_refresh_token": bool(sonic_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Sonic even
    when they also have a separate Sonic-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "sonic auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "sonic auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "sonic auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "sonic auth add minimax-oauth",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Sonic, so the disconnect
    ...
    user sees exactly what executes, and Sonic then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Sonic — run the disconnect command to remove it."
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("sonic_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        try:
            from sonic_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.sonic/.anthropic_oauth.json AND credential pool
...
# Guarded so sonic web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Sonic file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``sonic auth add anthropic``.
    """
    from agent.anthropic_adapter import _SONIC_OAUTH_FILE
    payload = {
    ...
    _SONIC_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _SONIC_OAUTH_FILE.with_name(
        f"{_SONIC_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _SONIC_OAUTH_FILE)
        try:
            _SONIC_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "sonic-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("SONIC_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from sonic_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from sonic_cli.auth import _save_codex_tokens
...
async def prune_sessions_endpoint(body: SessionPrune):
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_sonic_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from sonic_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_sonic_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from sonic_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SONIC_HOME)."""
    from sonic_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process SONIC_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (sonic_cli.mcp_config), so
# servers managed here show up under `sonic mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from sonic_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from sonic_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from sonic_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from sonic_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # SONIC_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `sonic mcp catalog` / `sonic mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds SONIC_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (sonic_cli.webhook); the webhook
...
# spawn the real `sonic gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_sonic_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_sonic_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from sonic_cli.auth import read_credential_pool
...
# `sonic memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `sonic memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_sonic_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_sonic_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_sonic_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `sonic import`. The spawned action runs with
...
async def list_checkpoints():
    ...
    # Checkpoints live under <sonic_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_sonic_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_sonic_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``sonic`` subprocess, and
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`sonic -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the SONIC_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_sonic_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "sonic setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local SONIC_HOME override so the write lands in the target
    ...
    """
    from sonic_constants import set_sonic_home_override, reset_sonic_home_override
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local SONIC_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the SONIC_HOME override. Returns the
    number of skills newly disabled.
    """
    from sonic_constants import set_sonic_home_override, reset_sonic_home_override
    from sonic_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's SONIC_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``sonic profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from SONIC_HOME).
    """
    from sonic_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``sonic profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the SONIC_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``sonic profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from sonic_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_sonic_home()`` at call
       time — the context-local override from ``set_sonic_home_override``
    ...
    retargeted to the *current* ``get_sonic_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a SONIC_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_sonic_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_sonic_home()`` at call time
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``sonic tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``sonic tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``sonic tools``
...
async def run_toolset_post_setup(
    ...
    ``profile`` spawns the hook as ``sonic -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SONIC_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Sonic,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``sonic computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``sonic --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from sonic_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``sonic --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SONIC_TUI_RESUME`` env var —
    matching what ``sonic_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``SONIC_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``SONIC_TUI_SIDECAR_URL`` so
    ...
    ``SONIC_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_sonic_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``sonic -p <name>`` performs. The in-process
    ``SONIC_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from sonic_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    # inherits the profile SONIC_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["SONIC_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from sonic_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Sonic inside WSL2 to use the dashboard's /chat "
...
# SONIC_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`sonic_cli.dashboard_auth.prefix.normalise_prefix`
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/sonic/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /sonic`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__SONIC_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/sonic``)
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__SONIC_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SONIC_BASE_PATH__="{prefix}";'
                f"window.__SONIC_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__SONIC_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__SONIC_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SONIC_BASE_PATH__="{prefix}";'
                f"window.__SONIC_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/sonic`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Sonic backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Sonic Teal",         "description": "Classic dark teal — the canonical Sonic look"},
    {"name": "default-large", "label": "Sonic Teal (Large)", "description": "Sonic Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.sonic/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.sonic/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_sonic_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.sonic/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as sonic_cli.plugins):
    1. User plugins:    ~/.sonic/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.sonic/plugins/  (only if SONIC_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from sonic_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_sonic_home() / "plugins", "user"),
    ...
    # ``sonic_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("SONIC_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".sonic" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_sonic_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"sonic auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from sonic_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sonic_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sonic_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.sonic/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"sonic_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    try:
        from sonic_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the sonic-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `sonic dashboard register` (Nous Portal) or "
    ...
    # SONIC_DASHBOARD_READY, open the browser, *then* serve.
    #
