...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    ink = root / "node_modules" / "@sonic" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "packages/sonic-ink/package-lock.json",
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("SONIC_TUI") == "1"
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.sonic/logs/update.log`` so nothing is lost.
        #
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    wants_tui = os.environ.get("SONIC_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("SONIC_TUI") == "1"):
        return False
