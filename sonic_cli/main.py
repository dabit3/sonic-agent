...
def select_provider_and_model(args=None):
    ...
    from sonic_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from sonic_constants import AI_GATEWAY_BASE_URL
    from sonic_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.sonic/.env by hand.
    ...
    from sonic_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def cmd_portal(args):
    ...
    from sonic_cli.portal_cli import portal_command
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``sonic_cli/web_dist/`` (per vite.config.ts
    outDir: "../sonic_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "sonic_cli" / "web_dist"
...
def cmd_update(args):
    ...
    """
    from sonic_cli.config import is_managed, managed_error
