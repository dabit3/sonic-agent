...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SONIC_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `sonic mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Sonic' own profile selector.
        """
        try:
    ...
    # after the subcommand (`sonic chat -p coder`), so keep scanning broadly.
    ...
    if profile_name is not None:
        try:
            ...
            sonic_home = _resolve_sudo_user_profile_env(profile_name)
            if not sonic_home:
...
# Provider model-selection wizard flows extracted to sonic_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (sonic_cli.main._model_flow_*) keep resolving unchanged.
from sonic_cli.model_setup_flows import (
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Sonic.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Sonic.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Sonic.exe: Access is
    ...
    an unrelated "Sonic" process is never touched. Best-effort: never raises.
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``sonic desktop --build-only`` → electron-builder ``--dir``).
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Sonic.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Sonic.exe, close any")
                    print("  running Sonic desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> sonic desktop --force-build")
    ...
    print(f"→ Launching packaged Sonic Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``sonic serve`` as
...
def _update_node_dependencies() -> None:
    ...
    # hung. The chatty npm-deprecation noise during `sonic update` comes from
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.sonic/logs/update.log`` only, never the terminal.

    During ``sonic update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _run_pre_update_backup(args) -> None:
    ...
    # ``sonic_cli/config.py``). Fall back to false if the key is missing
    ...
    try:
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            if build_result.returncode != 0:
                ...
                if tail:
                    print(tail)
                from sonic_constants import display_sonic_home as _dhh
        ...
        # don't have it. Tying the refresh to ``sonic update`` gives users a
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from sonic_cli.profiles import profile_exists, validate_alias_name
...
def cmd_dashboard(args):
    ...
    # launches the dashboard (`worker dashboard` → SONIC_HOME points into
    ...
    if (
        ...
        and os.environ.get("SONIC_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "sonic_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
...
def main():
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `sonic whatsapp` which sets up "
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Sonic shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from sonic_cli.pets import register_cli as _register_pets_cli
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: sonic computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
