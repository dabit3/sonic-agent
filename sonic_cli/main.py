...
Usage:
    sonic                     # Interactive chat (default)
    sonic chat                # Interactive chat
    sonic gateway             # Run gateway in foreground
    sonic gateway start       # Start gateway as service
    sonic gateway stop        # Stop gateway service
    sonic gateway status      # Show gateway status
    sonic gateway install     # Install gateway service
    sonic gateway uninstall   # Uninstall gateway service
    sonic setup               # Interactive setup wizard
    sonic logout              # Clear stored authentication
    sonic status              # Show status of all components
    sonic cron                # Manage cron jobs
    sonic cron list           # List cron jobs
    sonic cron status         # Check if cron scheduler is running
    sonic doctor              # Check configuration and dependencies
    sonic honcho setup                    # Configure Honcho AI memory integration
    sonic honcho status                   # Show Honcho config and connection status
    sonic honcho sessions                 # List directory → session name mappings
    sonic honcho map <name>               # Map current directory to a session name
    sonic honcho peer                     # Show peer names and dialectic settings
    sonic honcho peer --user NAME         # Set user peer name
    sonic honcho peer --ai NAME           # Set AI peer name
    sonic honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sonic honcho mode                     # Show current memory mode
    sonic honcho mode [hybrid|honcho|local]  # Set memory mode
    sonic honcho tokens                   # Show token budget settings
    sonic honcho tokens --context N       # Set session.context() token cap
    sonic honcho tokens --dialectic N     # Set dialectic result char cap
    sonic honcho identity                 # Show AI peer identity representation
    sonic honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sonic honcho migrate                  # Step-by-step migration guide: OpenClaw native → Sonic + Honcho
    sonic version             Show version
    sonic update              Update to latest version
    sonic uninstall           Uninstall Sonic Agent
    sonic acp                 Run as an ACP server for editor integration
    sonic sessions browse     Interactive session picker with search

    sonic claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: sonic_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``sonic_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``sonic update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``sonic_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, sonic crashes on import and the user can't run
# ``sonic update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
# that run *before* sonic_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SONIC_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".sonic", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``SONIC_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("SONIC_TUI") == "1" or "--tui" in argv:
        return True
...
def _print_fast_version_info() -> None:
    from sonic_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Sonic Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``sonic --version`` before config/logging imports on Termux."""
    if os.environ.get("SONIC_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to SONIC_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (sonic tools, sonic setup, sonic model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sonic {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any sonic module import.
#
# Many modules cache SONIC_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SONIC_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.sonic/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SONIC_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors sonic_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SONIC_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.sonic/profiles/coder or
    # /opt/data/profiles/coder).  If SONIC_HOME points to the sonic root
    # instead (e.g. systemd hardcodes SONIC_HOME=/root/.sonic), we must
    # still read active_profile — the user may have switched profiles via
    # `sonic profile use` and the gateway should honour that choice.
    # See issue #22502.
    sonic_home_env = os.environ.get("SONIC_HOME", "")
    if profile_name is None and sonic_home_env:
    ...
    if profile_name is None:
        try:
            ...
            active_path = get_default_sonic_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SONIC_HOME
    if profile_name is not None:
        try:
            from sonic_cli.profiles import resolve_profile_env

            sonic_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent sonic from starting
            print(
...
# Load .env from ~/.sonic/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from sonic_cli.config import get_sonic_home
from sonic_cli.env_loader import load_sonic_dotenv

load_sonic_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SONIC_REDACT_SECRETS env
# var BEFORE sonic_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_sonic_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "SONIC_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SONIC_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `sonic` subcommands
...
try:
    from sonic_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from sonic_constants import apply_ipv4_preference as _apply_ipv4
...
from sonic_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_sonic_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SONIC_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SONIC_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from sonic_cli.config import get_env_path, get_sonic_home, load_config
    from sonic_cli.auth import get_auth_status

    # Determine whether Sonic itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_sonic_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_sonic_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Sonic has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Sonic to use their tokens.
    if _has_sonic_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, sonic_bin
        cli_args: the original CLI arguments (everything after 'sonic')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    sonic_bin = container_info["sonic_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo sonic {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo sonic {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  sonic --tui --resume {target}")
    if title:
        print(f'  sonic --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sonic/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@sonic" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/sonic-ink/package.json",
    "packages/sonic-ink/index.js",
    "packages/sonic-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SONIC_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SONIC_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SONIC_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SONIC_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SONIC_HOME": sonic_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(sonic_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(sonic_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if sonic_cli_dir is None:
        sonic_cli_dir = Path(__file__).parent
    bundled = sonic_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SONIC_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SONIC_NODE")
    ...
    ext_dir = os.environ.get("SONIC_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SONIC_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SONIC_TUI_DIR (e.g. `unset SONIC_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @sonic/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @sonic/ink resolves through
        # packages/sonic-ink/dist/entry-exports.js. If that dist bundle is
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sonic_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="sonic-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SONIC_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["SONIC_PYTHON_SRC_ROOT"] = os.environ.get(
        "SONIC_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SONIC_PYTHON", sys.executable)
    env.setdefault("SONIC_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SONIC_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["SONIC_MODEL"] = model
        env["SONIC_INFERENCE_MODEL"] = model
    if provider:
        env["SONIC_TUI_PROVIDER"] = provider
        env["SONIC_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SONIC_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SONIC_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SONIC_TUI_SKILLS"] = value
    if query:
        env["SONIC_TUI_QUERY"] = query
    if image:
        env["SONIC_TUI_IMAGE"] = image
    if checkpoints:
        env["SONIC_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SONIC_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SONIC_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SONIC_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SONIC_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SONIC_ACCEPT_HOOKS"] = "1"
    ...
    # SONIC_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `sonic --tui` try to
    ...
    # still set SONIC_TUI_RESUME themselves.
    env.pop("SONIC_TUI_RESUME", None)
    if resume_session_id:
        env["SONIC_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `sonic update` so
    ...
    if code == 42:
        from sonic_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SONIC_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``sonic kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``sonic kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SONIC_KANBAN_BOARD"):
        return
    try:
        from sonic_cli.kanban_db import get_current_board

        os.environ["SONIC_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.sonic/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Sonic — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Sonic still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``SONIC_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("SONIC_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'sonic sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from sonic_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from sonic_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'sonic doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Sonic isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  sonic setup")
        print()

        from sonic_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'sonic setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from sonic_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["SONIC_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from sonic_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from sonic_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Sonic?")
        print()
    ...
    # has no creds.json.  Every subsequent `sonic gateway` then paid a 30s
    ...
    session_dir = get_sonic_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: sonic gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `sonic gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sonic gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Sonic Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sonic gateway")
        ...
        print()
        print("  Or install as a service: sonic gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'sonic whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from sonic_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from sonic_cli.config import stamp_install_method
    from sonic_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Sonic post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sonic model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("SONIC_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'sonic model' for "
                "available providers, or run 'sonic doctor' to diagnose config "
                "issues."
            )
    ...
    # a leftover OPENAI_BASE_URL in ~/.sonic/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sonic/.env if the active provider is not 'custom'.
    ...
    """
    from sonic_cli.config import get_env_value, save_env_value, load_config
...
#
# Sonic uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `sonic model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `sonic model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`sonic_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from sonic_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Sonic only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``sonic model`` flow, then route aux tasks to them here.
    """
    from sonic_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from sonic_cli.models import get_pricing_for_provider
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.sonic/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `sonic setup` from scratch. OpenRouter
    ...
    from sonic_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from sonic_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from sonic_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from sonic_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `sonic auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``sonic model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``sonic model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``sonic auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from sonic_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    from sonic_cli.config import get_env_value, load_config, save_config
    from sonic_cli.secret_prompt import masked_secret_prompt
    ...
    from sonic_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `sonic model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from sonic_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Sonic URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from sonic_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from sonic_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from sonic_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  Sonic will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Sonic will install it now (the preflight below "
...
# downstream call sites read `sonic_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `sonic` invocation — including
# fast paths like `sonic --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from sonic_cli.config import save_env_value, load_config, save_config
    from sonic_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from sonic_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from sonic_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from sonic_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Sonic turns to `copilot --acp`.")
    print("  Sonic currently starts its own ACP subprocess for each request.")
    print("  Sonic uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set SONIC_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``sonic setup`` / ``sonic model``.
    ...
    recover from a malformed paste without editing ``~/.sonic/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from sonic_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `sonic setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so sonic knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Sonic typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Sonic, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from sonic_cli.models import fetch_lmstudio_models
        ...
        from sonic_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from sonic_constants import display_sonic_home as _dhh_fn

            print(
                f"    Sonic will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'sonic model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Sonic CLI with a provider."""
    from sonic_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from sonic_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from sonic_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from sonic_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from sonic_cli.cron import cron_command
...
def cmd_slack(args):
    ...
    Dispatches ``sonic slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: sonic slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `sonic slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from sonic_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from sonic_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from sonic_cli.hooks import hooks_command
...
def cmd_security(args):
    """Dispatch `sonic security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from sonic_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from sonic_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from sonic_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from sonic_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Sonic home directory to a zip file."""
    if getattr(args, "quick", False):
        from sonic_cli.backup import run_quick_backup
...
def cmd_import(args):
    """Restore a Sonic backup from a zip file."""
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from sonic_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Sonic Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from sonic_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from sonic_cli.uninstall import run_gui_uninstall
    ...
    from sonic_cli.uninstall import run_uninstall
...
# Critical files that every ``sonic`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``sonic update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "sonic_cli/main.py",
    "sonic_cli/config.py",
    "sonic_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``sonic`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="sonic-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sonic update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``sonic_cli/web_dist/`` (per vite.config.ts
    outDir: "../sonic_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "sonic_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``sonic update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sonic web``).
    ...
    # build path usable on a stock `py -m sonic_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (sonic web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Sonic.app/Contents/MacOS/Sonic"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Sonic.exe",
            release_dir / "win-ia32-unpacked" / "Sonic.exe",
            release_dir / "win-arm64-unpacked" / "Sonic.exe",
        ]
    else:
        candidates = [
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Sonic is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Sonic.app/Contents/MacOS/Sonic  ->  app bundle = .../Sonic.app
    app = exe.parents[2]
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from sonic_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "ignore_existing", False):
        env["SONIC_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "sonic_root", None):
        env["SONIC_DESKTOP_SONIC_ROOT"] = str(Path(args.sonic_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["SONIC_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  sonic gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Sonic is
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``sonic dashboard`` processes other than ourselves.

    ``sonic dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``sonic update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Sonic Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``sonic dashboard`` as
    ...
    environment variable ``SONIC_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "sonic dashboard",
        "sonic_cli.main dashboard",
        "sonic_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "sonic.*dashboard"`) keeps us consistent
            # with `sonic_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `sonic update`.
    ...
    print("  Preview now:  sonic curator run --dry-run")
    print("  Pause it:     sonic curator pause")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``sonic update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``sonic update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: sonic curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``sonic dashboard`` processes.

    Called at the end of ``sonic update`` (default ``reason``) and also
    from ``sonic dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Sonic Desktop Electron app spawns this dashboard as a
    # backend child, it sets SONIC_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SONIC_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    sonic dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Sonic Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/dabit3/sonic-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="sonic-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"sonic-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to sonic-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sonic-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "sonic-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Sonic behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes sonic completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Sonic couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Sonic couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Sonic behaves unexpectedly.")
    return True
...
# =========================================================================
# Fork detection and upstream management for `sonic update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/dabit3/sonic-agent.git",
    "git@github.com:dabit3/sonic-agent.git",
    "https://github.com/dabit3/sonic-agent",
    "git@github.com:dabit3/sonic-agent",
}
OFFICIAL_REPO_URL = "https://github.com/dabit3/sonic-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sonic_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sonic_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Sonic repository.")
        print("  This means you may miss updates from dabit3/sonic-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/dabit3/sonic-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/dabit3/sonic-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``sonic update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from sonic_constants import get_default_sonic_root

    default_home = get_default_sonic_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``sonic update`` is still progressing even if pip/uv itself is silent.
    """
...
def _sonic_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "sonic.exe",
        scripts_dir / "sonic-gateway.exe",
    ]


def _detect_concurrent_sonic_instances(
    ...
    The Sonic Desktop Electron app spawns ``sonic.EXE`` as a backend child,
    so during ``sonic update`` the user-invoked process and the desktop's
    ...
    shims (``sonic.exe`` / ``sonic-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``sonic update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``sonic.exe``) is a separate process from the Python
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another sonic.exe is running:"]
...
def _quarantine_running_sonic_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``sonic.exe``.
    ...
    when ``sonic update`` runs, ``sonic.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``sonic.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next sonic invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Sonic Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Sonic Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _sonic_exe_shims(scripts_dir):
        ...
        print(
            "    Close Sonic Desktop, exit other `sonic` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `sonic update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_sonic_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``sonic.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``sonic.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``sonic`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_sonic_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``sonic.exe.old.*`` left by prior updates.

    Called early on every sonic invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``sonic update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `sonic update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``sonic.exe`` / ``sonic-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_sonic_exe`` for the rationale.
    """
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``sonic.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``sonic`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
...
class _UpdateOutputStream:
    """Stream wrapper used during ``sonic update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.sonic/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``sonic update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``sonic update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sonic/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``sonic update --gateway``) the update is already
    ...
    try:
        ...
        # sonic_cli.config.get_sonic_home to simulate setup failure.
        from sonic_cli.config import get_sonic_home as _get_sonic_home

        logs_dir = _get_sonic_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``sonic update --check``: fetch and report without installing.
    ...
    if method == "pip":
        from sonic_cli.config import recommended_update_command
        from sonic_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``sonic update`` without requiring a reinstall.  The
    ...
    ``sonic`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/sonic.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v sonic'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/sonic, code at /usr/local/lib/sonic-agent).
    fhs_link = Path("/usr/local/bin/sonic")
    ...
    path_comment = (
        "# Sonic Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SONIC_HOME before running the update.
    ...
    SONIC_HOME directories.  The ``--backup`` flag on ``sonic update``
    ...
    # Render path using display_sonic_home so the user sees ~/.sonic/...
    try:
        from sonic_constants import get_sonic_home, display_sonic_home

        home = get_sonic_home()
        try:
            display_path = f"{display_sonic_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  sonic import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``sonic update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Sonic Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Sonic Agent")
        return
...
def _cmd_update_pip(args):
    ...
    print("✓ Update complete! Restart sonic to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Sonic Agent...")
    print()

    # On Windows, abort early if another sonic.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_sonic_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $SONIC_HOME, this is cheap
        ...
        # orphan merge-conflict markers in sonic_cli/config.py bricked
        # every user who ran ``sonic update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``sonic
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``sonic update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_sonic_home added to sonic_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "sonic_cli.main", "desktop", "--build-only"]
        ...
        # modules in this process.  Reload sonic_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_sonic_home() added since the last release.
        try:
            import importlib
            import sonic_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SONIC_HOME so
        # it is not affected by sync_skills()'s module-level SONIC_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SONIC_HOME env var points at the default or a named profile.
        try:
        ...
        if version_bump_only:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: sonic config migrate")
            else:
                print()
                print("Skipped. Run 'sonic config migrate' later to configure.")
        else:
        ...
        # check `sonic curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``sonic update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from sonic_cli.tools_config import install_cua_driver
        ...
        # When running as ``sonic update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_sonic_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all sonic-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. sonic-gateway.service or sonic-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: sonic gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: sonic -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Sonic gateway unit files are still installed.
        # When both sonic.service (from a pre-rename install) and the
        # current sonic-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `sonic update` surfaces the issue until the user migrates.
        try:
            from sonic_cli.gateway import (
                has_legacy_sonic_units,
                _find_legacy_sonic_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_sonic_units():
                print()
                print("⚠ Legacy Sonic gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_sonic_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (sonic.service) fight the current")
                print("  sonic-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    sonic gateway migrate-legacy")
                print()
        ...
        print("  sonic model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sonic -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `sonic profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_sonic_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → sonic -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.sonic)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  sonic profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     sonic -p {name} chat")
                else:
        ...
        from sonic_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from sonic_constants import get_sonic_home as _hh
        ...
        from sonic_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `sonic profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → sonic -p {name}  ({wrapper})")
        print()
        ...
        from sonic_cli.profiles import profile_exists
        ...
        from sonic_cli.profiles import rename_profile
        ...
        from sonic_cli.profiles import export_profile
        ...
        from sonic_cli.profiles import import_profile
        ...
        import tempfile
        from sonic_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sonic_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  sonic -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      sonic -p {plan.manifest.name} chat")
        ...
        from sonic_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `sonic profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"sonic -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from sonic_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("sonic_requires"):
            print(f"Requires:     Sonic {data['sonic_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from sonic_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.sonic_requires:
        print(f"  Requires: Sonic {mf.sonic_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``sonic dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``sonic dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No sonic dashboard processes running.")
        return 0

    print(f"{len(pids)} sonic dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No sonic dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `sonic update`.
    ...
    # the same logs directory as every other Sonic surface.
    try:
        from sonic_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "SONIC_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SONIC_WEB_DIST"])
            if "SONIC_WEB_DIST" in os.environ
            else PROJECT_ROOT / "sonic_cli" / "web_dist"
        )
    ...
    from sonic_cli.web_server import start_server
...
# so that in ``sonic -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``sonic_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``sonic -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``sonic`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from sonic_cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SONIC_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SONIC_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SONIC_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `sonic --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for sonic CLI."""
    # Cosmetic: make the process show up as 'sonic' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from sonic_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``sonic.exe.old.*`` quarantine files left by previous
    # ``sonic update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_sonic_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: sonic-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from sonic_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `sonic model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.sonic/.env.  Currently supports "
    ...
    from sonic_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from sonic_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy sonic.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Sonic gateway unit files "
            "(e.g. sonic.service) left over from older installs. Profile "
            "units (sonic-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `sonic proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Sonic Agent with an interactive wizard. "
        "Run a specific section: sonic setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'sonic setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Sonic.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $SONIC_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Sonic")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from sonic_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Sonic CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: sonic-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Sonic with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set SONIC_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Sonic Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.sonic/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Sonic profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.sonic/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Sonic profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from sonic_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from sonic_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.sonic/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.sonic/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Sonic Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `sonic "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the Sonic "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.sonic/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Sonic setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Sonic Agent. Use 'sonic debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sonic debug share              Upload debug report and print URL
    sonic debug share --lines 500  Include more log lines
    sonic debug share --expire 30  Keep paste for 30 days
    sonic debug share --local      Print report locally (no upload)
    sonic debug share --no-redact  Disable upload-time secret redaction
    sonic debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sonic debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Sonic home directory to a zip file",
        description="Create a zip archive of your entire Sonic configuration, "
        "skills, sessions, and data (excludes the sonic-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/sonic-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.sonic/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo sonic uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from sonic_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Sonic backup from a zip file",
        description="Extract a previously created Sonic backup into your "
        "Sonic home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Sonic Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from sonic_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.sonic/skills/.bundled_manifest) "
            "so future 'sonic update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`sonic update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `sonic update`. Pass --sync to re-seed now."
        ),
    )
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from sonic_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from sonic_cli.skills_hub import skills_command
    ...
    from sonic_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/sonic-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `sonic plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from sonic_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``sonic --help``, ``sonic version``, ``sonic logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from sonic_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from sonic_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_sonic_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_sonic_home()}/memories/\n")
        else:
            from sonic_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'sonic tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # sonic tools disable <name...> [--platform cli]
    ...
    # sonic tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from sonic_cli.tools_config import tools_disable_enable_command
            ...
            _require_tty("tools")
            from sonic_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `sonic computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `sonic tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: sonic computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: sonic computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Sonic as an MCP server",
        description=(
            "Manage MCP server connections and run Sonic as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'sonic mcp add' to connect to a new server, or\n"
            "'sonic mcp serve' to expose Sonic conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Sonic as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `sonic mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from sonic_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch sonic --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Sonic",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Sonic",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.sonic/ (by default a "
        "single restore-point archive is written to ~/.sonic/backups/ "
        "before apply; restorable with 'sonic import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from sonic_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Sonic Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'sonic config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another sonic.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Sonic Agent",
        description="Remove Sonic Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Sonic Agent as an ACP (Agent Client Protocol) server",
        description="Start Sonic Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Sonic ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    def cmd_acp(args):
        """Launch Sonic Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Sonic instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `sonic update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `sonic profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Sonic profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Sonic Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `sonic dashboard` cmdlines and SIGTERM them directly — the same
    # path `sonic update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running sonic dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running sonic dashboard processes and exit",
    )
    # Backward-compat shim: older Sonic desktop app shells (<= 0.15.x) spawn the
    # backend as `sonic dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Sonic couldn't start" with no actionable cause. Accept and silently
    ...
    # for one release. The Sonic-Setup.exe success screen tells users to
    # run `sonic desktop` from a terminal, so the canonical name needs
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Sonic Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any sonic CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--sonic-root",
        help="Override the Sonic source root used by Desktop (sets SONIC_DESKTOP_SONIC_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets SONIC_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
...
""",
    ...
    # e.g. ``sonic -c Pokemon Agent Dev`` → ``sonic -c 'Pokemon Agent Dev'``
    ...
    from sonic_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'sonic -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (sonic hooks list, cron
