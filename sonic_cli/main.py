...
def _clean_managed_worktree(git_cmd: list[str], cwd: Path) -> bool:
    ...
    On a managed install (%LOCALAPPDATA%\\sonic\\sonic-agent or
    ~/.sonic/sonic-agent) the user never edits the source tree, so any
...
def _wait_for_interpreter_venv_ready(*, timeout: float = 15.0) -> bool:
    ...
    During ``sonic update`` the managed-uv path can rebuild the project venv
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → sonic -p {p.name}")
                break
        print()
        return
...
def cmd_dashboard(args):
    ...
    embedded_chat = args.tui or os.environ.get("SONIC_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
""",
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `sonic --tui` via PTY/WebSocket). "
            "Alternatively set SONIC_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
