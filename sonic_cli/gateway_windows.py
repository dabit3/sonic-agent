...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m sonic_cli.main gateway run --replace`` invocation) and
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from sonic_cli.config import get_sonic_home
        print(f"    type {Path(get_sonic_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_sonic_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from sonic_cli.config import get_sonic_home

    sonic_home = Path(get_sonic_home()).resolve()
    print()
    print("Next steps:")
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from sonic_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
