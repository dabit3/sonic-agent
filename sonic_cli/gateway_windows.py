...
_TASK_NAME_DEFAULT = "Sonic_Gateway"
_TASK_DESCRIPTION = "Sonic Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Sonic profile."""
    from sonic_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "sonic_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("SONIC_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("SONIC_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("SONIC_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["SONIC_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["SONIC_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["SONIC_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("SONIC_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("SONIC_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("SONIC_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Sonic_Gateway``
    Named profile X: ``Sonic_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during sonic_cli boot.
    from sonic_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\sonic\\gateway-service\\<task_name>.cmd``
    (or ``<SONIC_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Sonic installs stay self-contained).
    """
    _assert_windows()
    from sonic_cli.config import get_sonic_home

    script_dir = Path(get_sonic_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``SONIC_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SONIC_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m sonic_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "SONIC_HOME={sonic_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "SONIC_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports sonic_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_sonic_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_sonic_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_sonic_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [pythonw_path, "-m", "sonic_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('SONIC_HOME')}) = {_quote_vbs_string(sonic_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('SONIC_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_sonic_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    content = _build_gateway_cmd_script(python_path, working_dir, sonic_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, sonic_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Sonic builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "sonic_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "VIRTUAL_ENV": _preserve_sonic_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m sonic_cli.main gateway run``
    ...
    log_dir = Path(get_sonic_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("SONIC_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("SONIC_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from sonic_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `sonic -p <profile> gateway install` must be safe.
    ...
    from sonic_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"sonic {profile_arg} gateway start" if profile_arg else "sonic gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: sonic gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Sonic gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: sonic gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: sonic gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from sonic_cli.config import get_sonic_home
        print(f"    type {Path(get_sonic_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_sonic_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from sonic_cli.config import get_sonic_home

    sonic_home = Path(get_sonic_home())
    print()
    print("Next steps:")
    print("  sonic gateway status                      # Check status")
    print(f"  type {sonic_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Sonic gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: sonic gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: sonic gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: sonic gateway start")
            return
