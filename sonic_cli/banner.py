...
Pure display functions with no SonicCLI state dependency.
"""
...
from sonic_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built sonic — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/dabit3/sonic-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/dabit3/sonic-agent"
...
def _fetch_pypi_latest(package: str = "sonic-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Sonic update is available.

    Two paths: if ``SONIC_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = sonic_home / ".update_check"
    embedded_rev = os.environ.get("SONIC_REVISION") or None
    ...
    if embedded_rev:
        ...
        # $SONIC_HOME/sonic-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = sonic_home / "sonic-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Sonic git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SONIC_HOME/sonic-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``sonic_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from sonic_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/dabit3/sonic-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Sonic checkout. Cached per-process. Release URL always points at the
    canonical dabit3/sonic-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Sonic Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SONIC_BOLT
    except Exception:
        _bskin = None
        _hero = SONIC_BOLT
    left_lines = ["", _hero, ""]
    ...
    try:
        from sonic_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from sonic_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built sonic; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SONIC_AGENT_LOGO
        console.print(_logo)
        console.print()
