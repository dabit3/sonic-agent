"""
Status command for sonic CLI.

Shows the status of all Sonic Agent components.
"""
...
from sonic_cli.auth import AuthError, resolve_provider
from sonic_cli.colors import Colors, color
from sonic_cli.config import get_env_path, get_env_value, get_sonic_home, load_config
from sonic_cli.models import provider_label
from sonic_cli.nous_subscription import get_nous_subscription_features
from sonic_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``sonic config``'s
...
def show_status(args):
    """Show status of all Sonic Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Sonic Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: sonic auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: sonic model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: sonic auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: sonic auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: sonic model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from sonic_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'sonic-agent[vercel]')"
    ...
    try:
        from sonic_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   sonic gateway")
    ...
    jobs_file = get_sonic_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_sonic_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'sonic doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'sonic setup' to configure", Colors.DIM))
    print()
