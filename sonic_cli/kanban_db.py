...
``<root>`` is the **shared Sonic root** (the parent of any active
...
``sonic -p <profile>`` joins the same board as the dispatcher that
...
the profile's Sonic home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``SONIC_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``SONIC_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``sonic kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.sonic``. In Docker / custom
deployments where ``SONIC_HOME`` points outside ``~/.sonic`` (e.g.
``/opt/sonic``), ``<root>`` is ``SONIC_HOME``. Legacy env-var
overrides still work:

* ``SONIC_KANBAN_DB`` — pin the database file path directly.
* ``SONIC_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``SONIC_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``SONIC_KANBAN_DB``,
``SONIC_KANBAN_WORKSPACES_ROOT``, and ``SONIC_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/sonic-kanban-v1-spec.pdf`` for the full
...
# ``SONIC_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``SONIC_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("SONIC_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``SONIC_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "SONIC_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``sonic-agent``
...
def kanban_home() -> Path:
    """Return the shared Sonic root that anchors the kanban board.

    Resolution order:

    1. ``SONIC_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_sonic_root()``, which already returns ``<root>``
       when ``SONIC_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SONIC_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SONIC_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("SONIC_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``sonic kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``SONIC_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``sonic kanban boards
    ...
    env = os.environ.get("SONIC_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``sonic kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``SONIC_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("SONIC_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``SONIC_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``SONIC_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("SONIC_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``sonic kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``SONIC_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``SONIC_SESSION_ID``. NULL
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Sonic treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``SONIC_KANBAN_DB`` env → ``SONIC_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See sonic_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``sonic kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``SONIC_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from sonic_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``sonic --skills ...`` alongside the built-in
    ...
    # `sonic --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``sonic kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `sonic kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``sonic kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``sonic kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``sonic kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``sonic kanban complete <id>``) works without requiring
...
#      via ``sonic kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Sonic checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Sonic treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Sonic creates/reuses that linked worktree. With
      no ``workspace_path``, Sonic anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Sonic uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``SONIC_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Sonic
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``sonic kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``sonic kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Sonic profile.
    ...
    if not rows:
        return False
    try:
        from sonic_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from sonic_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Sonic profile.
        # `_default_spawn` invokes ``sonic -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Sonic
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `sonic kanban tail` — without
...
def _module_sonic_argv() -> list[str]:
    """Return the interpreter-bound Sonic CLI invocation."""
    # ``sonic_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``sonic`` package — there is no
    # ``sonic`` package to import.
    return [sys.executable, "-m", "sonic_cli.main"]


def _absolute_sonic_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Sonic shim."""
...
def _sonic_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Sonic executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_sonic_argv()
    return [_absolute_sonic_path(path)]


def _resolve_sonic_argv() -> list[str]:
    """Resolve the ``sonic`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$SONIC_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("sonic")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\sonic.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m sonic_cli.main`` — fallback for setups where
       Sonic is launched from a venv and the ``sonic`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_sonic_bin`` for the same reason. Kept
    local (not imported from gateway) because ``sonic_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("SONIC_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _sonic_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _sonic_path_argv(resolved_env_bin)
        return _module_sonic_argv()

    sonic_bin = _safe_which_no_cwd("sonic") if _IS_WINDOWS else shutil.which("sonic")
    if sonic_bin:
        return _sonic_path_argv(sonic_bin)
    return _module_sonic_argv()


def _kanban_worker_skill_available(sonic_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``sonic -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset SONIC_HOME means the worker falls back to the default root
    # home (``~/.sonic``), which ships the bundled skill.
    base = _Path(sonic_home) if sonic_home else (_Path.home() / ".sonic")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``sonic -p <profile> chat -q ...`` subprocess.
    ...
    ``SONIC_KANBAN_DB`` / ``SONIC_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject SONIC_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `sonic -p <name>` the
    # _apply_profile_override() runs *before* sonic_constants is imported.
    # If SONIC_HOME is absent from the child's env, get_sonic_home() falls
    # back to Path.home() / ".sonic" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from sonic_cli.profiles import resolve_profile_env
    try:
        env["SONIC_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via SONIC_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SONIC_HOME never had profiles created.
        pass
    if task.tenant:
        env["SONIC_TENANT"] = task.tenant
    env["SONIC_KANBAN_TASK"] = task.id
    env["SONIC_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["SONIC_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["SONIC_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SONIC_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["SONIC_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["SONIC_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`sonic -p <name>`
    # rewrites SONIC_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_sonic_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["SONIC_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["SONIC_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["SONIC_KANBAN_BOARD"] = resolved_board
    # SONIC_PROFILE is the author the kanban_comment tool defaults to.
    # `sonic -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["SONIC_PROFILE"] = profile_arg

    cmd = [
        *_resolve_sonic_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped SONIC_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("SONIC_HOME")):
    ...
    # `sonic kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`sonic` executable not found on PATH. "
            "Install Sonic Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``sonic kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled SONIC_PROFILE values like "sonic-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Sonic root exists

    Reads profile paths directly so this module has no import dependency on
    ``sonic_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from sonic_constants import get_default_sonic_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``sonic kanban assignees`` for the terminal.
