"""
Dump command for sonic CLI.

Outputs a compact, plain-text summary of the user's Sonic setup
...
from sonic_cli.config import get_sonic_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.sonic_build_sha`` by the Dockerfile's
    ``SONIC_GIT_SHA`` build-arg (see ``sonic_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``sonic dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from sonic_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(sonic_home: Path) -> int:
...
def _cron_summary(sonic_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = sonic_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"sonic_home:      {display_sonic_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `sonic auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `sonic auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["sonic-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(sonic_home)}")
    lines.append(f"  skills:             {_count_skills(sonic_home)}")
