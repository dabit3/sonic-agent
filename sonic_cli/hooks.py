"""sonic hooks — inspect and manage shell-script hooks.

Usage::

    sonic hooks list
    sonic hooks test <event> [--for-tool X] [--payload-file F]
    sonic hooks revoke <command>
    sonic hooks doctor

Consent records live under ``~/.sonic/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.sonic/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``sonic hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: sonic hooks {list|test|revoke|doctor}")
        print("Run 'sonic hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.sonic/config.yaml.")
        print("See `sonic hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `sonic hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `sonic hooks test` and `sonic hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Sonic wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `sonic hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `sonic hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `sonic hooks doctor`.")
