...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with sonic-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.sonic/plugins/<name>/``
3. **Project plugins** – ``./.sonic/plugins/<name>/`` (opt-in via
   ``SONIC_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``sonic_agent.plugins``
   entry-point group.
...
from sonic_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``SONIC_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SONIC_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SONIC_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.sonic/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SONIC_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SONIC_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Sonic loggers.
    """
    ...
    logger.debug(
        "SONIC_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Sonic' shipped guidance lives in the evidence-based
...
ENTRY_POINTS_GROUP = "sonic_agent.plugins"

_NS_PARENT = "sonic_plugins"
...
class PluginManifest:
    ...
    #              in ~/.sonic/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``sonic plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``sonic honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``sonic <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`sonic_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from sonic_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``sonic model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: sonic_cli.main imports plugins indirectly
        from sonic_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.sonic/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to sonic_cli/. Two layouts are
        ...
        # 2. User plugins (~/.sonic/plugins/)
        user_dir = get_sonic_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.sonic/plugins/)
        if _env_enabled("SONIC_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".sonic" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SONIC_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with sonic and must
            ...
            # all ~20 of them added several seconds to every `sonic`
            # invocation — including plain `sonic chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Sonic ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `sonic plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``sonic plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`sonic plugins
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``sonic_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``sonic_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="sonic-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``sonic tools`` TUI so plugin-provided toolsets appear
