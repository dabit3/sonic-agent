...
Used by ``sonic kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``sonic_cli/goals.py`` — same aux
...
_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Sonic Agent board.
...
def _profile_author() -> str:
    """Mirror of ``sonic_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("SONIC_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=SONIC_KANBAN_SPECIFY_MAX_TOKENS,
