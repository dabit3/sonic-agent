"""``sonic dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.sonic/.env``
as ``SONIC_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.sonic/auth.json``), refreshing it if needed. Fails fast with a
     "run `sonic setup`" hint when the user isn't logged in.
  ...
  3. Writes ``SONIC_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``SONIC_DASHBOARD_PORTAL_URL`` into ``~/.sonic/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from sonic_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `sonic auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from sonic_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    SONIC_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    SONIC_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    SONIC_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `sonic dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    sonic dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from sonic_cli.auth import AuthError, resolve_nous_access_token
    from sonic_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets SONIC_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `sonic dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `sonic setup` (or `sonic auth login nous`) first, then retry.")
        else:
    ...
    try:
        save_env_value("SONIC_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write SONIC_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  SONIC_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)
    ...
    try:
        existing_portal = get_env_value("SONIC_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("SONIC_DASHBOARD_PORTAL_URL", portal_base_url)
