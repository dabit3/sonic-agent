...
``$SONIC_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-sonic-setup
(the stage2 hook) has chowned the volume and seeded $SONIC_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$SONIC_HOME``,
    not under ``profiles/``). The dispatcher in ``sonic_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``sonic gateway start`` (no ``-p``) targets. Without it,
    bare ``sonic gateway start`` inside the container would land on
    ...
    ``$SONIC_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        sonic_home: The container's SONIC_HOME (typically /opt/data).
            Profiles live under ``<sonic_home>/profiles/<name>/``;
            the default profile lives at ``<sonic_home>`` itself.
    ...
    default_prior_state = legacy_default_state or _read_desired_state(sonic_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(sonic_home)
    ...
    profiles_root = sonic_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `sonic profile create` (config.yaml
            # is not — that comes later via `sonic setup`). Use it as the
            ...
            # via ``sonic -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(sonic_home, actions)
    return actions
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the sonic args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/sonic/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/sonic/docker/main-wrapper.sh
    ...
    ``sonic`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``sonic <subcommand>``, so an
    explicit leading ``sonic`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `sonic <subcommand>`; peel an explicit sonic.
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `sonic -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit sonic
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the sonic user) won't hit EACCES. See
...
def _write_reconcile_log(
    sonic_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $SONIC_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted SONIC_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        sonic_home=sonic_home, scandir=scandir,
    )
    for a in actions:
        print(
