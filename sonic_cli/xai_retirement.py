...
Pure logic: walks a Sonic config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `sonic doctor` and a future `sonic migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Sonic config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Sonic config and return retirement issues.
