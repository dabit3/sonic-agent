"""``sonic portal`` — the human-readable entry point for Nous Portal.

Running ``sonic portal`` with no subcommand performs the one-shot Portal
...
``sonic auth add nous --type oauth`` (which still works), is identical to
``sonic setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``sonic auth`` or ``sonic tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `sonic model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `sonic portal`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    ...
    print("Run `sonic portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `sonic portal` on the given argparse subparsers object."""
