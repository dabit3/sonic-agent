"""``sonic portal`` — small CLI surface for Nous Portal users.
...
already in ``sonic auth`` or ``sonic tools``. It's a discovery + status
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   sonic auth add nous --type oauth")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `sonic auth add nous --type oauth`.", Colors.YELLOW))
        print()
