...
the tool registry or launching Sonic (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``sonic-cli`` to the non-existent ``sonic``. ``resolve_toolset('sonic')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``sonic-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``sonic`` instead of ``sonic-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `sonic tools` to reconfigure."
        )
    return warnings
