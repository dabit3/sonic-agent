"""``sonic dashboard`` / ``sonic serve`` subcommand parsers.
...
``sonic_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `sonic dashboard` / `sonic serve` cmdlines and SIGTERM them directly —
    # the same path `sonic update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Sonic web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Sonic web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    # browser. It exists so the Sonic Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Sonic backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Sonic backend server — the JSON-RPC/WebSocket gateway the "
