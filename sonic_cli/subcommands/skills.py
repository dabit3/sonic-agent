"""``sonic skills`` subcommand parser.

Extracted from ``sonic_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `sonic update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `sonic update` reports as user-modified and skips. "
            "Use `sonic skills diff <name>` to see changes and `sonic skills reset "
