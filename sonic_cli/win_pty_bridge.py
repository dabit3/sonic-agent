"""Windows ConPTY bridge for the `sonic dashboard` chat tab.

Drop-in counterpart to ``sonic_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``sonic_cli.web_server`` consumes: ``spawn``, ``read``,
