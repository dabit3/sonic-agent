...
Config files are stored in ~/.sonic/ for easy access:
- ~/.sonic/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.sonic/.env         - API keys and secrets

This module provides:
- sonic config          - Show current configuration
- sonic config edit     - Open config in editor
- sonic config set      - Set a specific value
- sonic config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``sonic config set`` (which rewrites
    ...
    ``config.yaml`` in place: sonic never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.sonic/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``sonic logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  sonic config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Sonic makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Sonic itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``sonic update``, the TUI build.
...
#   on every plugin install / ``sonic update``.
...
# * ``SONIC_HOME`` / ``SONIC_PROFILE`` / ``SONIC_CONFIG`` /
#   ``SONIC_ENV`` — Sonic runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Sonic runtime location — never via dashboard env writer.
    # NOT a SONIC_* blanket: integration credentials (SONIC_GEMINI_*,
    # SONIC_LANGFUSE_*, SONIC_SPOTIFY_*, ...) ARE allowed.
    "SONIC_HOME", "SONIC_PROFILE", "SONIC_CONFIG", "SONIC_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Sonic runtime location "
            "(SONIC_HOME, SONIC_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SONIC_TOOL_PROGRESS", "SONIC_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `sonic plugins enable
    # observability/langfuse` or `sonic tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SONIC_LANGFUSE_ENV",
    "SONIC_LANGFUSE_RELEASE",
    "SONIC_LANGFUSE_SAMPLE_RATE",
    "SONIC_LANGFUSE_MAX_CHARS",
    "SONIC_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from sonic_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SONIC_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_sonic_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Sonic is running in package-manager-managed mode.

    Two signals: the SONIC_MANAGED env var (set by the systemd service),
    or a .managed marker file in SONIC_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``sonic_cli/`` — i.e. the git checkout for source
    installs, ``/opt/sonic`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$SONIC_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Sonic was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$SONIC_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SONIC_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.sonic``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SONIC_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.sonic:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SONIC_HOME``, a
    ...
    ``docker`` and ``sonic update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SONIC_HOME.
    try:
    ...
    #    host install whose shared $SONIC_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``sonic update``.
    try:
        method = (
            (get_sonic_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``sonic_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SONIC_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/sonic`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        return "pip install --upgrade sonic-agent"
    return "sonic update"
...
# Long-form text for ``sonic update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# sonic_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Sonic, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``sonic update`` inside Docker.
...
def format_managed_message(action: str = "modify this Sonic installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SONIC_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Sonic installation is managed by NixOS "
            f"(SONIC_MANAGED={env_hint}).\n"
            "Edit services.sonic-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Sonic installation is managed by Homebrew "
            f"(SONIC_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade sonic-agent"
        )

    return (
        f"Cannot {action}: this Sonic installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Sonic."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SONIC_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, sonic_bin
    or None if container mode is not active, we're already inside the
    container, or SONIC_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SONIC_DEV") == "1":
        return None
    ...
    container_mode_file = get_sonic_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "sonic-agent")
    exec_user = info.get("exec_user", "sonic")
    sonic_bin = info.get("sonic_bin", "/data/current-package/bin/sonic")
...
# Re-export from sonic_constants — canonical definition lives there.
from sonic_constants import get_sonic_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_sonic_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_sonic_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_sonic_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SONIC_UID / SONIC_GID env vars set by Docker deployments.

    Docker containers running Sonic commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SONIC_HOME once, but
    subdirectories created at runtime by ``ensure_sonic_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SONIC_UID", "").strip()
    gid_str = os.environ.get("SONIC_GID", "").strip()
    try:
...
def _chown_to_sonic_uid(path) -> None:
    """Chown ``path`` to ``SONIC_UID:SONIC_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_sonic_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_sonic_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the sonic group can
    share state with the gateway service.

    The mode can be overridden via the SONIC_HOME_MODE environment variable
    (e.g. SONIC_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SONIC_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SONIC_UID``/``SONIC_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SONIC_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_sonic_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Sonic runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SONIC_CONTAINER") or os.environ.get("SONIC_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SONIC_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SONIC_HOME, upgrading legacy empty templates.
...
def ensure_sonic_home():
    """Ensure ~/.sonic directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_sonic_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_sonic_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SONIC_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Sonic-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Sonic
        ...
        # identity slot (SOUL.md). Empty by default. The SONIC_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Sonic adds a coding operating brief
    ...
    "terminal": {
        ...
        # skipped. When empty, Sonic auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Sonic sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Sonic
        ...
        # bundled Sonic image, which drops to the `sonic` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Sonic sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Sonic should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``sonic chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.sonic/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: sonic sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .sonic.md, or .cursorrules before Sonic applies
    ...
    # single read_file page exceeds these limits, Sonic truncates the
    ...
    # See tools/kanban_tools.py and sonic_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``sonic dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``sonic kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``sonic kanban decompose`` and the kanban
        ...
        # ``sonic profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `sonic model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `sonic` (and `sonic chat`) launches by default:
        ...
        # REPL and `--tui` (or SONIC_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `sonic --tui` auto-resumes the most recent human-
        ...
        # e.g. `model · 68% · ~/projects/sonic`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `sonic pets`. Disabled until
        ...
        "pet": {
            ...
            # get_sonic_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # (``SONIC_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SONIC_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Sonic appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sonic/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sonic/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.sonic/cache/delegation/
    ...
    # response. If not, Sonic feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Sonic auto-pauses the goal and
    ...
    # always goes to ~/.sonic/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SONIC_SKILL_DIR} and ${SONIC_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `sonic curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `sonic curator run --consolidate` overrides
        ...
        # normally restored on every `sonic update`, so pruning them only
        ...
        # skipped), snapshot ~/.sonic/skills/ into
        # ~/.sonic/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `sonic curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for sonic-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Sonic Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SONIC_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Sonic' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.sonic/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SONIC_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `sonic doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``sonic_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Sonic to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SONIC_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via SONIC_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `sonic -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `sonic` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `sonic kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     sonic-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Sonic tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.sonic/logs/.
    ...
    # update model picker lists without shipping a sonic-agent release.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.sonic/.env,
        ...
        # behavior: files must live under the Sonic cache, under
        ...
        # user. Bridged to SONIC_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Sonic
        # cache (~/.sonic/cache/{documents,images,audio,video,screenshots})
        ...
        # to SONIC_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SONIC_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SONIC_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.sonic/state.db.
    ...
    "sessions": {
        ...
        # default of ``sonic sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.sonic/sessions/session_{sid}.json`` on every turn
    ...
    # ``sonic update`` behaviour.
    "updates": {
        # Run a full ``sonic backup``-style zip of SONIC_HOME before every
        # ``sonic update``.  Backups land in ``<SONIC_HOME>/backups/`` and
        # can be restored with ``sonic import <path>``.  Off by default:
        # zipping a large SONIC_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``sonic update
        ...
        "backup_keep": 5,
        # What `sonic update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<SONIC_HOME>/lsp/bin/`` on first use.
    ...
    # `sonic tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.sonic/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.sonic/.env (or your shell) and never in config.yaml.
            ...
            # ~/.sonic/bin/ on first use.  When False you must install
    ...
    # ── Speed profile ───────────────────────────────────────────────────────
    # Sonic is a latency-first agent. When enabled (the default), the speed
    ...
    #     startup so the first request starts streaming immediately
    # Set speed.enabled: false to restore stock upstream behaviour.
    "speed": {
        "enabled": True,
    ...
    "computer_use": {
        ...
        # by default upstream. Sonic disables it for our users unless they
        # explicitly opt in here. When false (default), Sonic sets
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sonic model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Sonic auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @sonic:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SONIC_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #sonic)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: sonic-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sonic-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Sonic API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Sonic API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Sonic API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SONIC_TOOL_PROGRESS and SONIC_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.sonic.config`` entries, then checks
    ...
    try:
        ...
        # should never break `sonic update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Sonic versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sonic_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sonic setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Sonic won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sonic doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SONIC_HOME", "~/.sonic")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SONIC_TOOL_PROGRESS")
            old_mode = get_env_value("SONIC_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SONIC_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SONIC_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SONIC_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SONIC_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SONIC_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `sonic plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `sonic curator status` has no
    ...
    #   3. Creates `~/.sonic/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_sonic_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_sonic_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: sonic config set <key> <value>")
    ...
    # metadata.sonic.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    backends) — while display paths (``sonic status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sonic_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sonic/config.yaml as-is, without merging defaults or migrating.
...
def _apply_speed_profile(config: Dict[str, Any]) -> Dict[str, Any]:
    """Apply the latency-first speed profile on top of the merged config.

    Sonic ships with ``speed.enabled: true``. The profile only fills in
    latency-sensitive values the user has not explicitly set in
    config.yaml (max_tokens), and zeroes out per-turn prompt extras that
    inflate prefill on every request (memory/skill nudges).
    """
    ...
    model_cfg = config.get("model")
    speed_max_tokens = speed.get("max_tokens")
    if (
        isinstance(model_cfg, dict)
        and model_cfg.get("max_tokens") is None
        and isinstance(speed_max_tokens, int)
        and speed_max_tokens > 0
    ):
        model_cfg["max_tokens"] = speed_max_tokens

    retries = speed.get("api_max_retries")
    if isinstance(retries, int) and retries >= 1:
        agent_cfg = config.setdefault("agent", {})
        if isinstance(agent_cfg, dict):
            agent_cfg["api_max_retries"] = min(
                int(agent_cfg.get("api_max_retries", retries) or retries), retries
            )

    if speed.get("disable_memory_nudges", False):
        mem_cfg = config.setdefault("memory", {})
        if isinstance(mem_cfg, dict):
            mem_cfg["nudge_interval"] = 0

    if speed.get("disable_skill_nudges", False):
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``SONIC_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        normalized = _apply_speed_profile(
...
#   openai-codex (OAuth — sonic auth) — OpenAI Codex
#   nous         (OAuth — sonic auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.sonic/config.yaml.\n
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sonic/.env.
    ...
    interactive menu render (`sonic tools`, `sonic setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `sonic tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Sonic env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sonic/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sonic/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sonic/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.sonic/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Sonic — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Sonic vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.sonic/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.sonic/.env`` over ``os.environ``.

    Used for Sonic-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Sonic Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Warn on stale SONIC_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SONIC_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SONIC_MAX_ITERATIONS={_env_ghost} "
                f"(run 'sonic doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  sonic config edit     # Edit config file", Colors.DIM))
    print(color("  sonic config set <key> <value>", Colors.DIM))
    print(color("  sonic setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: sonic config set <key> <value>")
            print()
            print("Examples:")
            print("  sonic config set model anthropic/claude-sonnet-4")
            print("  sonic config set terminal.backend docker")
            print("  sonic config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'sonic config migrate' to add them")
        ...
        print("  sonic config           Show current configuration")
        print("  sonic config edit      Open config in editor")
        print("  sonic config set <key> <value>   Set a config value")
        print("  sonic config check     Check for missing/outdated config")
        print("  sonic config migrate   Update config with new options")
        print("  sonic config path      Show config file path")
        print("  sonic config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``sonic config`` UI so users
