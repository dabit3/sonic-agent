...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.sonic/.env file
...
#
# Tokens live in ~/.sonic/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `sonic auth list` / `sonic status`."""
    try:
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("SONIC_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("SONIC_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
