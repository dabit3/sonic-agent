...
Add, remove, or reorder entries here — both `sonic setup` and
`sonic` provider-selection will pick up the change automatically.
"""
...
_SONIC_USER_AGENT = f"sonic-cli/{_SONIC_VERSION}"
...
OPENROUTER_MODELS: list[tuple[str, str]] = [
    ...
    ("openrouter/bodybuilder",                 "chat-only — no tool-capable endpoints"),
    # Free tier
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `sonic model`
    flow without maintaining a separate static list.
    """
    from sonic_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $SONIC_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Sonic-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SONIC_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Sonic refreshes the cache from
...
class _LazyProviderModels(dict):
    """dict whose expensive entries are computed on first access.

    ``_xai_curated_models()`` parses the multi-megabyte models.dev disk
    cache; doing that during module import costs ~100-200ms of CLI
    startup on every invocation. Entries registered in
    ``lazy_factories`` hold a cheap static placeholder until first read,
    then are replaced by the factory result.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.lazy_factories: dict[str, Any] = {}

    def _resolve(self, key: Any) -> None:
        factory = self.lazy_factories.pop(key, None)
        if factory is not None:
            super().__setitem__(key, factory())

    def _resolve_all(self) -> None:
        for key in list(self.lazy_factories):
            self._resolve(key)
...
_PROVIDER_MODELS: dict[str, list[str]] = _LazyProviderModels({
    ...
    "xai-oauth": list(_XAI_STATIC_FALLBACK),  # replaced lazily, see below
    "copilot-acp": [
        "copilot-acp",
    ],
    "copilot": [
    ...
    "xai": list(_XAI_STATIC_FALLBACK),  # replaced lazily, see below
    "nvidia": [
...
# The xAI curated lists come from the models.dev disk cache (a multi-MB
# JSON file). Registering them as lazy factories keeps that parse off the
# import path — it only runs if something actually asks for xAI models.
_PROVIDER_MODELS.lazy_factories = {
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Sonic that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Sonic that doesn't ship
...
# this list:  sonic model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `sonic model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `sonic model` TUI
...
# path. The *interactive* default (GUI onboarding / ``sonic model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in sonic_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``sonic auth add openai-codex`` without
    ``sonic model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``sonic model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    sonic-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from sonic_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — sonic-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from sonic_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``sonic model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from sonic_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from sonic_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Sonic' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``sonic_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Sonic should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``sonic auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.sonic/.env``.
    ...
    """
    try:
        from sonic_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Sonic release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Sonic release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from sonic_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Sonic release.
...
#
# Cache strategy:
#   - One JSON file at $SONIC_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from sonic_constants import get_sonic_home
    return get_sonic_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SONIC_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SONIC_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Sonic' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SONIC_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from sonic_constants import get_sonic_home
    return get_sonic_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Sonic will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Sonic cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Sonic actually ships.  (#46850)
