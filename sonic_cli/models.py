...
def check_nous_free_tier() -> bool:
    ...
    try:
        from sonic_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``sonic model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``sonic auth add openai-codex`` without
    ``sonic model``).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return sonic-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; sonic's picker expects
