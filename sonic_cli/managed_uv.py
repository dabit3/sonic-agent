...
Sonic owns its own uv binary at ``$SONIC_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$SONIC_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Sonic keeps *its* uv binary.

    ``$SONIC_HOME/bin/uv`` on POSIX, ``$SONIC_HOME\\bin\\uv.exe`` on
...
def rebuild_venv(uv_bin: str, venv_dir: Path, python_version: str = "3.11") -> bool:
    ...
    ``sonic.exe`` (gateway/desktop) holds ``venv\\Scripts\\python.exe`` open;
    ...
    venv back so Sonic is never left with no venv at all.
    """
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``sonic update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$SONIC_HOME/bin/`` instead of ``~/.local/bin/``.
    """
