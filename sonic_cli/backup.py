"""
Backup and import commands for sonic CLI.

`sonic backup` creates a zip archive of the entire ~/.sonic/ directory
(excluding the sonic-agent repo and transient files).

`sonic import` restores from a backup zip, overlaying onto the current
SONIC_HOME root.
"""
...
from sonic_constants import get_default_sonic_root, get_sonic_home, display_sonic_home
...
# File names that ``sonic import`` must never overwrite, matched by basename so
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to sonic root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Sonic home directory."""
    sonic_root = get_default_sonic_root()

    if not sonic_root.is_dir():
        print(f"Error: Sonic home directory not found at {sonic_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"sonic-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"sonic-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_sonic_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(sonic_root, followlinks=False):
        dp = Path(dirpath)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(sonic_root)
    ...
    print(f"\nRestore with: sonic import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Sonic backup.
    ...
    # Look for telltale files that a sonic home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Sonic backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.sonic/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a sonic dir name
        if prefix in {".sonic", "sonic"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_sonic_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (sonic_root / "config.yaml").exists()
        has_env = (sonic_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Sonic configuration.")
        ...
        for member in members:
            ...
            target = sonic_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(sonic_root.resolve())
            except ValueError:
        ...
        profiles_dir = sonic_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # sonic_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: sonic profile list  (after installing sonic)")

        # Guidance
        print()
        if not (sonic_root / "sonic-agent").is_dir():
            print("Note: The sonic-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: sonic update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  sonic -p {pname} gateway install")

        print("Done. Your Sonic configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and sonic backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SONIC_HOME).
...
# — `sonic update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(sonic_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(sonic_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(sonic_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for sonic backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_sonic_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, sonic_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``sonic_root`` to ``out_path``.
...
def _pre_update_backup_dir(sonic_home: Optional[Path] = None) -> Path:
    home = sonic_home or get_sonic_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SONIC_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SONIC_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``sonic update``) should continue even if the backup fails.
    """
    sonic_root = sonic_home or get_default_sonic_root()
    if not sonic_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(sonic_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, sonic_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `sonic claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SONIC_HOME under ``backups/`` before a
    ``sonic claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``sonic import <archive>``.  Writes to
    ``<SONIC_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `sonic import` and the
