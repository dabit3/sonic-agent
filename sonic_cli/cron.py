...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'sonic cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: sonic gateway restart")
        ...
        print("    sonic gateway install    # Install as a user service")
        print("    sudo sonic gateway install --system  # Linux servers: boot-time system service")
        print("    sonic gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: sonic cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
