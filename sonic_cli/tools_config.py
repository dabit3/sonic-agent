"""
Unified tool configuration for Sonic Agent.

`sonic tools` and `sonic setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.sonic/config.yaml under
...
from sonic_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from sonic_cli.nous_account import format_nous_portal_entitlement_message
...
from sonic_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _SONIC_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `sonic tools` → Video Generation, which walks
...
# `sonic tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from sonic_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `sonic tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``sonic tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from sonic_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``sonic tools`` checklist actually offers
    for ``platform``.
    ...
    so ``sonic tools`` never claims to add or remove a toolset the user was
...
from sonic_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Sonic routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set SONIC_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "SONIC_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "SONIC_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "SONIC_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "SONIC_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "SONIC_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `sonic model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("SONIC_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Sonic telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``sonic update`` and
      by ``sonic computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `sonic update` calls this
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    driver_cmd = _cua_driver_cmd()
    try:
        # When not verbose (e.g. `sonic update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `sonic update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.sonic/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Sonic process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_sonic_home()}/sonic-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in sonic-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.sonic/config.yaml")
        ...
        # Run the full `sonic auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.sonic/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from sonic_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: sonic auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `sonic auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: sonic auth spotify")
        except Exception as exc:
        ...
        try:
            from sonic_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: sonic plugins enable observability/langfuse")
        _print_info("    Restart Sonic for tracing to take effect.")
        _print_info("    Verify: sonic plugins list")
        ...
        try:
            ...
            _print_info("    Run later: sonic auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `sonic auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: sonic auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: sonic auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``sonic tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``sonic tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: sonic tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    # "sonic-cli" (which include all _SONIC_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[sonic-cli, spotify]`` after enabling Spotify via ``sonic
        ...
        # (e.g. "sonic-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``sonic tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `sonic tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"sonic-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("sonic-")}
    ...
    # shipped as a bundled plugin but user must opt in via `sonic tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `sonic tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by sonic tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (sonic-cli, sonic-telegram, etc.)
    ...
    # Opening `sonic tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `sonic tools` gets a silent no-op
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``sonic model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `sonic model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `sonic model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `sonic tools` and `sonic setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Sonic Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://lightning-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from sonic_constants import display_sonic_home
    print(color(f"  Tool configuration saved to {display_sonic_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'sonic' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, sonic mcp
