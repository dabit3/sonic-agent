"""`sonic checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.sonic/checkpoints/``.  Actions:

    sonic checkpoints               # same as `status`
    sonic checkpoints status        # total size, project count, breakdown
    sonic checkpoints list          # per-project checkpoint counts + workdir
    sonic checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    sonic checkpoints clear [-f]    # nuke the entire base (asks first)
    sonic checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    sonic checkpoints
    sonic checkpoints prune --retention-days 3 --max-size-mb 200
    sonic checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: sonic checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``sonic checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `sonic checkpoints` → status
