...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Sonic-managed OAuth credentials."""

    auth_hint = "sonic auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`sonic auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`sonic auth reset xai-oauth` or re-authenticate with "
                    "`sonic auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `sonic auth add xai-oauth --type oauth`."
            )
