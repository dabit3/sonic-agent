...
Reads the user's Nous OAuth state from ``~/.sonic/auth.json`` through the
...
:func:`sonic_cli.auth.resolve_nous_runtime_credentials`.
"""
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `sonic auth add nous` first."
                )
    ...
    # to sonic_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
