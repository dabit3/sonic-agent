"""Profile distributions — shareable, packaged Sonic profiles via git.

A distribution is a Sonic profile published as a git repository (or
...
* ``sonic profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``sonic skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``sonic profile``, not a parallel tree):

    sonic profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "sonic-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            sonic_requires=str(data.get("sonic_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.sonic_requires:
            out["sonic_requires"] = self.sonic_requires
        if self.author:
...
def check_sonic_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Sonic {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``sonic profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Sonic profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Sonic distribution."
        )

    # Version check up-front so we fail fast
    check_sonic_requires(manifest.sonic_requires, sonic_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.sonic).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="sonic_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `sonic profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`sonic profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="sonic_dist_update_") as tmp:
        plan = plan_install(
