"""Shell completion script generation for sonic CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_sonic_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Sonic Agent bash completion
# Add to ~/.bashrc:
...
_sonic_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_sonic_profiles)" -- "$cur"))
        return
    fi
...
complete -F _sonic_completion sonic
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef sonic
# Sonic Agent zsh completion
# Add to ~/.zshrc:
...
_sonic() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_sonic_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'sonic command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Sonic Agent fish completion",
        "# Add to your config:",
        "#   sonic completion fish | source",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c sonic -f -s p -l profile"
        " -d 'Profile name' -xa '(__sonic_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__sonic_profiles)' -d 'Profile name'"
                )
