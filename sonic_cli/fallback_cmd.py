"""
sonic fallback — manage the fallback provider chain.
...
Subcommands:
  sonic fallback [list]   Show the current fallback chain (default when no subcommand)
  sonic fallback add      Pick provider + model via the same picker as `sonic model`,
                           then append the selection to the chain
  sonic fallback remove   Pick an entry to delete from the chain
  sonic fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.sonic/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from sonic_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `sonic model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  sonic fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `sonic model`, then append the selection to the chain."""
    from sonic_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `sonic model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `sonic fallback list` to view, or `sonic fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``sonic fallback [subcommand]``."""
