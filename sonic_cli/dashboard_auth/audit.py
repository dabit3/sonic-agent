...
Profile-aware location: ``$SONIC_HOME/logs/dashboard-auth.log``.
...
from ``sonic_constants`` or other sonic_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SONIC_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``sonic_constants.get_sonic_home`` semantics: env var wins,
    else ``~/.sonic``. A local copy avoids an import cycle with the
    middleware which lives below ``sonic_cli``.
    """
    home = os.environ.get("SONIC_HOME") or str(Path.home() / ".sonic")
