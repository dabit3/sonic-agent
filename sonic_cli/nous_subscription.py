...
# coverage category (sonic_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `sonic tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("SONIC_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `sonic model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("SONIC_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("SONIC_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from sonic_cli.setup import prompt_checklist
    except Exception:
        return set()
