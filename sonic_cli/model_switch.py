...
- ``sonic_cli.providers``        -- canonical provider identity + overlays
- ``sonic_cli.model_normalize``  -- per-provider name formatting
...
_SONIC_MODEL_WARNING = (
    "Nous Research Sonic 3 & 4 models are NOT agentic and are not designed "
    "for use with Sonic Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Sonic 3 / Sonic 4 chat families.
# The previous substring check (`"sonic" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``sonic-brain:qwen3-14b-ctx16k`` that just
# happen to carry "sonic" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Sonic-3-Llama-3.1-70B, sonic-4-405b, openrouter/sonic3:70b
# Negative examples it must NOT match:
#   sonic-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_SONIC_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])sonic[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_sonic_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Sonic 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_SONIC_NON_AGENTIC_RE.search(model_name))


def _check_sonic_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Sonic 3/4 chat model."""
    if is_nous_sonic_non_agentic(model_name):
        return _SONIC_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``sonic config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from sonic_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'sonic model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'sonic doctor' — config issues detected:"
    ...
    # Mirrors the same logic in sonic_cli.runtime_provider.resolve_runtime_provider;
    ...
    sonic_warn = _check_sonic_model_warning(new_model)
    if sonic_warn:
        warnings.append(sonic_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from sonic_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from sonic_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # --- 1. Check Sonic-mapped providers ---
    from sonic_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from sonic_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for sonic_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(sonic_id)
        if (
            _alias_target
            and _alias_target != sonic_id
        ...
        pconfig = PROVIDER_REGISTRY.get(sonic_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (SONIC_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from sonic_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(sonic_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `sonic model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(sonic_id)
        if not model_ids:
            model_ids = curated.get(sonic_id, [])
    ...
    # --- 2. Check Sonic-only providers (nous, openai-codex, copilot, opencode-go) ---
    from sonic_cli.providers import SONIC_OVERLAYS
    from sonic_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Sonic provider ID.
    # SONIC_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Sonic IDs ("copilot").
    _mdev_to_sonic = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in SONIC_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Sonic slug — e.g. "github-copilot" → "copilot"
        sonic_slug = _mdev_to_sonic.get(pid, pid)
        if sonic_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(sonic_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, sonic_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or sonic_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(sonic_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", sonic_slug, exc)
        ...
        # configured.
        if not has_creds and sonic_slug == "anthropic":
            try:
                ...
                if (sonic_creds and sonic_creds.get("accessToken")) or \
        ...
        if sonic_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(sonic_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(sonic_slug)
                model_ids = _ids if _ids else (curated.get(sonic_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(sonic_slug, []) or curated.get(pid, [])
        elif sonic_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `sonic model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from sonic_cli.auth import get_provider_auth_state as _nous_state
        ...
        results.append({
            "slug": sonic_slug,
            "name": get_label(sonic_slug),
            "is_current": sonic_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "sonic",
        ...
        seen_slugs.add(sonic_slug.lower())
        _record_builtin_endpoint(sonic_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SONIC_OVERLAYS (keeps /model in sync
    # with `sonic model`).
    try:
        from sonic_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Sonic's canonical write key (matches
            ...
            # Sonic writes ``models:`` as a dict keyed by model id
            # (see sonic_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Sonic's own writer uses; a hyphen variant is accepted
            ...
            # active model. Sonic's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, sonic_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`sonic_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from sonic_cli.models import fetch_openrouter_models
