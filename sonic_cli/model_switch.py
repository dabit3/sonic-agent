...
def list_authenticated_providers(
    ...
    for sonic_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # show up in the picker without requiring a Sonic release.
    ...
    for pid, overlay in SONIC_OVERLAYS.items():
        ...
        if sonic_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(sonic_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(sonic_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(sonic_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Sonic slug, fall back to overlay key
            model_ids = curated.get(sonic_slug, []) or curated.get(pid, [])
