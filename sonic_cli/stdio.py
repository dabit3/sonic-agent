...
on Japanese locales, etc.).  Sonic's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``sonic_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Sonic is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SONIC_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SONIC_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Sonic-managed Git install directories so
    ...
    # already-running shells don't see the change — which means sonic
    ...
    # install without Sonic-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Sonic's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Sonic's
    ``sonic config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Sonic (User env var in Windows Settings, or
    export in a PowerShell profile) and Sonic picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Sonic-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\sonic\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``sonic`` from right after install) retain their old PATH.

    Any subprocess Sonic spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Sonic-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "sonic", "git", "cmd"),
        os.path.join(local_appdata, "sonic", "git", "bin"),
        os.path.join(local_appdata, "sonic", "git", "usr", "bin"),
        # Sonic venv Scripts directory — host of the sonic.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes sonic, but harmless to include.
        os.path.join(local_appdata, "sonic", "sonic-agent", "venv", "Scripts"),
