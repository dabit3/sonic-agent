...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or sonic -c.",
    ...
    "Ctrl+Z suspends Sonic to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "sonic -c resumes your most recent CLI session. sonic -c \"project name\" resumes by title.",
    "sonic -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "sonic -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "sonic chat -t web,terminal enables only specific toolsets for a focused session.",
    "sonic chat -s github-pr-workflow preloads a skill at launch.",
    "sonic chat -q \"query\" runs a single non-interactive query and exits.",
    "sonic chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "sonic chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "sonic --yolo bypasses all dangerous command approval prompts for the entire session.",
    "sonic chat --source telegram tags the session for filtering in sonic sessions list.",
    "sonic -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "sonic doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "sonic dump outputs a compact setup summary — great for bug reports.",
    "sonic config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "sonic config edit opens config.yaml in your default editor.",
    "sonic config check scans for missing or stale configuration options.",
    "sonic sessions browse opens an interactive session picker with search.",
    "sonic sessions stats shows session counts by platform and database size.",
    "sonic sessions prune --older-than 30 cleans up old sessions.",
    "sonic skills search react --source skills-sh searches the skills.sh public directory.",
    "sonic skills check scans installed hub skills for upstream updates.",
    "sonic skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "sonic skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "sonic mcp add github --command npx adds MCP servers from the command line.",
    "sonic mcp serve runs Sonic itself as an MCP server for other agents.",
    "sonic auth add lets you add multiple API keys for credential pool rotation.",
    "sonic completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "sonic logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "sonic backup creates a zip backup of your entire Sonic home directory.",
    "sonic profile create coder creates an isolated profile that becomes its own command.",
    "sonic profile create work --clone copies your current config and keys to a new profile.",
    "sonic update syncs new bundled skills to ALL profiles automatically.",
    "sonic gateway install sets up Sonic as a system service (systemd/launchd).",
    "sonic memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "sonic webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: sonic tools disables unused tools, sonic skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "sonic models routes vision, compression, and aux tasks to cheaper models — cuts background cost 85%+ without downgrading your chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as sonic -w).",
    ...
    "SONIC_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Sonic tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'sonic profile create coder' creates the 'coder' command.",
    "sonic profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Sonic prints a resume command with session ID and stats.",
    "sonic sessions export backup.jsonl exports all sessions for backup or analysis.",
    "sonic -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "sonic skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .sonic/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: sonic cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Sonic runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "sonic gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Sonic via a local macOS server.",
    ...
    "Context files (.sonic.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.sonic/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Sonic loads project context from .sonic.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "sonic mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "sonic mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "sonic mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — sonic tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.sonic/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "sonic chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "sonic plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "sonic acp runs Sonic as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SONIC_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "sonic auth add nous or sonic auth add openai-codex sets up OAuth-based providers.",
    ...
    "sonic status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.sonic/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Sonic your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.sonic/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Sonic auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.sonic/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SONIC_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SONIC_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SONIC_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.sonic/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'sonic profile create ls' would be rejected.",
    "sonic profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "sonic claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SONIC_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via sonic tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.sonic/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.sonic/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Sonic auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Sonic config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Sonic is working.',
    ...
    '/reload reloads ~/.sonic/.env into the running session — pick up new API keys without restarting.',
    ...
    "SONIC_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.sonic/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.sonic/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'sonic curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "sonic curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'sonic curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'sonic auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SONIC_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SONIC_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'sonic dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'sonic dashboard --tui embeds the full Sonic TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.sonic/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.sonic/dashboard-plugins/ — no npm build required.',
    ...
    'SONIC_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SONIC_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'SONIC_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SONIC_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.sonic/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.sonic/skills/ so drop-in skills appear without restarting the session.',
    ...
    'sonic -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'sonic chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'sonic chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'sonic chat --ignore-user-config skips ~/.sonic/config.yaml — reproducible bug reports and CI runs.',
    "sonic chat --source tool tags programmatic chats so they don't clutter sonic sessions list.",
    'sonic dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'sonic sessions rename <ID> "new title" renames any past session; sonic sessions delete <ID> removes one.',
    'sonic import restores a session export or profile archive produced by sessions export or profile export.',
    'sonic fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'sonic pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'sonic setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'sonic status --deep runs the full health sweep across every component; plain sonic status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SONIC_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SONIC_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.sonic/plugins/ — trust-gated by design.',
    "SONIC_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SONIC_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SONIC_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SONIC_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SONIC_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SONIC_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SONIC_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SONIC_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SONIC_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SONIC_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SONIC_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Sonic block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from sonic sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.sonic/dashboard-plugins/.',
]
