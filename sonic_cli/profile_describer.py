...
Used by ``sonic profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``sonic_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Sonic Agent kanban board.
...
Rules:
  ...
  - Never write "Sonic Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_sonic_home())
        else:
