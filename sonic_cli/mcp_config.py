"""
MCP Server Management CLI — ``sonic mcp`` subcommand.

Implements ``sonic mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.sonic/config.yaml under the ``mcp_servers`` key.
"""
...
from sonic_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_sonic_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from sonic_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # sonic_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  sonic mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  sonic mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  sonic mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_sonic_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: sonic mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_sonic_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `sonic mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  sonic mcp add <name> --url <endpoint>')
        _info('  sonic mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `sonic mcp remove` + `sonic mcp add` to reconfigure auth.")
        return False
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'sonic mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``sonic mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("sonic mcp catalog                            List Nous-approved MCPs")
        _info("sonic mcp install <name>                     Install a catalog MCP")
        _info("sonic mcp serve                              Run as MCP server")
        _info("sonic mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("sonic mcp add <name> --command <cmd>         Add a stdio server")
        _info("sonic mcp add <name> --preset <preset>       Add from a known preset")
        _info("sonic mcp remove <name>                      Remove a server")
        _info("sonic mcp list                               List configured servers")
        _info("sonic mcp test <name>                        Test connection")
        _info("sonic mcp configure <name>                   Toggle tools")
        _info("sonic mcp login <name>                       Re-authenticate OAuth")
        _info("sonic mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
