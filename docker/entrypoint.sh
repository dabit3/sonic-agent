#!/bin/bash
# Docker/Podman entrypoint: bootstrap config files into the mounted volume, then run sonic.
set -e
...
# optionally remap the sonic user/group to match host-side ownership, fix volume
# permissions, then re-exec as sonic.
if [ "$(id -u)" = "0" ]; then
    if [ -n "$SONIC_UID" ] && [ "$SONIC_UID" != "$(id -u sonic)" ]; then
        echo "Changing sonic UID to $SONIC_UID"
        usermod -u "$SONIC_UID" sonic
    fi

    if [ -n "$SONIC_GID" ] && [ "$SONIC_GID" != "$(id -g sonic)" ]; then
        echo "Changing sonic GID to $SONIC_GID"
        # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already exist
        # as "dialout" in the Debian-based container image)
        groupmod -o -g "$SONIC_GID" sonic 2>/dev/null || true
    fi

    # Fix ownership of the data volume. When SONIC_UID remaps the sonic user,
    ...
    if [ "$needs_chown" = true ]; then
        echo "Fixing ownership of $SONIC_HOME to sonic ($actual_sonic_uid)"
    ...
    # Ensure config.yaml is readable by the sonic runtime user even if it was
...
# --- Running as sonic from here ---
source "${INSTALL_DIR}/.venv/bin/activate"

# Stamp install method for detect_install_method()
echo "docker" > "${SONIC_HOME:=/opt/data}/.install_method" 2>/dev/null || true

# Create essential directory structure.  Cache and platform directories
# (cache/images, cache/audio, platforms/whatsapp, etc.) are created on
# demand by the application — don't pre-create them here so new installs
# get the consolidated layout from get_sonic_dir().
# The "home/" subdirectory is a per-profile HOME for subprocesses (git,
# ssh, gh, npm …).  Without it those tools write to /root which is
# ephemeral and shared across profiles.  See issue #4426.
mkdir -p "$SONIC_HOME"/{cron,sessions,logs,hooks,memories,skills,skins,plans,workspace,home}
...
if [ ! -f "$SONIC_HOME/SOUL.md" ]; then
    cp "$INSTALL_DIR/docker/SOUL.md" "$SONIC_HOME/SOUL.md"
fi

# auth.json: bootstrap from env on first boot only.  Used by orchestrators
# (e.g. provisioning a Sonic VPS from an account-management service) that
# need to seed the OAuth refresh credential non-interactively, instead of
# walking the user through `sonic setup` + the device-flow login dance.
...
# Optionally start `sonic dashboard` as a side-process.
#
...
# `exec sonic "$@"` so the user's chosen foreground command (chat, gateway,
...
case "${SONIC_DASHBOARD:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        ...
        (
            stdbuf -oL -eL sonic dashboard "${dash_args[@]}" 2>&1 \
                | sed -u 's/^/[dashboard] /'
        ) &
        ;;
esac
...
#
#   docker run <image>                 -> exec `sonic` with no args (legacy default)
#   docker run <image> chat -q "..."   -> exec `sonic chat -q "..."` (legacy wrap)
...
# Otherwise we treat the args as a sonic subcommand and wrap with `sonic`,
