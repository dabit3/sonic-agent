...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with lightning-agent)
2. User plugins: ``$LIGHTNING_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$LIGHTNING_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$LIGHTNING_HOME/plugins/model-providers/`` if it exists."""
    try:
        from lightning_constants import get_lightning_home

        d = get_lightning_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple LIGHTNING_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_lightning_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$LIGHTNING_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with lightning-agent.
    ...
    # 2. User plugins — under $LIGHTNING_HOME/plugins/model-providers/<name>/.
