"""Windows UTF-8 bootstrap for Lightning entry points.
...
should be imported at the very top of every Lightning entry point
(``lightning``, ``lightning-agent``, ``lightning-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import lightning_bootstrap``
# (or ``from lightning_bootstrap import apply_windows_utf8_bootstrap``) at
