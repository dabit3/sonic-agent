import { buildSonicWebSocketUrl } from "@sonic/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/sonic/). The Python backend
// injects ``window.__SONIC_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SONIC_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SONIC_BASE_PATH = readBasePath();
const BASE = SONIC_BASE_PATH;
...
declare global {
  interface Window {
    __SONIC_SESSION_TOKEN__?: string;
    __SONIC_BASE_PATH__?: string;
    ...
    __SONIC_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Sonic-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__SONIC_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Sonic dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``sonic_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SONIC_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Sonic-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``sonic_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SONIC_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateSonic: () =>
    fetchJSON<ActionResponse>("/api/sonic/update", { method: "POST" }),
  checkSonicUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/sonic/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``sonic update``. */
  can_update_sonic?: boolean;
  config_path: string;
