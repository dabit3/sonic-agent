/**
 * ChatPage — embeds `sonic --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (sonic_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { SONIC_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets SONIC_HOME pointed at the
  // selected profile, so the conversation runs with that profile's model,
  // skills, memory, and sessions (see web_server._resolve_chat_argv).
  if (profile) qs.set("profile", profile);
  return `${proto}//${window.location.host}${SONIC_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__SONIC_SESSION_TOKEN__ &&
    !window.__SONIC_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `sonic dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__SONIC_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Sonic TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[sonic-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__SONIC_AUTH_REQUIRED__``) this
    ...
    // forwarding them into Sonic. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="sonic-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
