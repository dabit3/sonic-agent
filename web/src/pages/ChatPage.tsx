/**
 * ChatPage — embeds `lightning --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (lightning_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { LIGHTNING_BASE_PATH } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${LIGHTNING_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__LIGHTNING_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `lightning dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Lightning TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[lightning-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Lightning. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="lightning-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __LIGHTNING_SESSION_TOKEN__?: string;
  }
}
