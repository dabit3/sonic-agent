...
`sonic skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.sonic.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.sonic.blueprint`` is a mapping containing
    ...
    sonic = meta.get("sonic") if isinstance(meta, dict) else None
    blueprint = sonic.get("blueprint") if isinstance(sonic, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.sonic.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.sonic.blueprint`` block) they can hand
    to ``sonic skills publish`` to share. ``body`` is the plain-language
