#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Sonic Skills Hub.
...
Used by sonic_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                sonic_meta = meta_block.get("sonic", {})
                if isinstance(sonic_meta, dict):
                    tags = sonic_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Sonic centralized index source
# ---------------------------------------------------------------------------

SONIC_INDEX_URL = "https://lightning-agent.nousresearch.com/docs/api/skills-index.json"
SONIC_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "sonic-index.json"
SONIC_INDEX_TTL = 6 * 3600  # 6 hours


def _load_sonic_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SONIC_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if SONIC_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - SONIC_INDEX_CACHE_FILE.stat().st_mtime
            if age < SONIC_INDEX_TTL:
                return json.loads(SONIC_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(SONIC_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Sonic index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Sonic index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        SONIC_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        SONIC_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class SonicIndexSource(SkillSource):
    """Skill source backed by the centralized Sonic Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "sonic-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "sonic-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SonicIndexSource(auth=auth), # Centralized index (search + resolved install paths)
