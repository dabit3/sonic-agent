...
def _is_sonic_provider_credential(name: str) -> bool:
    """True if ``name`` is a Sonic-managed provider credential (API key,
    token, or similar) per ``_SONIC_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Sonic API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _SONIC_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Sonic-managed provider credentials (from
    ``_SONIC_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Sonic-managed
    ...
    Non-Sonic third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_sonic_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Sonic provider "
                "credential %r (blocked by _SONIC_PROVIDER_ENV_BLOCKLIST). "
