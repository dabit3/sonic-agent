...
`mcp_tool.py`, and `sonic_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# SonicMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_sonic_provider_class() -> Optional[type]:
    ...
    class SonicMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._sonic_server_name = server_name
            ...
            self._sonic_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`SonicTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, SonicTokenStorage)
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._sonic_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._sonic_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import SonicTokenStorage
            if not isinstance(storage, SonicTokenStorage):
                return
    ...
    return SonicMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_SONIC_PROVIDER_CLS: Optional[type] = _make_sonic_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`SonicMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `sonic mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``sonic mcp remove <name>`` and (indirectly) by
        ``sonic mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
