...
`mcp_tool.py`, and `lightning_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# LightningMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_lightning_provider_class() -> Optional[type]:
    ...
    class LightningMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._lightning_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`LightningTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, LightningTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._lightning_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._lightning_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import LightningTokenStorage
                        if isinstance(storage, LightningTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._lightning_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import LightningTokenStorage
            if not isinstance(storage, LightningTokenStorage):
                return
    ...
    return LightningMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_LIGHTNING_PROVIDER_CLS: Optional[type] = _make_lightning_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`LightningMCPOAuthProvider` directly using the helpers
        ...
        return _LIGHTNING_PROVIDER_CLS(
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``lightning mcp remove <name>`` and (indirectly) by
        ``lightning mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
