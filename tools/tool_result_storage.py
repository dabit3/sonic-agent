...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/lightning-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/lightning-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/lightning-results"
HEREDOC_MARKER = "LIGHTNING_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/lightning-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"LIGHTNING_PERSIST_{uuid.uuid4().hex[:8]}"
