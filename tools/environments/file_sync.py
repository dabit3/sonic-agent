...
_FORCE_SYNC_ENV = "LIGHTNING_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.lightning") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.lightning to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.lightning", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``LIGHTNING_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, lightning_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.lightning/`` directory as a tar archive,
        ...
        # against an uninitialized remote .lightning/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (lightning_home or get_lightning_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="lightning-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.lightning/skills/a.md`` →
        ``~/.lightning/skills/a.md``, a new remote file at
        ``/root/.lightning/skills/b.md`` maps to ``~/.lightning/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
