...
One ``CDPSupervisor`` runs per Sonic ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "sonic-dialog-bridge.invalid"
...
(() => {
  if (window.__sonicDialogBridgeInstalled) return;
  window.__sonicDialogBridgeInstalled = true;
  const ENDPOINT = "http://sonic-dialog-bridge.invalid/";
