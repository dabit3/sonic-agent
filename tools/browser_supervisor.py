...
One ``CDPSupervisor`` runs per Lightning ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "lightning-dialog-bridge.invalid"
...
(() => {
  if (window.__lightningDialogBridgeInstalled) return;
  window.__lightningDialogBridgeInstalled = true;
  const ENDPOINT = "http://lightning-dialog-bridge.invalid/";
