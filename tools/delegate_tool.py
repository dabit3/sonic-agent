...
# Must match sonic_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (sonic-* prefixed), and scenario toolsets.
#
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``sonic-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. sonic-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.sonic/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_sonic_dir("cache/delegation", "delegation_cache")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'sonic auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (sonic_cli/config.py load_config()) so that
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Sonic subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
