...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``lightning tools`` repaint, tool-registration scans,
    ...
    2. ``~/.lightning/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_lightning_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.lightning/.env`` first, then ``os.environ``.

    Wraps :func:`lightning_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from lightning_cli.config import get_env_value as _lightning_get_env_value

        value = _lightning_get_env_value(name)
...
def lightning_xai_user_agent() -> str:
    """Return a stable Lightning-specific User-Agent for xAI HTTP calls."""
    try:
        from lightning_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Lightning-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Lightning-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``lightning_cli.config.get_env_value`` so keys
    stored in ``~/.lightning/.env`` (the standard Lightning location) are honored —
