...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``sonic tools`` repaint, tool-registration scans,
    ...
    2. ``~/.sonic/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_sonic_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.sonic/.env`` first, then ``os.environ``.

    Wraps :func:`sonic_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from sonic_cli.config import get_env_value as _sonic_get_env_value

        value = _sonic_get_env_value(name)
...
def sonic_xai_user_agent() -> str:
    """Return a stable Sonic-specific User-Agent for xAI HTTP calls."""
    try:
        from sonic_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Sonic-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Sonic-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``sonic_cli.config.get_env_value`` so keys
    stored in ``~/.sonic/.env`` (the standard Sonic location) are honored —
