...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SONIC_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SONIC_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SONIC_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SONIC_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SONIC_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SONIC_SESSION_PLATFORM via
    ...
    if env_var_enabled("SONIC_CRON_SESSION"):
        return False
    if env_var_enabled("SONIC_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $SONIC_HOME, or by the resolved absolute
# active profile home path such as /home/sonic/.sonic/config.yaml. The
# resolved-absolute form is folded into the ~/.sonic/ patterns at detection
...
# go stale when SONIC_HOME is set after this module is imported, e.g. under the
...
# ~/.sonic/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _SONIC_ENV_PATH; matches the SONIC_HOME override form as
# well as ~/.sonic/.
_SONIC_CONFIG_PATH = (
    r'(?:~\/\.sonic/|'
    r'(?:\$home|\$\{home\})/\.sonic/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\b(pkill|killall)\b.*\b(sonic|gateway|cli\.py)\b', "kill sonic/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill sonic` but not
    # `kill -9 $(pgrep -f sonic)` because the substitution is opaque
    ...
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Sonic file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.sonic/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Sonic-managed security file (~/.sonic/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_SONIC_CONFIG_PATH}|{_SONIC_ENV_PATH})', "in-place edit of Sonic config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_SONIC_CONFIG_PATH}|{_SONIC_ENV_PATH})', "in-place edit of Sonic config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_SONIC_CONFIG_PATH}|{_SONIC_ENV_PATH})', "in-place edit of Sonic config/env (perl/ruby)"),
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / SONIC_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.sonic``); the matched
...
def _rewrite_resolved_sonic_home(command: str) -> str:
    ...
    /home/sonic/.sonic/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$SONIC_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.sonic")
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("SONIC_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("SONIC_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    try:
        ...
        try:
            from sonic_cli.config import load_config as _load_cfg
