...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("LIGHTNING_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("LIGHTNING_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("LIGHTNING_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set LIGHTNING_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind LIGHTNING_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds LIGHTNING_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("LIGHTNING_CRON_SESSION"):
        return False
    if env_var_enabled("LIGHTNING_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $LIGHTNING_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_LIGHTNING_ENV_PATH = (
    r'(?:~\/\.lightning/|'
    r'(?:\$home|\$\{home\})/\.lightning/|'
    r'(?:\$lightning_home|\$\{lightning_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\blightning\s+gateway\s+(stop|restart)\b', "stop/restart lightning gateway (kills running agents)"),
    (r'\blightning\s+update\b', "lightning update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart lightning-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart lightning-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(lightning|gateway|cli\.py)\b', "kill lightning/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill lightning` but not
    # `kill -9 $(pgrep -f lightning)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("LIGHTNING_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("LIGHTNING_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("LIGHTNING_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("LIGHTNING_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("LIGHTNING_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("LIGHTNING_EXEC_ASK")
