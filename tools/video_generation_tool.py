...
provider** — turn on a backend by enabling a plugin (``lightning plugins
enable video_gen/<name>``) and selecting it in ``lightning tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/lightning-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `lightning tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `lightning plugins list` to see "
            f"installed video gen backends, or `lightning tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `lightning tools` → "
...
# `lightning tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`lightning tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `lightning tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
