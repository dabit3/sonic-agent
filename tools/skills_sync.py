...
Copies bundled skills from the repo's skills/ directory into ~/.lightning/skills/
...
The manifest lives at ~/.lightning/skills/.bundled_manifest.
"""
...
from lightning_constants import get_bundled_skills_dir, get_lightning_home
...
def _get_bundled_dir() -> Path:
    ...
    Checks LIGHTNING_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.lightning/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.lightning/skills/ using the manifest.
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `lightning skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `lightning skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `lightning update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.lightning/skills/ ...")
