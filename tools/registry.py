"""Central registry for all lightning-agent tools.
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``lightning tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``lightning tools enable foo``)
