...
it is automatically downloaded from GitHub releases to $SONIC_HOME/bin/tirith.
...
def _get_sonic_home() -> str:
    """Return the Sonic home directory, respecting SONIC_HOME env var."""
    return str(get_sonic_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sonic_home(), ".tirith-install-failed")
...
def _sonic_bin_dir() -> str:
    """Return $SONIC_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sonic_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SONIC_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sonic_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SONIC_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SONIC_HOME/bin/tirith
    ...
    sonic_bin = os.path.join(_sonic_bin_dir(), "tirith")
    if os.path.isfile(sonic_bin) and os.access(sonic_bin, os.X_OK):
        _resolved_path = sonic_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sonic_bin
