...
it is automatically downloaded from GitHub releases to $LIGHTNING_HOME/bin/tirith.
...
def _get_lightning_home() -> str:
    """Return the Lightning home directory, respecting LIGHTNING_HOME env var."""
    return str(get_lightning_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_lightning_home(), ".tirith-install-failed")
...
def _lightning_bin_dir() -> str:
    """Return $LIGHTNING_HOME/bin, creating it if needed."""
    d = os.path.join(_get_lightning_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $LIGHTNING_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_lightning_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $LIGHTNING_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $LIGHTNING_HOME/bin/tirith
    ...
    lightning_bin = os.path.join(_lightning_bin_dir(), "tirith")
    if os.path.isfile(lightning_bin) and os.access(lightning_bin, os.X_OK):
        _resolved_path = lightning_bin
        _install_failure_reason = ""
        _clear_install_failed()
