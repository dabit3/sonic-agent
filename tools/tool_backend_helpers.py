...
def managed_nous_tools_enabled() -> bool:
    ...
    try:
        ...
        from lightning_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.lightning/.env`` (via
    ``lightning_cli.config.get_env_value`` when available) so tool-side
