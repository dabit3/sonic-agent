#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Sonic desktop GUI.
...
``read_terminal`` it is gated on ``SONIC_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — SONIC_DESKTOP is set on the gateway the app spawns."""
    return (os.getenv("SONIC_DESKTOP") or "").strip().lower() in ("1", "true", "yes")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Sonic desktop GUI (the tabs mirroring terminal(background=true) runs). "
