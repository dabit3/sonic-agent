...
Backend is selected during ``sonic tools`` setup (web.backend in config.yaml).
When available, Sonic can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Sonic config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Sonic' config/.env layer (``sonic config set``, ``sonic tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.sonic/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``sonic tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `sonic tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_sonic_dir("cache/web", "web_cache")
