...
from sonic_constants import agent_browser_runnable, get_sonic_home
from utils import env_int, is_truthy_value
from sonic_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Sonic secret
...
def _find_agent_browser() -> str:
    ...
    # node_modules binary, which disappears on the next ``sonic update`` and
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_sonic_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_sonic_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_sonic_home() / "node")),
            ]
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Sonic
