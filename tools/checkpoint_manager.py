...
-----------------------------------------------------------------------------
    ...
    ~/.sonic/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/sonic/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_sonic_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/sonic"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Sonic convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("SONIC_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Sonic infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``sonic checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `sonic checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "sonic@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Sonic Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``SONIC_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at SONIC_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "SONIC_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `sonic checkpoints` CLI
# ---------------------------------------------------------------------------
