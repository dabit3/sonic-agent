...
etc.) surface as specific blocked checks via `sonic computer-use doctor`
...
# fetches the latest release, so a `SONIC_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `SONIC_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("SONIC_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Sonic should disable cua-driver telemetry for this user.
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or SONIC_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `sonic computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `sonic tools` and enable the Computer Use toolset to install it automatically."
    )
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Sonic-managed secrets out of the child env.
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        #   - Gets a distinct agent-cursor color per Sonic run, with
        ...
        #     concurrent Sonic runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Sonic run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"sonic-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Sonic' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps sonic `capture(mode, app)` → cua-driver `list_windows` +
