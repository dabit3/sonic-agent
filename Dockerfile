...
# subprocesses, git, bun, etc.) that would otherwise accumulate when sonic
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main sonic process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Sonic WebUI' catalog). The image has moved to s6-overlay /init\
...
# `file:` workspace dependency (same pattern as sonic-ink above).
...
# lock on the @sonic/ink entry, tripped the TUI launcher's
...
# gives the non-root sonic user read + traverse but no write; root retains
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. sonic
# subcommand vs. no-args), drops to the sonic user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/sonic/docker/main-wrapper.sh" ]
CMD [ ]
