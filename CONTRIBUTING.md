# Contributing to Lightning Agent

Thank you for contributing to Lightning Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Lightning to work everywhere.
...
Bundled skills (in `skills/`) ship with every Lightning install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `lightning skills browse` (labeled "official") and install it with `lightning skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `lightning skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.lightning/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(lightning_home, config)` for setup-wizard integration
...
- Integrate with `lightning memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/lightning-agent.git
cd lightning-agent
...
```bash
mkdir -p ~/.lightning/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.lightning/config.yaml
touch ~/.lightning/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.lightning/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/lightning" ~/.local/bin/lightning

# Verify
lightning doctor
lightning chat -q "Hello"
```
...
├── cli.py                    # LightningCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (lightning-cli, lightning-telegram, etc.)
├── lightning_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── lightning_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.lightning/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (lightning-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.lightning/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.lightning/`)

| Path | Purpose |
|------|---------|
| `~/.lightning/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.lightning/.env` | API keys and secrets |
| `~/.lightning/auth.json` | OAuth credentials (Nous Portal) |
| `~/.lightning/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.lightning/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.lightning/state.db` | SQLite session database |
| `~/.lightning/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.lightning/cron/` | Scheduled job data |
| `~/.lightning/whatsapp/session/` | WhatsApp bridge credentials |
...
(for example `_LIGHTNING_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.lightning`:
...
The user may skip setup and keep loading the skill. Lightning only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `lightning setup` or update `~/.lightning/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Lightning tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Lightning Agent" is the secondary collaborator. If the contributor's commit shows "Lightning Agent" as author because they used Lightning to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Lightning tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `lightning --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Lightning uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.lightning/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `lightning_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `lightning_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Lightning runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the lightning wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `lightning_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `lightning_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `lightning_cli/gateway_windows.py::
...
Lightning has terminal access. Security matters.
...
2. **Test manually**: Run `lightning` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/lightning-agent/issues)
- Include: OS, Python version, Lightning version (`lightning version`), full error traceback
