import type { InputEvent, Key } from '@lightning/ink'
import * as Ink from '@lightning/ink'
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.LIGHTNING_TUI_TERMUX_FAST_ECHO ?? '').trim().toLowerCase()
