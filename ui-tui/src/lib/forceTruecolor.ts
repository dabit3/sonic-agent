...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SONIC_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Sonic on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SONIC_TUI_TRUECOLOR=1.
