...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LIGHTNING_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Lightning on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LIGHTNING_TUI_TRUECOLOR=1.
