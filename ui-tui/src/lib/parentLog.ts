...
const logDir = join(process.env.SONIC_HOME?.trim() || join(homedir(), '.sonic'), 'logs')
...
// a real ~/.sonic (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('sonic-tui: parent lifecycle log unavailable\n')
    }
  }
}
