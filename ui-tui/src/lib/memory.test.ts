...
describe('heapdump retention guard (#21767)', () => {
  ...
  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.SONIC_HEAPDUMP_DIR
    } else {
      process.env.SONIC_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.SONIC_HEAPDUMP_MAX_BYTES
    } else {
      process.env.SONIC_HEAPDUMP_MAX_BYTES = savedMax
    }
