import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@sonic/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Sonic'
  )
  ...
  const cwd = ui.info?.cwd || process.env.SONIC_CWD || process.cwd()
