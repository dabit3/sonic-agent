...
- `src/app/setupHandoff.ts` — launches external `sonic setup`, suspends Ink while it runs, opens a new session on success
...
### Setup (`setup.ts`)
`/setup` — launches external `sonic setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external sonic setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.sonic/tui-parent.log
