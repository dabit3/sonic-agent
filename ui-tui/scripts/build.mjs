...
await build({
  ...
  // Skip the prebuilt @sonic/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@sonic/ink': resolve(root, 'packages/sonic-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The sonic_cli launcher
