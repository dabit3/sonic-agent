...
await build({
  ...
  // Skip the prebuilt @lightning/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@lightning/ink': resolve(root, 'packages/lightning-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The lightning_cli launcher
