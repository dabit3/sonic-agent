...
_SONIC_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on SONIC_DESKTOP via check_fn —
...
TOOLSETS = {
    ...
    # Coding posture (base Sonic — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in sonic_cli/tools_config.py).
        "posture": True,
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``sonic-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SONIC_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``sonic-gateway``
