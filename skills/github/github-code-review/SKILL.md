...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _sonic_env="${SONIC_HOME:-$HOME/.sonic}/.env"; [ -f "$_sonic_env" ] && grep -q "^GITHUB_TOKEN=" "$_sonic_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_sonic_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Sonic Agent\",
    \"comments\": [
...
```bash
source "${SONIC_HOME:-$HOME/.sonic}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Sonic Agent. Code looks clean — good test coverage, no security concerns."
