...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LIGHTNING_HOME", os.path.expanduser("~/.lightning")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LIGHTNING_HOME", Path.home() / ".lightning")) / "skills" / "red-teaming" / "godmode"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Lightning models — already uncensored, just needs clean prompt
    "lightning": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "lightning" in model_lower or "nous" in model_lower:
        return "lightning"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Lightning config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.lightning/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Lightning for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Lightning for changes to take effect.")
