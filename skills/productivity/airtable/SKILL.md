...
4. Store the token in `~/.sonic/.env` (or via `sonic setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.sonic/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
