# Lightning Agent Has Had "Routines" Since March
...
Lightning equivalent — works today:
```bash
...
Lightning equivalent — works today:
```bash
lightning webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Lightning implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Lightning Agent |
|---|---|---|
...
## Things Lightning Does That Routines Can't
...
Lightning has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Lightning Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
lightning gateway setup    # enable webhooks
lightning webhook subscribe pr-review \
  --events "pull_request" \
...
GitHub: [github.com/NousResearch/lightning-agent](https://github.com/NousResearch/lightning-agent)

---

*Lightning Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
