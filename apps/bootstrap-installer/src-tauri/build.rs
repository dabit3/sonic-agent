...
fn main() {
    ...
    // local checkout hasn't pushed). Set SONIC_BUILD_PIN_COMMIT to bake an
    // immutable commit pin for reproducible/release installers.
    //
    // Commit pin resolution:
    //   - SONIC_BUILD_PIN_COMMIT, if set and non-empty. Accepts a SHA, tag,
    ...
    //   1. SONIC_BUILD_PIN_BRANCH, if set and non-empty.
    ...
    if let Some(c) = &commit {
        ...
        println!(
            "cargo:warning=sonic-bootstrap: pinning to commit {}",
            short(c)
        );
    }
    if let Some(b) = &branch {
        ...
        match &commit {
            Some(_) => println!("cargo:warning=sonic-bootstrap: pinning to branch {b}"),
            None => println!(
                "cargo:warning=sonic-bootstrap: following branch {b} HEAD (no commit pin; \
                 set SONIC_BUILD_PIN_COMMIT for an immutable pin)"
            ),
        }
    }
...
fn resolve_commit_pin() -> Option<String> {
    ...
    // asks for one via SONIC_BUILD_PIN_COMMIT. With no env var, we return
    // None and the installer follows the branch HEAD at install time.
    let requested = std::env::var("SONIC_BUILD_PIN_COMMIT").ok()?;
