...
pub fn run() {
    // Tracing → bootstrap-installer.log under SONIC_HOME/logs/ so install
    ...
    // Sonic is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Sonic installer starting");
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_sonic_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_sonic_home,
            paths::open_log_dir,
        ])
        .run(tauri::generate_context!())
        .expect("error while running Sonic Setup");
}
