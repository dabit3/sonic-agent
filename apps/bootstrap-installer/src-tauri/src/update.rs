...
//!
//!   1. wait for the old Sonic desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `sonic update`
...
/// install tree before giving up and letting `sonic update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_sonic`
    ...
    // retry-once `sonic update` already does above, and `sonic update`'s own
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_sonic_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Sonic to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend sonic.exe (or the desktop Sonic.exe
            ...
            // force-kill every Sonic.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Sonic still holding install files ({}); force-killing stragglers…",
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_sonic(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Sonic.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Sonic.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// which completes BEFORE we spawn `venv\Scripts\sonic.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startSonic()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// sonic.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named sonic.exe.)
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let sonic_home = crate::paths::sonic_home();
    let mut envs = vec![(
        "SONIC_HOME".to_string(),
        sonic_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        sonic_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_sonic_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Sonic.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.sonic-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.sonic-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::sonic_home())
        .status()
        .await
...
mod tests {
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_sonic(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/sonic-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Sonic.app"]),
            Some(PathBuf::from("/Applications/Sonic.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Sonic.app");
        let tmp = base.join("Sonic.app.sonic-update-new");
        let old = base.join("Sonic.app.sonic-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Sonic.app");
        let tmp = base.join("Sonic.app.sonic-update-new"); // intentionally absent
        let old = base.join("Sonic.app.sonic-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Sonic.app");
        let tmp = base.join("Sonic.app.sonic-update-new"); // absent
        let old = base.join("Sonic.app.sonic-update-old");
        write_marker(&target, "OLD");
