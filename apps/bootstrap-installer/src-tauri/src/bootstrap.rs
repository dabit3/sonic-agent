...
//! SONIC_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for SONIC_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub sonic_home: Option<String>,
}
...
/// Spawn the locally-built Sonic desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_sonic_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_sonic_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Sonic desktop at {}. The desktop build step \
             may have been skipped or failed. Run `sonic desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Sonic desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Sonic
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_sonic_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Sonic.exe"),
            ("win-arm64-unpacked", "Sonic.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Sonic.app/Contents/MacOS", "Sonic"),
            ("mac-arm64/Sonic.app/Contents/MacOS", "Sonic"),
        ]
    } else {
        &[("linux-unpacked", "sonic")]
    };
...
pub(crate) fn resolve_sonic_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_sonic_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Sonic.app/Contents/MacOS/Sonic -> .../Sonic.app
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::sonic_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $SonicHome which Stage-Repository clones
    // the repo INTO at $SonicHome\sonic-agent. Mirrors sonic_constants.
    let sonic_home = args
        .sonic_home
        .clone()
        .unwrap_or_else(|| crate::paths::sonic_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&sonic_home).join("sonic-agent");

    // Copy ourselves to SONIC_HOME/sonic-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_sonic_home() {
        tracing::warn!(?err, "failed to copy installer into SONIC_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    powershell::run_script(script_path, args, sink, sonic_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Sonic"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Sonic.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Sonic.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("sonic");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Sonic.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_sonic_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_sonic_desktop_app(&root)
    ...
    #[test]
    fn resolve_sonic_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_sonic_desktop_app(&root).is_none(),
