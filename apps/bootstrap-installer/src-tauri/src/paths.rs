...
//!
//! Mirrors `sonic_constants.get_sonic_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\sonic
//!   macOS:   ~/.sonic
//!   Linux:   ~/.sonic  (override via $SONIC_HOME)
//!
//! NOTE (macOS): Python's get_sonic_home(), scripts/install.sh, and the
//! Electron desktop's resolveSonicHome() ALL use ~/.sonic on macOS — there
...
/// Returns the canonical Sonic home directory, respecting $SONIC_HOME if set.
pub fn sonic_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SONIC_HOME") {
    ...
    {
        // %LOCALAPPDATA%\sonic — matches scripts/install.ps1's $SonicHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("sonic");
        }
    }

    // macOS + Linux + fallback: ~/.sonic (matches Python get_sonic_home(),
    // install.sh, and the Electron desktop's resolveSonicHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".sonic");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    sonic_home().join("bootstrap-cache")
}
...
/// SONIC_HOME so it survives repo checkout deletion (unlike anything under
/// sonic-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\sonic\sonic-setup.exe`; on other
...
pub fn copy_self_to_sonic_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SONIC_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SONIC_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".sonic-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SONIC_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[sonic-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SONIC_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_sonic_home() -> String {
    sonic_home().to_string_lossy().into_owned()
}
