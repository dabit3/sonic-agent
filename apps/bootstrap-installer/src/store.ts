...
/// desktop app handing off via `Sonic-Setup.exe --update`.
...
export const $sonicHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, sonicHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_sonic_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $sonicHome.set(sonicHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Sonic" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Sonic-Setup.exe --update);
...
export async function launchSonicDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_sonic_desktop', { installRoot })
}
