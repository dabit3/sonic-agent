 ...
 * Background
 * ----------
 * After `sonic update` + `sonic desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything SONIC_DESKTOP_* is preserved
// plus SONIC_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['SONIC_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['SONIC_DESKTOP_']
