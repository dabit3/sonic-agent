...
// Regression guards for Windows `sonic` resolution in main.cjs.
//
...
//      Git-Bash `sonic` shim shadowed the real sonic.cmd/sonic.exe; the
...
//      --repair by checking ONLY venv\Scripts\sonic.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow sonic.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\sonic.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvSonic\) \?/,
    'recovery regressed to gating only on the sonic.exe shim, which forces destructive --repair'
  )
})
