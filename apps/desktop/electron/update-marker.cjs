...
function markerPath(sonicHome) {
  return path.join(sonicHome, '.sonic-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(sonicHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(sonicHome)
  let raw
  try {
