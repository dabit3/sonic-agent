...
// even though a fresh shell — and the Sonic CLI — sees it immediately. The
// desktop's SONIC_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\sonic. Reading
