...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('exit', (code, signal) => {
    ...
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Sonic backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })
...
ipcMain.handle('sonic:setting:defaultProjectDir:get', async () => ({
  ...
  defaultLabel: path.join(app.getPath('home'), 'sonic-projects')
}))
