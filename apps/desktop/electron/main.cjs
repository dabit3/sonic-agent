...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig()) {
  ...
  return {
    ...
    envOverride: Boolean(process.env.SONIC_DESKTOP_REMOTE_URL)
  }
}
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (config.mode === 'remote') {
    ...
    const remote = (await resolveRemoteBackend()) || (await startSonic())
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    throw new Error('Profiles are unavailable when connected to a remote Sonic backend.')
  }
...
ipcMain.handle('sonic:connection-config:get', async () => sanitizeDesktopConnectionConfig())
