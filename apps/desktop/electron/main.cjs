...
// HEAD commit of ACTIVE_SONIC_ROOT so an adopted marker carries the same
...
function readActiveHeadCommit() {
  try {
    const sha = execFileSync(resolveGitBinary(), ['-C', ACTIVE_SONIC_ROOT, 'rev-parse', 'HEAD'], {
      encoding: 'utf8',
      stdio: ['ignore', 'pipe', 'ignore']
    }).trim()
...
function resolveSonicBackend(dashboardArgs) {
  ...
  // 3b. Existing-but-unmarked install at ACTIVE_SONIC_ROOT. The marker is
  ...
  //     the WHOLE install on top of a working one. ACTIVE_SONIC_ROOT is our
  //     canonical location (unlike a random `sonic` on PATH), so adopt it:
  ...
  if (!forceBootstrapRepair && isActiveRuntimeUsable()) {
    rememberLog(`[bootstrap] adopting existing install at ${ACTIVE_SONIC_ROOT}; skipping first-launch setup`)
    try {
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  const remote =
    config.mode === 'remote'
      ...
      : resolveRemoteBackend() || (await startSonic())
