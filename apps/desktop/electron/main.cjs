...
async function spawnPoolBackend(profile, entry) {
  ...
  let webDist
  try {
    backend = await ensureRuntime(resolveSonicBackend(dashboardArgs))
    sonicCwd = resolveSonicCwd()
...
async function startSonic() {
  ...
  connectionPromise = (async () => {
    ...
    sonicProcess = spawn(backend.command, backend.args, hiddenWindowsChildOptions({
      cwd: sonicCwd,
      env: {
        ...process.env,
...
ipcMain.handle('sonic:window:openSession', async (_event, sessionId) => {
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'sonic_cli.main', 'uninstall', '--gui-summary'], hiddenWindowsChildOptions({
        cwd: agentRoot,
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SONIC_PROTOCOL, process.execPath, [
