...
//   - sonic_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - sonic_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / sonic #37247). When the AT cookie
...
const RT_COOKIE_VARIANTS = ['__Host-sonic_session_rt', '__Secure-sonic_session_rt', 'sonic_session_rt']
