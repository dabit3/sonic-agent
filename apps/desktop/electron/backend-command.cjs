...
// Backend subcommand routing for the desktop-managed Sonic process.
//
// The desktop app launches its own headless backend via `sonic serve` — it
...
// app hasn't updated yet, or an older `sonic` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Sonic profile to pin via `--profile`.
 */
 ...
 * `-m sonic_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `sonic_cli/subcommands/dashboard.py` source registers
