...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'sonic_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'sonic_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'sonic_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Sonic.app/Contents/MacOS/Sonic', 'darwin'),
    '/Applications/Sonic.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Sonic.app/Contents/MacOS/Sonic', 'darwin'),
    '/Users/x/Applications/Sonic.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Sonic\\Sonic.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Sonic'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\sonic-desktop\\Sonic.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\sonic-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Sonic.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_SonicXXXX/sonic', 'linux', { APPIMAGE: '/home/x/Apps/Sonic.AppImage' }),
    '/home/x/Apps/Sonic.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/sonic/linux-unpacked/sonic', 'linux', {}), '/opt/sonic/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/sonic', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Sonic.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Sonic.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.sonic/sonic-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.sonic/sonic-agent',
    uninstallArgs: ['-m', 'sonic_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/sonic/linux-unpacked',
    sonicHome: '/home/x/.sonic'
  })
  ...
  assert.match(script, /'-m' 'sonic_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/sonic\/linux-unpacked'/)
  assert.match(script, /export SONIC_HOME='\/home\/x\/\.sonic'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.sonic/sonic-agent',
    agentRoot: '/home/x/.sonic/sonic-agent',
    uninstallArgs: ['-m', 'sonic_cli.uninstall', '--mode', 'full'],
    appPath: null,
    sonicHome: '/home/x/.sonic'
  })
  // System python + source on PYTHONPATH so import sonic_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.sonic\/sonic-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'sonic_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'sonic_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    sonicHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'sonic_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\sonic',
    uninstallArgs: ['-m', 'sonic_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Sonic',
    sonicHome: 'C:\\Users\\x\\AppData\\Local\\sonic'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import sonic_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\sonic;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "sonic_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Sonic" >nul 2>&1/)
