 ...
 *
 * Drives apps/desktop's first-launch install of Sonic Agent by spawning
...
function cachedScriptPath(sonicHome, commit) {
  return path.join(bootstrapCacheDir(sonicHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, sonicHome, emit }) {
  ...
  const cached = cachedScriptPath(sonicHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, sonicHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...process.env,
        // Pass SONIC_HOME through so install.ps1 respects the caller's
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, sonicHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, sonicHome }) {
  const args = ['--dir', activeRoot, '--sonic-home', sonicHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, sonicHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, sonicHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, sonicHome, activeRoot, abortSignal, installStamp }) {
  const startedAt = Date.now()
  emit({ type: 'stage', name: stage.name, state: 'running' })

  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--stage', stage.name, '--non-interactive', '--json', ...buildPosixPinArgs({ installStamp, activeRoot, sonicHome })]
    : ['-Stage', stage.name, '-NonInteractive', '-Json', ...buildPinArgs(installStamp)]
  const result = await (isPosix ? spawnBash : spawnPowerShell)(
    scriptPath,
    args,
    { emit, stageName: stage.name, abortSignal, sonicHome }
  )
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(sonicHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, sonicHome, emit })
