 ...
 *      internals; SONIC_HOME / SONIC_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.sonic/sonic-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'sonic')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Sonic12345/AppRun', ROOT, 'linux'), null)
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'sonic')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'sonic://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]
  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'sonic://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves SONIC_HOME + SONIC_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    SONIC_HOME: '/home/u/.sonic',
    SONIC_DESKTOP_REMOTE_URL: 'http://box:9119',
    SONIC_DESKTOP_REMOTE_TOKEN: 'secret',
    SONIC_DESKTOP_SONIC_ROOT: '/home/u/dev/sonic',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.sonic/sonic-agent/apps/desktop/release/linux-unpacked/Sonic',
    args: ['sonic://open/agent/42', "--note=it's fine"],
    env: { SONIC_HOME: '/home/u/.sonic', SONIC_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export SONIC_HOME='\/home\/u\/\.sonic'/)
  assert.match(script, /export SONIC_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Sonic' 'sonic:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `sonic-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Sonic/Sonic',
    args: [],
    env: {},
    cwd: ''
  })
  const tmp = path.join(os.tmpdir(), `sonic-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  try {
    execFileSync('bash', ['-n', tmp], { stdio: 'pipe' })
  } finally {
  ...
  assert.match(script, /exec '\/opt\/Sonic\/Sonic'\n/)
})
