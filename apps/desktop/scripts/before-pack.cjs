 ...
 * product name (`Sonic`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Sonic'
 *
 * This is a hard failure with no obvious cause for the user — `sonic desktop`
 ...
 * (the mac-unpacked Sonic.app bundle) and Windows (win-unpacked), so we
