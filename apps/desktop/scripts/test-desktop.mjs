...
// no longer bundles the Sonic Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Sonic.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Sonic'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Sonic.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'sonic'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SONIC_HOME for non-sandboxed runs -- matches main.cjs's
// resolveSonicHome(). On Windows it's %LOCALAPPDATA%\sonic; elsewhere
// it's ~/.sonic. The fresh-install sandbox launchFresh() sets its own
// SONIC_HOME and never touches this.
const DEFAULT_SONIC_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'sonic')
  }
  return path.join(os.homedir(), '.sonic')
})()
const VENV_ROOT = path.join(DEFAULT_SONIC_HOME, 'sonic-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'sonic-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SONIC_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Sonic-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Sonic-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Sonic-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Sonic-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SONIC_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SONIC_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const sonicHome = path.join(sandbox, 'sonic-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(sonicHome, { recursive: true })
  ...
  env.SONIC_DESKTOP_CWD = cwd
  env.SONIC_DESKTOP_IGNORE_EXISTING = '1'
  env.SONIC_DESKTOP_TEST_MODE = 'fresh-install'
  env.SONIC_DESKTOP_USER_DATA_DIR = userDataDir
  env.SONIC_HOME = sonicHome
  delete env.SONIC_DESKTOP_SONIC
  delete env.SONIC_DESKTOP_SONIC_ROOT
  ...
  console.log(`  SONIC_HOME: ${sonicHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(sonicHome, 'sonic-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Sonic Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of sonic_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'sonic-agent', 'sonic_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Sonic
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SONIC_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SONIC_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
