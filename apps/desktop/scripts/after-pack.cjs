 ...
 *
 * Stamps the Sonic icon + identity onto the packed Windows Sonic.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `sonic desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Sonic')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Sonic'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Sonic.exe keeps the stock Electron icon`)
  }
}
