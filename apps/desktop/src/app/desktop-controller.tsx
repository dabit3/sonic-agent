...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../sonic'
...
import { useSonicConfig } from './session/hooks/use-sonic-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.sonicDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.sonicDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.sonicDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.sonicDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // sonic:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.sonicDesktop?.onDeepLink?.(payload => {
    ...
    void window.sonicDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.sonicDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshSonicConfig, sttEnabled, voiceMaxRecordingSeconds } = useSonicConfig({
    activeSessionIdRef,
  ...
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshSonicConfig])
