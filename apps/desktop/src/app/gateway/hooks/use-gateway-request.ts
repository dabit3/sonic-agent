...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<SonicGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.sonicDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SonicGateway | null> | null>(null)
