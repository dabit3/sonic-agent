...
// bug that drives the REAL useGatewayBoot hook + REAL SonicGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshSonicConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { sonicDesktop?: unknown }).sonicDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { sonicDesktop?: unknown }).sonicDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSonic) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSonic()'s remote branch awaits waitForSonic() for 45s before it
    ...
    ;(window as { sonicDesktop?: unknown }).sonicDesktop = desktop
    ...
    // After ~45s waitForSonic gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Sonic backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
