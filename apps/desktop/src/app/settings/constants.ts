...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Sonic & Nous-trained models',
  ...
  { prefix: 'SONIC_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'SONIC_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // sonic_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Sonic stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Sonic can read from one file request.',
  approvals: {
    mode: 'How Sonic handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Sonic updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
