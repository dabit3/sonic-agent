...
export function GatewaySettings() {
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.sonicDesktop.getConnectionConfig(scope)
        setState(refreshed)
    ...
    <SettingsContent>
      <div className="mb-5">
        ...
        <p className="mt-2 max-w-2xl text-[length:var(--conversation-caption-font-size)] leading-(--conversation-caption-line-height) text-(--ui-text-tertiary)">
          ...
          an already-running Sonic backend on another machine or behind a trusted proxy. Pick a profile below to give it
          its own remote host.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <ModeCard
          ...
          description="Connect this desktop shell to a remote Sonic backend. Hosted gateways use OAuth or a username and password; self-hosted ones may use a session token."
          disabled={state.envOverride}
          icon={Globe}
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.sonicDesktop?.revealLogs()} size="sm" variant="textStrong">
