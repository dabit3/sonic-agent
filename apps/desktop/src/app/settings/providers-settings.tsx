...
import { disconnectOAuthProvider, listOAuthProviders } from '@/sonic'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.sonicDesktop?.terminal)
...
//      in sonic_cli/provider_catalog.py) — the SAME provider identity
//      `sonic model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Sonic can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Sonic can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Sonic never deletes creds another tool owns behind a silent API call.
