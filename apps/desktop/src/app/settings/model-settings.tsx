...
import type { AuxiliaryModelsResponse, MoaConfigResponse, MoaModelSlot, ModelOptionProvider, StaleAuxAssignment } from '@/sonic'
...
// Sonic' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Sonic default (medium), shown as Medium.
...
// backend now surfaces the full `sonic model` universe (every canonical
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `sonic model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Sonic runs the flow for you.`}
          </p>
        )}
