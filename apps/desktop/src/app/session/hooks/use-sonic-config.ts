...
import { getSonicConfig, getSonicConfigDefaults } from '@/sonic'
...
interface SonicConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useSonicConfig({ activeSessionIdRef, refreshProjectBranch }: SonicConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshSonicConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getSonicConfig(), getSonicConfigDefaults().catch(() => ({}))])
  ...
  return { refreshSonicConfig, sttEnabled, voiceMaxRecordingSeconds }
}
