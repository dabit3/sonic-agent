...
export type ProjectTreeEntry = SonicReadDirEntry
...
async function gitRootFor(start: string) {
  if (!window.sonicDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.sonicDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.sonicDesktop?.readDir || !window.sonicDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.sonicDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.sonicDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: SonicReadDirEntry) {
...
async function filterIgnored(entries: SonicReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SonicReadDirResult> {
  if (!window.sonicDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.sonicDesktop.readDir(dirPath)
