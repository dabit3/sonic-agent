...
import type { SonicReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<SonicReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { sonicDesktop: { readDir: typeof readDir } }).sonicDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { sonicDesktop?: unknown }).sonicDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SonicReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { sonicDesktop: unknown }).sonicDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SonicReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SonicReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { sonicDesktop: unknown }).sonicDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.sonicDesktop is missing', async () => {
    delete (window as unknown as { sonicDesktop?: unknown }).sonicDesktop
