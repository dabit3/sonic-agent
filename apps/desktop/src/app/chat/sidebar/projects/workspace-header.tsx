...
const branchActionLabel = (branch: SonicGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `sonic/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<SonicGitBranch[]>([])
  ...
  const convert = async (branch: SonicGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
