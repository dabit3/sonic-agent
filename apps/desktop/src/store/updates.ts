...
const UPDATE_TOAST_SNOOZE_KEY = 'sonic:update-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: { label: translateNow('notifications.updateSonic'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  ...
  // /api/sonic/update/check so behind-count + "what's changed" describe the
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  // path the command-center "Update Sonic" button uses), then poll the action
