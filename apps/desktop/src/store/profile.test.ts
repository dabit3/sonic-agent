...
const remoteConn = (over: Partial<SonicConnection> = {}): SonicConnection =>
  ({ baseUrl: 'https://sonic-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SonicConnection

const localConn = (over: Partial<SonicConnection> = {}): SonicConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SonicConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SonicConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { sonicDesktop: { getConnection } })
})
