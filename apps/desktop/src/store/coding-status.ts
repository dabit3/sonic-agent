...
import type { SonicGitWorktree, SonicRepoStatus } from '@/global'
...
export const $repoStatus = atom<SonicRepoStatus | null>(null)
...
export const $repoWorktrees = atom<SonicGitWorktree[]>([])
...
async function loadWorktrees(target: string): Promise<void> {
  const list = window.sonicDesktop?.git?.worktreeList
...
export async function refreshRepoStatus(cwd?: null | string): Promise<void> {
  ...
  const probe = window.sonicDesktop?.git?.repoStatus
