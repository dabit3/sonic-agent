...
import { getSonicConfigRecord, saveSonicConfig } from '@/sonic'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSonicConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSonicConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
