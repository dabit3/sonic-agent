 ...
 * app's bounds and stays visible while Sonic is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'sonic.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'sonic.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.sonicDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.sonicDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.sonicDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.sonicDesktop?.petOverlay?.close()
}
