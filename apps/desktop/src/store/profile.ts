...
import { getProfiles, setApiRequestProfile } from '@/sonic'
...
// /api/profiles/active `current`). "default" is the root ~/.sonic. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'sonic.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'sonic.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.sonicDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new SONIC_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.sonicDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.sonicDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'sonic.desktop.showAllProfiles'
...
// Switch to the default (root ~/.sonic) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.sonicDesktop?.touchBackend?.(target).catch(() => undefined)
}
