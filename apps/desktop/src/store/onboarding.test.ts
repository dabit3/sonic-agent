...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('sonic-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('sonic-desktop-onboarded-v1')).toBe('1')
  })
