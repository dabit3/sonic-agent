...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('sonic.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('sonic.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  })
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('sonic.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
