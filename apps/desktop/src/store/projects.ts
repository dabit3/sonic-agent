...
import type { SonicGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/sonic'
...
const PROJECT_SCOPE_KEY = 'sonic.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Sonic gateway is not connected')
  }
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  const scan = window.sonicDesktop?.git?.scanRepos
...
async function writeProjectIdea(folder: null | string | undefined, idea: string): Promise<void> {
  ...
  const write = window.sonicDesktop?.writeTextFile
...
// repo, returning where Sonic should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SonicGitBranch[]> {
  const git = window.sonicDesktop?.git
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.sonicDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.sonicDesktop?.writeClipboard?.(path)
  }
}
...
export async function pickProjectFolder(): Promise<null | string> {
  const pick = window.sonicDesktop?.settings?.pickDefaultProjectDir
