...
import type { SonicReviewFile, SonicReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Sonic' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'sonic.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'sonic.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'sonic.desktop.reviewTreeMode'
const SELECTED_KEY = 'sonic.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SonicReviewFile[]>([])
...
export const $reviewShipInfo = atom<SonicReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['sonicDesktop']>['git']>['review']>
...
function reviewCtx(): { cwd: string; review: ReviewBridge } | null {
  const cwd = repoCwd()
  const review = window.sonicDesktop?.git?.review
...
export async function selectReviewFile(file: SonicReviewFile): Promise<void> {
...
export async function stageReviewFile(path: null | string): Promise<void> {
  await window.sonicDesktop?.git?.review?.stage(repoCwd() ?? '', path)
  await afterMutation()
}

export async function unstageReviewFile(path: null | string): Promise<void> {
  await window.sonicDesktop?.git?.review?.unstage(repoCwd() ?? '', path)
  await afterMutation()
}

export async function revertReviewFile(path: null | string): Promise<void> {
  await window.sonicDesktop?.git?.review?.revert(repoCwd() ?? '', path)
  await afterMutation()
}
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.sonicDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.sonicDesktop?.openExternal?.(url)
    }
