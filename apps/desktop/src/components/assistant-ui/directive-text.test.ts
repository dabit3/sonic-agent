...
import { formatRefValue, sonicDirectiveFormatter } from './directive-text'
...
describe('sonicDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = sonicDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = sonicDirectiveFormatter.parse('@file:src/main.tsx the entry point')
