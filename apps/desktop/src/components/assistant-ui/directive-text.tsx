...
const SONIC_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.sonicDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.sonicDesktop?.readFileDataUrl(id)
