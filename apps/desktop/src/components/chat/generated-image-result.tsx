...
async function resolveImageSrc(path: string): Promise<string> {
  ...
  if (window.sonicDesktop && isRemoteGateway()) {
  ...
  return window.sonicDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
    ...
    return (
      <a
        ...
        onClick={event => {
          ...
          void window.sonicDesktop?.openExternal(mediaExternalUrl(image))
        }}
      >
