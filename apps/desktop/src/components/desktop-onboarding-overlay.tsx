...
const FLOW_SUBTITLES: Record<OAuthProvider['flow'], string> = {
  ...
  loopback: 'Opens your browser to sign in — Sonic connects automatically',
...
function FlowPanel({ ctx, flow }: { ctx: OnboardingContext; flow: OnboardingFlow }) {
  ...
  if (flow.status === 'awaiting_browser') {
    return (
      <Step title={`Sign in with ${title}`}>
        <p className="text-sm text-muted-foreground">
          We opened {title} in your browser. Authorize Sonic there and you'll be connected automatically — nothing to
          copy or paste.
        </p>
