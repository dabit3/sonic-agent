...
const SKIN_KEY = 'sonic-desktop-theme-v2'
const MODE_KEY = 'sonic-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'sonic-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'sonic-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'sonic-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.sonicTheme = skinName
  root.dataset.sonicMode = rendered
  ...
  window.sonicDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('sonic-boot-background', chromeBg)
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.sonicThemeFont = 'true'
...
/** Sync the desktop skin with the active Sonic backend theme on connect. */
