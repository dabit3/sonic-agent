...
import { getSonicConfigRecord, type SonicConfigRecord, saveSonicConfig } from '@/sonic'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SonicConfigRecord>
  saveConfig: (config: SonicConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.sonicDesktop?.api) {
      return Promise.resolve({})
    }

    return getSonicConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.sonicDesktop?.api) {
...
export function getConfigDisplayLanguage(config: SonicConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SonicConfigRecord, locale: Locale): SonicConfigRecord {
