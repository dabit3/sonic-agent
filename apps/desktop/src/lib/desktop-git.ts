...
// (window.sonicDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['sonicDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.sonicDesktop

  if (!desktop) {
    throw new Error('Sonic Desktop bridge is unavailable')
  }
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: SonicGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: SonicGitBranch[] }>('branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<SonicRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<SonicReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<SonicReviewShipInfo>('review/ship-info', { path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  return isDesktopFsRemoteMode() ? remoteGit : window.sonicDesktop?.git
}
