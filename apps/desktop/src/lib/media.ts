...
export function mediaStreamUrl(path: string): string {
  return `sonic-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// expose GET /api/media (sonic_cli/web_server.py).
export async function gatewayMediaDataUrl(path: string): Promise<string> {
  const file = filePathFromMediaPath(path)

  const result = await window.sonicDesktop!.api<{ data_url: string }>({
