...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Sonic doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in sonic_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `sonic model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `sonic model` CLI / onboarding OAuth flow. */
  auth_type?: string
 ...
 *  not Sonic); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/sonic/update/check` — the backend's own update state.
