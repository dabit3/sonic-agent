"""Centralized logging setup for Lightning Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.lightning/logs/`` (profile-aware via ``get_lightning_home()``).
...
from lightning_constants import get_config_path, get_lightning_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_lightning_session_injector", False):
    ...
    _session_record_factory._lightning_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``lightning logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "lightning_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("lightning_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the Lightning logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    lightning_home
        Override for the Lightning home directory.  Falls back to
        ``get_lightning_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_lightning_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._lightning_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the lightning group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from lightning_cli.config import is_managed
