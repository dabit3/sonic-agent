...
:::info
아래 내용은 이 skill이 트리거될 때 Sonic가 실제로 로드하는 **전체 skill 정의**입니다. 즉, skill이 활성화되었을 때 agent가 실제 지침으로 보는 텍스트입니다.
:::
...
**Tenant 상속.** env에 `SONIC_TENANT`가 설정되어 있다면, 모든 `kanban_create` 호출에 `tenant=os.environ.get("SONIC_TENANT")`를 넣어 child task도 같은 namespace에 머물게 하세요.
