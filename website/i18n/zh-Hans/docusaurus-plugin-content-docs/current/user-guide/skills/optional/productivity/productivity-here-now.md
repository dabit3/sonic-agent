...
| | |
|---|---|
| 来源 | 可选 — 通过 `sonic skills install official/productivity/here-now` 安装 |
...
- Skill 辅助脚本路径：
  - `${SONIC_SKILL_DIR}/scripts/publish.sh` 用于发布站点
  - `${SONIC_SKILL_DIR}/scripts/drive.sh` 用于私有 Drive 存储

## 创建站点

```bash
PUBLISH="${SONIC_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client sonic
```
...
```bash
PUBLISH="${SONIC_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client sonic
```
...
```bash
DRIVE="${SONIC_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}` | 用于归因的 agent 名称（如 `sonic`） |
