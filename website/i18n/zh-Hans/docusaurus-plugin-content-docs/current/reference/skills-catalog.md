...
sidebar_position: 5
title: "内置技能目录"
description: "随 Sonic Agent 附带的内置技能目录"
---

# 内置技能目录

Sonic 附带一个大型内置技能库，安装时会复制到 `~/.sonic/skills/`。下方每个技能均链接至专属页面，包含完整定义、配置和用法说明。

Sonic 在执行 `sonic update` 时也会同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.sonic/skills/` 目录树中缺失，它仍随 Sonic 一同发布；可通过 `sonic skills reset <name> --restore` 恢复。
...
| [`sonic-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sonic-agent) | 配置、扩展或贡献 Sonic Agent。 | `autonomous-ai-agents/sonic-agent` |
