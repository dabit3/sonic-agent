...
sonic portal info        # 随时查看登录状态和路由信息
```
...
付费 Nous Portal 订阅者还可访问 **[Tool Gateway](/user-guide/features/tool-gateway)**——网页搜索、图像生成、TTS 和浏览器自动化，均通过你的订阅路由。无需额外 API key。全新安装时，`sonic setup --portal` 一条命令即可完成登录、设置 Nous 为提供商并开启网关。现有用户可通过 `sonic model` 或 `sonic tools` 按工具启用。随时使用 `sonic portal info` 查看路由状态。
:::
