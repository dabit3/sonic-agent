...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/dabit3/sonic-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    sonic_meta = (fm.get("metadata") or {}).get("sonic") or {}
    tags = sonic_meta.get("tags") or []
    related = sonic_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sonic skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Sonic Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Sonic ships with a large built-in skill library copied into `~/.sonic/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Sonic also syncs bundled skills on `sonic update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sonic/skills/` tree, it is still shipped with Sonic; restore it with `sonic skills reset <name> --restore`.",
        "",
