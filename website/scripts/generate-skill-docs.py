...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/lightning-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    lightning_meta = (fm.get("metadata") or {}).get("lightning") or {}
    tags = lightning_meta.get("tags") or []
    related = lightning_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lightning skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lightning Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lightning ships with a large built-in skill library copied into `~/.lightning/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lightning also syncs bundled skills on `lightning update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lightning/skills/` tree, it is still shipped with Lightning; restore it with `lightning skills reset <name> --restore`.",
        "",
