...
- **Anthropic** — Claude models (direct API, OAuth via `sonic login anthropic`, OpenRouter, or any compatible proxy)
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Sonic — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
