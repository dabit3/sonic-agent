...
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `SONIC_TUI=1`. Always wins over `display.interface`. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Sonic itself. |
...
sonic skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
sonic skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
sonic skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- **npm lockfile churn.** Before stashing or switching branches, Sonic makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `sonic update`.
