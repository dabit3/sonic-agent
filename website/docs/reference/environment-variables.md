...
All variables go in `~/.sonic/.env`. You can also set them with `sonic config set VAR value`.
...
| `SONIC_AGENT_TIMEOUT` | Gateway inactivity timeout for a running agent in seconds (default: `900`). Resets on every tool call and streamed token. Set to `0` to disable. |
