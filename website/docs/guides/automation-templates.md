...
Copy-paste recipes for common automation patterns. Each template uses Sonic's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`sonic webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `sonic webhook subscribe`) |
...
```bash
sonic cron create "0 2 * * *" \
  "You are a project manager triaging the dabit3/sonic-agent GitHub repo.

1. Run: gh issue list --repo dabit3/sonic-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
sonic webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
sonic cron create "0 9 * * 1" \
  "Scan the dabit3/sonic-agent repo for documentation drift.

1. Run: gh pr list --repo dabit3/sonic-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (sonic_cli/commands.py, sonic_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (sonic_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
sonic cron create "0 6 * * *" \
  "Run a dependency security audit on the sonic-agent project.

1. cd ~/.sonic/sonic-agent && source .venv/bin/activate
...
```bash
sonic webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
sonic webhook subscribe alert-triage \
...
```python title="~/.sonic/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Sonic-Monitor/1.0"})
...
sonic cron create "every 30m" \
  ...
  --script ~/.sonic/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
sonic cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Sonic Agent development." \
...
```bash
sonic webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
sonic cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the sonic-agent codebase.
