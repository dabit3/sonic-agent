...
Copy-paste recipes for common automation patterns. Each template uses Lightning's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`lightning webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `lightning webhook subscribe`) |
...
```bash
lightning cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/lightning-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/lightning-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
lightning webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
lightning cron create "0 9 * * 1" \
  "Scan the NousResearch/lightning-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/lightning-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (lightning_cli/commands.py, lightning_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (lightning_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
lightning cron create "0 6 * * *" \
  "Run a dependency security audit on the lightning-agent project.

1. cd ~/.lightning/lightning-agent && source .venv/bin/activate
...
```bash
lightning webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
lightning webhook subscribe alert-triage \
...
```python title="~/.lightning/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Lightning-Monitor/1.0"})
...
```bash
lightning cron create "every 30m" \
  ...
  --script ~/.lightning/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
lightning cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Lightning Agent development." \
...
```bash
lightning webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
lightning cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the lightning-agent codebase.
