...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Sonic Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`sonic claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Sonic. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::tip
If your OpenClaw setup was multi-provider, `sonic setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
sonic claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.sonic/` (by default a single restore-point archive is written before apply, under `~/.sonic/backups/pre-migration-*.zip`; restorable with `sonic import`). |
...
| What | OpenClaw source | Sonic destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.sonic/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.sonic/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.sonic/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.sonic/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Sonic destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.sonic/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.sonic/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.sonic/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.sonic/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Sonic skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Sonic destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.sonic/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Sonic config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Sonic config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Sonic field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.sonic/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Sonic `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Sonic path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `SONIC_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Sonic equivalent)

These are saved to `~/.sonic/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Sonic |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Sonic has built-in tool instructions |
...
| Multi-agent list | `archive/agents-list.json` | Use Sonic profiles |
...
8. **Archive cleanup** — after confirming everything works, run `sonic claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `sonic config set`.

### Skills not appearing after migration

Imported skills land in `~/.sonic/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `sonic config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
