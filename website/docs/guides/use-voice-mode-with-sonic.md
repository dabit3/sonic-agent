---
sidebar_position: 8
title: "Use Voice Mode with Sonic"
description: "A practical guide to setting up and using Sonic voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Sonic
...
- you want Sonic sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Sonic.
...
## Step 1: make sure normal Sonic works first
...
Sonic supports both local and cloud speech stacks.
...
### If you use `sonic setup`

If you choose NeuTTS in the setup wizard, Sonic checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Sonic transcribes and responds
...
- asking Sonic to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Sonic loop.

## Tuning CLI behavior

### Silence threshold

If Sonic starts/stops too aggressively, tune:
...
Sonic stays a normal chat bot, but can speak replies.
...
- you want Sonic to function like a portable research or ops assistant
...
Sonic joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Sonic detects speech boundaries
- transcripts are posted in the associated text channel
- Sonic responds in text and audio
...
1. get text Sonic working
2. install `sonic-agent[voice]`
