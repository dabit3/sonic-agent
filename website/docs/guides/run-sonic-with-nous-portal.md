---
sidebar_position: 1
title: "Run Sonic with Nous Portal"
...
3. Sets `model.provider: nous` in `~/.sonic/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Sonic is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
sonic setup --portal                              # on the remote, open the printed URL in your local browser
...
```
Hey, search the web for "Sonic release notes" and summarize the top 3 hits.
```

You should see Sonic call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Sonic will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
sonic cron create "every day at 9am" \
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Sonic doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/sonic-agent/issues) — most gaps are routing config we can update.
