...
If the consent page renders the authorization code directly on the page (xAI's current behavior on browser-based consoles) instead of redirecting to your `127.0.0.1:56121/callback`, paste **just the bare code value** at the `Callback URL:` prompt — Sonic accepts the full URL, a bare `?code=...&state=...` query fragment, or a bare code interchangeably.
...
4. From then on, Sonic refreshes the access token in the background — you stay signed in until you `sonic auth logout xai-oauth` or revoke access from your xAI account settings.
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `sonic setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.
