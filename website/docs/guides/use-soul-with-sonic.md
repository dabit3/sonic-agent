---
sidebar_position: 7
title: "Use SOUL.md with Sonic"
description: "How to use SOUL.md to shape Sonic Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Sonic

`SOUL.md` is the **primary identity** for your Sonic instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Sonic to feel like the same assistant every time you talk to it — or if you want to replace the Sonic persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Sonic should be
- what Sonic should avoid stylistically
- how Sonic should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Sonic is and how Sonic speaks
...
Sonic now uses only the global SOUL file for the current instance:

```text
~/.sonic/SOUL.md
```

If you run Sonic with a custom home directory, it becomes:
...
Sonic automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Sonic does not overwrite it
- if the file exists but is empty, Sonic adds nothing from it to the prompt

## How Sonic uses it

When Sonic starts a session, it reads `SOUL.md` from `SONIC_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Sonic feels.
...
Sonic already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Sonic should sound.

# Avoid
What Sonic should not do.

# Defaults
How Sonic should behave when ambiguity appears.
```
...
Then restart Sonic or start a new session.
...
### I edited SOUL.md but Sonic still sounds the same

Check:
- you edited `~/.sonic/SOUL.md` or `$SONIC_HOME/SOUL.md`
...
### Sonic is ignoring parts of my SOUL.md
