...
This tutorial walks you through setting up a Telegram bot powered by Lightning Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Lightning Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.lightning/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Lightning itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Lightning Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_lightning_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Lightning Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.lightning/.env`:
...
```
[Gateway] Starting Lightning Gateway...
...
# View live logs
journalctl --user -u lightning-gateway -f
...
lightning gateway start
lightning gateway stop
tail -f ~/.lightning/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `lightning gateway install` to update the plist.
:::
...
```bash
lightning gateway stop && lightning gateway start
```
...
lightning pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.lightning/.env`:
...
Control how much detail the bot shows when using tools. In `~/.lightning/config.yaml`:
...
Customize how the bot communicates by editing `~/.lightning/SOUL.md`:

For a full guide, see [Use SOUL.md with Lightning](/docs/guides/use-soul-with-lightning).
...
```bash
# From the CLI
lightning cron list          # View all scheduled jobs
lightning cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u lightning-gateway` (Linux) or `~/.lightning/logs/gateway.log` (macOS) |
| Cron job output | `~/.lightning/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.lightning/cron/jobs.json` |
| Pairing data | `~/.lightning/pairing/` |
| Session history | `~/.lightning/sessions/` |
