...
This tutorial walks you through setting up a Telegram bot powered by Sonic Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Sonic Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.sonic/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Sonic itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Sonic Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_sonic_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Sonic Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Sonic Gateway...
...
# View live logs
journalctl --user -u sonic-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `sonic gateway install` to update the plist.
:::
...
```bash
sonic gateway stop && sonic gateway start
```
...
sonic pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.sonic/.env`:
...
Control how much detail the bot shows when using tools. In `~/.sonic/config.yaml`:
...
Customize how the bot communicates by editing `~/.sonic/SOUL.md`:

For a full guide, see [Use SOUL.md with Sonic](/docs/guides/use-soul-with-sonic).
...
```bash
# From the CLI
sonic cron list          # View all scheduled jobs
sonic cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u sonic-gateway` (Linux) or `~/.sonic/logs/gateway.log` (macOS) |
| Cron job output | `~/.sonic/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.sonic/cron/jobs.json` |
| Pairing data | `~/.sonic/pairing/` |
| Session history | `~/.sonic/sessions/` |
