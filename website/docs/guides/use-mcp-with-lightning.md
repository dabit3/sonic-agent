---
sidebar_position: 6
title: "Use MCP with Lightning"
description: "A practical guide to connecting MCP servers to Lightning Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Lightning

This guide shows how to actually use MCP with Lightning Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Lightning tool
- you want Lightning to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Lightning to internal APIs, databases, or company systems without modifying Lightning core

Do not use MCP when:
- a built-in Lightning tool already solves the job well
...
- Lightning remains the agent
- MCP servers contribute tools
- Lightning discovers those tools at startup or reload time
...
If you installed Lightning with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Lightning banner/status should show MCP integration when configured
- ask Lightning what tools it has available
...
## WSL2: bridge Lightning in WSL to Windows Chrome

This is the practical setup when:

- Lightning runs inside WSL2
...
In this setup, Lightning does **not** connect to Chrome directly. Instead:

- Lightning runs in WSL
- Lightning starts a local stdio MCP server
...
```text
Lightning (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Lightning stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Lightning core browser transport
...
```bash
lightning mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
lightning mcp test chrome-devtools-win
```

Then start a fresh Lightning session or run:
...
Once loaded, Lightning can use the MCP-prefixed browser tools directly. For example:
...
If Lightning runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Lightning from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Lightning from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Lightning:
...
2. Lightning-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Lightning will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Lightning to reason over a bounded workspace.
...
Now Lightning can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Lightning core.
...
Because Lightning now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
