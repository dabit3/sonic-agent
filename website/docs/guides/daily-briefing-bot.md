...
2. **Lightning spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.lightning/cron/output/` and you can read them anytime.
:::
...
Lightning will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Lightning Agent
```
...
Before creating cron jobs, ensure Lightning has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Lightning what you want:
...
Lightning will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Lightning to delegate each topic to a sub-agent:
...
Lightning will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
