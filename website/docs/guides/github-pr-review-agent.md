...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Lightning Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Lightning instantly when PRs are opened or updated.
:::
...
- **Lightning Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  lightning gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.lightning/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Lightning can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/lightning-agent --state open --limit 3
```
...
Still in the chat, ask Lightning to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/lightning-agent
```
...
A skill gives Lightning consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.lightning/skills/code-review
```

Create `~/.lightning/skills/code-review/SKILL.md`:
...
Verify it loaded — start `lightning` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Lightning your team's standards:
...
2. Teach Lightning your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
lightning gateway status    # Is the gateway running?
lightning cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-lightning-plugin)** — wrap the review logic into a shareable plugin
