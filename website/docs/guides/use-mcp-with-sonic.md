---
sidebar_position: 6
title: "Use MCP with Sonic"
description: "A practical guide to connecting MCP servers to Sonic Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Sonic

This guide shows how to actually use MCP with Sonic Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Sonic tool
- you want Sonic to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Sonic to internal APIs, databases, or company systems without modifying Sonic core

Do not use MCP when:
- a built-in Sonic tool already solves the job well
...
- Sonic remains the agent
- MCP servers contribute tools
- Sonic discovers those tools at startup or reload time
...
If you installed Sonic with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Sonic banner/status should show MCP integration when configured
- ask Sonic what tools it has available
...
## WSL2: bridge Sonic in WSL to Windows Chrome

This is the practical setup when:

- Sonic runs inside WSL2
...
In this setup, Sonic does **not** connect to Chrome directly. Instead:

- Sonic runs in WSL
- Sonic starts a local stdio MCP server
...
```text
Sonic (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Sonic stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Sonic core browser transport
...
```bash
sonic mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
sonic mcp test chrome-devtools-win
```

Then start a fresh Sonic session or run:
...
Once loaded, Sonic can use the MCP-prefixed browser tools directly. For example:
...
If Sonic runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Sonic from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Sonic from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Sonic:
...
2. Sonic-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Sonic will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Sonic to reason over a bounded workspace.
...
Now Sonic can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Sonic core.
...
Because Sonic now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
