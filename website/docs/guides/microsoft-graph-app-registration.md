...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.lightning/.env` at the end.
...
4. Fill in:
   - **Name:** `Lightning Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `lightning-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Lightning app
New-CsApplicationAccessPolicy `
  -Identity "Lightning-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Lightning meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Lightning-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.lightning/.env`:
...
Lightning ships a Graph auth smoke-test. From your Lightning install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.lightning/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `lightning gateway restart`.
