...
Subscription-based access to Sonic-4 models (`Sonic-4-70B`, `Sonic-4.3-36B`, `Sonic-4-405B`) via Nous Research's portal. Two paths:

- **Fresh install:** `sonic setup --portal` — runs OAuth, sets Nous as your provider, and offers the [Tool Gateway](/docs/user-guide/features/tool-gateway) in one command.
- **Existing install:** `sonic model`, pick **Nous Portal**, sign in through the browser.

Either path stores a long-lived refresh token at `~/.sonic/auth.json`. Check status with `sonic portal status`.
