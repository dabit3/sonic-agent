...
[PR #12550](https://github.com/dabit3/sonic-agent/pull/12550) proposed a
...
(`sonic-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
- `toolsets.py` — register `browser_dialog` in `browser`, `sonic-acp`, `sonic-api-server`, core toolsets (gated on CDP reachability)
...
- `sonic_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
