...
[PR #12550](https://github.com/NousResearch/lightning-agent/pull/12550) proposed a
...
(`lightning-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Lightning `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `lightning-acp`, `lightning-api-server`, core toolsets (gated on CDP reachability)
...
- `lightning_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
