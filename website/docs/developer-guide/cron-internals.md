...
description: "How Sonic stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `sonic_cli/cron.py` | CLI `sonic cron` subcommands |
...
Jobs are stored in `~/.sonic/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `sonic cron` commands are run or during active CLI sessions.
...
```python
# ~/.sonic/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.sonic/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `sonic cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `sonic cron` CLI provides direct job management:

```bash
sonic cron list                    # Show all jobs
sonic cron create                  # Interactive job creation (alias: add)
sonic cron edit <job_id>           # Edit job configuration
sonic cron pause <job_id>          # Pause a running job
sonic cron resume <job_id>         # Resume a paused job
sonic cron run <job_id>            # Trigger immediate execution
sonic cron remove <job_id>         # Delete a job
```
