...
description: "How Lightning stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `lightning_cli/cron.py` | CLI `lightning cron` subcommands |
...
Jobs are stored in `~/.lightning/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `lightning cron` commands are run or during active CLI sessions.
...
```python
# ~/.lightning/scripts/check_competitors.py
...
2. **Environment variable** — `LIGHTNING_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.lightning/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `lightning cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `lightning cron` CLI provides direct job management:

```bash
lightning cron list                    # Show all jobs
lightning cron create                  # Interactive job creation (alias: add)
lightning cron edit <job_id>           # Edit job configuration
lightning cron pause <job_id>          # Pause a running job
lightning cron resume <job_id>         # Resume a paused job
lightning cron run <job_id>            # Trigger immediate execution
lightning cron remove <job_id>         # Delete a job
```
