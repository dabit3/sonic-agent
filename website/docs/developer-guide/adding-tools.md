...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Sonic Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Sonic tool** to the repository itself.
...
2. **`toolsets.py`** — add tool name to `_SONIC_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `sonic_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `sonic_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `sonic chat -q "Use the weather tool for London"`
