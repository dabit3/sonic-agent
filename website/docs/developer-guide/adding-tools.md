...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Lightning Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Lightning tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Lightning core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Lightning Plugin](/docs/guides/build-a-lightning-plugin)
...
2. **`toolsets.py`** — add tool name to `_LIGHTNING_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `lightning_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `lightning_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `lightning chat -q "Use the weather tool for London"`
