...
Sonic contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY` and `OPENAI_API_KEY`).
...
- `tests/sonic_cli/test_fallback_cmd.py` — the `/fallback` CLI command
