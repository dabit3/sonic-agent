...
description: "How to add a new inference provider to Lightning Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Lightning can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `lightning model` menu entries
...
1. `lightning_cli/auth.py` decides how credentials are found.
2. `lightning_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `lightning_cli/models.py` and `lightning_cli/main.py` make the provider show up in the CLI. (`lightning_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `lightning_cli/auth.py`
2. `lightning_cli/models.py`
3. `lightning_cli/runtime_provider.py`
4. `lightning_cli/main.py`
...
:::tip
`lightning_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `lightning setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$LIGHTNING_HOME/plugins/model-providers/` both get picked up.
...
7. `lightning model` menu includes the provider
8. `lightning setup` wizard delegates to `main.py` automatically
9. `provider:model` alias syntax works
10. Runtime resolver returns the correct `base_url` and `api_key`
11. `LIGHTNING_INFERENCE_PROVIDER` env-var override accepts the provider id
12. Fallback model activation can switch into the provider cleanly

User plugins at `$LIGHTNING_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `lightning model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `lightning_cli/auth.py`
- `_PROVIDER_LABELS` in `lightning_cli/models.py`
- `_PROVIDER_ALIASES` in both `lightning_cli/auth.py` and `lightning_cli/models.py`
- CLI `--provider` choices in `lightning_cli/main.py`
...
## Step 2: Add auth metadata in `lightning_cli/auth.py`
...
- What env vars should Lightning check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `lightning_cli/models.py`
...
## Step 4: Resolve runtime data in `lightning_cli/runtime_provider.py`
...
Be careful with API-key precedence. Lightning already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `lightning_cli/main.py`

A provider is not discoverable until it shows up in the interactive `lightning model` flow.

Update these in `lightning_cli/main.py`:
...
:::tip
`lightning_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `lightning model` and `lightning setup` automatically.
:::
...
When you add a native provider, double-check that Lightning is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m lightning_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m lightning_cli.main model
...
- [ ] `ProviderConfig` added in `lightning_cli/auth.py`
- [ ] aliases added in `lightning_cli/auth.py` and `lightning_cli/models.py`
- [ ] model catalog added in `lightning_cli/models.py`
- [ ] runtime branch added in `lightning_cli/runtime_provider.py`
- [ ] CLI wiring added in `lightning_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `lightning model` but not `lightning setup`
