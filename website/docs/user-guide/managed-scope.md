...
`~/.sonic/config.yaml`, `~/.sonic/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/sonic`:

```text
/etc/sonic/
├── config.yaml     # managed config layer (wins over ~/.sonic/config.yaml)
└── .env            # managed env layer (wins over ~/.sonic/.env + shell)
```
...
The location can be relocated with the `SONIC_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SONIC_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Sonic.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SONIC_MANAGED_DIR=/opt/org/sonic-policy
```

:::warning
A user who can set `SONIC_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `sonic doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/sonic/config.yaml` (managed) | `/etc/sonic/.env` (managed) |
| 2 | `~/.sonic/config.yaml` (user) | `~/.sonic/.env` (user) |
...
```bash
sonic config        # shows a header naming the managed source + the pinned keys
sonic doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Sonic refuses and names the source:

```bash
$ sonic config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/sonic/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `sonic config set` / setup will not write
...
sudo tee /etc/sonic/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/sonic/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/sonic
sudo chmod 0644 /etc/sonic/config.yaml /etc/sonic/.env
```

Changes take effect on the next Sonic start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`sonic doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Sonic as `root`), managed scope is advisory.
