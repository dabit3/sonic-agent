...
<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Sonic session." />
...
`~/.sonic/sessions/sessions.json` is the **gateway routing index** — it maps
...
`sonic sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.sonic/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `sonic sessions list`, the cause is
`state.db` not receiving them — run `sonic sessions repair` and watch for a
