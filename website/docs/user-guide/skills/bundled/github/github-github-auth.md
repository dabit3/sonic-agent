...
- Give it a name like "lightning-agent"
- Select scopes:
...
- Give it a title like "lightning-agent-&lt;machine-name>"
...
elif [ -f ~/.lightning/.env ] && grep -q "^GITHUB_TOKEN=" ~/.lightning/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.lightning/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
