...
| | |
|---|---|
| Source | Optional — install with `sonic skills install official/devops/watchers` |
...
All three live in `$SONIC_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$SONIC_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $SONIC_HOME/skills/devops/watchers/scripts/watch_rss.py \
...
python $SONIC_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name sonic-issues --repo dabit3/sonic-agent --scope issues
```

Poll an arbitrary JSON API:

```bash
python $SONIC_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$SONIC_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $SONIC_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $SONIC_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$SONIC_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
