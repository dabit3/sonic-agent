...
| | |
|---|---|
| Source | Optional — install with `lightning skills install official/devops/pinggy-tunnel` |
| Path | `optional-skills/devops/pinggy-tunnel` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Lightning Agent |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Lightning' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
