...
| | |
|---|---|
| Source | Optional — install with `sonic skills install official/productivity/siyuan` |
...
3. Store it in `${SONIC_HOME:-~/.sonic}/.env`:
   ```
...
```yaml
# In ~/.sonic/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
