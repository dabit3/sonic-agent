...
| | |
|---|---|
| Source | Optional — install with `lightning skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Lightning Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.lightning/.env`:
...
```bash
CANVAS="python $LIGHTNING_HOME/skills/productivity/canvas/scripts/canvas_api.py"
