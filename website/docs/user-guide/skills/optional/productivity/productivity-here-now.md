...
| | |
|---|---|
| Source | Optional — install with `lightning skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LIGHTNING_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LIGHTNING_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${LIGHTNING_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client lightning
```
...
```bash
PUBLISH="${LIGHTNING_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client lightning
```
...
```bash
DRIVE="${LIGHTNING_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `lightning`)    |
