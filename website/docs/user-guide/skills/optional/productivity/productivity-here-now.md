...
| | |
|---|---|
| Source | Optional — install with `sonic skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SONIC_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SONIC_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `sonic`)    |
