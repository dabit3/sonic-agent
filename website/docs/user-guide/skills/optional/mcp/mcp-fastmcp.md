...
| | |
|---|---|
| Source | Optional — install with `lightning skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Lightning or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Lightning. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.lightning/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.lightning/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Lightning integration, either:

- configure the server in `~/.lightning/config.yaml` using the `native-mcp` skill, or
...
### Lightning cannot see the deployed server

The server-building part may be correct while the Lightning config is not. Load the `native-mcp` skill and configure the server in `~/.lightning/config.yaml`, then restart Lightning.
