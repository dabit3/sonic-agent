...
| | |
|---|---|
| Source | Optional — install with `sonic skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Sonic or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Sonic. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sonic/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sonic/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Sonic integration, either:

- configure the server in `~/.sonic/config.yaml` using the `native-mcp` skill, or
...
### Sonic cannot see the deployed server

The server-building part may be correct while the Sonic config is not. Load the `native-mcp` skill and configure the server in `~/.sonic/config.yaml`, then restart Sonic.
