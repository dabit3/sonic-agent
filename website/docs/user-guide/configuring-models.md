...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `sonic setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `sonic portal status`.
...
Every auxiliary task defaults to `auto` — meaning Sonic uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
`provider: auto` with `model: ''` tells Sonic to use the main model for that task.
