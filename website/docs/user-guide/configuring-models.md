...
Lightning uses two kinds of model slots:
...
[Nous Portal](/docs/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `lightning setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `lightning portal status`.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models Lightning recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Lightning writes it to `~/.lightning/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning Lightning uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `lightning skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, Lightning writes to `~/.lightning/config.yaml`:
...
`provider: auto` with `model: ''` tells Lightning to use the main model for that task.

## When does it take effect?

- **CLI** (`lightning chat`): next `lightning chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`lightning gateway restart`) if you want to force all sessions to pick up the change.
...
Lightning lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `lightning setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Lightning switched providers on me
...
Inside any `lightning chat` session:
...
```bash
lightning config set model.aliases.fav anthropic/claude-opus-4.6
lightning config set model.aliases.grok x-ai/grok-4
```
...
### `lightning model` subcommand

```bash
lightning model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`lightning model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.lightning/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `lightning config get model` and `lightning status`.

### Direct config edit

Edit `~/.lightning/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-Lightning-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Lightning-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Lightning-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__LIGHTNING_SESSION_TOKEN__`) if you're scripting against a running dashboard.
