...
- **`${SONIC_HOME}/logs/gateways/<profile>/current`** (mapped to `~/.sonic/logs/gateways/<profile>/current` on the host via the volume mount) — rotated, with an ISO 8601 timestamp prepended per line. Rotation is 10 archives × 1 MB each, so it can't fill the disk. This is what `sonic logs` reads and what survives container restarts.

The per-profile reconciler keeps a separate audit log at `${SONIC_HOME}/logs/container-boot.log` — one line per profile per container boot, recording whether each gateway was restored to its prior state.
:::
...
The built-in web dashboard runs as an optional side-process inside the same container as the gateway. Set `SONIC_DASHBOARD=1` to run the dashboard on container loopback (`127.0.0.1`) by default:
...
The entrypoint starts `sonic dashboard` in the background (running as the non-root `sonic` user) before `exec`-ing the main command. Dashboard output is prefixed with `[dashboard]` in `docker logs` so it's easy to separate from gateway logs.

| Environment variable | Description | Default |
|---------------------|-------------|---------|
| `SONIC_DASHBOARD` | Set to `1` (or `true` / `yes`) to launch the dashboard alongside the main command | *(unset — dashboard not started)* |
| `SONIC_DASHBOARD_HOST` | Bind address for the dashboard HTTP server | `127.0.0.1` |
...
`SONIC_DASHBOARD_HOST=0.0.0.0`. The dashboard's OAuth auth gate engages
...
`SONIC_DASHBOARD_OAUTH_CLIENT_ID` is set (see
...
set `SONIC_DASHBOARD_INSECURE=1`. This re-enables the legacy "no auth,
...
Sonic supports [multiple profiles](../reference/profile-commands.md) — separate `~/.sonic/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using Sonic' built-in multi-profile feature is not recommended.**
...
- **Independent lifecycle** — upgrade, restart, pause, or roll back each agent separately (`docker restart sonic-work` leaves `sonic-personal` untouched).
