...
Sonic can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Sonic becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).
...
These ship with `codex app-server` itself — no Sonic involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
When you enable the runtime, Sonic queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Sonic tool callback (MCP server, registered in `~/.codex/config.toml`)

Sonic registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
**One thing to be aware of:** each continuation prompt is a fresh codex turn, which means codex re-evaluates command approval policy from scratch. If you're doing a long-running goal with lots of writes, expect more approval prompts than you'd see on a single in-session task. Set `default_permissions = ":workspace"` (which Sonic does automatically when you enable the runtime) so simple workspace writes don't require prompting.

### Kanban (multi-agent worktree dispatch)

**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `sonic chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- The Sonic tool callback for browser_*, vision, image_gen, skills, TTS

What also works because the MCP callback exposes them:
- **`kanban_complete` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `SONIC_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `SONIC_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `SONIC_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `sonic-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, Sonic also passes narrow app-server sandbox overrides when `SONIC_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`SONIC_KANBAN_WORKSPACES_ROOT`, `SONIC_KANBAN_WORKSPACE`, legacy `SONIC_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive — [issue #27941](https://github.com/dabit3/sonic-agent/issues/27941)).
...
|  | Sonic default runtime | Codex app-server (opt-in) |
|---|---|---|
...
3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Sonic auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ...
   ```
   Sonic will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
You can also set it manually in `~/.sonic/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Sonic runtime)
```
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Sonic runtime.
...
For `apply_patch` (file edit) approvals, Sonic shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.
...
(Sonic will preserve your override on re-migration as long as it lives outside the `# managed by sonic-agent` markers.)
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.sonic/config.yaml`:
...
Sonic wraps everything it manages between two marker comments:
...
- Add your own MCP servers Sonic doesn't know about
...
By default, Sonic points the codex subprocess at `~/.codex/` regardless of which Sonic profile is active. This means `sonic -p work` and `sonic -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `SONIC_HOME`:

```bash
# Inside the work profile, you might wrap sonic:
CODEX_HOME=~/.sonic/profiles/work/codex sonic chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `sonic -p work` will operate on isolated Codex state.
...
Sonic does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
What's not migrated:
- Sonic-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Sonic doesn't touch credentials.

## Sonic tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Sonic registers itself as an MCP server in `~/.codex/config.toml`:
...
When the model calls `web_search` (or another exposed Sonic tool), codex spawns the `sonic_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Sonic runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Sonic Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Sonic UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `sonic-tools` callback)
...
- Sonic web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)
...
If you find a bug, [open an issue](https://github.com/dabit3/sonic-agent/issues) with the output of `sonic logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
For implementation details, see [PR #24182](https://github.com/dabit3/sonic-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
