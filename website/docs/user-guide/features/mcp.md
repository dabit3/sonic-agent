...
description: "Connect Sonic Agent to external tool servers via MCP — and control exactly which MCP tools Sonic loads"
---

# MCP (Model Context Protocol)

MCP lets Sonic Agent connect to external tool servers so the agent can use tools that live outside Sonic itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Sonic to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Sonic tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Sonic to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.sonic/config.yaml`:
...
4. Ask Sonic to use the MCP-backed capability.
...
Sonic will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Sonic connects to directly.
...
- you do not want Sonic spawning a local subprocess for that integration
...
**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `sonic mcp login` can list the tools and look like it worked, but every real tool call later times out. `sonic mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `sonic mcp login googledrive` — with the pre-registered client, Sonic skips registration and runs the normal browser authorization flow.
...
Sonic reads MCP config from `~/.sonic/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Sonic skips the server entirely |
...
For well-known MCP servers, `sonic mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
sonic mcp add codex --preset codex
```
...
You can pick any local name (`sonic mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Sonic registers MCP tools

Sonic prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Sonic sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Sonic also registers utility tools around MCP resources and prompts:
...
- Sonic only registers resource utilities if the MCP session actually supports resource operations
- Sonic only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Sonic, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Sonic skips the server completely and does not even attempt a connection.
...
You can also separately disable Sonic-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Sonic does not create an empty runtime MCP toolset for that server.
...
Sonic discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Sonic when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Sonic receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Sonic does not blindly pass your full shell environment.
...
Then verify your config and restart Sonic.
...
Because Sonic now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Sonic may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Sonic via the `sampling/createMessage` protocol. This allows an MCP server to ask Sonic to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Sonic as an MCP server

In addition to connecting **to** MCP servers, Sonic can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Sonic's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Sonic
- You want a single MCP server that bridges to all of Sonic's connected messaging platforms at once
- You already have a running Sonic gateway with connected platforms
...
Add Sonic to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Sonic in a specific location:

```json
{
  "mcpServers": {
    "sonic": {
      "command": "/home/user/.sonic/sonic-agent/venv/bin/sonic",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Sonic-specific channel browser:
...
The MCP server includes a live event bridge that polls Sonic's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
sonic mcp serve              # Normal mode
sonic mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Sonic's session store (`~/.sonic/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `sonic send` CLI.
...
- The embedded `sonic mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Sonic, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
