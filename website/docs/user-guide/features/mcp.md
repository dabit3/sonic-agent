...
description: "Connect Lightning Agent to external tool servers via MCP — and control exactly which MCP tools Lightning loads"
---

# MCP (Model Context Protocol)

MCP lets Lightning Agent connect to external tool servers so the agent can use tools that live outside Lightning itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Lightning to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Lightning tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Lightning to see
...
2. Add an MCP server to `~/.lightning/config.yaml`:
...
4. Ask Lightning to use the MCP-backed capability.
...
Lightning will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Lightning connects to directly.
...
- you do not want Lightning spawning a local subprocess for that integration

## Basic configuration reference

Lightning reads MCP config from `~/.lightning/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Lightning skips the server entirely |
...
For well-known MCP servers, `lightning mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
lightning mcp add codex --preset codex
```
...
You can pick any local name (`lightning mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Lightning registers MCP tools

Lightning prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Lightning sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Lightning also registers utility tools around MCP resources and prompts:
...
- Lightning only registers resource utilities if the MCP session actually supports resource operations
- Lightning only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Lightning, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Lightning skips the server completely and does not even attempt a connection.
...
You can also separately disable Lightning-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Lightning does not create an empty runtime MCP toolset for that server.
...
Lightning discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Lightning when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Lightning receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Lightning does not blindly pass your full shell environment.
...
Then verify your config and restart Lightning.
...
Because Lightning now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Lightning may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Lightning via the `sampling/createMessage` protocol. This allows an MCP server to ask Lightning to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Lightning as an MCP server

In addition to connecting **to** MCP servers, Lightning can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Lightning's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Lightning
- You want a single MCP server that bridges to all of Lightning's connected messaging platforms at once
- You already have a running Lightning gateway with connected platforms
...
Add Lightning to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Lightning in a specific location:

```json
{
  "mcpServers": {
    "lightning": {
      "command": "/home/user/.lightning/lightning-agent/venv/bin/lightning",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Lightning-specific channel browser:
...
The MCP server includes a live event bridge that polls Lightning's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
lightning mcp serve              # Normal mode
lightning mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Lightning's session store (`~/.lightning/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Lightning agent itself.
...
- The embedded `lightning mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Lightning, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
