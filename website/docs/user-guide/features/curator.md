...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.sonic/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
sonic curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
sonic curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
sonic curator run --background  # fire-and-forget: start the run in a background thread
...
2. Its name is **not** in `~/.sonic/skills/.hub/lock.json` (hub-installed skills).
...
**To see which skills the curator actually manages**, run `sonic curator status`.
...
- Archives are recoverable via `sonic curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `sonic curator pin <name>`. See the next
section.
