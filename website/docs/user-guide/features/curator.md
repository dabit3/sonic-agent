...
The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, Sonic checks whether:
...
On a brand-new install (or the first time a pre-curator install ticks after `sonic update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `sonic curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.sonic/skills/.archive/`.
...
```bash
sonic model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`sonic model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
sonic curator status         # last run, counts, pinned list, LRU top 5
sonic curator run            # trigger a review now (blocks until the LLM pass finishes)
sonic curator run --background  # fire-and-forget: start the LLM pass in a background thread
sonic curator run --dry-run  # preview only — report without any mutations
sonic curator backup         # take a manual snapshot of ~/.sonic/skills/
sonic curator rollback       # restore from the newest snapshot
sonic curator rollback --list     # list available snapshots
sonic curator rollback --id <ts>  # restore a specific snapshot
sonic curator rollback -y         # skip the confirmation prompt
sonic curator pause          # stop runs until resumed
sonic curator resume
...
Before every real curator pass, Sonic takes a tar.gz snapshot of `~/.sonic/skills/` at `~/.sonic/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
sonic curator rollback        # restore newest snapshot (with confirmation)
sonic curator rollback -y     # skip the prompt
sonic curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Sonic takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `sonic curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `sonic curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`sonic curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
- `~/.sonic/skills/.bundled_manifest` (skills copied from the repo on install), and
- `~/.sonic/skills/.hub/lock.json` (skills installed via `sonic skills install`).

Everything else in `~/.sonic/skills/` is fair game for the curator. This includes:
...
- Skills added via external skill directories you've pointed Sonic at.
...
1. Run `sonic curator run --dry-run` to see exactly what the curator would propose.
2. Use `sonic curator pin <name>` to fence off anything you don't want touched.
3. Or set `curator.enabled: false` in `config.yaml` if you'd rather manage the library yourself.

Archives are always recoverable via `sonic curator restore <name>`, but it's easier to pin up-front than to chase down a consolidation after the fact.
:::

If you want to protect a specific skill from ever being touched — for example a hand-authored skill you rely on — use `sonic curator pin <name>`. See the next section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `sonic curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
sonic curator pin <skill>
sonic curator unpin <skill>
```
...
The curator maintains a sidecar at `~/.sonic/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.sonic/logs/curator/`:
...
```bash
sonic curator restore <skill-name>
```

This moves the skill back from `~/.sonic/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.sonic/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `sonic curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/dabit3/sonic-agent/issues/7816) — original proposal and design discussion
