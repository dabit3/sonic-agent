...
title: "Plugins"
description: "Extend Sonic with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Sonic has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Sonic
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Sonic Plugin](/docs/guides/build-a-sonic-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.sonic/plugins/` with a `plugin.yaml` and Python code:
...
Start Sonic — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.sonic/plugins/hello-world/plugin.yaml`**
...
**`~/.sonic/plugins/hello-world/__init__.py`**

```python
"""Minimal Sonic plugin — registers a tool and a hook."""
...
Drop both files into `~/.sonic/plugins/hello-world/`, restart Sonic, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.sonic/plugins/` are disabled by default. Enable them only for trusted repositories by setting `SONIC_ENABLE_PROJECT_PLUGINS=true` before starting Sonic.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `sonic <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `sonic plugins install` |
| Distribute via pip | `[project.entry-points."sonic_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Sonic — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.sonic/plugins/` | Personal plugins |
| Project | `.sonic/plugins/` | Project-specific plugins (requires `SONIC_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `sonic_agent.plugins` entry_points | Distributed packages |
| Nix | `services.sonic-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Sonic also recognizes sub-category directories that route plugins to specialized discovery systems:
...
```bash
sonic plugins                    # interactive toggle (space to check/uncheck)
sonic plugins enable <name>      # add to allow-list
sonic plugins disable <name>     # remove from allow-list + add to disabled
```

After `sonic plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Sonic' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.sonic/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.sonic/plugins/`.

### Migration for existing users

When you upgrade to a version of Sonic that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.sonic/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Sonic has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.sonic/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Sonic also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Sonic Plugin](/docs/guides/build-a-sonic-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Sonic Plugin](/docs/guides/build-a-sonic-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Sonic Plugin](/docs/guides/build-a-sonic-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `sonic <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `SONIC_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Sonic auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `sonic skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.sonic/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `sonic plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
Running `sonic plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `sonic plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-sonic-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
