...
title: "Plugins"
description: "Extend Lightning with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Lightning has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Lightning
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Lightning Plugin](/docs/guides/build-a-lightning-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.lightning/plugins/` with a `plugin.yaml` and Python code:

```
~/.lightning/plugins/my-plugin/
...
Start Lightning — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.lightning/plugins/hello-world/plugin.yaml`**
...
**`~/.lightning/plugins/hello-world/__init__.py`**

```python
"""Minimal Lightning plugin — registers a tool and a hook."""
...
Drop both files into `~/.lightning/plugins/hello-world/`, restart Lightning, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.lightning/plugins/` are disabled by default. Enable them only for trusted repositories by setting `LIGHTNING_ENABLE_PROJECT_PLUGINS=true` before starting Lightning.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `lightning <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `lightning plugins install` |
| Distribute via pip | `[project.entry-points."lightning_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Lightning — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.lightning/plugins/` | Personal plugins |
| Project | `.lightning/plugins/` | Project-specific plugins (requires `LIGHTNING_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `lightning_agent.plugins` entry_points | Distributed packages |
| Nix | `services.lightning-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Lightning also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.lightning/plugins/model-providers/<name>/` and `~/.lightning/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

Sub-category plugins surface in `lightning plugins list` and the interactive `lightning plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `lightning plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `lightning plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.lightning/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
lightning plugins                    # interactive toggle (space to check/uncheck)
lightning plugins enable <name>      # add to allow-list
lightning plugins disable <name>     # remove from allow-list + add to disabled
```

After `lightning plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Lightning' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.lightning/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.lightning/plugins/`.

### Migration for existing users

When you upgrade to a version of Lightning that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.lightning/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Lightning has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.lightning/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Lightning also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Lightning Plugin](/docs/guides/build-a-lightning-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Lightning Plugin](/docs/guides/build-a-lightning-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Lightning Plugin](/docs/guides/build-a-lightning-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `lightning <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `LIGHTNING_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Lightning auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `lightning skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.lightning/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `lightning plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
lightning plugins                                       # unified interactive UI
lightning plugins list                                  # table: enabled / disabled / not enabled
lightning plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
lightning plugins install user/repo --enable            # install AND enable (no prompt)
lightning plugins install user/repo --no-enable         # install but leave disabled (no prompt)
lightning plugins update my-plugin                      # pull latest
lightning plugins remove my-plugin                      # uninstall
lightning plugins enable my-plugin                      # add to allow-list (flat plugin)
lightning plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
lightning plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `lightning plugins list` shows in the **Name** column.

### Interactive UI

Running `lightning plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `lightning plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-lightning-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
