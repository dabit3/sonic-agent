...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Sonic
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Sonic Plugin](/guides/build-a-sonic-plugin)** — step-by-step guide with a complete working example.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Sonic — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
...
| Nix | `services.sonic-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Sonic Plugin](/guides/build-a-sonic-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Sonic Plugin](/guides/build-a-sonic-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Sonic Plugin](/guides/build-a-sonic-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `sonic <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `SONIC_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Sonic auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `sonic skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.sonic/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `sonic plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
See the **[full guide](/guides/build-a-sonic-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
