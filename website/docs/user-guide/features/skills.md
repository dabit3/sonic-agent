...
By default every profile is seeded with the bundled skill catalog, and each `sonic update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.sonic` profile):

```bash
curl -fsSL https://raw.githubusercontent.com/dabit3/sonic-agent/main/scripts/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
sonic profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
sonic skills opt-out            # stop future seeding — nothing on disk is touched
sonic skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
sonic skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `sonic update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `sonic skills opt-in`) to re-enable.

:::note Safe by default
`sonic skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Sonic installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
description, the standard section order, Sonic-tool framing, no invented
commands).
...
These are maintained in the Sonic repository itself and install with built-in trust.
