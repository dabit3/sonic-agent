...
When Lightning Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.lightning/config.yaml` under `agent.custom_instructions`.
...
If you use Lightning' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.lightning/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Lightning Agent's deliverable
