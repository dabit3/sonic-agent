...
When Sonic Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.sonic/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.sonic/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.sonic/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Sonic Agent's deliverable
