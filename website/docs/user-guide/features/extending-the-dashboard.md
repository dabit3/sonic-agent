...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.sonic/plugins/<name>/dashboard/` | `user` |
...
2. Restart `sonic dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.sonic/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
