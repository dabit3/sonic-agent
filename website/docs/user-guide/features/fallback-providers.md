...
If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.sonic/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — Sonic still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `sonic fallback` writes to this key. When both are set, Sonic merges them with `fallback_providers` taking priority.
:::
