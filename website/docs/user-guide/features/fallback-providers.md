...
When a task's provider is set to `"auto"` (the default), Sonic tries providers in order until one works:
...
If the resolved provider fails at call time, Sonic also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
