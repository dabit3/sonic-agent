...
Sonic can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SONIC_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Sonic renders the sprite at full fidelity when your
...
sonic pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `sonic pets list [query] [--limit N]` |
| List installed pets | `sonic pets list --installed` |
| Install a pet | `sonic pets install <slug> [--select] [--force]` |
| Set the active pet | `sonic pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `sonic pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `sonic pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `sonic pets off` |
| Remove an installed pet | `sonic pets remove <slug>` |
| Diagnose setup | `sonic pets doctor` |

`sonic pets show` flags:
...
Beyond installing pre-made pets from the gallery, Sonic can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `sonic pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `sonic tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SONIC_PET_IMAGE_PROVIDER` env var (e.g. `SONIC_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Sonic is
...
Run `sonic pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Sonic uses its own
  profile-scoped `<SONIC_HOME>/pets/` instead — install through `sonic pets`.
