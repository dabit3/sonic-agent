...
Lightning Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
lightning computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `lightning computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `lightning tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or Lightning app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   lightning -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.lightning/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Lightning refreshes the binary in two
places so you don't get stuck on a stale release:

- **`lightning update`** — when you update Lightning itself, if `cua-driver` is
...
- **`lightning computer-use install --upgrade`** — manual force-refresh.
...
`lightning computer-use status` shows the installed version next to the
binary path.
...
Lightning applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.lightning/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Lightning applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `LIGHTNING_CUA_DRIVER_VERSION`
...
```
LIGHTNING_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
LIGHTNING_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
LIGHTNING_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`lightning computer-use install` to fetch the cua-driver binary, or run
`lightning tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/lightning-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
