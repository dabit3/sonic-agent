...
Sonic Agent can drive your desktop — clicking, typing, scrolling,
...
```
sonic computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `sonic computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `sonic tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Sonic app). `sonic computer-use doctor` will tell you which permission is missing. |
...
```
sonic -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.sonic/config.yaml`.

## `sonic computer-use doctor` — your first triage stop

`sonic computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ sonic computer-use doctor
...
agent is acting here." Each Sonic run declares its own cua-driver
**session id** (something like `sonic-3a7b9c14d2e8`); the cursor's
...
Sonic intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Sonic-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Sonic skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Sonic
...
Sonic applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.sonic/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Sonic applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Sonic agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Sonic agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Sonic warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Sonic from inside the RDP / console
...
```
SONIC_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SONIC_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Sonic disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Sonic sets
...
When it's on, `sonic computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Sonic at a binary you built from source instead
of the published release. Sonic resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SONIC_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Sonic testing (see notes).
...
### Option B — point Sonic straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SONIC_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SONIC_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SONIC_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Sonic is using your build

- `sonic computer-use status` prints the resolved binary path and
  version.
- `sonic computer-use doctor` confirms the binary is reachable and
...
- **Sonic spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Sonic
  change needed either way.
- **Local builds skip the version check.** Sonic warns when the
...
**First action when anything's off: run `sonic computer-use doctor`.**
...
Run `sonic computer-use install` to fetch the cua-driver binary, or
run `sonic tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `sonic computer-use doctor` will
...
- **Sonic-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Sonic `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Sonic (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Sonic among them)
