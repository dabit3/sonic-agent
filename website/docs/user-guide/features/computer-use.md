...
Sonic Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `sonic computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `sonic tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Sonic refreshes the binary in two
places so you don't get stuck on a stale release:

- **`sonic update`** — when you update Sonic itself, if `cua-driver` is
...
- **`sonic computer-use install --upgrade`** — manual force-refresh.
...
`sonic computer-use status` shows the installed version next to the
binary path.
...
Pair with `approvals.mode: manual` in `~/.sonic/config.yaml` if you want every action confirmed.
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `SONIC_CUA_DRIVER_VERSION`
...
SONIC_CUA_DRIVER_VERSION=0.5.0    # optional pin
```
...
`sonic computer-use install` to fetch the cua-driver binary, or run
`sonic tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/dabit3/sonic-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
