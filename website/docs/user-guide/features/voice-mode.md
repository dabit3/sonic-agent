...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Lightning Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Lightning Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Lightning](/docs/guides/use-voice-mode-with-lightning).

## Prerequisites

Before using voice features, make sure you have:

1. **Lightning Agent installed** — `pip install lightning-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `lightning model` or set your preferred provider credentials in `~/.lightning/.env`
3. **A working base setup** — run `lightning` to verify the agent responds to text before enabling voice

:::tip
The `~/.lightning/` directory and default `config.yaml` are created automatically the first time you run `lightning`. You only need to create `~/.lightning/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/docs/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `lightning setup --portal` wires both up at once.
:::
...
pip install "lightning-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "lightning-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "lightning-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "lightning-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`lightning chat`) and the **TUI** (`lightning --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.lightning/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
lightning                # Start the interactive CLI
```
...
1. Start the CLI with `lightning` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.lightning/config.yaml` (default: `ctrl+b`).
:::
...
```bash
lightning gateway        # Start the gateway (connects to configured platforms)
lightning gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@lightningbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.lightning/.env`:
```bash
...
```bash
lightning gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.lightning/.env`
...
- Review gateway logs: `tail -f ~/.lightning/logs/gateway.log`
