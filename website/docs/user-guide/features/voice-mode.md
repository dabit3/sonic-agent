...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Sonic Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Sonic Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Sonic](/guides/use-voice-mode-with-sonic).
...
:::tip
The `~/.sonic/` directory and default `config.yaml` are created automatically the first time you run `sonic`. You only need to create `~/.sonic/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `sonic setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`sonic chat`) and the **TUI** (`sonic --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.sonic/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
sonic                # Start the interactive CLI
```
...
1. Start the CLI with `sonic` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.sonic/config.yaml` (default: `ctrl+b`).
:::
...
```bash
sonic gateway        # Start the gateway (connects to configured platforms)
sonic gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@sonicbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.sonic/.env`:
```bash
...
```bash
sonic gateway        # Start with existing configuration
```
...
If you are running Sonic inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.sonic/.env`
...
- Review gateway logs: `tail -f ~/.sonic/logs/gateway.log`
