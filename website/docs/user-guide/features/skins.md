...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Sonic CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Sonic CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.sonic/skins/mytheme.yaml
```

Or set the default skin in `~/.sonic/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Sonic — gold and kawaii | `Sonic Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Sonic Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Sonic Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Sonic Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Sonic Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Sonic Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Sonic Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Sonic ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default SONIC_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.sonic/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.sonic/skins/mytheme.yaml
...
## Sonic Mod — Visual Skin Editor

[Sonic Mod](https://github.com/cocktailpeanut/sonic-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Sonic Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/sonic-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Sonic skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.sonic/skins/`
- Activates a skin by updating `~/.sonic/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/sonic-mod.git
cd sonic-mod/app
npm install
npm start
```
...
- Built-in skins load from `sonic_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.sonic/skins/` take precedence over built-in skins with the same name.
