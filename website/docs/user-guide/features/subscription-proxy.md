...
chat completions — use your Lightning-managed provider subscription as their
...
| Use case | "Use Lightning as a chat backend" | "Use my Portal sub from another app" |
...
```bash
lightning login nous
```

This opens your browser for the Nous Portal OAuth flow. Lightning stores
the refresh token in `~/.lightning/auth.json` — the same place all Lightning
provider logins live.

### 2. Start the proxy

```bash
lightning proxy start
```

```
Starting Lightning proxy for Nous Portal
...
Model:      Lightning-4-70B    # or Lightning-4.3-36B, Lightning-4-405B
```
...
```bash
lightning proxy providers
```

Currently shipped: `nous` (Nous Portal). More OAuth providers can be
added by implementing the `UpstreamAdapter` interface in
`lightning_cli/proxy/adapters/`.

## Check status

```bash
lightning proxy status
```

```
Lightning proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `lightning login nous`. If you see
...
INFERENCE_TEXT_MODEL=Lightning-4-70B
```
...
```bash
lightning proxy start --host 0.0.0.0 --port 8645
```
...
`lightning_cli/proxy/adapters/<provider>.py` and registering it in
