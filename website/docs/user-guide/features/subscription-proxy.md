...
chat completions — use your Sonic-managed provider subscription as their
...
| Use case | "Use Sonic as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Sonic stores
the refresh token in `~/.sonic/auth.json` — the same place all Sonic
...
Model:      Sonic-4-70B    # or Sonic-4.3-36B, Sonic-4-405B
```
...
interface in `sonic_cli/proxy/adapters/`.
...
If you see `not logged in`, run `sonic auth add nous`. If you see
...
`sonic_cli/proxy/adapters/<provider>.py` and registering it in
