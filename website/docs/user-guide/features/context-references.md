...
Type `@` followed by a reference to inject content directly into your message. Lightning expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Lightning env: `$LIGHTNING_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$LIGHTNING_HOME/skills/.hub/`
