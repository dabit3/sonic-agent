...
Lightning runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Lightning
...
1. Lightning captures a baseline of current diagnostics for the file.
...
…). Lightning auto-detects the binary on PATH or in
`<LIGHTNING_HOME>/lsp/bin/`.
...
`node_modules` tree — Lightning installs both packages together when you
run `lightning lsp install typescript` or auto-install fires on first
use.

## CLI

```
lightning lsp status          # service state + per-server install status
lightning lsp list            # registry, optionally --installed-only
lightning lsp install <id>    # eagerly install one server
lightning lsp install-all     # try every server with a known recipe
lightning lsp restart         # tear down running clients
lightning lsp which <id>      # print resolved binary path
```

`lightning lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <LIGHTNING_HOME>/lsp/bin
...
When `install_strategy: auto`, Lightning installs binaries into
`<LIGHTNING_HOME>/lsp/bin/`. NPM packages land in
`<LIGHTNING_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Lightning-owned and is
...
Servers are kept alive for the life of the Lightning process. There's
...
**`lightning lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<LIGHTNING_HOME>/lsp/bin/`. Run
`lightning lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lightning lsp status`**
...
When `lightning lsp status` shows a `Backend warnings` section, install
...
`~/.lightning/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.lightning/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `lightning lsp restart` to clear the set;
