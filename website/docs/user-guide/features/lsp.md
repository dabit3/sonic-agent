...
Sonic runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Sonic
...
1. Sonic captures a baseline of current diagnostics for the file.
...
…). Sonic auto-detects the binary on PATH or in
`<SONIC_HOME>/lsp/bin/`.
...
`node_modules` tree — Sonic installs both packages together when you
run `sonic lsp install typescript` or auto-install fires on first
use.

## CLI

```
sonic lsp status          # service state + per-server install status
sonic lsp list            # registry, optionally --installed-only
sonic lsp install <id>    # eagerly install one server
sonic lsp install-all     # try every server with a known recipe
sonic lsp restart         # tear down running clients
sonic lsp which <id>      # print resolved binary path
```

`sonic lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SONIC_HOME>/lsp/bin
...
When `install_strategy: auto`, Sonic installs binaries into
`<SONIC_HOME>/lsp/bin/`. NPM packages land in
`<SONIC_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Sonic-owned and is
...
Servers are kept alive for the life of the Sonic process. There's
...
**`sonic lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SONIC_HOME>/lsp/bin/`. Run
`sonic lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `sonic lsp status`**
...
When `sonic lsp status` shows a `Backend warnings` section, install
...
`~/.sonic/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.sonic/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `sonic lsp restart` to clear the set;
