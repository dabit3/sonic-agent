...
The Chat tab is part of every `sonic dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
sonic dashboard --no-open --insecure --host 0.0.0.0 --port 9119
```
