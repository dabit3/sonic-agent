...
```bash
cd ~/.sonic/sonic-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.sonic/sonic-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`cd ~/.sonic/sonic-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
SONIC_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
SONIC_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
| `GET /api/sonic/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Sonic Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
| `sonic dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
When you run `sonic dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Sonic doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `sonic dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
|---------|-----------|--------|----------------|
| `SONIC_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `sonic dashboard register` |
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `SONIC_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
| `SONIC_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `SONIC_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# 1. Set the dashboard login credentials in ~/.sonic/.env (secrets file, 0600).
cat >> ~/.sonic/.env <<'EOF'
SONIC_DASHBOARD_BASIC_AUTH_USERNAME=admin
SONIC_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
Prefer no plaintext at rest? Use `SONIC_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.sonic/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.sonic/.env`, so the credentials are in the environment at boot.
...
Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `SONIC_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.
...
- **Signed out on every restart** — set `SONIC_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
