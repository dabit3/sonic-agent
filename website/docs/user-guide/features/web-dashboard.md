...
The web dashboard is a browser-based UI for managing your Sonic Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `sonic setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `sonic --tui` via PTY/WebSocket). Alternatively set `SONIC_DASHBOARD_TUI=1`. |

```bash
# Custom port
sonic dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
sonic dashboard --host 0.0.0.0
...
The default `sonic-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'sonic-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install sonic-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `sonic dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `sonic dashboard` launch. Start the dashboard with `sonic dashboard --tui` or set `SONIC_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `sonic --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'sonic-agent[web,pty]'`, or `[all]` covers both)
...
1. **The embedded chat must be enabled.** `/api/ws` and `/api/pty` close immediately with WS code **4403** unless the dashboard was started with `--tui` (or `SONIC_DASHBOARD_TUI=1`). A plain `sonic dashboard` or `sonic gateway` serves the status page but refuses the chat socket.
2. **The session token must match.** Even with chat enabled, the socket closes with WS code **4401** if the token Desktop sends doesn't match the dashboard's session token. By default the dashboard generates a **fresh random token on every restart**, so a token you saved in Desktop yesterday is invalid after the service restarts. Pin it by setting `SONIC_DASHBOARD_SESSION_TOKEN` to a stable value.
...
```ini
[Service]
Environment="SONIC_DASHBOARD_SESSION_TOKEN=<long-random-token>"
ExecStart=/path/to/venv/bin/python -m sonic_cli.main dashboard \
...
```bash
curl -i -H "X-Sonic-Session-Token: <long-random-token>" http://VM_IP:9119/api/config
```
...
The REST API reads the token from the `X-Sonic-Session-Token` header; the WebSocket reads the same token from the `?token=` query parameter. Both compare against `SONIC_DASHBOARD_SESSION_TOKEN`, so a 200 here means the WS handshake will authenticate too.
:::
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `sonic config set` and the gateway read from.
:::
...
Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.sonic/skills/` and grouped by category.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Sonic versions, CPU core count + utilization, memory, disk usage of the Sonic home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Sonic version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `sonic update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
...
This re-reads `~/.sonic/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/sonic/update/check` | Report update availability (commits behind, install method) without applying. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public address — anything other than `127.0.0.1` / `localhost` — Sonic Agent engages an OAuth-based auth gate. Every request must carry a verified session cookie or it's bounced through a full OAuth round-trip via the Nous Portal.
...
|-------|-----------|----------|
| `sonic dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `sonic dashboard --host 0.0.0.0` | **ON** | Production / Fly.io deployment |
| `sonic dashboard --host 192.168.1.10 --insecure` | OFF | Trusted LAN; user opts into legacy session-token auth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `sonic dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
|---------|-----------|--------|----------------|
| `SONIC_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | Nous Portal at Fly.io provisioning time |
| `SONIC_DASHBOARD_PORTAL_URL` | `dashboard.oauth.portal_url` | URL (default: `https://portal.nousresearch.com`) | Portal — override only for staging or a custom deployment |

Per the Sonic Agent convention (`~/.sonic/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so Fly.io's platform-secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain Fly deploys with partial proxy chains), set `dashboard.public_url` (or `SONIC_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/sonic"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `SONIC_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `SONIC_DASHBOARD_PUBLIC_URL` env var | — | Fly.io platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `sonic_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `sonic_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `sonic_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `sonic_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$SONIC_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.sonic/plugins/dashboard-auth-myidp/__init__.py
from sonic_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path (Fly.io shape). SONIC_DASHBOARD_PORTAL_URL is
# optional — defaults to production.
SONIC_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  sonic dashboard --host 0.0.0.0
...
Sonic Desktop can drive a Sonic backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a **Session token**. (For the desktop app itself — install, settings, chat — see the [Sonic Desktop](/user-guide/desktop) page.)

The "session token" is the dashboard's session token — the same secret the local web UI uses for `/api` and WebSocket calls. **Sonic does not print it for you to copy.** By default the dashboard mints a fresh random token on every boot and injects it straight into the served HTML, so there is nothing sitting in `config.yaml`, in `/gateway`, or in the logs to grab. For a remote connection you set the token yourself on the backend, then paste that same value into the desktop app.

The desktop app sends the token as an `X-Sonic-Session-Token` header. The backend accepts it only in legacy session-token mode — i.e. when bound non-loopback **with `--insecure`**. A non-loopback bind *without* `--insecure` engages the [OAuth gate](#oauth-authentication-gated-mode) instead, which ignores the session token. So a remote desktop connection means: `--insecure` + a token you control.

The backend must also be started with **`--tui`** (or `SONIC_DASHBOARD_TUI=1`). The desktop's chat runs over the `/api/ws` + `/api/pty` WebSockets, and those are refused unless the embedded-chat surface is enabled. Without `--tui` the desktop still passes the `/api/status` health check (so the app reports the backend "ready") but the chat WebSocket is closed on connect — connects, looks ready, chat stays dead. A plain `sonic dashboard` or `sonic gateway` is not enough.

### On the backend (the remote machine)

```bash
# 1. Mint a stable token and store it in ~/.sonic/.env (secrets file, 0600).
#    Setting SONIC_DASHBOARD_SESSION_TOKEN pins the token so it survives
...
If you run the dashboard as a systemd service, `~/.sonic/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.sonic/.env`, so the token is in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/sonic` are supported if you front it with a reverse proxy)
...
Instead of the in-app settings, you can point the desktop at a backend with two env vars before launching it. When `SONIC_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app settings (the Gateway settings panel shows an "env override" badge and disables editing):

| Env var | Value |
|---------|-------|
| `SONIC_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |
| `SONIC_DESKTOP_REMOTE_TOKEN` | the same token as `SONIC_DASHBOARD_SESSION_TOKEN` on the backend |
...
- **Test remote fails with 401** — the token doesn't match the backend's `SONIC_DASHBOARD_SESSION_TOKEN`, or the backend is running *without* `--insecure` on a non-loopback bind (the OAuth gate is on and ignores the session token). Confirm `--insecure` and that the env var is actually loaded (`curl -s -H "X-Sonic-Session-Token: $TOKEN" http://<host>:9119/api/status` should return JSON, not 401).
- **Backend reports "ready" but chat does nothing** — the backend was started without `--tui` (or `SONIC_DASHBOARD_TUI=1`), so `/api/status` answers but the chat WebSocket (`/api/ws` / `/api/pty`) is refused. Restart the backend with `--tui`.
...
- **No token anywhere to copy** — expected. You mint it (`SONIC_DASHBOARD_SESSION_TOKEN`); Sonic never auto-surfaces the default ephemeral one.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `sonic_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `sonic update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `sonic dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Sonic Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Sonic Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
