...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.sonic/.env` (the secrets file, mode 0600):
...
Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.sonic/.env` so the credentials are in the environment at boot.
