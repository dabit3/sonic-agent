...
The packaged app ships only the Electron shell. On first launch it installs the Sonic Agent runtime into `SONIC_HOME` (`~/.sonic`, or `%LOCALAPPDATA%\sonic` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. The React renderer talks to a `sonic dashboard` backend over the standard gateway APIs and reuses the agent rather than reimplementing it. Install, backend-resolution, and self-update logic live in the Electron main process.
