...
- Using the **OpenAI-compatible API server** (exposed by `sonic gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
The Sonic [Tool Gateway](/user-guide/features/tool-gateway) and the API server are long-lived processes. In WSL2 you have a few options for keeping them up.

### Desktop shortcut for opening Sonic quickly

If you just want a double-click launcher for an interactive Sonic shell, create
...
2. For the target, use your distro name (replace `Ubuntu` if needed):
   ...
   ```text
   wt.exe -w 0 -p "Ubuntu" wsl.exe -d Ubuntu --cd ~ -- bash -ic "sonic"
   ```

3. Name it something obvious like `Sonic`.

That opens Windows Terminal, starts your WSL distro, drops you in your Linux
home directory, and launches Sonic. If `sonic` is not on PATH yet, open WSL
once manually and run `source ~/.bashrc`, or replace the command with
`uv run sonic` inside your project checkout.

Optional polish:

- **Custom icon:** open **Properties -> Change Icon** and point it at an `.ico`
  file, such as the Sonic favicon from the repo.
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-sonic#wsl2-bridge-sonic-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Sonic in WSL.
