...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for Sonic — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for Sonic — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run Sonic interactively.
...
sonic --tui -r 20260409_000000_aa11bb
sonic --tui --resume "my t0p session"
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while Sonic is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `sonic doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch Sonic installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Sonic version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point Sonic at it:

```bash
export SONIC_TUI_DIR=/path/to/prebuilt/ui-tui
sonic --tui
```
...
| `/reload` | Re-reads `~/.sonic/.env` into the running TUI process so newly added API keys take effect without a restart |
...
1. `SONIC_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
```bash
export SONIC_TUI_THEME=light
```

## Busy indicator styles

The status-bar busy indicator is pluggable — the default rotates Sonic' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
By default, `sonic --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export SONIC_TUI_RESUME=1          # most-recent TUI session
# or:
export SONIC_TUI_RESUME=<session-id>   # specific session
```
...
- **Working directory with git branch** — `~/projects/sonic-agent (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
- **`⚠ YOLO`** — visible warning whenever YOLO mode is on (`sonic --yolo`, `/yolo`, or `SONIC_YOLO_MODE=1`). The same badge also appears in the startup banner so you cannot launch an auto-approving session without noticing.

## Configuration

The TUI respects all standard Sonic config: `~/.sonic/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.sonic/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.

See [Sessions](sessions.md) for lifecycle, search, compression, and export.

## Attaching to a running gateway

By default the TUI spawns its own in-process gateway, so each TUI instance is self-contained. If you already have a long-lived gateway running (e.g. `sonic gateway run` in tmux, or the systemd / launchd service), you can point the TUI at that gateway instead — the TUI then becomes a thin client and shares state with every other surface (messaging platforms, web dashboard, other TUI sessions) that's attached to the same gateway.

Set the websocket URL via env before launching:

```bash
export SONIC_TUI_GATEWAY_URL="ws://localhost:8765/api/ws?token=<auth-token>"
sonic --tui
```
...
Launching `sonic` (without `--tui`) stays on the classic CLI by default. To make a machine prefer the TUI, set `display.interface: tui` in `~/.sonic/config.yaml` (persistent) or `SONIC_TUI=1` in your shell profile (per-shell). To go back, set `interface: cli` / unset the env var, or pass `sonic --cli` for a one-off.

If the TUI fails to launch (no Node, missing bundle, TTY issue), Sonic prints a diagnostic and falls back — rather than leaving you stuck.
