...
If you only run one Sonic agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Sonic agents that should all
...
(`ai.sonic.gateway-<name>.plist`) or systemd user service
(`sonic-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
sonic profile create coder
sonic profile create personal-bot
sonic profile create research
...
`~/.local/bin/sonic-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: sonic-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    sonic gateway "$action"
  else
    sonic -p "$profile" gateway "$action"
  fi
}
...
```bash
sonic-gateways start      # start every configured profile
sonic-gateways stop       # stop every configured profile
sonic-gateways restart    # restart all
sonic-gateways status     # status across all
sonic-gateways list       # delegates to `sonic gateway list`
```

:::tip
The `default` profile is targeted with `sonic gateway <action>` (no `-p`),
not `sonic -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `sonic -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.sonic.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/sonic-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.sonic.gateway.plist` /
`sonic-gateway.service`.
...
```bash
# Default profile
tail -f ~/.sonic/logs/gateway.log
tail -f ~/.sonic/logs/gateway.error.log

# Named profile
tail -f ~/.sonic/profiles/<name>/logs/gateway.log
tail -f ~/.sonic/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.sonic/logs/gateway.log ~/.sonic/profiles/*/logs/gateway.log
```
...
```bash
sonic profile list             # profiles + model + gateway state
sonic-gateways status          # full status across every profile
launchctl list | grep sonic    # macOS — PIDs and labels
systemctl --user list-units 'sonic-gateway-*'   # Linux — units
```
...
The default profile uses `~/.sonic/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
sonic config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.sonic/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=sonic --why="gateways running" \
  sleep infinity &
...
`sonic-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.sonic/.env ~/.sonic/profiles/*/.env
```

## Updating the code

`sonic update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `sonic gateway start` after a previous `sonic gateway stop`. The
...
```bash
ps -ef | grep "sonic_cli.*-p <profile>"
cat ~/.sonic/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.sonic.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.sonic.gateway-<profile>.plist

# Linux
systemctl --user restart sonic-gateway-<profile>.service
```

### Health check

```bash
sonic doctor                  # default profile
sonic -p <profile> doctor     # one profile
```
