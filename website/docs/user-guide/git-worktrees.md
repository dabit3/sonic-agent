...
title: "Git Worktrees"
description: "Run multiple Sonic agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Sonic Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Sonic so each session has a clean, isolated working directory.

## Why Use Worktrees with Sonic?
...
git worktree add ../repo-feature feature/sonic-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/sonic-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Sonic there:

```bash
cd ../repo-feature

# Start Sonic in the worktree
sonic
```
...
git worktree add ../repo-experiment-a feature/sonic-a
git worktree add ../repo-experiment-b feature/sonic-b
```
...
- Works on its own branch (`feature/sonic-a` vs `feature/sonic-b`).
...
- Sonic checkpoint data under `~/.sonic/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Sonic experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/sonic-checkpoints-docs`, `feature/sonic-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Sonic from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sonic -w` (Automatic Worktree Mode)

Sonic has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `sonic/sonic-<hash>`).
...
- Use **git worktrees** to give each Sonic session its own clean checkout.
