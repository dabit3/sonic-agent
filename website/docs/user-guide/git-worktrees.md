...
title: "Git Worktrees"
description: "Run multiple Lightning agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Lightning Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Lightning so each session has a clean, isolated working directory.

## Why Use Worktrees with Lightning?

Lightning treats the **current working directory** as the project root:

- CLI: the directory where you run `lightning` or `lightning chat`
...
git worktree add ../repo-feature feature/lightning-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/lightning-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Lightning there:

```bash
cd ../repo-feature

# Start Lightning in the worktree
lightning
```
...
git worktree add ../repo-experiment-a feature/lightning-a
git worktree add ../repo-experiment-b feature/lightning-b
```
...
Each Lightning process:

- Works on its own branch (`feature/lightning-a` vs `feature/lightning-b`).
...
- Lightning checkpoint data under `~/.lightning/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Lightning experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/lightning-checkpoints-docs`, `feature/lightning-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Lightning from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `lightning -w` (Automatic Worktree Mode)

Lightning has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `lightning/lightning-<hash>`).
...
```bash
lightning -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `lightning -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Lightning session its own clean checkout.
