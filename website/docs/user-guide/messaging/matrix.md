...
sidebar_position: 9
title: "Matrix"
description: "Set up Lightning Agent as a Matrix bot"
---

# Matrix Setup

Lightning Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Lightning Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Lightning works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Lightning behaves once it's connected.

## How Lightning Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Lightning responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Lightning requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Lightning supports Matrix threads (MSC3440). If you reply in a thread, Lightning keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Lightning auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Lightning isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `lightning` — the full user ID will be `@lightning:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `lightning-bot`).

### Option C: Use Your Own Account

You can also run Lightning as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Lightning needs an access token to authenticate with the homeserver. You have two options:
...
curl -X POST https://your-server/_matrix/client/v3/login \
  ...
  -d '{
    ...
    "user": "@lightning:your-server.org",
...
Instead of providing an access token, you can give Lightning the bot's user ID and password. Lightning will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@lightning:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Lightning Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure Lightning Agent
...
Add the following to your `~/.lightning/.env` file:
...
# MATRIX_USER_ID=@lightning:matrix.example.org
...
Optional behavior settings in `~/.lightning/config.yaml`:
...
:::tip
You can run `lightning gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Lightning supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with lightning extras
pip install 'lightning-agent[matrix]'
```
...
Add to your `~/.lightning/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Lightning:

- Stores encryption keys in `~/.lightning/platforms/matrix/store/` (legacy installs: `~/.lightning/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Lightning imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning[Deleting the crypto store]
If you delete `~/.lightning/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Lightning detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
1. Stop Synapse and delete the old device from its database:
   ...
   sudo sqlite3 /var/lib/matrix-synapse/homeserver.db "
     DELETE FROM e2e_device_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@lightning:your-server';
     DELETE FROM e2e_one_time_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@lightning:your-server';
     DELETE FROM e2e_fallback_keys_json WHERE device_id = 'DEVICE_ID' AND user_id = '@lightning:your-server';
     DELETE FROM devices WHERE device_id = 'DEVICE_ID' AND user_id = '@lightning:your-server';
   "
   ...
   curl -X DELETE -H "Authorization: Bearer ADMIN_TOKEN" \
     'https://your-server/_synapse/admin/v2/users/%40lightning%3Ayour-server/devices/DEVICE_ID'
   ```
...
2. Delete the local crypto store and restart Lightning:
   ```bash
   rm -f ~/.lightning/platforms/matrix/store/crypto.db*
   # restart lightning
   ```
...
Add this to your `~/.lightning/.env`:
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/NousResearch/lightning-agent/issues/12614).
...
If you previously used Lightning with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@lightning:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Lightning Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.lightning/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.lightning/platforms/matrix/store/crypto.db
   rm -f ~/.lightning/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.lightning/.env`:
...
a working E2EE setup with a previous version of Lightning and are upgrading.
...
Matrix E2EE requires `libolm`, which doesn't compile on macOS ARM64 (Apple Silicon). The `lightning-agent[matrix]` extra is gated to Linux only. If you're on macOS, proxy mode lets you run E2EE in a Docker container on a Linux VM while the actual agent runs natively on macOS with full access to your local files, memory, and skills.
...
services:
  lightning-matrix:
    ...
    environment:
      ...
      MATRIX_ENCRYPTION: "true"
      MATRIX_DEVICE_ID: "LIGHTNING_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.lightning/platforms/matrix/store
```
...
RUN pip install 'lightning-agent[matrix]'

CMD ["lightning", "gateway"]
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Lightning API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Lightning-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Lightning logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Lightning gateway isn't running, or it failed to connect.

**Fix**: Check that `lightning gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.lightning/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Lightning Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: Lightning can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
