...
Bridge and appservice deployments need extra loop protection. Sonic always
...
cd ~/.sonic/sonic-agent && uv pip install -e ".[matrix]"
```
...
RUN cd ~/.sonic/sonic-agent && uv pip install -e ".[matrix]"
