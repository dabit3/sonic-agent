...
Connect Sonic to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Sonic — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Sonic runs it in a
...
2. **Finds or creates** the `Sonic Agent` project on your account.
...
Runtime credentials are written to `~/.sonic/.env`
...
(device token, dashboard project id) lives in `~/.sonic/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Sonic
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Sonic replies with a pairing code. Approve it with:

```bash
sonic pairing approve photon <CODE>
```

Use `sonic pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.sonic/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.sonic/.env`):
...
By default Sonic responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Sonic` and
`@Sonic agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Sonic will reply.
...
- **`sidecar deps : ✗ run sonic photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `sonic photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `sonic photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `sonic photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Sonic sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Sonic wake words  | JSON list / comma / newline regex patterns for group mentions |
