...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Lightning Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Lightning to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.lightning/.env`:
...
All standard Lightning commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.lightning/logs/gateway.log`
...
4. Consider enabling verbose logging: `LIGHTNING_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Lightning automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
LIGHTNING_LOG_LEVEL=debug lightning gateway
```
...
```bash
lightning chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
