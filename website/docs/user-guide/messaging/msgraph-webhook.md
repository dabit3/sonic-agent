...
description: "Receive Microsoft Graph change notifications (meetings, calendar, chat, etc.) in Sonic"
---

# Microsoft Graph Webhook Listener

The `msgraph_webhook` gateway platform is an inbound event listener. It's how Sonic receives **change notifications** from Microsoft Graph — "a Teams meeting ended," "a new message landed in this chat," "this calendar event was updated." Different from the `teams` platform (which is a chat bot users type to) — this one is M365 telling Sonic something happened, not a person.

Right now the primary consumer is the Teams meeting summary pipeline: Graph notifies when a meeting produces a transcript, the pipeline fetches it, and Sonic posts a summary back into Teams. Other Graph resources (`/chats/.../messages`, `/users/.../events`) use the same listener — the pipeline consumers land with their own PRs.
...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/.sonic/.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.

## Quick Start

Minimum `~/.sonic/config.yaml`:
...
Or via env vars in `~/.sonic/.env` (auto-merged on startup):
...
Start the gateway: `sonic gateway run`. The listener exposes:
...
- [Microsoft Teams bot setup](/docs/user-guide/messaging/teams) — the different platform that lets users chat with Sonic in Teams
