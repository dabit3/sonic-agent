...
Connect Sonic to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Sonic supports two WeCom integration modes:
...
3. Sonic decrypts the message, queues it for the agent
...
(Use `sonic gateway start` only after `sonic gateway install` has registered the systemd/launchd service.)
