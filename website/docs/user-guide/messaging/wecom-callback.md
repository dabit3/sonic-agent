...
Connect Lightning to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Lightning supports two WeCom integration modes:
...
3. Lightning decrypts the message, queues it for the agent
...
(Use `lightning gateway start` only after `lightning gateway install` has registered the systemd/launchd service.)
