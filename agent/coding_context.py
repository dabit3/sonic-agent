"""Coding-context awareness — base Sonic, every interactive surface.

When the user runs Sonic inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Sonic shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("sonic.coding_context")
...
# in _SONIC_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``SONIC_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Sonic should operate in its coding posture right now."""
