...
row taxonomy is inferred from the atlas shape so Sonic can render both legacy
...
# rows (1536x1872), while older Sonic/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Sonic' activity state names. They are not always identical to the
...
# Legacy Sonic/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Sonic activity names -> accepted row-name aliases in descending
