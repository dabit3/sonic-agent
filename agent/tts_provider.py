...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Sonic with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.sonic/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Sonic
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sonic tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``sonic setup`` for
