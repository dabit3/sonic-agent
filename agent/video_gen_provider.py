...
as ``kind: backend``) or ``~/.lightning/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``lightning tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``lightning tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``lightning tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``lightning tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$LIGHTNING_HOME/cache/videos/``, creating parents as needed."""
    from lightning_constants import get_lightning_home

    path = get_lightning_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$LIGHTNING_HOME/cache/videos/``.
