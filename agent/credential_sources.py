"""Unified removal contract for every credential source Sonic reads from.

Sonic seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.sonic/.env
    claude_code   — ~/.claude/.credentials.json
    sonic_pkce   — ~/.sonic/.anthropic_oauth.json
    ...
    manual        — user ran `sonic auth add`
...
unify here is **removal**:
    ...
    ``sonic auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), sonic_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.sonic/.env  → clear it
    ...
    """
    from sonic_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Sonic.",
            f"  The pool entry is now suppressed — Sonic will ignore "
            f"{env_var} until you run `sonic auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Sonic stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `sonic auth add anthropic` to re-enable if needed.",
    ])


def _remove_sonic_pkce(provider: str, removed) -> RemovalResult:
    """~/.sonic/.anthropic_oauth.json is ours — delete it outright."""
    from sonic_constants import get_sonic_home

    result = RemovalResult()
    oauth_file = get_sonic_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Sonic Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `sonic login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `sonic auth add nous` to re-engage, which is the documented re-add
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``sonic auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``sonic auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `sonic model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Sonic auth store is not enough — _seed_from_singletons() would
    ...
    via ``sonic auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `sonic auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `sonic auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Sonic stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `sonic auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="sonic_pkce",
        remove_fn=_remove_sonic_pkce,
        description="~/.sonic/.anthropic_oauth.json",
    ))
    register(RemovalStep(
