...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(sonic_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(sonic_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved SONIC_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ':' (Unix PATH-style). E.g., '/opt/data:/var/www/html'."""
    env = os.getenv("SONIC_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
#
# Sonic profiles are separate SONIC_HOME dirs under
...
#
# Reference: May 2026 incident where a sonic-security profile session
# edited skills under both ``~/.sonic/profiles/sonic-security/skills/``
# AND ``~/.sonic/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SONIC_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SONIC_HOME.
    ...
    """
    try:
        home_real = _sonic_home_path().resolve()
        root_real = _sonic_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Sonic scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Sonic entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Sonic profile {info['target_profile']!r}, but the "
