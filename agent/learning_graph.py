...
def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or (fm.get("metadata", {}).get("sonic", {}) or {}).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or (fm.get("metadata", {}).get("sonic", {}) or {}).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_sonic_home() / "skills" / ".usage.json"
        try:
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_sonic_home() / "skills"
