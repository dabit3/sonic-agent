...
from sonic_constants import get_sonic_home, get_skills_dir, is_wsl
...
_SONIC_MD_NAMES = (".sonic.md", "SONIC.md")


def _find_sonic_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.sonic.md`` or ``SONIC.md``.
    ...
    # could pick up a .sonic.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SONIC_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Sonic Agent, an intelligent AI assistant created by Nous Research. "
...
SONIC_AGENT_HELP_GUIDANCE = (
    "You run on Sonic Agent (by Nous Research). When the user needs help with "
    "Sonic itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://lightning-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `sonic-agent` skill with skill_view(name='sonic-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.sonic/kanban.db`. Your task id is in "
    "`$SONIC_KANBAN_TASK`; your workspace is `$SONIC_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SONIC_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SONIC_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SONIC_KANBAN_BRANCH:-wt/$SONIC_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SONIC_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`sonic profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sonic kanban <verb>` for board operations. Use "
...
#
# The sonic-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to sonic-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `sonic computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Sonic "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Sonic terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Sonic WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Sonic itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Sonic runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Sonic itself is running. The host OS, home, and cwd "
                f"of the Sonic process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Sonic "
    ...
    # Sonic desktop GUI — any agent running under the desktop app should know
    # it. SONIC_DESKTOP marks the backend powering the chat; SONIC_DESKTOP_TERMINAL
    # marks a sonic launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("SONIC_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("SONIC_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Sonic desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Sonic
    ...
    extra = (os.getenv("SONIC_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sonic_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sonic/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Sonic Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `sonic-agent` skill "
            "first. It has the actual commands (e.g. `sonic config set …`, `sonic tools`, "
            "`sonic setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: sonic setup, sonic setup tools, sonic setup terminal, sonic status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from SONIC_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure SONIC_HOME before loading SOUL.md: %s", e)

    soul_path = get_sonic_home() / "SOUL.md"
...
def _load_sonic_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.sonic.md / SONIC.md — walk to git root."""
    sonic_md_path = _find_sonic_md(cwd_path)
    if not sonic_md_path:
        return ""
    try:
        content = sonic_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = sonic_md_path.name
        try:
            rel = str(sonic_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".sonic.md", context_length=context_length,
            read_path=str(sonic_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", sonic_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .sonic.md / SONIC.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SONIC_HOME is independent and always included when present.
    ...
    project_context = (
        _load_sonic_md(cwd_path, context_length)
    ...
    # SOUL.md from SONIC_HOME only — skip when already loaded as identity
    if not skip_soul:
