...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SONIC_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Sonic runtime / deployment
    "SONIC_HOME", "SONIC_PROFILE", "SONIC_GATEWAY_LOCK_DIR",
    "SONIC_MAX_ITERATIONS", "SONIC_MAX_TOKENS", "SONIC_API_TIMEOUT",
    "SONIC_REDACT_SECRETS", "SONIC_NOUS_TIMEOUT_SECONDS",
    "_SONIC_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SONIC_KANBAN_DB", "SONIC_KANBAN_WORKSPACES_ROOT", "SONIC_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "SONIC_KANBAN_",
    "SONIC_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(sonic_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(sonic_home) / ".env")
