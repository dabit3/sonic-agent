...
from sonic_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SONIC_PLATFORM`` or
            ``SONIC_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``sonic`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sonic/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``sonic`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SONIC_HOME, not cwd
        if not p.is_absolute():
            p = (sonic_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sonic/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    sonic = metadata.get("sonic") or {}
    if not isinstance(sonic, dict):
        sonic = {}
    return {
        "fallback_for_toolsets": sonic.get("fallback_for_toolsets", []),
        "requires_toolsets": sonic.get("requires_toolsets", []),
        "fallback_for_tools": sonic.get("fallback_for_tools", []),
        "requires_tools": sonic.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    sonic = metadata.get("sonic")
    if not isinstance(sonic, dict):
        return []
    raw = sonic.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Sonic metadata, VCS, virtualenv/dependency, cache, and skill
