...
from lightning_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``LIGHTNING_PLATFORM`` or
            ``LIGHTNING_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("LIGHTNING_PLATFORM")
...
# which becomes the dominant cost of ``lightning`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.lightning/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``lightning`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against LIGHTNING_HOME, not cwd
        if not p.is_absolute():
            p = (lightning_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.lightning/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    lightning = metadata.get("lightning") or {}
    if not isinstance(lightning, dict):
        lightning = {}
    return {
        "fallback_for_toolsets": lightning.get("fallback_for_toolsets", []),
        "requires_toolsets": lightning.get("requires_toolsets", []),
        "fallback_for_tools": lightning.get("fallback_for_tools", []),
        "requires_tools": lightning.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    lightning = metadata.get("lightning")
    if not isinstance(lightning, dict):
        return []
    raw = lightning.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Lightning metadata, VCS, virtualenv/dependency, and cache
