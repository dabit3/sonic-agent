...
Plugins built on Sonic Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Sonic
...
:class:`~sonic_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Sonic' main config
...
handles every provider, fallback chain, and per-task override Sonic
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`sonic_cli.plugins.PluginContext`
