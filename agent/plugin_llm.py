...
Plugins built on Lightning Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Lightning
...
:class:`~lightning_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Lightning' main config
...
handles every provider, fallback chain, and per-task override Lightning
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`lightning_cli.plugins.PluginContext`
