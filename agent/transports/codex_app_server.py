...
projection into Lightning' display, approval bridging, transcript projection into
...
"codex_app_server"`. Lightning' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("LIGHTNING_KANBAN_TASK"):
            kanban_db = spawn_env.get("LIGHTNING_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "LIGHTNING_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("LIGHTNING_HOME", os.path.expanduser("~/.lightning")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "lightning",
        client_title: str = "Lightning Agent",
