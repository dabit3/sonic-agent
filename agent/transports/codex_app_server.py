...
projection into Sonic's display, approval bridging, transcript projection into
...
"codex_app_server"`. Sonic's default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("SONIC_KANBAN_TASK"):
            kanban_db = spawn_env.get("SONIC_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SONIC_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SONIC_HOME", os.path.expanduser("~/.sonic")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "sonic",
        client_title: str = "Sonic Agent",
