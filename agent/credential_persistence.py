...
``auth.json``.  They intentionally have no dependency on ``sonic_cli.auth`` so
...
# Sources Sonic owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Sonic-owned OAuth/device-code state)
