...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.sonic/auth.json active provider)
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Sonic owns auxiliary retry + provider/model fallback policy (the
    ...
    # Sonic sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Sonic control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Sonic triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``SONIC_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``SONIC_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("SONIC_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("SONIC_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "SonicAgent",
}
...
# in lockstep with sonic_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``sonic_cli.__version__`` is
...
_AUTH_JSON_PATH = get_sonic_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Sonic Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``sonic_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.sonic/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from sonic_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``sonic auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``sonic_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Sonic auth store.
    ...
    try:
        from sonic_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from sonic_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `sonic setup` or `sonic model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`sonic_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from sonic_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second sonic
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``sonic config aux reset``)."""
...
def _resolve_auto(
    ...
    #    scenario where a user switches providers via `sonic model` but the
    #    old OPENAI_BASE_URL lingers in ~/.sonic/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: sonic model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.sonic/.env",
    ...
    if main_provider == "moa":
        try:
            from sonic_cli.config import load_config
            from sonic_cli.moa_config import resolve_moa_preset
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Sonic owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: sonic auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: sonic model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: sonic model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from sonic_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: sonic doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("sonic_cli.auth not available for provider %s", provider)
        return None, None
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`sonic_cli.plugins.PluginContext.register_auxiliary_task`) the
