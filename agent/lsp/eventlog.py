...
Grep recipe::
    ...
    tail -f ~/.lightning/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("lightning.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `lightning lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
