...
``kind: backend``) or ``~/.lightning/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`lightning_cli.tools_config` to inject this provider as a
