...
upstream prefix cache warm.  See ``sonic-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Sonic never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SONIC_HOME persona while keeping
    ...
    # Pointer to the sonic-agent skill + docs for user questions about Sonic itself.
    stable_parts.append(SONIC_AGENT_HELP_GUIDANCE)
    ...
    # SONIC_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Sonic, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Sonic profile the agent is running
    # under so it doesn't conflate ~/.sonic/skills/ (default profile) with
    # ~/.sonic/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Sonic profile: default. Other profiles (if any) live "
            "under ~/.sonic/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Sonic profile: {active_profile}. This session reads "
            f"and writes ~/.sonic/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.sonic/skills/, ~/.sonic/plugins/, "
            f"~/.sonic/cron/, ~/.sonic/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Sonic never rebuilds or reinjects parts of it
