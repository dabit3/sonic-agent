...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``sonic -c`` / ``--resume`` (relaunch
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Sonic-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.sonic/sessions/``.  Without
...
class AIAgent:
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        if cfg is not None:
            return cfg
        return env_float("SONIC_API_TIMEOUT", 1800.0)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        sonic-agent #21444 for the symptom history.  The upstream backend
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Sonic against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("SONIC_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("sonic_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from sonic_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``SONIC_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        # untrusted input (X-Sonic-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects SONIC_REDACT_SECRETS via
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.sonic/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (SONIC_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (SONIC_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset SONIC_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("SONIC_DEV_CREDITS"))
        ...
        if _dev:
            # SONIC_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `sonic logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
