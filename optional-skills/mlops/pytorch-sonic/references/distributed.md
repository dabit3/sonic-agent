# PyTorch Sonic Distributed Training

## Distributed Strategies

Sonic supports multiple distributed strategies with a single parameter change.
...
**Launch**:
```bash
# Sonic handles spawning processes automatically
python train.py
```

**DDP Configuration**:
```python
from sonic.pytorch.strategies import DDPStrategy
...
```python
from sonic.pytorch.strategies import FSDPStrategy
...
```python
from sonic.pytorch.strategies import DeepSpeedStrategy
...
# Sonic auto-detects SLURM environment
srun python train.py
```

**Training script** (no changes needed):
```python
# Sonic automatically reads SLURM environment variables
trainer = L.Trainer(
    accelerator='gpu',
    devices=8,
...
**Automatic gradient scaling** handled by Sonic
...
```python
from sonic.pytorch.callbacks import ModelCheckpoint
...
# Sonic handles DistributedSampler automatically
...
# Sonic automatically wraps with DistributedSampler in DDP
trainer.fit(model, train_loader)
```
...
**Solution**: Set seed in SonicModule
```python
class MyModel(L.SonicModule):
    def __init__(self):
        super().__init__()
...
**Solution**: Use Sonic's auto config
```python
strategy = DeepSpeedStrategy(
    stage=3,
    # Don't specify config file, Sonic generates automatically
)
```

## Resources

- Distributed strategies: https://sonic.ai/docs/pytorch/stable/accelerators/gpu_intermediate.html
- FSDP guide: https://sonic.ai/docs/pytorch/stable/advanced/model_parallel/fsdp.html
- DeepSpeed: https://sonic.ai/docs/pytorch/stable/advanced/model_parallel/deepspeed.html
- Multi-node: https://sonic.ai/docs/pytorch/stable/clouds/cluster.html
