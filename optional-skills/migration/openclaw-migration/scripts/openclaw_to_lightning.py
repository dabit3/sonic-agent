#!/usr/bin/env python3
"""OpenClaw -> Lightning migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Lightning Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Lightning.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Lightning memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Lightning memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Lightning-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Lightning-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Lightning command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.lightning/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.lightning/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Lightning .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Lightning .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Lightning .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Lightning .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Lightning .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Lightning config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Lightning config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Lightning.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Lightning MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Lightning config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'lightning cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Lightning config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Lightning session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Lightning custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Lightning config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Lightning config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Lightning config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Lightning config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Lightning in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Lightning`` (prose), but lowercase matches
# like ``openclaw`` → ``lightning`` (so filesystem paths like ``~/.openclaw``
# become ``~/.lightning`` — the real Lightning home — not the broken ``~/.Lightning``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Lightning'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Lightning'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Lightning'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Lightning`` but maps ``openclaw`` → ``lightning`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.lightning/config.yaml`` (the real Lightning home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Lightning.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Lightning Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-lightning/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Lightning config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-lightning/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Lightning session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Lightning config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Lightning
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Lightning-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Lightning-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: lightning config set",
                        )
                    continue
        ...
        for oc_key, lightning_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and lightning_key not in secret_additions:
                secret_additions[lightning_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, lightning_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and lightning_key not in secret_additions:
                        secret_additions[lightning_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lightning_config = load_yaml_file(destination)
        current_model = lightning_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = lightning_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                lightning_config["model"] = {"default": model_str}
            dump_yaml_file(destination, lightning_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Lightning still uses "edge"
        ...
        lightning_config = load_yaml_file(destination)
        existing_tts = lightning_config.get("tts", {})
        ...
        if self.execute:
            ...
            lightning_config["tts"] = merged_tts
            dump_yaml_file(destination, lightning_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Lightning destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Lightning-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Lightning-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lightning_cfg_path = self.target_root / "config.yaml"
        lightning_cfg = load_yaml_file(lightning_cfg_path)
        existing_mcp = lightning_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Lightning config")
                continue

            lightning_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                lightning_srv["command"] = srv["command"]
                if srv.get("args"):
                    lightning_srv["args"] = srv["args"]
                if srv.get("env"):
                    lightning_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    lightning_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                lightning_srv["url"] = srv["url"]
                if srv.get("headers"):
                    lightning_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    lightning_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                lightning_srv["enabled"] = False
            if srv.get("timeout"):
                lightning_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                lightning_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                lightning_srv["tools"] = {}
                if tools_cfg.get("include"):
                    lightning_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    lightning_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                lightning_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = lightning_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(lightning_cfg_path)
            lightning_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(lightning_cfg_path, lightning_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'lightning cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = lightning_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Lightning reasoning_effort
        ...
        if compaction:
            compression = lightning_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = lightning_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            lightning_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = lightning_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'lightning gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("LIGHTNING_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lightning_cfg_path = self.target_root / "config.yaml"
        lightning_cfg = load_yaml_file(lightning_cfg_path)
        sr = lightning_cfg.get("session_reset") or {}
        changes = False
        ...
        if changes:
            lightning_cfg["session_reset"] = sr
            if self.execute:
                self.maybe_backup(lightning_cfg_path)
                dump_yaml_file(lightning_cfg_path, lightning_cfg)
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lightning_cfg_path = self.target_root / "config.yaml"
        lightning_cfg = load_yaml_file(lightning_cfg_path)
        custom_providers = lightning_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(lightning_cfg_path)
            lightning_cfg["custom_providers"] = custom_providers
            dump_yaml_file(lightning_cfg_path, lightning_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Lightning config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            lightning_cfg_path = self.target_root / "config.yaml"
            lightning_cfg = load_yaml_file(lightning_cfg_path)
            discord_lightning = lightning_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_lightning["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_lightning["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                lightning_cfg["discord"] = discord_lightning
                dump_yaml_file(lightning_cfg_path, lightning_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        lightning_cfg_path = self.target_root / "config.yaml"
        lightning_cfg = load_yaml_file(lightning_cfg_path)
        browser_lightning = lightning_cfg.get("browser") or {}
        changed = False

        # Map fields that have Lightning equivalents
        if browser.get("cdpUrl"):
            browser_lightning["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_lightning["headless"] = browser["headless"]
            changed = True

        if changed:
            lightning_cfg["browser"] = browser_lightning
            if self.execute:
                self.maybe_backup(lightning_cfg_path)
                dump_yaml_file(lightning_cfg_path, lightning_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            lightning_mode = mode_map.get(mode, "manual")
            lightning_cfg.setdefault("approvals", {})["mode"] = lightning_mode
            if self.execute:
                self.maybe_backup(lightning_cfg_path)
                dump_yaml_file(lightning_cfg_path, lightning_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{lightning_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Lightning Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Lightning. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Lightning Config Not Overwritten)",
                "",
                "These items already existed in your Lightning config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Lightning agent discovers these",
            "directories, it may read/write to them instead of the Lightning state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `lightning claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `lightning claw cleanup` to fix it.",
            "",
            "## Lightning-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `lightning claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `lightning setup` to configure any remaining settings",
            "- Run `lightning mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `lightning cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `lightning cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `lightning gateway install` if you need the gateway service",
            "- Review `~/.lightning/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Lightning Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("LIGHTNING_HOME") or str(Path.home() / ".lightning"), help="Lightning home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Lightning targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Lightning-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Lightning Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.lightning/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.lightning/config.yaml")
        print("    2. Run: lightning mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: lightning cron")
