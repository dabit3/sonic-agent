...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SONIC_HOME", os.path.expanduser("~/.sonic")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SONIC_HOME", Path.home() / ".sonic")) / "skills" / "red-teaming" / "godmode"
...
PREFILL_PATH = SONIC_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Sonic models — already uncensored, just needs clean prompt
    "sonic": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "sonic" in model_lower or "nous" in model_lower:
        return "sonic"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Sonic config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.sonic/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Sonic for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Sonic for changes to take effect.")
