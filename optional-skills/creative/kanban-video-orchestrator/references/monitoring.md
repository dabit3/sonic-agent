...
lightning kanban watch --tenant <project-slug>

# Snapshot of the board
lightning kanban list --tenant <project-slug>
lightning kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
lightning kanban stats --tenant <project-slug>

# Visual dashboard (browser)
lightning dashboard

# Inspect a specific task (includes comments + events)
lightning kanban show <task-id>

# Follow a single task's event stream
lightning kanban tail <task-id>
```

Verify available subcommands with `lightning kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `lightning kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `lightning kanban block <id>` then `lightning kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `lightning kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `lightning kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `lightning profile list` (profile exists?), `lightning status` (gateway/dispatcher up?), and verify tenant. |
...
lightning kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
lightning kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(lightning kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
lightning kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
lightning kanban block <task-id>
lightning kanban archive <task-id>

# Diagnose what happened
lightning kanban show <task-id>      # task body, comments, recent events
lightning kanban tail <task-id>      # follow the live event stream
lightning kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `lightning kanban create` for the director
...
while true; do
    clear
    lightning kanban list --tenant <slug>
    echo "---"
    lightning kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `lightning kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `lightning profile list` and start
