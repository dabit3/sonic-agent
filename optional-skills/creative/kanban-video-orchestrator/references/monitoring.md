...
sonic kanban watch --tenant <project-slug>

# Snapshot of the board
sonic kanban list --tenant <project-slug>
sonic kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
sonic kanban stats --tenant <project-slug>

# Visual dashboard (browser)
sonic dashboard

# Inspect a specific task (includes comments + events)
sonic kanban show <task-id>

# Follow a single task's event stream
sonic kanban tail <task-id>
```

Verify available subcommands with `sonic kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `sonic kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `sonic kanban block <id>` then `sonic kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `sonic kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `sonic kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `sonic profile list` (profile exists?), `sonic status` (gateway/dispatcher up?), and verify tenant. |
...
sonic kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
sonic kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(sonic kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
sonic kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
sonic kanban block <task-id>
sonic kanban archive <task-id>

# Diagnose what happened
sonic kanban show <task-id>      # task body, comments, recent events
sonic kanban tail <task-id>      # follow the live event stream
sonic kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `sonic kanban create` for the director
...
while true; do
    clear
    sonic kanban list --tenant <slug>
    echo "---"
    sonic kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `sonic kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `sonic profile list` and start
