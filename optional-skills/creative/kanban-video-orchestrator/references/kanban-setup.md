...
the actual `setup.sh` that creates the project workspace, configures Sonic
...
2. **Create profiles** — `sonic profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.sonic/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `sonic kanban create` assigned to the director
...
```bash
sonic profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.sonic/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.sonic/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Sonic
...
Each profile gets a `SOUL.md` at `~/.sonic/profiles/<name>/SOUL.md` that
...
```bash
sonic kanban create "Direct production of <video title>" \
    --assignee director \
...
the Sonic `.env` (`${SONIC_HOME:-$HOME/.sonic}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _sonic_env="${SONIC_HOME:-$HOME/.sonic}/.env"
    if grep -q "^${var}=" "$_sonic_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_sonic_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_sonic_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY sonic ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY sonic OPENROUTER_API_KEY || exit 1
# ...
```
