...
Maps each role archetype to the Lightning skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public lightning-agent
...
## Lightning skills relevant to video production

### Visual / rendering skills (`lightning-agent/skills/creative/`)
...
### Design / pre-production skills (`lightning-agent/skills/creative/`)
...
### Audio / media skills (`lightning-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`lightning-agent/skills/devops/`)
...
These are **not** Lightning skills but external CLIs / APIs that profiles invoke.
...
## Built-in Lightning tools for media review

These are native Lightning tools — not invoked via terminal but through their own
...
key is present in `~/.lightning/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Lightning profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.lightning/.env` → environment variable.
...
When a new Lightning-public video skill ships:
