...
Maps each role archetype to the Sonic skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public sonic-agent
...
## Sonic skills relevant to video production

### Visual / rendering skills (`sonic-agent/skills/creative/`)
...
### Design / pre-production skills (`sonic-agent/skills/creative/`)
...
### Audio / media skills (`sonic-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`sonic-agent/skills/devops/`)
...
These are **not** Sonic skills but external CLIs / APIs that profiles invoke.
...
## Built-in Sonic tools for media review

These are native Sonic tools — not invoked via terminal but through their own
...
key is present in `${SONIC_HOME:-~/.sonic}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Sonic profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SONIC_HOME:-~/.sonic}/.env` → environment variable.
...
When a new Sonic-public video skill ships:
