...
# 5. Symlinks the 'sonic' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Sonic Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sonic-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink sonic into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sonic command..."

SONIC_BIN="$SCRIPT_DIR/venv/bin/sonic"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SONIC_BIN" "$COMMAND_LINK_DIR/sonic"
echo -e "${GREEN}✓${NC} Symlinked sonic → $COMMAND_LINK_DISPLAY_DIR/sonic"
...
# ============================================================================
# Seed bundled skills into ~/.sonic/skills/
# ============================================================================

SONIC_SKILLS_DIR="${SONIC_HOME:-$HOME/.sonic}/skills"
mkdir -p "$SONIC_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sonic/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SONIC_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  sonic status        # Check configuration"
if is_termux; then
    echo "  sonic gateway       # Run gateway in foreground"
else
    echo "  sonic gateway install # Install gateway service (messaging + cron)"
fi
echo "  sonic cron list     # View scheduled jobs"
echo "  sonic doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sonic_cli.main setup
fi
