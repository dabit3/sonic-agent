"""
Backup and import commands for lightning CLI.

`lightning backup` creates a zip archive of the entire ~/.lightning/ directory
(excluding the lightning-agent repo and transient files).

`lightning import` restores from a backup zip, overlaying onto the current
LIGHTNING_HOME root.
"""
...
from lightning_constants import get_default_lightning_root, get_lightning_home, display_lightning_home
...
_EXCLUDED_DIRS = {
    "lightning-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to lightning root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Lightning home directory."""
    lightning_root = get_default_lightning_root()

    if not lightning_root.is_dir():
        print(f"Error: Lightning home directory not found at {lightning_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"lightning-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"lightning-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_lightning_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(lightning_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(lightning_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(lightning_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside lightning root
            try:
    ...
    print(f"\nRestore with: lightning import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Lightning backup.
    ...
    # Look for telltale files that a lightning home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Lightning backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.lightning/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a lightning dir name
        if prefix in {".lightning", "lightning"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_lightning_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (lightning_root / "config.yaml").exists()
        has_env = (lightning_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Lightning configuration.")
        ...
        for member in members:
            ...
            target = lightning_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(lightning_root.resolve())
            except ValueError:
        ...
        profiles_dir = lightning_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # lightning_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: lightning profile list  (after installing lightning)")

        # Guidance
        print()
        if not (lightning_root / "lightning-agent").is_dir():
            print("Note: The lightning-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: lightning update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  lightning -p {pname} gateway install")

        print("Done. Your Lightning configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and lightning backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to LIGHTNING_HOME).
...
# — `lightning update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(lightning_home: Optional[Path] = None) -> Path:
    home = lightning_home or get_lightning_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    lightning_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(lightning_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(lightning_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for lightning backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_lightning_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, lightning_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``lightning_root`` to ``out_path``.
...
def _pre_update_backup_dir(lightning_home: Optional[Path] = None) -> Path:
    home = lightning_home or get_lightning_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of LIGHTNING_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<LIGHTNING_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``lightning update``) should continue even if the backup fails.
    """
    lightning_root = lightning_home or get_default_lightning_root()
    if not lightning_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(lightning_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, lightning_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `lightning claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of LIGHTNING_HOME under ``backups/`` before a
    ``lightning claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``lightning import <archive>``.  Writes to
    ``<LIGHTNING_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `lightning import` and the
