...
on Japanese locales, etc.).  Lightning's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``lightning_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Lightning is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``LIGHTNING_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("LIGHTNING_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Lightning-managed Git install directories so
    ...
    # already-running shells don't see the change — which means lightning
    ...
    # install without Lightning-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Lightning's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Lightning's
    ``lightning config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Lightning (User env var in Windows Settings, or
    export in a PowerShell profile) and Lightning picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Lightning-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\lightning\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``lightning`` from right after install) retain their old PATH.

    Any subprocess Lightning spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Lightning-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "lightning", "git", "cmd"),
        os.path.join(local_appdata, "lightning", "git", "bin"),
        os.path.join(local_appdata, "lightning", "git", "usr", "bin"),
        # Lightning venv Scripts directory — host of the lightning.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes lightning, but harmless to include.
        os.path.join(local_appdata, "lightning", "lightning-agent", "venv", "Scripts"),
