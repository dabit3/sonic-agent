"""
Configuration management for Lightning Agent.

Config files are stored in ~/.lightning/ for easy access:
- ~/.lightning/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.lightning/.env         - API keys and secrets

This module provides:
- lightning config          - Show current configuration
- lightning config edit     - Open config in editor
- lightning config set      - Set a specific value
- lightning config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.lightning/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``lightning logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  lightning config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `lightning plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "LIGHTNING_LANGFUSE_ENV",
    "LIGHTNING_LANGFUSE_RELEASE",
    "LIGHTNING_LANGFUSE_SAMPLE_RATE",
    "LIGHTNING_LANGFUSE_MAX_CHARS",
    "LIGHTNING_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from lightning_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("LIGHTNING_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_lightning_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Lightning is running in package-manager-managed mode.

    Two signals: the LIGHTNING_MANAGED env var (set by the systemd service),
    or a .managed marker file in LIGHTNING_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade lightning-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Lightning was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.lightning/.install_method`` file (written by installers)
    2. LIGHTNING_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_lightning_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.lightning/.install_method."""
    stamp = get_lightning_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade lightning-agent"
    if method == "docker":
        return "docker pull nousresearch/lightning-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade lightning-agent"
        return "pip install --upgrade lightning-agent"
...
def format_managed_message(action: str = "modify this Lightning installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("LIGHTNING_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Lightning installation is managed by NixOS "
            f"(LIGHTNING_MANAGED={env_hint}).\n"
            "Edit services.lightning-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Lightning installation is managed by Homebrew "
            f"(LIGHTNING_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade lightning-agent"
        )

    return (
        f"Cannot {action}: this Lightning installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Lightning."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from LIGHTNING_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, lightning_bin
    or None if container mode is not active, we're already inside the
    container, or LIGHTNING_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("LIGHTNING_DEV") == "1":
        return None
    ...
    container_mode_file = get_lightning_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "lightning-agent")
    exec_user = info.get("exec_user", "lightning")
    lightning_bin = info.get("lightning_bin", "/data/current-package/bin/lightning")
...
# Re-export from lightning_constants — canonical definition lives there.
from lightning_constants import get_lightning_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_lightning_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_lightning_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the lightning group can
    share state with the gateway service.

    The mode can be overridden via the LIGHTNING_HOME_MODE environment variable
    (e.g. LIGHTNING_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse LIGHTNING_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("LIGHTNING_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Lightning runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("LIGHTNING_CONTAINER") or os.environ.get("LIGHTNING_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set LIGHTNING_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into LIGHTNING_HOME if the user doesn't have one yet."""
...
def ensure_lightning_home():
    """Ensure ~/.lightning directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_lightning_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_lightning_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"LIGHTNING_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Lightning-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Lightning auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Lightning sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Lightning
        ...
        # bundled Lightning image, which drops to the `lightning` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Lightning sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Lightning should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``lightning chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.lightning/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: lightning sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Lightning truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``lightning kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``lightning kanban decompose`` and the kanban
        ...
        # ``lightning profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `lightning model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `lightning --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `lightning -c` muscle memory.  Default off so existing
        # users aren't surprised.  LIGHTNING_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/lightning`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.lightning/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.lightning/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Lightning feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Lightning auto-pauses the goal and
    ...
    # always goes to ~/.lightning/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${LIGHTNING_SKILL_DIR} and ${LIGHTNING_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `lightning curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.lightning/skills/ into
        # ~/.lightning/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `lightning curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for lightning-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Lightning Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (LIGHTNING_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.lightning/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or LIGHTNING_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `lightning doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``lightning_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Lightning to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via LIGHTNING_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `lightning -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `lightning` launches with
        ...
        # decomposition is manual via `lightning kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     lightning-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.lightning/logs/.
    ...
    # update model picker lists without shipping a lightning-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://lightning-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `lightning model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.lightning/state.db.
    ...
    "sessions": {
        ...
        # default of ``lightning sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.lightning/sessions/session_{sid}.json`` on every turn
    ...
    # ``lightning update`` behaviour.
    "updates": {
        # Run a full ``lightning backup``-style zip of LIGHTNING_HOME before every
        # ``lightning update``.  Backups land in ``<LIGHTNING_HOME>/backups/`` and
        # can be restored with ``lightning import <path>``.  Off by default —
        # on large LIGHTNING_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<LIGHTNING_HOME>/lsp/bin/`` on first use.
    ...
    # `lightning tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.lightning/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.lightning/.env (or your shell) and never in config.yaml.
            ...
            # ~/.lightning/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "LIGHTNING_GEMINI_CLIENT_SECRET": {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'lightning model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Lightning auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @lightning:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. LIGHTNING_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #lightning)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: lightning-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'lightning-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Lightning API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Lightning API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Lightning API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # LIGHTNING_TOOL_PROGRESS and LIGHTNING_TOOL_PROGRESS_MODE are deprecated —
    ...
    "LIGHTNING_PREFILL_MESSAGES_FILE": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.lightning.config`` entries, then checks
    ...
    try:
        ...
        # should never break `lightning update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Lightning versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``lightning_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'lightning setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Lightning won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'lightning doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("LIGHTNING_HOME", "~/.lightning")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("LIGHTNING_TOOL_PROGRESS")
            old_mode = get_env_value("LIGHTNING_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from LIGHTNING_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from LIGHTNING_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("LIGHTNING_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from LIGHTNING_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$LIGHTNING_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_lightning_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `lightning plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `lightning curator status` has no
    ...
    #   3. Creates `~/.lightning/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_lightning_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_lightning_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: lightning config set <key> <value>")
    ...
    # metadata.lightning.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _lightning_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.lightning/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.lightning/config.yaml.
    ...
    (which change ``LIGHTNING_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — lightning auth) — OpenAI Codex
#   nous         (OAuth — lightning auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.lightning/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.lightning/.env.
    ...
    interactive menu render (`lightning tools`, `lightning setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `lightning tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Lightning env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.lightning/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.lightning/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.lightning/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.lightning/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Lightning — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Lightning vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.lightning/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Lightning Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  lightning config edit     # Edit config file", Colors.DIM))
    print(color("  lightning config set <key> <value>", Colors.DIM))
    print(color("  lightning setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: lightning config set <key> <value>")
            print()
            print("Examples:")
            print("  lightning config set model anthropic/claude-sonnet-4")
            print("  lightning config set terminal.backend docker")
            print("  lightning config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'lightning config migrate' to add them")
        ...
        print("  lightning config           Show current configuration")
        print("  lightning config edit      Open config in editor")
        print("  lightning config set <key> <value>   Set a config value")
        print("  lightning config check     Check for missing/outdated config")
        print("  lightning config migrate   Update config with new options")
        print("  lightning config path      Show config file path")
        print("  lightning config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``lightning config`` UI so users
