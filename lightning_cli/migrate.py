"""CLI handlers for ``lightning migrate ...``.

Currently exposes only ``lightning migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``lightning migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: lightning migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `lightning migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `lightning doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
