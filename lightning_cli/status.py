"""
Status command for lightning CLI.

Shows the status of all Lightning Agent components.
"""
...
from lightning_cli.auth import AuthError, resolve_provider
from lightning_cli.colors import Colors, color
from lightning_cli.config import get_env_path, get_env_value, get_lightning_home, load_config
from lightning_cli.models import provider_label
from lightning_cli.nous_subscription import get_nous_subscription_features
from lightning_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``lightning config``'s
...
def show_status(args):
    """Show status of all Lightning Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Lightning Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: lightning auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: lightning model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: lightning auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: lightning auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: lightning model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from lightning_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'lightning-agent[vercel]')"
    ...
    try:
        from lightning_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   lightning gateway")
    ...
    jobs_file = get_lightning_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_lightning_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'lightning doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'lightning setup' to configure", Colors.DIM))
    print()
