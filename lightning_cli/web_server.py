"""
Lightning Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m lightning_cli.main web          # Start on http://127.0.0.1:9119
    python -m lightning_cli.main web --port 8080
"""
...
try:
    ...
    # running `lightning dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["LIGHTNING_WEB_DIST"]) if "LIGHTNING_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Lightning Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Lightning-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``lightning dashboard --tui``
# or LIGHTNING_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.lightning/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_lightning_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "lightning-update": "lightning-update.log",
}
...
def _spawn_lightning_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``lightning <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``lightning_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "lightning_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "LIGHTNING_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``lightning gateway restart`` in the background."""
    try:
        proc = _spawn_lightning_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/lightning/update")
async def update_lightning():
    """Kick off ``lightning update`` in the background."""
    try:
        proc = _spawn_lightning_action(["update"], "lightning-update")
    except Exception as exc:
        _log.exception("Failed to spawn lightning update")
        raise HTTPException(status_code=500, detail=f"Failed to start update: {exc}")
    return {
        "ok": True,
        "pid": proc.pid,
        "name": "lightning-update",
    }
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Lightning supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in lightning_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.lightning/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``lightning auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Lightning resolves Anthropic creds in this order at runtime:
    1. ``~/.lightning/.anthropic_oauth.json`` — Lightning-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_lightning_oauth_credentials,
            read_claude_code_credentials,
            _LIGHTNING_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_lightning_oauth_credentials = None  # type: ignore
        _LIGHTNING_OAUTH_FILE = None  # type: ignore

    lightning_creds = None
    if read_lightning_oauth_credentials:
        try:
            lightning_creds = read_lightning_oauth_credentials()
        except Exception:
            lightning_creds = None
    if lightning_creds and lightning_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "lightning_pkce",
            "source_label": f"Lightning PKCE ({_LIGHTNING_OAUTH_FILE})",
            "token_preview": _truncate_token(lightning_creds.get("accessToken")),
            "expires_at": lightning_creds.get("expiresAt"),
            "has_refresh_token": bool(lightning_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Lightning even
    when they also have a separate Lightning-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "lightning auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "lightning auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lightning auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "lightning auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lightning auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from lightning_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("lightning_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Lightning-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _LIGHTNING_OAUTH_FILE
            if _LIGHTNING_OAUTH_FILE.exists():
                _LIGHTNING_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from lightning_cli.auth import clear_provider_auth
...
#          → persists to ~/.lightning/.anthropic_oauth.json AND credential pool
...
# Guarded so lightning web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Lightning file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``lightning auth add anthropic``.
    """
    from agent.anthropic_adapter import _LIGHTNING_OAUTH_FILE
    payload = {
    ...
    _LIGHTNING_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _LIGHTNING_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "lightning-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("LIGHTNING_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from lightning_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``lightning auth add minimax-oauth``.
    """
    from lightning_cli.auth import (
        _minimax_poll_token,
...
async def get_logs(
    ...
    from lightning_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_lightning_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from lightning_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from lightning_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, LIGHTNING_HOME)."""
    from lightning_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["lightning_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process LIGHTNING_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_lightning_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "lightning setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from lightning_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from lightning_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from lightning_cli.tools_config import (
...
#
# The endpoint spawns the same ``lightning --tui`` binary the CLI uses, behind
...
try:
    from lightning_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``lightning --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``LIGHTNING_TUI_RESUME`` env var —
    matching what ``lightning_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``LIGHTNING_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from lightning_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("LIGHTNING_TUI_DISABLE_MOUSE", "1")
    env.setdefault("LIGHTNING_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["LIGHTNING_TUI_RESUME"] = resume

    if sidecar_url:
        env["LIGHTNING_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Lightning inside WSL2 to use the dashboard's /chat "
...
# LIGHTNING_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/lightning"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/lightning/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /lightning`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__LIGHTNING_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/lightning``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__LIGHTNING_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__LIGHTNING_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__LIGHTNING_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/lightning`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Lightning backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Lightning Teal",         "description": "Classic dark teal — the canonical Lightning look"},
    {"name": "default-large", "label": "Lightning Teal (Large)", "description": "Lightning Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.lightning/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.lightning/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_lightning_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.lightning/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as lightning_cli.plugins):
    1. User plugins:    ~/.lightning/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.lightning/plugins/  (only if LIGHTNING_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from lightning_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_lightning_home() / "plugins", "user"),
    ...
    # ``lightning_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("LIGHTNING_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".lightning" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from lightning_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_lightning_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"lightning auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from lightning_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from lightning_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lightning_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lightning_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.lightning/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.lightning/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"lightning_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Lightning Web UI → http://{host}:{port}")
