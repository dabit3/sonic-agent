...
# xAI's public API is OpenAI-compatible for the endpoints Lightning commonly
# uses. The Responses endpoint is included because Lightning' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Lightning-managed OAuth credentials."""

    auth_hint = "lightning auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`lightning auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`lightning auth reset xai-oauth` or re-authenticate with "
                    "`lightning auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `lightning auth add xai-oauth --type oauth`."
            )
