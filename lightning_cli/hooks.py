"""lightning hooks — inspect and manage shell-script hooks.

Usage::

    lightning hooks list
    lightning hooks test <event> [--for-tool X] [--payload-file F]
    lightning hooks revoke <command>
    lightning hooks doctor

Consent records live under ``~/.lightning/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.lightning/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``lightning hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lightning hooks {list|test|revoke|doctor}")
        print("Run 'lightning hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.lightning/config.yaml.")
        print("See `lightning hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lightning hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lightning hooks test` and `lightning hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Lightning wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lightning hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lightning hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lightning hooks doctor`.")
