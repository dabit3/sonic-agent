"""Profile distributions — shareable, packaged Lightning profiles via git.

A distribution is a Lightning profile published as a git repository (or
...
* ``lightning profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``lightning skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``lightning profile``, not a parallel tree):

    lightning profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "lightning_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".lightning_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "lightning-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            lightning_requires=str(data.get("lightning_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.lightning_requires:
            out["lightning_requires"] = self.lightning_requires
        if self.author:
...
def check_lightning_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Lightning {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``lightning profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Lightning profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Lightning distribution."
        )

    # Version check up-front so we fail fast
    check_lightning_requires(manifest.lightning_requires, lightning_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.lightning).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="lightning_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `lightning profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`lightning profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="lightning_dist_update_") as tmp:
        plan = plan_install(
