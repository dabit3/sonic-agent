"""
Gateway subcommand for lightning CLI.

Handles: lightning gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_lightning_home is imported lazily at call sites to avoid ImportError
# when lightning_constants is cached from a pre-update version during `lightning update`.
from lightning_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from lightning_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "lightning-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "lightning CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``lightning gateway status`` from falsely counting the ``lightning`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``lightning gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "lightning_cli.main gateway",
        "lightning_cli.main --profile",
        "lightning_cli.main -p",
        "lightning_cli/main.py gateway",
        "lightning_cli/main.py --profile",
        "lightning_cli/main.py -p",
        "lightning gateway",
        "gateway/run.py",
    ]
    current_home = str(get_lightning_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"LIGHTNING_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. LIGHTNING_HOME
        ...
        # LIGHTNING_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "LIGHTNING_HOME=" in command and f"LIGHTNING_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``lightning update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Lightning profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Lightning profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from lightning_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "lightning_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``lightning update`` users
    # with no running gateway until they re-invoke ``lightning gateway``
...
def _sync_lightning_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``LIGHTNING_HOME``.

    Under ``sudo``, ``LIGHTNING_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_lightning_home` falls back to ``/root/.lightning`` — the wrong
    profile. The unit file pins ``LIGHTNING_HOME`` for the actual gateway
    ...
    unit_home = env.get("LIGHTNING_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("LIGHTNING_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["LIGHTNING_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}lightning gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}lightning gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `lightning gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``lightning gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from lightning_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (LIGHTNING_HOME-scoped).
...
from lightning_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``lightning gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``LIGHTNING_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("LIGHTNING_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "lightning-gateway"
SERVICE_DESCRIPTION = "Lightning Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current LIGHTNING_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (LIGHTNING_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from lightning_constants import get_default_lightning_root
    home = get_lightning_home().resolve()
    default = get_default_lightning_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary LIGHTNING_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(lightning_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when LIGHTNING_HOME is a named profile.

    For ``~/.lightning/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        lightning_home: Optional explicit LIGHTNING_HOME path. Defaults to the current
            ``get_lightning_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from lightning_constants import get_default_lightning_root
    home = Path(lightning_home or str(get_lightning_home())).resolve()
    default = get_default_lightning_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this LIGHTNING_HOME.

    Default ``~/.lightning`` returns ``lightning-gateway`` (backward compatible).
    Profile ``~/.lightning/profiles/coder`` returns ``lightning-gateway-coder``.
    Any other LIGHTNING_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Lightning installs that predate the
# lightning-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (lightning-gateway-*.service) and unrelated third-party
# "lightning" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("lightning.service",)
...
def _find_legacy_lightning_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Lightning gateway units.

    Detects unit files installed by older Lightning versions that used a
    different service name (e.g. ``lightning.service`` before the rename to
    ``lightning-gateway.service``). When both a legacy unit and the current
    ``lightning-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``lightning-gateway-coder.service`` and unrelated third-party
      ``lightning-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``lightning.service`` running an unrelated
...
def has_legacy_lightning_units() -> bool:
    """Return True when any legacy Lightning gateway unit files exist."""
    return bool(_find_legacy_lightning_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Lightning gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_lightning_units()
    if not legacy:
        return
    print_warning("Legacy Lightning gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current lightning-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    lightning gateway migrate-legacy")


def remove_legacy_lightning_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Lightning gateway unit files.

    Iterates over whatever ``_find_legacy_lightning_units()`` returns — which is
    ...
    legacy = _find_legacy_lightning_units()
    if not legacy:
        print("No legacy Lightning gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Lightning gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: lightning gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo lightning gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    lightning gateway uninstall")
    print_info("    sudo lightning gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Lightning can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo lightning gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo lightning gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo lightning gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Lightning often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.lightning`` → ``ai.lightning.gateway.plist`` (backward compatible).
    Profile ``~/.lightning/profiles/coder`` → ``ai.lightning.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.lightning.gateway-{suffix}" if suffix else "ai.lightning.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.lightning/lightning-agent  -> /home/alice/.lightning/lightning-agent
      /opt/lightning                 -> /opt/lightning  (kept as-is)
...
def _lightning_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current LIGHTNING_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_lightning_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.lightning                    → /home/alice/.lightning
      /root/.lightning/profiles/coder     → /home/alice/.lightning/profiles/coder
      /opt/custom-lightning               → /opt/custom-lightning  (kept as-is)
    """
    current_lightning = get_lightning_home().resolve()
    current_default = (Path.home() / ".lightning").resolve()
    target_default = Path(target_home_dir) / ".lightning"

    # Default ~/.lightning → remap to target user's default
    if current_lightning == current_default:
        return str(target_default)

    # Profile or subdir of ~/.lightning → preserve the relative structure
    try:
        relative = current_lightning.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.lightning) — keep as-is
        return str(current_lightning)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    lightning_node = lightning_home / "node" / "bin"
    if _is_dir(lightning_node):
        candidates.append(str(lightning_node))
    lightning_nm = lightning_home / "node_modules" / ".bin"
    if _is_dir(lightning_nm):
        candidates.append(str(lightning_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        lightning_home = _lightning_home_for_target_user(home_dir)
        profile_arg = _profile_arg(lightning_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m lightning_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="LIGHTNING_HOME={lightning_home}"
Restart=always
RestartSec=5
...
"""
    ...
    lightning_home = str(get_lightning_home().resolve())
    profile_arg = _profile_arg(lightning_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only LIGHTNING_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp LIGHTNING_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../lightning_test`` path into
    # ``Environment="LIGHTNING_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Lightning install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo lightning gateway uninstall --system")
    print_info("         lightning gateway install")
    print_info("         lightning gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (lightning.service from pre-rename installs)
    # before installing the new lightning-gateway.service. If both remain, they
    ...
    if has_legacy_lightning_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_lightning_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}lightning gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}lightning gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}lightning gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_lightning_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `lightning gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `lightning gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}lightning gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}lightning gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}lightning gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}lightning gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.lightning.gateway-{suffix}" if suffix else "ai.lightning.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    lightning_home = str(get_lightning_home().resolve())
    log_dir = get_lightning_home() / "logs"
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>lightning_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>LIGHTNING_HOME</key>
        <string>{lightning_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Lightning install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  lightning gateway status             # Check status")
    from lightning_constants import display_lightning_home as _dhh
...
def launchd_stop():
    ...
    # `lightning gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Lightning install")
    else:
        print("⚠ Service definition is stale relative to the current Lightning install")
        print("  Run: lightning gateway start")
    ...
    if deep:
        log_file = get_lightning_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/lightning"
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("LIGHTNING_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Lightning gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'lightning' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/lightning/docker/entrypoint.sh before the Lightning command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$LIGHTNING_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set LIGHTNING_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `lightning gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set LIGHTNING_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `lightning gateway restart` which already
    ...
    # `lightning gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Lightning Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # LIGHTNING_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("LIGHTNING_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Lightning will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @lightning:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. lightning) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Lightning delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Lightning agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Lightning will use (e.g., lightning@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Lightning connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: lightning pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Lightning will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``lightning setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run lightning under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.lightning/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_lightning_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'lightning pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: lightning pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'lightning gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from lightning_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from lightning_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from lightning_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Lightning will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Lightning will store the returned account_id/token in ~/.lightning/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `lightning gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_lightning_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `lightning pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `lightning pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"LightningAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``lightning_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from lightning_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.lightning/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.lightning/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: lightning gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: lightning gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: lightning gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo lightning gateway install --system")
                    print_info("  Or run in foreground:  lightning gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: lightning gateway run")
                print_info("  For persistence:   tmux new -s lightning 'lightning gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from lightning_constants import display_lightning_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: lightning gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup lightning gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: lightning gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'lightning gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``lightning gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: lightning gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: lightning gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s lightning 'lightning gateway run'")
                print()
            ...
            print()
            print("  lightning gateway run                              # direct foreground")
            print("  tmux new -s lightning 'lightning gateway run'         # persistent via tmux")
            print("  nohup lightning gateway run > ~/.lightning/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: lightning gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: lightning gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: lightning gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: lightning gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    lightning gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: lightning gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  lightning gateway install")
                else:
                    print("To install as a service:")
                    print("  lightning gateway install")
                    print("  sudo lightning gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  lightning gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup lightning gateway run > ~/.lightning/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s lightning 'lightning gateway run'         # persistent via tmux")
                    print("  nohup lightning gateway run > ~/.lightning/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  lightning gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  lightning gateway install  # Install as user service")
                    print("  sudo lightning gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Lightning gateway unit files from
        # pre-rename installs (e.g. lightning.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_lightning_units(interactive=not yes, dry_run=dry_run)
