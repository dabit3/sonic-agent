"""
Dump command for lightning CLI.

Outputs a compact, plain-text summary of the user's Lightning setup
...
from lightning_cli.config import get_lightning_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``lightning dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from lightning_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(lightning_home: Path) -> int:
...
def _cron_summary(lightning_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = lightning_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_lightning_dotenv(
        lightning_home=env_path.parent,
    ...
    lines.append(f"lightning_home:      {display_lightning_home()}")
    ...
    toolsets = config.get("toolsets", ["lightning-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(lightning_home)}")
    lines.append(f"  skills:             {_count_skills(lightning_home)}")
