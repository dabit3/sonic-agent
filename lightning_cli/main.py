#!/usr/bin/env python3
"""
Lightning CLI - Main entry point.

Usage:
    lightning                     # Interactive chat (default)
    lightning chat                # Interactive chat
    lightning gateway             # Run gateway in foreground
    lightning gateway start       # Start gateway as service
    lightning gateway stop        # Stop gateway service
    lightning gateway status      # Show gateway status
    lightning gateway install     # Install gateway service
    lightning gateway uninstall   # Uninstall gateway service
    lightning setup               # Interactive setup wizard
    lightning logout              # Clear stored authentication
    lightning status              # Show status of all components
    lightning cron                # Manage cron jobs
    lightning cron list           # List cron jobs
    lightning cron status         # Check if cron scheduler is running
    lightning doctor              # Check configuration and dependencies
    lightning honcho setup                    # Configure Honcho AI memory integration
    lightning honcho status                   # Show Honcho config and connection status
    lightning honcho sessions                 # List directory → session name mappings
    lightning honcho map <name>               # Map current directory to a session name
    lightning honcho peer                     # Show peer names and dialectic settings
    lightning honcho peer --user NAME         # Set user peer name
    lightning honcho peer --ai NAME           # Set AI peer name
    lightning honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    lightning honcho mode                     # Show current memory mode
    lightning honcho mode [hybrid|honcho|local]  # Set memory mode
    lightning honcho tokens                   # Show token budget settings
    lightning honcho tokens --context N       # Set session.context() token cap
    lightning honcho tokens --dialectic N     # Set dialectic result char cap
    lightning honcho identity                 # Show AI peer identity representation
    lightning honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    lightning honcho migrate                  # Step-by-step migration guide: OpenClaw native → Lightning + Honcho
    lightning version             Show version
    lightning update              Update to latest version
    lightning uninstall           Uninstall Lightning Agent
    lightning acp                 Run as an ACP server for editor integration
    lightning sessions browse     Interactive session picker with search

    lightning claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: lightning_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``lightning_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``lightning update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``lightning_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, lightning crashes on import and the user can't run
# ``lightning update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _print_fast_version_info() -> None:
    from lightning_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Lightning Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``lightning --version`` before config/logging imports on Termux."""
    if os.environ.get("LIGHTNING_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to LIGHTNING_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (lightning tools, lightning setup, lightning model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'lightning {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any lightning module import.
#
# Many modules cache LIGHTNING_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("LIGHTNING_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.lightning/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set LIGHTNING_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors lightning_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If LIGHTNING_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.lightning/profiles/coder or
    # /opt/data/profiles/coder).  If LIGHTNING_HOME points to the lightning root
    # instead (e.g. systemd hardcodes LIGHTNING_HOME=/root/.lightning), we must
    # still read active_profile — the user may have switched profiles via
    # `lightning profile use` and the gateway should honour that choice.
    # See issue #22502.
    lightning_home_env = os.environ.get("LIGHTNING_HOME", "")
    if profile_name is None and lightning_home_env:
        if Path(lightning_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the lightning root
    if profile_name is None:
        try:
            from lightning_constants import get_default_lightning_root

            active_path = get_default_lightning_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set LIGHTNING_HOME
    if profile_name is not None:
        try:
            from lightning_cli.profiles import resolve_profile_env

            lightning_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent lightning from starting
            print(
...
# Load .env from ~/.lightning/.env first, then project root as dev fallback.
...
load_lightning_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → LIGHTNING_REDACT_SECRETS env
# var BEFORE lightning_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "LIGHTNING_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_lightning_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["LIGHTNING_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `lightning` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from lightning_logging import setup_logging as _setup_logging
...
try:
    from lightning_cli.config import load_config as _load_config_early
    from lightning_constants import apply_ipv4_preference as _apply_ipv4
...
from lightning_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_lightning_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("LIGHTNING_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("LIGHTNING_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from lightning_cli.config import get_env_path, get_lightning_home, load_config
    from lightning_cli.auth import get_auth_status

    # Determine whether Lightning itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_lightning_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_lightning_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Lightning has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Lightning to use their tokens.
    if _has_lightning_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, lightning_bin
        cli_args: the original CLI arguments (everything after 'lightning')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    lightning_bin = container_info["lightning_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo lightning {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo lightning {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  lightning --tui --resume {target}")
    if title:
        print(f'  lightning --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @lightning/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@lightning" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/lightning-ink/package.json",
    "packages/lightning-ink/package-lock.json",
    "packages/lightning-ink/index.js",
    "packages/lightning-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``LIGHTNING_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("LIGHTNING_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``LIGHTNING_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("LIGHTNING_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "LIGHTNING_HOME": lightning_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(lightning_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(lightning_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if lightning_cli_dir is None:
        lightning_cli_dir = Path(__file__).parent
    bundled = lightning_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (LIGHTNING_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("LIGHTNING_NODE")
    ...
    ext_dir = os.environ.get("LIGHTNING_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with LIGHTNING_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset LIGHTNING_TUI_DIR (e.g. `unset LIGHTNING_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("LIGHTNING_QUIET"):
    ...
    if tui_dev:
        # Keep the local @lightning/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @lightning/ink resolves through
        # packages/lightning-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "lightning-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from lightning_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="lightning-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["LIGHTNING_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["LIGHTNING_PYTHON_SRC_ROOT"] = os.environ.get(
        "LIGHTNING_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("LIGHTNING_PYTHON", sys.executable)
    env.setdefault("LIGHTNING_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["LIGHTNING_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["LIGHTNING_MODEL"] = model
        env["LIGHTNING_INFERENCE_MODEL"] = model
    if provider:
        env["LIGHTNING_TUI_PROVIDER"] = provider
        env["LIGHTNING_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["LIGHTNING_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["LIGHTNING_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["LIGHTNING_TUI_SKILLS"] = value
    if query:
        env["LIGHTNING_TUI_QUERY"] = query
    if image:
        env["LIGHTNING_TUI_IMAGE"] = image
    if checkpoints:
        env["LIGHTNING_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["LIGHTNING_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["LIGHTNING_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["LIGHTNING_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["LIGHTNING_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["LIGHTNING_ACCEPT_HOOKS"] = "1"
    ...
    # LIGHTNING_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `lightning --tui` try to
    ...
    # still set LIGHTNING_TUI_RESUME themselves.
    env.pop("LIGHTNING_TUI_RESUME", None)
    if resume_session_id:
        env["LIGHTNING_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `lightning update` so
    ...
    if code == 42:
        from lightning_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``LIGHTNING_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``lightning kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``lightning kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("LIGHTNING_KANBAN_BOARD"):
        return
    try:
        from lightning_cli.kanban_db import get_current_board

        os.environ["LIGHTNING_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("LIGHTNING_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'lightning sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from lightning_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from lightning_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'lightning doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Lightning isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  lightning setup")
        print()

        from lightning_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'lightning setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from lightning_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["LIGHTNING_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.lightning/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["LIGHTNING_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["LIGHTNING_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["LIGHTNING_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from lightning_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from lightning_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Lightning?")
        print()
    ...
    # has no creds.json.  Every subsequent `lightning gateway` then paid a 30s
    ...
    session_dir = get_lightning_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: lightning gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `lightning gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  lightning gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Lightning Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  lightning gateway")
        ...
        print()
        print("  Or install as a service: lightning gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'lightning whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from lightning_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from lightning_cli.config import stamp_install_method
    from lightning_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Lightning post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``lightning model``) and the setup wizard
    ...
    from lightning_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from lightning_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("LIGHTNING_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'lightning model' for "
                "available providers, or run 'lightning doctor' to diagnose config "
                "issues."
            )
    ...
    from lightning_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.lightning/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.lightning/.env if the active provider is not 'custom'.
    ...
    """
    from lightning_cli.config import get_env_value, save_env_value, load_config
...
#
# Lightning uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `lightning model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `lightning model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from lightning_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Lightning only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``lightning model`` flow, then route aux tasks to them here.
    """
    from lightning_cli.config import load_config
    from lightning_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from lightning_cli.auth import _prompt_model_selection
    from lightning_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from lightning_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from lightning_constants import OPENROUTER_BASE_URL
    from lightning_cli.auth import (
        ProviderConfig,
    ...
    from lightning_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.lightning/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `lightning setup` from scratch. OpenRouter
    ...
    from lightning_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from lightning_constants import AI_GATEWAY_BASE_URL
    from lightning_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.lightning/.env by hand.
    ...
    from lightning_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from lightning_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from lightning_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from lightning_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from lightning_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `lightning auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from lightning_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``lightning model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``lightning model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``lightning auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from lightning_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.lightning/config.yaml.
    """
    from lightning_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from lightning_cli.auth import _save_model_choice, deactivate_provider
    from lightning_cli.config import get_env_value, load_config, save_config
    ...
    from lightning_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"Lightning will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `lightning model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from lightning_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Lightning URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from lightning_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from lightning_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from lightning_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Lightning will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Lightning will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from lightning_cli.curses_ui import flush_stdin
...
# it to the model-selection handlers so plain `lightning --tui` does not pay for
...
def _model_flow_copilot(config, current_model=""):
    ...
    from lightning_cli.config import save_env_value, load_config, save_config
    from lightning_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from lightning_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from lightning_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from lightning_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Lightning turns to `copilot --acp`.")
    print("  Lightning currently starts its own ACP subprocess for each request.")
    print("  Lightning uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set LIGHTNING_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``lightning setup`` / ``lightning model``.
    ...
    recover from a malformed paste without editing ``~/.lightning/.env`` by hand.
    ...
    from lightning_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `lightning setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from lightning_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so lightning knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Lightning typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Lightning, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from lightning_cli.auth import AuthError
        from lightning_cli.models import fetch_lmstudio_models
        ...
        from lightning_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from lightning_constants import display_lightning_home as _dhh_fn

            print(
                f"    Lightning will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               lightning model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from lightning_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'lightning model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Lightning CLI with a provider."""
    from lightning_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from lightning_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from lightning_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from lightning_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from lightning_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from lightning_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from lightning_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    Dispatches ``lightning slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: lightning slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `lightning slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from lightning_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from lightning_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from lightning_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from lightning_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from lightning_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from lightning_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from lightning_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Lightning home directory to a zip file."""
    if getattr(args, "quick", False):
        from lightning_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from lightning_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Lightning backup from a zip file."""
    from lightning_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from lightning_cli.banner import check_for_updates
        from lightning_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Lightning Agent."""
    _require_tty("uninstall")
    from lightning_cli.uninstall import run_uninstall
...
# Critical files that every ``lightning`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``lightning update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "lightning_cli/main.py",
    "lightning_cli/config.py",
    "lightning_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "lightning_constants.py",
)
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``lightning`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="lightning-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``lightning update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``lightning_cli/web_dist/`` (per vite.config.ts
    outDir: "../lightning_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "lightning_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``lightning update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``lightning web``).
    ...
    # build path usable on a stock `py -m lightning_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (lightning web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``lightning dashboard`` processes other than ourselves.

    ``lightning dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``lightning update`` replaces files on disk, the running
    ...
    patterns = [
        "lightning dashboard",
        "lightning_cli.main dashboard",
        "lightning_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "lightning.*dashboard"`) keeps us consistent
            # with `lightning_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `lightning update`.
    ...
    print("  Preview now:  lightning curator run --dry-run")
    print("  Pause it:     lightning curator pause")
    print(
        "  Docs:         https://lightning-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``lightning update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``lightning update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: lightning curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``lightning dashboard`` processes.

    Called at the end of ``lightning update`` (default ``reason``) and also
    from ``lightning dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    lightning dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Lightning Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/lightning-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="lightning-update-")
        zip_path = os.path.join(tmp_dir, f"lightning-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to lightning-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"lightning-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "lightning-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Lightning behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes lightning completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Lightning couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Lightning couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Lightning behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `lightning update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/lightning-agent.git",
    "git@github.com:NousResearch/lightning-agent.git",
    "https://github.com/NousResearch/lightning-agent",
    "git@github.com:NousResearch/lightning-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/lightning-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_lightning_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_lightning_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Lightning repository.")
        print("  This means you may miss updates from NousResearch/lightning-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/lightning-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/lightning-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``lightning update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from lightning_constants import get_default_lightning_root

    default_home = get_default_lightning_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``lightning update`` is still progressing even if pip/uv itself is silent.
    """
...
def _lightning_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "lightning.exe",
        scripts_dir / "lightning-gateway.exe",
    ]


def _detect_concurrent_lightning_instances(
    ...
    The Lightning Desktop Electron app spawns ``lightning.EXE`` as a backend child,
    so during ``lightning update`` the user-invoked process and the desktop's
    ...
    shims (``lightning.exe`` / ``lightning-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID is excluded so the running
    ``lightning update`` invocation never reports itself.
    ...
    for shim in _lightning_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "lightning.exe"
    lines = ["✗ Another lightning.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Lightning Desktop, exit any open `lightning` REPLs, and")
    lines.append("  stop the gateway (`lightning gateway stop`) before retrying.")
    lines.append("  Override with `lightning update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_lightning_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``lightning.exe``.
    ...
    when ``lightning update`` runs, ``lightning.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``lightning.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next lightning invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Lightning Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Lightning Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _lightning_exe_shims(scripts_dir):
        ...
        print(
            "    Close Lightning Desktop, exit other `lightning` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `lightning update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_lightning_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``lightning.exe.old.*`` left by prior updates.

    Called early on every lightning invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``lightning update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `lightning update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``lightning.exe`` / ``lightning-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_lightning_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_lightning_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.lightning/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``lightning update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.lightning/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``lightning update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``lightning update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.lightning/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``lightning update --gateway``) the update is already
    ...
    try:
        ...
        # lightning_cli.config.get_lightning_home to simulate setup failure.
        from lightning_cli.config import get_lightning_home as _get_lightning_home

        logs_dir = _get_lightning_home() / "logs"
...
def _cmd_update_check():
    """Implement ``lightning update --check``: fetch and report without installing."""
    from lightning_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from lightning_cli.config import recommended_update_command
        from lightning_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``lightning update`` without requiring a reinstall.  The
    ...
    ``lightning`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/lightning.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v lightning'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/lightning, code at /usr/local/lib/lightning-agent).
    fhs_link = Path("/usr/local/bin/lightning")
    ...
    path_comment = (
        "# Lightning Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of LIGHTNING_HOME before running the update.
    ...
    LIGHTNING_HOME directories.  The ``--backup`` flag on ``lightning update``
    ...
    try:
        from lightning_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_lightning_home so the user sees ~/.lightning/...
    try:
        from lightning_constants import get_lightning_home, display_lightning_home

        home = get_lightning_home()
        try:
            display_path = f"{display_lightning_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  lightning import {out_path}")
...
def cmd_update(args):
    """Update Lightning Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from lightning_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Lightning Agent")
        return
...
def _cmd_update_pip(args):
    """Update Lightning via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "lightning-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "lightning-agent"]
    ...
    print("✓ Update complete! Restart lightning to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Lightning Agent...")
    print()

    # On Windows, abort early if another lightning.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_lightning_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in lightning_cli/config.py bricked
        # every user who ran ``lightning update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``lightning
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``lightning update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_lightning_home added to lightning_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload lightning_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_lightning_home() added since the last release.
        try:
            import importlib
            import lightning_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit LIGHTNING_HOME so
        # it is not affected by sync_skills()'s module-level LIGHTNING_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's LIGHTNING_HOME env var points at the default or a named profile.
        try:
            from lightning_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: lightning config migrate")
            else:
                print()
                print("Skipped. Run 'lightning config migrate' later to configure.")
        else:
        ...
        # check `lightning curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``lightning update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from lightning_cli.tools_config import install_cua_driver
        ...
        # When running as ``lightning update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_lightning_home() / ".update_exit_code"
            try:
        ...
        try:
            from lightning_cli.gateway import (
                is_macos,
            ...
            try:
                from lightning_constants import (
            ...
            # Discover all lightning-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. lightning-gateway.service or lightning-gateway-coder.service
                            ...
                            # path in `lightning gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: lightning gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: lightning -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Lightning gateway unit files are still installed.
        # When both lightning.service (from a pre-rename install) and the
        # current lightning-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `lightning update` surfaces the issue until the user migrates.
        try:
            from lightning_cli.gateway import (
                has_legacy_lightning_units,
                _find_legacy_lightning_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_lightning_units():
                print()
                print("⚠ Legacy Lightning gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_lightning_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (lightning.service) fight the current")
                print("  lightning-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    lightning gateway migrate-legacy")
                print()
        ...
        print("  lightning model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``lightning -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `lightning profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_lightning_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → lightning -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.lightning)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  lightning profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     lightning -p {name} chat")
                else:
        ...
        from lightning_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from lightning_constants import get_lightning_home as _hh
        ...
        from lightning_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `lightning profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from lightning_cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec lightning -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from lightning_cli.profiles import rename_profile
        ...
        from lightning_cli.profiles import export_profile
        ...
        from lightning_cli.profiles import import_profile
        ...
        import tempfile
        from lightning_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="lightning_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  lightning -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      lightning -p {plan.manifest.name} chat")
        ...
        from lightning_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `lightning profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"lightning -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from lightning_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("lightning_requires"):
            print(f"Requires:     Lightning {data['lightning_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from lightning_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.lightning_requires:
        print(f"  Requires: Lightning {mf.lightning_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``lightning dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``lightning dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No lightning dashboard processes running.")
        return 0

    print(f"{len(pids)} lightning dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No lightning dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `lightning update`.
    ...
    if "LIGHTNING_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["LIGHTNING_WEB_DIST"])
            if "LIGHTNING_WEB_DIST" in os.environ
            else PROJECT_ROOT / "lightning_cli" / "web_dist"
        )
    ...
    from lightning_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("LIGHTNING_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from lightning_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Lightning log files."""
    from lightning_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from lightning_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``lightning -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``lightning_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``lightning -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``lightning`` or only flags → defaults to ``chat``.
        return False
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from lightning_cli.plugins import discover_plugins
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("LIGHTNING_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from lightning_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from lightning_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["LIGHTNING_DEFER_AGENT_STARTUP"] = "1"
            os.environ["LIGHTNING_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["LIGHTNING_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `lightning --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("LIGHTNING_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("LIGHTNING_TUI") == "1"):
        return False
...
def main():
    ...
    try:
        from lightning_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``lightning.exe.old.*`` quarantine files left by previous
    # ``lightning update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_lightning_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: lightning-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from lightning_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://lightning-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `lightning model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.lightning/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://lightning-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from lightning_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from lightning_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy lightning.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Lightning gateway unit files "
            "(e.g. lightning.service) left over from older installs. Profile "
            "units (lightning-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `lightning proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Lightning Agent with an interactive wizard. "
        "Run a specific section: lightning setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'lightning setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Lightning.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $LIGHTNING_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Lightning")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from lightning_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Lightning CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: lightning-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Lightning with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set LIGHTNING_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Lightning Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.lightning/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Lightning profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.lightning/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Lightning profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from lightning_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from lightning_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.lightning/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.lightning/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Lightning Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `lightning "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Lightning setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Lightning Agent. Use 'lightning debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    lightning debug share              Upload debug report and print URL
    lightning debug share --lines 500  Include more log lines
    lightning debug share --expire 30  Keep paste for 30 days
    lightning debug share --local      Print report locally (no upload)
    lightning debug share --no-redact  Disable upload-time secret redaction
    lightning debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'lightning debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Lightning home directory to a zip file",
        description="Create a zip archive of your entire Lightning configuration, "
        "skills, sessions, and data (excludes the lightning-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/lightning-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.lightning/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo lightning uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from lightning_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Lightning backup from a zip file",
        description="Extract a previously created Lightning backup into your "
        "Lightning home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Lightning Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from lightning_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.lightning/skills/.bundled_manifest) "
            "so future 'lightning update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from lightning_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from lightning_cli.skills_hub import skills_command
    ...
    from lightning_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/lightning-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `lightning plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from lightning_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``lightning --help``, ``lightning version``, ``lightning logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from lightning_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from lightning_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_lightning_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_lightning_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_lightning_home()}/memories/\n")
        else:
            from lightning_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'lightning tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # lightning tools list [--platform cli]
    ...
    # lightning tools disable <name...> [--platform cli]
    ...
    # lightning tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from lightning_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from lightning_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `lightning computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `lightning tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: lightning computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: lightning computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Lightning as an MCP server",
        description=(
            "Manage MCP server connections and run Lightning as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'lightning mcp add' to connect to a new server, or\n"
            "'lightning mcp serve' to expose Lightning conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Lightning as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `lightning mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from lightning_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_lightning_home() / "sessions"
            ...
            # Launch lightning --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Lightning",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Lightning",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.lightning/ (by default a "
        "single restore-point archive is written to ~/.lightning/backups/ "
        "before apply; restorable with 'lightning import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from lightning_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Lightning Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'lightning config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--force",
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another lightning.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Lightning Agent",
        description="Remove Lightning Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Lightning Agent as an ACP (Agent Client Protocol) server",
        description="Start Lightning Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Lightning ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    def cmd_acp(args):
        """Launch Lightning Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Lightning instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `lightning update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `lightning profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Lightning profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Lightning Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `lightning --tui` via PTY/WebSocket). "
            "Alternatively set LIGHTNING_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `lightning dashboard` cmdlines and SIGTERM them directly — the same
    # path `lightning update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running lightning dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running lightning dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Lightning log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    lightning logs                    Show last 50 lines of agent.log
    lightning logs -f                 Follow agent.log in real time
    lightning logs errors             Show last 50 lines of errors.log
    lightning logs gateway -n 100     Show last 100 lines of gateway.log
    lightning logs --level WARNING    Only show WARNING and above
    lightning logs --session abc123   Filter by session ID
    lightning logs --component tools  Only show tool-related lines
    lightning logs --since 1h         Lines from the last hour
    lightning logs --since 30m -f     Follow, starting from 30 min ago
    lightning logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``lightning -c Pokemon Agent Dev`` → ``lightning -c 'Pokemon Agent Dev'``
    ...
    from lightning_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'lightning -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (lightning hooks list, cron
