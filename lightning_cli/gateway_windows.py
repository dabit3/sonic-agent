...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m lightning_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Lightning_Gateway"
_TASK_DESCRIPTION = "Lightning Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Lightning profile."""
    from lightning_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "lightning_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("LIGHTNING_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("LIGHTNING_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("LIGHTNING_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["LIGHTNING_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["LIGHTNING_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["LIGHTNING_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("LIGHTNING_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("LIGHTNING_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("LIGHTNING_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Lightning_Gateway``
    Named profile X: ``Lightning_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during lightning_cli boot.
    from lightning_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\lightning\\gateway-service\\<task_name>.cmd``
    (or ``<LIGHTNING_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Lightning installs stay self-contained).
    """
    _assert_windows()
    from lightning_cli.config import get_lightning_home

    script_dir = Path(get_lightning_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports LIGHTNING_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m lightning_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "LIGHTNING_HOME={lightning_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "LIGHTNING_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports lightning_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "lightning_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    lightning_home = str(Path(get_lightning_home()).resolve())
    profile_arg = _profile_arg(lightning_home)

    content = _build_gateway_cmd_script(python_path, working_dir, lightning_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Lightning builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "lightning_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m lightning_cli.main gateway run``
    ...
    log_dir = Path(get_lightning_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("LIGHTNING_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("LIGHTNING_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from lightning_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `lightning -p <profile> gateway install` must be safe.
    ...
    from lightning_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"lightning {profile_arg} gateway start" if profile_arg else "lightning gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: lightning gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Lightning gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: lightning gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: lightning gateway start")
        _print_next_steps()
        return
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from lightning_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from lightning_cli.config import get_lightning_home
        print(f"    type {Path(get_lightning_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_lightning_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from lightning_cli.config import get_lightning_home

    lightning_home = Path(get_lightning_home()).resolve()
    print()
    print("Next steps:")
    print("  lightning gateway status                      # Check status")
    print(f"  type {lightning_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Lightning gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: lightning gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: lightning gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: lightning gateway start")
            return
...
def stop() -> None:
    ...
    _assert_windows()
    from lightning_cli.gateway import kill_gateway_processes
