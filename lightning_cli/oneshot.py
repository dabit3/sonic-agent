...
configured for "cli" in `lightning tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (LIGHTNING_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `lightning chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    ...
    - LIGHTNING_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"lightning -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "lightning -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"lightning -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "lightning -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "lightning -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to LIGHTNING_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            LIGHTNING_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("LIGHTNING_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "lightning -z: --provider requires --model (or LIGHTNING_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``lightning -z`` / oneshot mode.

    Oneshot bypasses ``LightningCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when lightning is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from lightning_cli.config import load_config
    from lightning_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("LIGHTNING_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from lightning_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("LIGHTNING_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                LIGHTNING_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via LIGHTNING_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via LIGHTNING_YOLO_MODE=1
