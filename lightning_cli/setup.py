"""
Interactive setup wizard for Lightning Agent.
...
from lightning_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://lightning-agent.nousresearch.com/docs"
...
# display_lightning_home imported lazily at call sites (stale-module safety during lightning update)
...
from lightning_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Lightning Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Lightning using environment variables or config commands:")
    print_info("  lightning config set model.provider custom")
    print_info("  lightning config set model.base_url http://localhost:8080/v1")
    print_info("  lightning config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'lightning setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from lightning_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from lightning_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'lightning setup')")


def _print_setup_summary(config: dict, lightning_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'lightning setup' to configure"))
    ...
    # Video generation — opt-in via `lightning tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from lightning_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'lightning setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'lightning setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'lightning setup terminal'"))
    ...
    # Spotify (OAuth via lightning auth spotify — check auth.json, not env vars)
    try:
        from lightning_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'lightning setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {lightning_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('lightning setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('lightning setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('lightning setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('lightning setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('lightning setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('lightning config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('lightning config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('lightning config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('lightning', Colors.GREEN)}              Start chatting")
    print(f"   {color('lightning gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('lightning doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `lightning tools` and `lightning setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``lightning model``)
    ...
    provider added to ``lightning model`` is automatically available here.
    ...
    # Delegate to the shared lightning model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from lightning_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: lightning model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from lightning_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Lightning can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Lightning can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'lightning setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``lightning model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from lightning_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.lightning/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Lightning auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via lightning setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'lightning setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Lightning runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'lightning-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'lightning-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into LIGHTNING_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("LIGHTNING_MAX_ITERATIONS")
    ...
    print_info("  Run `lightning setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # LIGHTNING_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Lightning delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `lightning update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under LIGHTNING_HOME, and print
    ...
    """
    try:
        from lightning_cli.slack_cli import _build_full_manifest
        from lightning_constants import get_lightning_home

        manifest = _build_full_manifest(
            bot_name="Lightning",
            bot_description="Your Lightning agent on Slack",
        )
        target = Path(get_lightning_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `lightning slack manifest --write` anytime to refresh after "
            "Lightning adds new commands."
        )
        ...
        print_info(
            ...
            "lightning slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Lightning delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Lightning delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  lightning config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Lightning to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from lightning_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://lightning-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://lightning-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  lightning config edit")
    print_info("   Open config in your editor:  lightning config edit")
...
def setup_gateway(config: dict):
    ...
    from lightning_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Lightning from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'lightning setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     lightning config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_lightning_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  lightning gateway")
        else:
            from lightning_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   lightning gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   lightning gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `lightning setup tools` and `lightning tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``lightning_cli.auth`` — lists every supported
    ...
    """
    try:
        from lightning_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from lightning_cli.config
    ...
    if section_key == "model":
        ...
        from lightning_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_lightning.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_lightning migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_lightning", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Lightning.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Lightning to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Lightning at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Lightning at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Lightning at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Lightning at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Lightning equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Lightning config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Lightning.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Lightning's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(lightning_home: Path) -> bool:
    ...
    print_info("Lightning can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: lightning claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=lightning_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: lightning claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Lightning configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Lightning config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Lightning (use lightning claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``lightning setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``lightning setup --portal``)
    that gets a brand-new user from zero to a fully working Lightning session
    ...
    from lightning_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ Lightning Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `lightning setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `lightning auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `lightning auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `lightning model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `lightning portal status` to inspect routing.")
    print_info("  Run `lightning` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      lightning setup           — full or quick (auto-detected)
      lightning setup model     — just model/provider
      lightning setup tts       — just text-to-speech
      lightning setup terminal  — just terminal backend
      lightning setup gateway   — just messaging platforms
      lightning setup tools     — just tool configuration
      lightning setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Lightning Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Lightning Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Lightning Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, lightning_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Lightning configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'lightning setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(lightning_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Lightning?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, lightning_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {lightning_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'lightning config edit'")
    ...
    _print_setup_summary(config, lightning_home)


def _run_first_time_quick_setup(config: dict, lightning_home, is_existing: bool):
    ...
    the user can customize later via ``lightning setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'lightning setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    lightning setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  lightning setup gateway")
    print()

    _print_setup_summary(config, lightning_home)


def _run_quick_setup(config: dict, lightning_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'lightning setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Lightning to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'lightning setup gateway'.")
