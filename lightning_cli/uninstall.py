"""
Lightning Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.lightning/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Lightning PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing lightning-agent or lightning PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Lightning Agent" comment and following line
                if '# Lightning Agent' in line or '# lightning-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('lightning' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing lightning
                if 'lightning' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the lightning wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "lightning",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains lightning_cli reference)
                content = wrapper.read_text()
                if 'lightning_cli' in content or 'lightning-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``lightning gateway run`` processes
    ...
    try:
        from lightning_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from lightning_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``LIGHTNING_HOME`` and ``LIGHTNING_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\lightning\git\cmd``, ``%LOCALAPPDATA%\lightning\git\bin``,
#      ``%LOCALAPPDATA%\lightning\git\usr\bin``, ``%LOCALAPPDATA%\lightning\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\lightning\git\`` and Node to
#      ``%LOCALAPPDATA%\lightning\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``lightning dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\lightning\gateway-service\`` and sometimes
...
def _lightning_path_markers(lightning_home: Path) -> list[str]:
    """Path-entry substrings that identify Lightning-owned User-PATH entries."""
    root = str(lightning_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare lightning-agent install dir.
    markers = [root + "\\lightning-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if LIGHTNING_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "lightning".  We don't want to catch
    # unrelated entries like "clightning-foo" or "ephermeral", so we look for
    # backslash-lightning as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(lightning_home: Path) -> list[str]:
    """Strip Lightning-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _lightning_path_markers(lightning_home)
            kept: list[str] = []
...
def remove_lightning_env_vars_windows() -> list[str]:
    """Delete LIGHTNING_HOME and LIGHTNING_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("LIGHTNING_HOME", "LIGHTNING_GIT_BASH_PATH"):
                try:
...
def _is_default_lightning_home(lightning_home: Path) -> bool:
    """Return True when ``lightning_home`` points at the default (non-profile) root."""
    try:
        from lightning_constants import get_default_lightning_root
        return lightning_home.resolve() == get_default_lightning_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its LIGHTNING_HOME directory.

    We shell out to ``lightning -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current LIGHTNING_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m lightning_cli.main` so we don't depend on a `lightning`
    #    wrapper that may be half-removed mid-uninstall.
    lightning_invocation = [_sys.executable, "-m", "lightning_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                lightning_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's LIGHTNING_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.lightning/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.lightning/ for future reinstall
    """
    project_root = get_project_root()
    lightning_home = get_lightning_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie LIGHTNING_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_lightning_home(lightning_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Lightning Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {lightning_home / 'config.yaml'}")
    print(f"  Secrets: {lightning_home / '.env'}")
    print(f"  Data:    {lightning_home / 'cron/'}, {lightning_home / 'sessions/'}, {lightning_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their LIGHTNING_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Lightning data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Lightning code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in lightning_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\lightning\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(lightning_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Lightning-owned PATH entries in User environment")

        log_info("Removing LIGHTNING_HOME / LIGHTNING_GIT_BASH_PATH User env vars...")
        removed_env = remove_lightning_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Lightning-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing lightning command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.lightning/, just remove the lightning-agent subdir
            if lightning_home in project_root.parents or project_root.parent == lightning_home:
    ...
    #     under LIGHTNING_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(lightning_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(lightning_home)
    ...
    # 5. Optionally remove ~/.lightning/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile LIGHTNING_HOME dirs live under
        ...
        try:
            if lightning_home.exists():
                shutil.rmtree(lightning_home)
                log_success(f"Removed {lightning_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {lightning_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {lightning_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {lightning_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/lightning-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/lightning-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Lightning Agent! ⚕")
    print()
