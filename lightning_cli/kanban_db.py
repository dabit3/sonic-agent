...
``<root>`` is the **shared Lightning root** (the parent of any active
...
``lightning -p <profile>`` joins the same board as the dispatcher that
...
the profile's Lightning home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``LIGHTNING_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``LIGHTNING_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``lightning kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.lightning``. In Docker / custom
deployments where ``LIGHTNING_HOME`` points outside ``~/.lightning`` (e.g.
``/opt/lightning``), ``<root>`` is ``LIGHTNING_HOME``. Legacy env-var
overrides still work:

* ``LIGHTNING_KANBAN_DB`` — pin the database file path directly.
* ``LIGHTNING_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``LIGHTNING_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``LIGHTNING_KANBAN_DB``,
``LIGHTNING_KANBAN_WORKSPACES_ROOT``, and ``LIGHTNING_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/lightning-kanban-v1-spec.pdf`` for the full
...
# ``LIGHTNING_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``LIGHTNING_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("LIGHTNING_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``lightning-agent``
...
def kanban_home() -> Path:
    """Return the shared Lightning root that anchors the kanban board.

    Resolution order:

    1. ``LIGHTNING_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_lightning_root()``, which already returns ``<root>``
       when ``LIGHTNING_HOME`` is ``<root>/profiles/<name>``, and returns
       ``LIGHTNING_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``LIGHTNING_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("LIGHTNING_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``lightning kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``LIGHTNING_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``lightning kanban boards
    ...
    """
    env = os.environ.get("LIGHTNING_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``lightning kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``LIGHTNING_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("LIGHTNING_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``LIGHTNING_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("LIGHTNING_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``lightning kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``LIGHTNING_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``LIGHTNING_SESSION_ID``. NULL
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Lightning treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``LIGHTNING_KANBAN_DB`` env → ``LIGHTNING_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See lightning_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``lightning kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``LIGHTNING_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from lightning_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``lightning --skills ...`` alongside the built-in
    ...
    # `lightning --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``lightning kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `lightning kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``lightning kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``lightning kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``lightning kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``lightning kanban complete <id>``) works without requiring
...
#      via ``lightning kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Lightning
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``lightning kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Lightning profile.
    ...
    if not rows:
        return False
    try:
        from lightning_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Lightning profile.
        # `_default_spawn` invokes ``lightning -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Lightning
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `lightning kanban tail` — without
...
def _module_lightning_argv() -> list[str]:
    """Return the interpreter-bound Lightning CLI invocation."""
    # ``lightning_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``lightning`` package — there is no
    # ``lightning`` package to import.
    return [sys.executable, "-m", "lightning_cli.main"]


def _absolute_lightning_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Lightning shim."""
...
def _lightning_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Lightning executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_lightning_argv()
    return [_absolute_lightning_path(path)]


def _resolve_lightning_argv() -> list[str]:
    """Resolve the ``lightning`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$LIGHTNING_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("lightning")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\lightning.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m lightning_cli.main`` — fallback for setups where
       Lightning is launched from a venv and the ``lightning`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_lightning_bin`` for the same reason. Kept
    local (not imported from gateway) because ``lightning_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("LIGHTNING_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _lightning_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _lightning_path_argv(resolved_env_bin)
        return _module_lightning_argv()

    lightning_bin = _safe_which_no_cwd("lightning") if _IS_WINDOWS else shutil.which("lightning")
    if lightning_bin:
        return _lightning_path_argv(lightning_bin)
    return _module_lightning_argv()


def _kanban_worker_skill_available(lightning_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``lightning -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset LIGHTNING_HOME means the worker falls back to the default root
    # home (``~/.lightning``), which ships the bundled skill.
    base = _Path(lightning_home) if lightning_home else (_Path.home() / ".lightning")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``lightning -p <profile> chat -q ...`` subprocess.
    ...
    ``LIGHTNING_KANBAN_DB`` / ``LIGHTNING_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject LIGHTNING_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `lightning -p <name>` the
    # _apply_profile_override() runs *before* lightning_constants is imported.
    # If LIGHTNING_HOME is absent from the child's env, get_lightning_home() falls
    # back to Path.home() / ".lightning" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from lightning_cli.profiles import resolve_profile_env
    try:
        env["LIGHTNING_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via LIGHTNING_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # LIGHTNING_HOME never had profiles created.
        pass
    if task.tenant:
        env["LIGHTNING_TENANT"] = task.tenant
    env["LIGHTNING_KANBAN_TASK"] = task.id
    env["LIGHTNING_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["LIGHTNING_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["LIGHTNING_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["LIGHTNING_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`lightning -p <name>`
    # rewrites LIGHTNING_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_lightning_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["LIGHTNING_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["LIGHTNING_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["LIGHTNING_KANBAN_BOARD"] = resolved_board
    # LIGHTNING_PROFILE is the author the kanban_comment tool defaults to.
    # `lightning -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["LIGHTNING_PROFILE"] = profile_arg

    cmd = [
        *_resolve_lightning_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped LIGHTNING_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("LIGHTNING_HOME")):
    ...
    # `lightning kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`lightning` executable not found on PATH. "
            "Install Lightning Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``lightning kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled LIGHTNING_PROFILE values like "lightning-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Lightning root exists

    Reads profile paths directly so this module has no import dependency on
    ``lightning_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from lightning_constants import get_default_lightning_root
        default_root = get_default_lightning_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``lightning kanban assignees`` for the terminal.
