"""
Profile management for multiple isolated Lightning instances.

Each profile is a fully independent LIGHTNING_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.lightning/profiles/<name>/`` by default.

The "default" profile is ``~/.lightning`` itself — backward compatible,
zero migration needed.

Usage::

    lightning profile create coder          # fresh profile + bundled skills
    lightning profile create coder --clone  # also copy config, .env, SOUL.md, skills
    lightning profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    lightning -p coder chat                 # or via flag
    lightning profile use coder             # set as sticky default
    lightning profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See lightning_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.lightning``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   lightning-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `lightning profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `lightning
...
# `lightning skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Lightning infrastructure directories that only the default profile
         (``~/.lightning``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_lightning_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.lightning) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "lightning-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "lightning", "default", "test", "tmp", "root", "sudo",
})

# Lightning subcommands that cannot be used as profile names/aliases
_LIGHTNING_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the lightning root, NOT to the current LIGHTNING_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where LIGHTNING_HOME points outside
    ``~/.lightning``, profiles live under ``LIGHTNING_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_lightning_home() / "profiles"


def _get_default_lightning_home() -> Path:
    """Return the default (pre-profile) LIGHTNING_HOME path.

    In standard deployments this is ``~/.lightning``.
    In Docker/custom deployments where LIGHTNING_HOME is outside ``~/.lightning``
    (e.g. ``/opt/data``), returns LIGHTNING_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_lightning_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Lightning installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its LIGHTNING_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_lightning_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, lightning subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _LIGHTNING_SUBCOMMANDS:
        return f"'{canon}' conflicts with a lightning subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``lightning profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Lightning config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_lightning_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``lightning update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.lightning)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.lightning/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `lightning update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`lightning profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `lightning update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `lightning profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches LIGHTNING_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``lightning profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LIGHTNING_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.lightning).\n"
            "To remove everything, use: lightning uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LIGHTNING_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("LIGHTNING_HOME")
    try:
        os.environ["LIGHTNING_HOME"] = str(profile_dir)
        from lightning_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["LIGHTNING_HOME"] = old_home
        elif "LIGHTNING_HOME" in os.environ:
            del os.environ["LIGHTNING_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.lightning/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: lightning profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from LIGHTNING_HOME.

    Returns ``"default"`` if LIGHTNING_HOME is not set or points to ``~/.lightning``.
    Returns the profile name if LIGHTNING_HOME points into ``~/.lightning/profiles/<name>``.
    Returns ``"custom"`` if LIGHTNING_HOME is set to an unrecognized path.
    """
    from lightning_constants import get_lightning_home
    lightning_home = get_lightning_home()
    resolved = lightning_home.resolve()

    default_resolved = _get_default_lightning_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.lightning itself — its parent is ~/ and its
        # directory name is ".lightning", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: lightning profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.lightning itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.lightning). "
            "Specify a different name: lightning profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="lightning_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"lightning.{old_name}"
    new_host = f"lightning.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_lightning_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a LIGHTNING_HOME path string.

    Called early in the CLI entry point, before any lightning modules
    are imported, to set the LIGHTNING_HOME environment variable.
    """
