...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or lightning -c.",
    ...
    "Ctrl+Z suspends Lightning to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "lightning -c resumes your most recent CLI session. lightning -c \"project name\" resumes by title.",
    "lightning -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "lightning -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "lightning chat -t web,terminal enables only specific toolsets for a focused session.",
    "lightning chat -s github-pr-workflow preloads a skill at launch.",
    "lightning chat -q \"query\" runs a single non-interactive query and exits.",
    "lightning chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "lightning chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "lightning --yolo bypasses all dangerous command approval prompts for the entire session.",
    "lightning chat --source telegram tags the session for filtering in lightning sessions list.",
    "lightning -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "lightning doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "lightning dump outputs a compact setup summary — great for bug reports.",
    "lightning config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "lightning config edit opens config.yaml in your default editor.",
    "lightning config check scans for missing or stale configuration options.",
    "lightning sessions browse opens an interactive session picker with search.",
    "lightning sessions stats shows session counts by platform and database size.",
    "lightning sessions prune --older-than 30 cleans up old sessions.",
    "lightning skills search react --source skills-sh searches the skills.sh public directory.",
    "lightning skills check scans installed hub skills for upstream updates.",
    "lightning skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "lightning skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "lightning mcp add github --command npx adds MCP servers from the command line.",
    "lightning mcp serve runs Lightning itself as an MCP server for other agents.",
    "lightning auth add lets you add multiple API keys for credential pool rotation.",
    "lightning completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "lightning logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "lightning backup creates a zip backup of your entire Lightning home directory.",
    "lightning profile create coder creates an isolated profile that becomes its own command.",
    "lightning profile create work --clone copies your current config and keys to a new profile.",
    "lightning update syncs new bundled skills to ALL profiles automatically.",
    "lightning gateway install sets up Lightning as a system service (systemd/launchd).",
    "lightning memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "lightning webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: lightning tools disables unused tools, lightning skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "lightning models routes vision, compression, and aux tasks to cheaper models — cuts background cost 85%+ without downgrading your chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as lightning -w).",
    ...
    "LIGHTNING_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Lightning tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'lightning profile create coder' creates the 'coder' command.",
    "lightning profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Lightning prints a resume command with session ID and stats.",
    "lightning sessions export backup.jsonl exports all sessions for backup or analysis.",
    "lightning -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "lightning skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .lightning/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: lightning cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Lightning runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "lightning gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Lightning via a local macOS server.",
    ...
    "Context files (.lightning.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.lightning/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Lightning loads project context from .lightning.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — lightning tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.lightning/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "lightning chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "lightning plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "lightning acp runs Lightning as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "LIGHTNING_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "lightning login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "lightning status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.lightning/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Lightning your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.lightning/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Lightning auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.lightning/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at LIGHTNING_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "LIGHTNING_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in LIGHTNING_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.lightning/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'lightning profile create ls' would be rejected.",
    "lightning profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "lightning claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "LIGHTNING_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via lightning tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.lightning/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.lightning/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Lightning auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Lightning config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Lightning is working.',
    ...
    '/reload reloads ~/.lightning/.env into the running session — pick up new API keys without restarting.',
    ...
    "LIGHTNING_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.lightning/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.lightning/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'lightning curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "lightning curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'lightning curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'lightning auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'LIGHTNING_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "LIGHTNING_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'lightning dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'lightning dashboard --tui embeds the full Lightning TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.lightning/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.lightning/dashboard-plugins/ — no npm build required.',
    ...
    'LIGHTNING_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'LIGHTNING_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'LIGHTNING_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'LIGHTNING_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.lightning/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.lightning/skills/ so drop-in skills appear without restarting the session.',
    ...
    'lightning -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'lightning chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'lightning chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'lightning chat --ignore-user-config skips ~/.lightning/config.yaml — reproducible bug reports and CI runs.',
    "lightning chat --source tool tags programmatic chats so they don't clutter lightning sessions list.",
    'lightning dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'lightning sessions rename <ID> "new title" renames any past session; lightning sessions delete <ID> removes one.',
    'lightning import restores a session export or profile archive produced by sessions export or profile export.',
    'lightning fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'lightning pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'lightning setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'lightning status --deep runs the full health sweep across every component; plain lightning status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'LIGHTNING_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'LIGHTNING_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.lightning/plugins/ — trust-gated by design.',
    "LIGHTNING_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'LIGHTNING_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'LIGHTNING_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'LIGHTNING_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'LIGHTNING_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'LIGHTNING_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'LIGHTNING_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'LIGHTNING_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'LIGHTNING_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'LIGHTNING_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'LIGHTNING_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Lightning block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from lightning sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.lightning/dashboard-plugins/.',
]
