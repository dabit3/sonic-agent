"""Helpers for loading Lightning .env files consistently across entrypoints."""
...
# load_lightning_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_lightning_dotenv().
# Used by setup / `lightning model` flows to label detected credentials so
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_lightning_dotenv()`` call.  Returns
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `lightning setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``lightning_cli.config._sanitize_env_lines`` which
    already knows all valid Lightning env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from lightning_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_lightning_dotenv(
    *,
    lightning_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Lightning environment files with user config taking precedence.

    Behavior:
    - `~/.lightning/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(lightning_home or os.getenv("LIGHTNING_HOME", Path.home() / ".lightning"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Lightning reads
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `lightning model`
