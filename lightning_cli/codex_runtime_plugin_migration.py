"""Migrate Lightning' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Lightning' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Lightning mcp_servers.<n>.command/args/env  → codex stdio transport
  Lightning mcp_servers.<n>.url/headers       → codex streamable_http transport
  Lightning mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Lightning mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Lightning-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Lightning' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Lightning config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Lightning' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, lightning_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Lightning MCP server config to the codex inline-table dict
    ...
    has_command = bool(lightning_cfg.get("command"))
    has_url = bool(lightning_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(lightning_cfg["command"])
        args = lightning_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = lightning_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = lightning_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(lightning_cfg["url"])
        headers = lightning_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Lightning' transport: sse hint is informational; codex auto-negotiates
        if lightning_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in lightning_cfg:
        try:
            out["tool_timeout_sec"] = float(lightning_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in lightning_cfg:
        try:
            out["startup_timeout_sec"] = float(lightning_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if lightning_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in lightning_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_LIGHTNING_KEYS:
            skipped.append(f"{key} (unknown Lightning key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (LIGHTNING_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Lightning)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Lightning' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Lightning' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Lightning
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="lightning-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a LIGHTNING_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed lightning-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.lightning`` symlink that happens to live under ``/private/var/folders``
...
def _build_lightning_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Lightning' own
    ...
    so a lightning installed under /opt/, /usr/local/, or a venv all work.
    LIGHTNING_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # LIGHTNING_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_lightning_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever LIGHTNING_HOME its launcher sets
    ...
    # — that would override the launcher's LIGHTNING_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("LIGHTNING_HOME", tmp_path) would
    ...
    lightning_home = os.environ.get("LIGHTNING_HOME") or ""
    if lightning_home and _looks_like_test_tempdir(lightning_home):
        lightning_home = ""
    if lightning_home:
        env["LIGHTNING_HOME"] = lightning_home
    # PYTHONPATH passes through so a worktree-launched lightning finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["LIGHTNING_QUIET"] = "1"
    env["LIGHTNING_REDACT_SECRETS"] = env.get("LIGHTNING_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_lightning_tools: bool = True,
) -> MigrationReport:
    """Translate Lightning mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        lightning_config: full ~/.lightning/config.yaml dict
        ...
        expose_lightning_tools: when True (default), register Lightning' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Lightning for tools
    ...
    lightning_servers = (lightning_config or {}).get("mcp_servers") or {}
    if not isinstance(lightning_servers, dict):
        report.errors.append(
            "mcp_servers in Lightning config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in lightning_servers.items():
    ...
    # Inject Lightning' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Lightning for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/lightning_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_lightning_tools:
        translated["lightning-tools"] = _build_lightning_tools_mcp_entry()
        if "lightning-tools" not in report.migrated:
            report.migrated.append("lightning-tools")
