"""``lightning logs`` — view and filter Lightning log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.lightning/logs/``.

Usage examples::

    lightning logs                    # last 50 lines of agent.log
    lightning logs -f                 # follow agent.log in real time
    lightning logs errors             # last 50 lines of errors.log
    lightning logs gateway -n 100    # last 100 lines of gateway.log
    lightning logs --level WARNING    # only WARNING+ lines
    lightning logs --session abc123   # filter by session ID substring
    lightning logs --component tools  # only tool-related lines
    lightning logs --since 1h         # lines from the last hour
    lightning logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_lightning_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Lightning runs — try 'lightning chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_lightning_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_lightning_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_lightning_home()}/logs/")
        return

    print(f"Log files in {display_lightning_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'lightning chat' to generate logs)")
