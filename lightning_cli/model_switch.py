...
- ``lightning_cli.providers``        -- canonical provider identity + overlays
- ``lightning_cli.model_normalize``  -- per-provider name formatting
...
_LIGHTNING_MODEL_WARNING = (
    "Nous Research Lightning 3 & 4 models are NOT agentic and are not designed "
    "for use with Lightning Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Lightning 3 / Lightning 4 chat families.
# The previous substring check (`"lightning" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``lightning-brain:qwen3-14b-ctx16k`` that just
# happen to carry "lightning" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Lightning-3-Llama-3.1-70B, lightning-4-405b, openrouter/lightning3:70b
# Negative examples it must NOT match:
#   lightning-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_LIGHTNING_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])lightning[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_lightning_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Lightning 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_LIGHTNING_NON_AGENTIC_RE.search(model_name))


def _check_lightning_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Lightning 3/4 chat model."""
    if is_nous_lightning_non_agentic(model_name):
        return _LIGHTNING_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``lightning config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from lightning_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'lightning model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'lightning doctor' — config issues detected:"
    ...
    # Mirrors the same logic in lightning_cli.runtime_provider.resolve_runtime_provider;
    ...
    lightning_warn = _check_lightning_model_warning(new_model)
    if lightning_warn:
        warnings.append(lightning_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from lightning_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from lightning_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by lightning provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://lightning-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Lightning release. Falls back to the in-repo
    ...
    # --- 1. Check Lightning-mapped providers ---
    for lightning_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(lightning_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (LIGHTNING_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from lightning_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(lightning_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Lightning release.
        model_ids = curated.get(lightning_id, [])
        if lightning_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(lightning_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Lightning-only providers (nous, openai-codex, copilot, opencode-go) ---
    from lightning_cli.providers import LIGHTNING_OVERLAYS
    from lightning_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Lightning provider ID.
    # LIGHTNING_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Lightning IDs ("copilot").
    _mdev_to_lightning = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in LIGHTNING_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Lightning slug — e.g. "github-copilot" → "copilot"
        lightning_slug = _mdev_to_lightning.get(pid, pid)
        if lightning_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(lightning_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, lightning_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or lightning_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(lightning_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", lightning_slug, exc)
        ...
        # configured.
        if not has_creds and lightning_slug == "anthropic":
            try:
                ...
                if (lightning_creds and lightning_creds.get("accessToken")) or \
        ...
        if lightning_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(lightning_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(lightning_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(lightning_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Lightning slug, fall back to overlay key
            model_ids = curated.get(lightning_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if lightning_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(lightning_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": lightning_slug,
            "name": get_label(lightning_slug),
            "is_current": lightning_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "lightning",
        ...
        seen_slugs.add(lightning_slug.lower())
        _record_builtin_endpoint(lightning_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or LIGHTNING_OVERLAYS (keeps /model in sync
    # with `lightning model`).
    try:
        from lightning_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Lightning's canonical write key (matches
            ...
            # Lightning writes ``models:`` as a dict keyed by model id
            # (see lightning_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Lightning's own writer uses; a hyphen variant is accepted
            ...
            # active model. Lightning's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, lightning_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`lightning_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from lightning_cli.models import fetch_openrouter_models
