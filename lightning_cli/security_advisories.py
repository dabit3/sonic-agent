"""
Security advisory checker for Lightning Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``lightning doctor --ack <id>``; the ack is persisted to
...
1. ``lightning doctor`` (and ``lightning doctor --ack <id>``)
...
run in environments where the rest of Lightning failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including lightning when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.lightning/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: lightning doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Lightning requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'lightning doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.lightning/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_lightning_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``lightning doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `lightning doctor` on the gateway host for details.")
