...
Pure logic: walks a Lightning config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `lightning doctor` and a future `lightning migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Lightning config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Lightning config and return retirement issues.
