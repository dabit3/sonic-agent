"""Shell completion script generation for lightning CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_lightning_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Lightning Agent bash completion
# Add to ~/.bashrc:
#   eval "$(lightning completion bash)"

_lightning_profiles() {{
    local profiles_dir="$HOME/.lightning/profiles"
...
_lightning_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_lightning_profiles)" -- "$cur"))
        return
    fi
...
complete -F _lightning_completion lightning
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef lightning
# Lightning Agent zsh completion
# Add to ~/.zshrc:
...
_lightning_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.lightning/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.lightning/profiles 2>/dev/null)}}")
    fi
...
_lightning() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_lightning_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'lightning command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Lightning Agent fish completion",
        "# Add to your config:",
        "#   lightning completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __lightning_profiles",
        "    echo default",
        "    if test -d $HOME/.lightning/profiles",
        "        ls $HOME/.lightning/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c lightning -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c lightning -f -s p -l profile"
        " -d 'Profile name' -xa '(__lightning_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__lightning_profiles)' -d 'Profile name'"
                )
