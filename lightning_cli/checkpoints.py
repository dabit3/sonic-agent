"""`lightning checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.lightning/checkpoints/``.  Actions:

    lightning checkpoints               # same as `status`
    lightning checkpoints status        # total size, project count, breakdown
    lightning checkpoints list          # per-project checkpoint counts + workdir
    lightning checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    lightning checkpoints clear [-f]    # nuke the entire base (asks first)
    lightning checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    lightning checkpoints
    lightning checkpoints prune --retention-days 3 --max-size-mb 200
    lightning checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lightning checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lightning checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lightning checkpoints` → status
