...
Used by ``lightning profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``lightning_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Lightning Agent kanban board.
...
Rules:
  ...
  - Never write "Lightning Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_lightning_home())
        else:
