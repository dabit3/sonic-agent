...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with lightning-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.lightning/plugins/<name>/``
3. **Project plugins** – ``./.lightning/plugins/<name>/`` (opt-in via
   ``LIGHTNING_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``lightning_agent.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``LIGHTNING_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("LIGHTNING_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``LIGHTNING_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.lightning/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("LIGHTNING_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When LIGHTNING_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Lightning loggers.
    """
    ...
    logger.debug(
        "LIGHTNING_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "lightning_agent.plugins"

_NS_PARENT = "lightning_plugins"
...
class PluginManifest:
    ...
    #              in ~/.lightning/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``lightning plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``lightning honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``lightning <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.lightning/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to lightning_cli/. Two layouts are
        ...
        # 2. User plugins (~/.lightning/plugins/)
        user_dir = get_lightning_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.lightning/plugins/)
        if _env_enabled("LIGHTNING_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".lightning" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set LIGHTNING_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with lightning and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Lightning ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `lightning plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``lightning_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``lightning_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="lightning-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``lightning tools`` TUI so plugin-provided toolsets appear
