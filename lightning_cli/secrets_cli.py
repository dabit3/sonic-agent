"""CLI handlers for ``lightning secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``lightning_cli.main`` as part of building the top-level
    ``lightning secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Lightning process."
    )
    console.print(
        "  Status:  [cyan]lightning secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]lightning secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]lightning secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]lightning secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Lightning will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`lightning secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]lightning[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
