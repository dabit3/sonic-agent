"""CLI subcommand: ``lightning send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"lightning send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"lightning send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"lightning send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"lightning send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"lightning send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `lightning gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.lightning/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_lightning_env() -> None:
    """Populate ``os.environ`` from ``~/.lightning/.env`` AND bridge top-level
    ...
    startup that ``lightning send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.lightning/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.lightning/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``lightning send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.lightning/.env and ~/.lightning/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "lightning send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  lightning send --to telegram \"hello\"\n"
            "  lightning send --to discord:#ops --file report.md\n"
            "  lightning send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "lightning send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `lightning send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Lightning "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.lightning/.env + ~/.lightning/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  lightning send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  lightning send --list                  # all platforms\n"
            "  lightning send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `lightning send --list telegram`.",
    )
