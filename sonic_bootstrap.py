"""Windows UTF-8 bootstrap for Sonic entry points.
...
should be imported at the very top of every Sonic entry point
(``sonic``, ``sonic-agent``, ``sonic-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Sonic modules.

    Sonic ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Sonic source root
    ...
    """
    root = src_root or os.environ.get("SONIC_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SONIC_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SONIC_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import sonic_bootstrap``
# (or ``from sonic_bootstrap import apply_windows_utf8_bootstrap``) at
