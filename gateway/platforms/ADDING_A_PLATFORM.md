...
There are two ways to add a platform to the Sonic gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.sonic/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Sonic code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `sonic gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `sonic_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Sonic core.
...
And add it to the `sonic-gateway` composite:

```python
"sonic-gateway": {
    "includes": [..., "sonic-your-platform"]
}
```
...
## 12. Status Display (`sonic_cli/status.py`)
...
## 13. Gateway Setup Wizard (`sonic_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ sonic_cli/ toolsets.py \
  --include="*.py" -l | sort -u
