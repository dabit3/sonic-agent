...
There are two ways to add a platform to the Lightning gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.lightning/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Lightning code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lightning gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `lightning_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Lightning core.
...
And add it to the `lightning-gateway` composite:

```python
"lightning-gateway": {
    "includes": [..., "lightning-your-platform"]
}
```
...
## 12. Status Display (`lightning_cli/status.py`)
...
## 13. Gateway Setup Wizard (`lightning_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ lightning_cli/ toolsets.py \
  --include="*.py" -l | sort -u
