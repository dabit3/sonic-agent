...
# Audio file extensions Sonic recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Sonic' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Sonic-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.sonic/.env manually."
)
...
# Default location: {SONIC_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_sonic_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_sonic_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_sonic_dir("cache/videos", "video_cache")
...
MEDIA_DELIVERY_STRICT_ENV = "SONIC_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SONIC_HOME / "image_cache",
    _SONIC_HOME / "audio_cache",
    _SONIC_HOME / "video_cache",
    _SONIC_HOME / "document_cache",
    _SONIC_HOME / "browser_screenshots",
    ...
    _SONIC_HOME / "cache" / "images",
    _SONIC_HOME / "cache" / "audio",
    _SONIC_HOME / "cache" / "videos",
    _SONIC_HOME / "cache" / "documents",
    _SONIC_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # SONIC_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.sonic
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Sonic
    secrets (``~/.sonic/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``SONIC_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Sonic-managed cache, under an operator-allowlisted root
    (``SONIC_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Sonic root (~/.sonic/.env,
    ...
    # ``MEDIA:~/.sonic/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?sonic\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+sonic[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``SONIC_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Sonic commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("SONIC_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "SONIC_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SONIC_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Sonic sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.sonic/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from sonic_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SONIC_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SONIC_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SONIC_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SONIC_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SONIC_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SONIC_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _sonic_run_generation is set on
