...
# Audio file extensions Lightning recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Lightning' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Lightning-created Telegram private-chat
...
from lightning_constants import get_lightning_dir, get_lightning_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.lightning/.env manually."
)
...
# Default location: {LIGHTNING_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_lightning_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; LightningAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_lightning_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_lightning_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_lightning_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_lightning_dir("cache/screenshots", "browser_screenshots")
_LIGHTNING_HOME = get_lightning_home()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "LIGHTNING_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _LIGHTNING_HOME / "image_cache",
    _LIGHTNING_HOME / "audio_cache",
    _LIGHTNING_HOME / "video_cache",
    _LIGHTNING_HOME / "document_cache",
    _LIGHTNING_HOME / "browser_screenshots",
)
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    existing regular files under Lightning-managed media caches, or roots the
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?lightning\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+lightning[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from lightning_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          LIGHTNING_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          LIGHTNING_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          LIGHTNING_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("LIGHTNING_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("LIGHTNING_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("LIGHTNING_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _lightning_run_generation is set on
