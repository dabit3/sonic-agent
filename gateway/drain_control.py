...
Why the epoch (NS-570). ``SONIC_HOME`` is a **durable** store — on Sonic
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``sonic gateway restart``| same    | same(init) | SAME   | honour |
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable SONIC_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (SONIC_HOME is a
    durable Fly volume on Sonic Cloud) and the lifecycle action that triggered
