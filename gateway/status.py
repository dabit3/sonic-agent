...
The PID file lives at ``{LIGHTNING_HOME}/gateway.pid``.  LIGHTNING_HOME defaults to
``~/.lightning`` but can be overridden via the environment variable.  This means
separate LIGHTNING_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "lightning-gateway"
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting LIGHTNING_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("LIGHTNING_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "lightning" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Lightning gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different LIGHTNING_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``lightning.service`` + ``lightning-
# gateway.service``).
#
