...
The PID file lives at ``{SONIC_HOME}/gateway.pid``.  SONIC_HOME defaults to
``~/.sonic`` but can be overridden via the environment variable.  This means
separate SONIC_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SONIC_HOME."""
    home = get_sonic_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("SONIC_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "sonic" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Sonic gateway lifecycle subcommand from a command line.
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `sonic gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Sonic-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Sonic gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a SONIC_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.sonic`` or ``$SONIC_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``sonic_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``SONIC_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"sonic_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit SONIC_HOME= on the argv. SONIC_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "sonic_home=" in command_lc and f"sonic_home={home_lc}" not in command_lc:
        return False
    return True
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``SONIC_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    SONIC_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SONIC_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``sonic.service`` + ``sonic-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different SONIC_HOME. When two profile gateway
    # services share the same default ~/.sonic (SONIC_HOME not set
    ...
    # field is absent in markers written by older Sonic versions; treat
    ...
    replacer_home = record.get("replacer_sonic_home")
    if replacer_home is not None and replacer_home != str(get_sonic_home()):
        return False
    ...
    # ``sonic gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_sonic_home": str(get_sonic_home()),
