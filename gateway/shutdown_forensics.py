    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if lightning_home_str:
            takeover_path = Path(lightning_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(lightning_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded lightning-agent but never re-ran ``lightning setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../lightning-gateway.service"
    ...
    # that's the common case for lightning.
    ...
    # constant in lightning_cli/gateway.py.
    headroom = 30.0
