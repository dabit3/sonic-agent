...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_lightning_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.lightning/config.yaml (primary user-facing config)
    3. ~/.lightning/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``lightning config set gateway.streaming.*``
