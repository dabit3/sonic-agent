...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_sonic_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SONIC_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.sonic/config.yaml (primary user-facing config)
    3. ~/.sonic/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``sonic config set gateway.streaming.*``
