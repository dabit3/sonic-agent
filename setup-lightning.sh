...
# 5. Symlinks the 'lightning' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Lightning Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/lightning-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink lightning into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up lightning command..."

LIGHTNING_BIN="$SCRIPT_DIR/venv/bin/lightning"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LIGHTNING_BIN" "$COMMAND_LINK_DIR/lightning"
echo -e "${GREEN}✓${NC} Symlinked lightning → $COMMAND_LINK_DISPLAY_DIR/lightning"
...
# ============================================================================
# Seed bundled skills into ~/.lightning/skills/
# ============================================================================

LIGHTNING_SKILLS_DIR="${LIGHTNING_HOME:-$HOME/.lightning}/skills"
mkdir -p "$LIGHTNING_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.lightning/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LIGHTNING_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  lightning status        # Check configuration"
if is_termux; then
    echo "  lightning gateway       # Run gateway in foreground"
else
    echo "  lightning gateway install # Install gateway service (messaging + cron)"
fi
echo "  lightning cron list     # View scheduled jobs"
echo "  lightning doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m lightning_cli.main setup
fi
