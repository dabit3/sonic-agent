...
from sonic_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
...
def load_cli_config() -> Dict[str, Any]:
    ...
    # sonic_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `sonic config`/`doctor`/guards (which use load_config) honor it. The
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"sonic pid={os.getpid()}", str(wt_path)],
...
class SonicCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `sonic update` to rebuild the venv) to restore persistence."
                )
            except Exception:
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            print(f"\n  [Sonic #{visible_index}]{_ts_suffix(msg)}")
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            try:
                # Discover from disk (bundled + user), matching `sonic plugins
                ...
                # catalog lives behind `sonic plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: sonic plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_sonic_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: sonic plugins list)")
                else:
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `sonic portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from sonic_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from sonic_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `sonic portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from sonic_cli.auth import step_up_nous_billing_scope
...
def main(
    ...
    if toolsets:
        ...
        # Coding posture (base Sonic): with no explicit --toolsets, collapse
