"""Shared constants for Sonic Agent.
...
_UNSET = object()
_SONIC_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_SONIC_HOME_OVERRIDE", default=_UNSET
)


def set_sonic_home_override(path: str | Path | None) -> Token:
    """Set a context-local Sonic home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _SONIC_HOME_OVERRIDE.set(value)


def reset_sonic_home_override(token: Token) -> None:
    """Restore the previous context-local Sonic home override."""
    _SONIC_HOME_OVERRIDE.reset(token)


def get_sonic_home_override() -> str | None:
    """Return the active context-local Sonic home override, if any."""
    override = _SONIC_HOME_OVERRIDE.get()
...
def get_sonic_home() -> Path:
    ...
    ``sonic_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
...
def get_default_sonic_root() -> Path:
    ...
    In profile mode where ``SONIC_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.sonic/profiles/coder``) and Docker
    ...
    try:
        ...
        # SONIC_HOME is under ~/.sonic (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — SONIC_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Sonic is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``SONIC_OPTIONAL_SKILLS``.
    """
    override = os.getenv("SONIC_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_sonic_home() / "optional-skills"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``SONIC_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<SONIC_HOME>/skills`` last-resort
    """
    override = os.getenv("SONIC_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_sonic_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Sonic subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to SONIC_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to SONIC_HOME (e.g. ``"image_cache"``).
...
def iter_sonic_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Sonic-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\sonic\\node``. POSIX installs use
    ``$SONIC_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_sonic_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with sonicManagedNodePathEntries() in
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Sonic-managed Node trees live under ``$SONIC_HOME/node`` (or a profile's
    ``SONIC_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_sonic_node_executable``
    used to trust file presence alone, so ``sonic update`` would pick that
...
def sonic_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Sonic-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_sonic_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%SONIC_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_sonic_home()
    index_url = f"https://nodejs.org/dist/latest-v{_SONIC_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_SONIC_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_sonic_managed_node() -> bool:
    """Redownload Sonic-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not sonic_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SONIC_HOME": str(get_sonic_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_sonic_node_executable(command: str) -> str | None:
    """Return a Sonic-managed Node/npm executable path, healing broken trees."""
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Sonic-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Sonic-managed installs.

    This is for Sonic-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_sonic_node_executable(command)
    if managed:
        return managed
    if sonic_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_sonic_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Sonic-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_sonic_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``sonic update`` — leaving a **dangling symlink** that ``which`` still
...
def display_sonic_home() -> str:
    """Return a user-friendly display string for the current SONIC_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.sonic``
        profile:  ``~/.sonic/profiles/coder``
        custom:   ``/opt/sonic-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.sonic``.  For code that needs a real ``Path``, use
    :func:`get_sonic_home` instead.
    """
    home = get_sonic_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``SONIC_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{SONIC_HOME}/home`` when the profile-home directory exists."""
    sonic_home = get_sonic_home_override() or (env or {}).get("SONIC_HOME") or os.getenv("SONIC_HOME")
    if not sonic_home:
        return None
    profile_home = os.path.join(sonic_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("SONIC_REAL_HOME") or os.getenv("SONIC_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Sonic profile HOME.

    ``SONIC_HOME`` scopes Sonic state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={SONIC_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{SONIC_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{SONIC_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Sonic' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["SONIC_REAL_HOME"] = real_home
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under SONIC_HOME.

    Replaces the ``get_sonic_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, sonic_logging.py, sonic_time.py, etc.).
    """
    return get_sonic_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under SONIC_HOME."""
    return get_sonic_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under SONIC_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_sonic_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._sonic_ipv4_patched = True  # type: ignore[attr-defined]
