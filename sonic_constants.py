...
def get_sonic_home() -> Path:
    ...
    ``sonic_cli/kanban_db.py``).  See https://github.com/dabit3/sonic-agent/issues/18594.
    """
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/dabit3/sonic-agent/issues/25821.
    """
...
def get_subprocess_home() -> str | None:
    ...
    When ``{SONIC_HOME}/home/`` exists on disk, subprocesses should use it
    as ``HOME`` so system tools (git, ssh, gh, npm …) write their configs
    inside the Sonic data directory instead of the OS-level ``/root`` or
    ``~/``.  This provides:
    ...
    **never** modified — only subprocess environments should inject this value.
    ...
    """
    sonic_home = get_sonic_home_override() or os.getenv("SONIC_HOME")
    if not sonic_home:
        return None
