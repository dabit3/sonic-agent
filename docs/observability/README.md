# Sonic Observer Hooks

Sonic observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Sonic' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Sonic catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Sonic only builds
