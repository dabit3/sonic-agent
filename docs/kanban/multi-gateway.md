...
Sonic supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.sonic/config.yaml`:
...
Or set the env var: `SONIC_KANBAN_DISPATCH_IN_GATEWAY=false`
