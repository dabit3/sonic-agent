...
> (`git log --oneline a957ef083..a6f7171a5 -- 'docker/*' 'sonic_cli/service_manager.py' …`).
> Open Questions are collapsed into a single Decision Log table; full
> deliberations live in PR review comments.

**Goal:** Replace `tini` with s6-overlay as PID 1 in the Sonic Docker image so
that the main sonic process, the dashboard, and dynamically-created
...
s6-svscan as PID 1. Main sonic and the dashboard are declared as static
s6-rc services at image build time. Per-profile gateways — which users create
*after* the image is built (`sonic profile create coder` →
...
- Existing systemd/launchd/windows surface in `sonic_cli/gateway.py` and
  `sonic_cli/gateway_windows.py`.
...
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  `gateway-default` slot is always registered for the root SONIC_HOME
  profile so `sonic gateway start` (no `-p`) has somewhere to land.
...
1. `gosu` privilege drop from root → `sonic` UID.
2. Copied `.env.example`, `cli-config.yaml.example`, `SOUL.md` into
   `$SONIC_HOME` if missing.
3. Synced bundled skills via `tools/skills_sync.py`.
4. Optionally backgrounded `sonic dashboard` in a subshell when
   `SONIC_DASHBOARD=1` — **not supervised**, no restart.
5. `exec sonic "$@"` — tini's sole direct child.
...
All init-system logic lives in **`sonic_cli/gateway.py`** (~5,400 LOC at
re-validation). The systemd/launchd code is ~1,500 lines of that, plus a
separate **`sonic_cli/gateway_windows.py`** (~690 LOC) for Windows
Scheduled Tasks.
...
- `sonic_cli/status.py` — gained an `s6` branch for in-container runs.
- `sonic_cli/profiles.py` — `create_profile` / `delete_profile` register and
  unregister with s6 inside the container (no-op on host).
- `sonic_cli/doctor.py` — `_check_gateway_service_linger` skips on s6, and a
  new "Service Supervisor" section reports main-sonic / dashboard /
  profile-gateway counts via the ServiceManager.
- `sonic_cli/gateway.py::gateway_command` — the
...
`sonic gateway start`, `coder gateway start` (profile alias), and
`sonic -p <profile> gateway start` all spawn a gateway process scoped to a
given profile. See
[Profiles: Running Gateways](https://sonic-agent.nousresearch.com/docs/user-guide/profiles#running-gateways).
On host, lifecycle is managed via per-profile systemd units
(`sonic-gateway-<profile>.service`); inside the container, an s6 service at
...
`sonic_cli/container_boot.py`, and auto-starts those whose last recorded
...
- **Root/non-root model:** `/init` runs as root to set up the supervision
  ...
  `s6-setuidgid sonic` in its `run` script. The per-service `s6-supervise`
  monitor stays root so it can signal its child regardless of UID. Net
  effect: sonic and all its subprocesses run as UID 10000 exactly as
...
- `/command/with-contenv` sources `/run/s6/container_environment/*` into
  service env — convenient for passing `SONIC_HOME` etc.
...
- Zombie reaping: PID 1 (s6-svscan) reaps all zombies non-blockingly on
  SIGCHLD. Any subagent subprocess spawned by the main sonic process is
...
Container ENTRYPOINT is `/init`, PID 1 is s6-svscan. The main sonic
...
### D2. Main sonic is an s6 service with container-exit semantics

The contract "container exits when `sonic` exits" is preserved via a
...
|---|---|
| (no args) | `sonic` with no args, container exits when sonic exits |
| `chat -q "..."` | `sonic chat -q "..."`, container exits with sonic exit code |
...
- **s6-rc** (declarative, compile-then-swap): used for main sonic and the
...
(tmpfs, sonic-writable). s6-svscan picks them up on rescan.
...
(`sonic -p <profile> gateway run`, or `sonic gateway run` for the default
...
and silently failed for the unprivileged sonic user (UID 10000), making
...
Existing per-profile `sonic -p <profile> gateway start/stop/restart` CLI
...
`sonic gateway start` (no `-p`) targets a special `gateway-default` slot
that's always registered by the cont-init reconciler. Its run script omits
the `-p` flag and runs against the root `$SONIC_HOME` profile.

`--all` lifecycle (`sonic gateway stop --all`, `... restart --all`)
...
main-sonic service so the sonic CMD becomes that main program.
...
_is_tui_invocation() {
    ...
    case "${SONIC_TUI:-}" in 1|true|TRUE|yes) return 0 ;; esac
    if [ -t 0 ] && [ $# -eq 0 ]; then return 0; fi
    return 1
}
```

And in `docker/s6-rc.d/main-sonic/run`:

```sh
if [ -f /var/run/s6/container_environment/SONIC_TUI_MODE ]; then
    exec sleep infinity   # s6-overlay will exec CMD as the TTY-connected main
fi
exec s6-setuidgid sonic sonic ${SONIC_ARGS:-}
```

In TUI mode main sonic is effectively unsupervised (same as the pre-s6
...
| Profile gateway crash loop masks a real config error | Low | Medium | s6 `finish` script `max_restarts` cap (planned follow-up); operators see crash-looping logs in `$SONIC_HOME/logs/gateways/<profile>/` |
...
| `docker restart` silently loses per-profile gateway registrations (tmpfs scandir wiped) | High (without mitigation) | High | Cont-init reconciliation re-registers from persistent `$SONIC_HOME/profiles/` and auto-starts those last seen `running`; outcome recorded to `$SONIC_HOME/logs/container-boot.log` (size-bounded, rotates to `.1` at 256 KiB) |
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `sonic doctor` alerts when N consecutive container restarts ended in `startup_failed` |
| `_s6_running()` detection works as root but silently fails for unprivileged sonic user, making runtime-registration path inert | High (without mitigation) | High | **Caught in PR review.** Detection now probes `/proc/1/comm` (world-readable) + `/run/s6/basedir`. Docker integration tests refactored to `docker exec -u sonic` so the realistic runtime user is exercised |
| `s6-svscanctl` from sonic hits EACCES on the root-owned control FIFO | Medium | Medium | `02-reconcile-profiles` chowns `/run/service/.s6-svscan/{control,lock}` to sonic after stage1 creates them |
| Per-service `supervise/control` FIFO is root-owned by s6-supervise, blocking `s6-svc` from sonic | Known | Medium | Surfaced cleanly as `S6CommandError` (with rc + stderr) instead of raw `CalledProcessError`. Permission fix tracked as a follow-up (small SUID helper, polling chown loop in cont-init.d, or replace `s6-svc` with `down`-marker manipulation) |
...
| # | Question | Decision |
|---|---|---|
| OQ1 | Gate Phase 2 behind env var? | Ship directly (Sonic is pre-1.0; users can pin the previous image) |
| OQ2 | s6 root model | Root `/init`, drop per-service via `s6-setuidgid sonic` |
| OQ3 | Dashboard opt-in mechanism | Always declared as an s6 service; `03-dashboard-toggle` cont-init script writes a `down` marker when `SONIC_DASHBOARD` is unset so `s6-svstat` reports the slot's real state |
| OQ4 | Podman rootless | Supported, fix reactively |
| OQ5 | Service naming | `gateway-<profile>` (matches pre-existing `sonic-gateway-<profile>.service` systemd convention) |
...
| OQ8 | Log persistence | `$SONIC_HOME/logs/gateways/<profile>/`. The log path is sourced from runtime `$SONIC_HOME` via `with-contenv`, NOT Python-substituted at registration time |
...
- **`gateway-default` slot:** always registered by the reconciler so
  `sonic gateway start` (no `-p`) has somewhere to land.
...
- [x] `docker run -it --rm sonic-agent --tui` starts the Ink TUI with
      working keyboard input, cursor control, and resize (SIGWINCH)
- [x] Dashboard crashes are recovered by s6 within ~2s
- [x] `sonic profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `sonic -p test gateway start` inside a container dispatches through s6
- [x] `sonic -p test gateway stop` inside a container cleanly stops via s6
- [x] `sonic profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$SONIC_HOME/logs/gateways/test/current`
- [x] `sonic status` inside the container shows `Manager: s6`
- [x] `sonic gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `sonic gateway stop --all` / `... restart --all` iterate every
...
- [x] `sonic gateway install/start/stop` on Linux host and macOS host
