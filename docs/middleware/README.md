...
Sonic middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Sonic'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Sonic calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Sonic retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Sonic guardrails,
...
- `telemetry_schema_version`: currently `sonic.observer.v1`
- `middleware_schema_version`: currently `sonic.middleware.v1`
...
Sonic stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Sonic runs
them as a nested chain in registration order. Middleware failures are fail-open:
Sonic logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Sonic applies middleware in this order:
...
For each tool call, Sonic applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Sonic pre-execution path against the effective arguments:
...
```bash
sonic plugins enable <plugin-name>
```

For isolated local testing, use one `SONIC_HOME` for plugin enablement and the
agent run:

```bash
export SONIC_HOME=/tmp/sonic-middleware-test
mkdir -p "$SONIC_HOME"
sonic plugins enable <plugin-name>
sonic chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run sonic plugins enable <plugin-name>
uv run sonic chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["sonic_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/sonic-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Sonic expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Sonic treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Sonic preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Sonic does not convert downstream
