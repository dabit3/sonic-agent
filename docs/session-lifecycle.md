...
The canonical transcript store is SQLite via `SessionDB` (from `sonic_state`). The
...
Counts consecutive restarts via a JSON file (`{SONIC_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `sonic update`, `sonic gateway restart`,
or `/restart`.
