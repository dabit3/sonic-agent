...
When running Sonic inside Docker, the default `network_mode: host` gives the
...
The Sonic [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   SONIC_UID=$(id -u) SONIC_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=sonic,sonic-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://sonic-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Sonic trust model and vulnerability reporting
