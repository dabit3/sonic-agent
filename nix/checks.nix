...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        bundled-locales = pkgs.runCommand "sonic-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${sonic-agent}/share/sonic-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${sonic-agent}/share/sonic-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${sonic-agent}/share/sonic-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SONIC_BUNDLED_LOCALES" ${sonic-agent}/bin/sonic || \
            (echo "FAIL: SONIC_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SONIC_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (SONIC_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SONIC_BUNDLED_LOCALES=${sonic-agent}/share/sonic-agent/locales \
            ${sonicVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SONIC_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${sonicVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${sonicVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without SONIC_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        messaging-variant = pkgs.runCommand "sonic-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.sonicVenv}/bin/python3 -c \
