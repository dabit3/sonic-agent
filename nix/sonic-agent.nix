# nix/sonic-agent.nix — Overridable Sonic Agent package
#
...
let
  nodejs = nodejs_22;
  sonicVenv = callPackage ./python.nix {
    inherit uv2nix pyproject-nix pyproject-build-systems;
    dependency-groups = [ "all" ] ++ extraDependencyGroups;
  };

  sonicNpmLib = callPackage ./lib.nix {
    inherit npm-lockfile-fix nodejs;
  };

  sonicTui = callPackage ./tui.nix {
    inherit sonicNpmLib;
  };

  sonicWeb = callPackage ./web.nix {
    inherit sonicNpmLib;
  };
  ...
  # to a dev checkout — the loader reads them from SONIC_BUNDLED_PLUGINS.
  ...
  # at by SONIC_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${sonicVenv}/${sitePackagesPath}')
        ...
        for di in sp.glob('*.dist-info'):
            ...
            for line in meta.read_text().splitlines():
                if line.startswith('Name:'):
                    ...
                    if pkg in core:
                        print(f'ERROR: plugin package \"{pkg}\" collides with a package in sonic sealed venv', file=sys.stderr)
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sonic-agent $out/bin
    cp -r ${bundledSkills} $out/share/sonic-agent/skills
    cp -r ${bundledPlugins} $out/share/sonic-agent/plugins
    cp -r ${bundledLocales} $out/share/sonic-agent/locales
    cp -r ${sonicWeb} $out/share/sonic-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${sonicTui}/lib/sonic-tui/* $out/ui-tui/

    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${sonicVenv}/bin/${name} $out/bin/${name} \
          --suffix PATH : "${runtimePath}" \
          --set SONIC_BUNDLED_SKILLS $out/share/sonic-agent/skills \
          --set SONIC_BUNDLED_PLUGINS $out/share/sonic-agent/plugins \
          --set SONIC_BUNDLED_LOCALES $out/share/sonic-agent/locales \
          --set SONIC_WEB_DIST $out/share/sonic-agent/web_dist \
          --set SONIC_TUI_DIR $out/ui-tui \
          --set SONIC_PYTHON ${sonicVenv}/bin/python3 \
          --set SONIC_NODE ${lib.getExe nodejs} \
          ${lib.optionalString (rev != null) ''--set SONIC_REVISION ${rev} \''}
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${sonicVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru = {
    ...
    devShellHook = ''
      STAMP=".nix-stamps/sonic-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "sonic-agent: installing Python dependencies..."
