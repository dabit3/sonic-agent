...
load_sonic_dotenv(
    sonic_home=_sonic_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.sonic/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_sonic_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("SONIC_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("SONIC_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("SONIC_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent SonicCLI subprocess for slash commands."""
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from sonic_cli.active_sessions import transfer_active_session
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `sonic --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("SONIC_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `sonic --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from sonic_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its SONIC_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_sonic_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s SONIC_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_sonic_home``).
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `sonic` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # SONIC_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_sonic_home_override(profile_home)
                try:
                    from sonic_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from sonic_cli.runtime_provider import canonical_custom_identity
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _sonic_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_sonic_home_override()
        home = override if isinstance(override, str) and override else _sonic_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    sonic_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def resolve_skin() -> dict:
    try:
        from sonic_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("SONIC_MODEL", "")
        or os.environ.get("SONIC_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("SONIC_TUI_PROVIDER", "").strip()
    ...
    try:
        from sonic_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("SONIC_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("SONIC_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("SONIC_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Sonic): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `sonic --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] SONIC_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from sonic_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown SONIC_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in SONIC_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid SONIC_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid SONIC_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from sonic_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (SONIC_MODEL /
    # SONIC_INFERENCE_MODEL / SONIC_TUI_PROVIDER / SONIC_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # SONIC_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("SONIC_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from sonic_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from sonic_cli.banner import get_update_result
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from sonic_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("SONIC_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("SONIC_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses sonic_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("SONIC_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("SONIC_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("SONIC_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("SONIC_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind SONIC_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `SONIC_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_sonic_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds SONIC_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``sonic --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``sonic gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Sonic has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from sonic_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from sonic_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sonic_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sonic_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_sonic_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("SONIC_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
# (agent/billing_view.py + sonic_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sonic_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sonic_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from sonic_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Sonic profile home
    # (~/.sonic/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $SONIC_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn SONIC_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_sonic_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from sonic_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from sonic_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: sonic_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from sonic_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Sonic Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Sonic/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Sonic Desktop. If it serves Sonic/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from sonic_cli.model_switch import parse_model_flags
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from sonic_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_sonic_home), "display": display_sonic_home()})
    if key == "project":
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Sonic provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.sonic/.env`` into the gateway process via
    ``sonic_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `sonic` is interactive — use `/sonic chat -q …` or run `sonic` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`sonic setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`sonic gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`sonic sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`sonic config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m sonic_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "sonic_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from sonic_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from sonic_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from sonic_cli.config import is_managed, save_env_value
        from sonic_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `sonic model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.sonic/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from sonic_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from sonic_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("SONIC_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("SONIC_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["SONIC_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from sonic_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["SONIC_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from sonic_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from sonic_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from sonic_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from sonic_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("SONIC_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SONIC_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_sonic_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from sonic_cli.skills_hub import do_install
        ...
        if action == "browse":
            from sonic_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from sonic_cli.skills_hub import inspect_skill
