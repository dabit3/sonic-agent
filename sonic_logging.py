"""Centralized logging setup for Sonic Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.sonic/logs/`` (profile-aware via ``get_sonic_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``sonic_cli.web_server``,
    ``sonic_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Sonic (TUI, gateway, ``hy_memory`` server, MCP
...
from sonic_constants import get_config_path, get_sonic_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_sonic_session_injector", False):
    ...
    _session_record_factory._sonic_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``sonic logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``sonic logs --component gateway``.
    "gateway": ("gateway", "sonic_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    ...
    """Configure the Sonic logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    sonic_home
        Override for the Sonic home directory.  Falls back to
        ``get_sonic_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_sonic_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._sonic_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the sonic group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from sonic_cli.config import is_managed
