...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("lightning/lightning-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned lightning/* and pr-* branches."""

    def test_prunes_orphaned_lightning_branch(self, git_repo):
        """lightning/lightning-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "lightning/lightning-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "lightning/lightning-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "lightning/lightning-deadbeef" in result.stdout
        ...
        assert "lightning/lightning-deadbeef" in orphaned
        ...
        assert "lightning/lightning-deadbeef" not in result.stdout
