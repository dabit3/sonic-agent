...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("sonic/sonic-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned sonic/* and pr-* branches."""

    def test_prunes_orphaned_sonic_branch(self, git_repo):
        """sonic/sonic-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "sonic/sonic-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "sonic/sonic-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "sonic/sonic-deadbeef" in result.stdout
        ...
        assert "sonic/sonic-deadbeef" in orphaned
        ...
        assert "sonic/sonic-deadbeef" not in result.stdout
