...
minimal ``LightningCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def lightning_home(tmp_path, monkeypatch):
    """Isolated LIGHTNING_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".lightning"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("LIGHTNING_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves LIGHTNING_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal LightningCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, lightning_home):
        ...
        with patch("lightning_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, lightning_home):
        ...
        with patch("lightning_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, lightning_home):
    ...
    def test_no_assistant_message_skipped(self, lightning_home):
...
class TestHealthyTurnStillRuns:
    def test_clean_response_enqueues_continuation_when_judge_says_continue(
        ...
        with patch(
            "lightning_cli.goals.judge_goal",
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, lightning_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, lightning_home):
        ...
        src = inspect.getsource(LightningCLI.chat)
