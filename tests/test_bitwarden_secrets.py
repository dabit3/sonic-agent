...
binary download are exercised manually by `sonic secrets bitwarden
...
def sonic_home(tmp_path, monkeypatch):
    """Point Sonic at an isolated home directory."""
    home = tmp_path / ".sonic"
    home.mkdir()
    monkeypatch.setenv("SONIC_HOME", str(home))
    # Some modules cache get_sonic_home; clear if needed.
    import sonic_constants
    if hasattr(sonic_constants, "_SONIC_HOME_CACHE"):
        sonic_constants._SONIC_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(sonic_home, monkeypatch):
...
def test_install_bws_happy_path(sonic_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(sonic_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(sonic_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from sonic_cli.env_loader import _apply_external_secret_sources
