...
binary download are exercised manually by `lightning secrets bitwarden
...
def lightning_home(tmp_path, monkeypatch):
    """Point Lightning at an isolated home directory."""
    home = tmp_path / ".lightning"
    home.mkdir()
    monkeypatch.setenv("LIGHTNING_HOME", str(home))
    # Some modules cache get_lightning_home; clear if needed.
    import lightning_constants
    if hasattr(lightning_constants, "_LIGHTNING_HOME_CACHE"):
        lightning_constants._LIGHTNING_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(lightning_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(lightning_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(lightning_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from lightning_cli.env_loader import _apply_external_secret_sources
