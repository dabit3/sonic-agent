...
from lightning_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(lightning_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <lightning_home>/plugins/<name> and opt it in."""
    plugin_dir = lightning_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    lightning_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        lightning_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LIGHTNING_HOME", str(tmp_path / "lightning_empty"))
