...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://lightning-agent.nousresearch.com"
    assert headers["X-Title"] == "Lightning Agent"
    assert headers["User-Agent"].startswith("LightningAgent/")
...
def test_nvidia_cloud_base_url_applies_billing_origin_header(mock_openai):
    ...
    assert agent._client_kwargs["default_headers"]["X-BILLING-INVOKE-ORIGIN"] == "LightningAgent"
...
def test_routed_client_preserves_openai_sdk_custom_headers(mock_openai):
    ...
    routed_client = SimpleNamespace(
        api_key="test-key",
        base_url="https://integrate.api.nvidia.com/v1",
        _custom_headers={"X-BILLING-INVOKE-ORIGIN": "LightningAgent"},
    )
