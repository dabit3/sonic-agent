...
AlexKucera reported on Discord (2026-04-16) that after ``lightning update`` pulled
...
Opt-in — not part of default CI:
    LIGHTNING_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.lightning/.env).
"""
...
# Load ~/.lightning/.env so live runs pick up OPENROUTER_API_KEY without
# needing the runner to shell-source it first. Silent if the file is absent.
def _load_user_env() -> None:
    env_file = Path.home() / ".lightning" / ".env"
...
LIVE = os.environ.get("LIGHTNING_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set LIGHTNING_LIVE_TESTS=1"),
