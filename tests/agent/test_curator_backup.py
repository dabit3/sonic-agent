...
def backup_env(monkeypatch, tmp_path):
    """Isolate SONIC_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".sonic"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_sonic_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd sonic_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under SONIC_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level SONIC_DIR picks up the tmp HOME."""
    import sonic_constants
    importlib.reload(sonic_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.sonic/cron/jobs.json
