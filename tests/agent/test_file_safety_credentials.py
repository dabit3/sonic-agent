"""Tests for LIGHTNING_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/lightning-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``LIGHTNING_HOME`` itself,
...
for the credential stores while leaving arbitrary ``LIGHTNING_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_lightning_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "lightning_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_lightning_home_path", lambda: home)
    return home
...
def test_arbitrary_lightning_home_file_not_blocked(fake_home):
    """Non-credential files inside LIGHTNING_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Lightning cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into LIGHTNING_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "lightning_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at LIGHTNING_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == LIGHTNING_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at LIGHTNING_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in LIGHTNING_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_files_outside_lightning_home_not_blocked(
    fake_home, tmp_path
):
    """A project's ``.env``, ``auth.json``, or ``mcp-tokens/`` outside
    LIGHTNING_HOME must remain readable — the gate is per-location, not
    per-filename."""
    from agent.file_safety import get_read_block_error
    ...
    for rel in (".env", "auth.json"):
        ...
        assert get_read_block_error(str(p)) is None, (
            f"{rel} outside LIGHTNING_HOME should NOT be blocked"
        )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, LIGHTNING_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_lightning_home_path", lambda: profile)
    monkeypatch.setattr(fs, "_lightning_root_path", lambda: root)
