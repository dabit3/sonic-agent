...
thinking phase.  Sonic's default cloud-stream stale detector
(``SONIC_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``SONIC_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_reasoning_floor_applies_to_nemotron_3_ultra(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("SONIC_API_CALL_STALE_TIMEOUT", raising=False)
    _write_config(tmp_path, "")

    # Clear any cached config from prior tests in this session.
    import importlib
    from sonic_cli import config as cfg_mod, timeouts as to_mod
...
def test_reasoning_floor_loses_to_env_var_when_no_floor_match(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    (tmp_path / ".env").write_text("", encoding="utf-8")
    monkeypatch.setenv("SONIC_API_CALL_STALE_TIMEOUT", "300")
    _write_config(tmp_path, "")
