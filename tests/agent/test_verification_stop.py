...
def clear_verify_env(monkeypatch):
    ...
    set SONIC_SESSION_SOURCE and the gateway sets SONIC_SESSION_PLATFORM.
    """
    for var in (
        "SONIC_VERIFY_ON_STOP",
        "SONIC_PLATFORM",
...
def test_verify_on_stop_env_can_disable(clear_verify_env):
    clear_verify_env.setenv("SONIC_VERIFY_ON_STOP", "0")
...
def test_verify_on_stop_auto_uses_sonic_platform_override(clear_verify_env):
...
def test_ad_hoc_pass_satisfies_no_suite_stop_loop(tmp_path, monkeypatch):
    ...
    (tmp_path / "package.json").write_text("{}", encoding="utf-8")
    changed = str(tmp_path / "src" / "app.ts")
    script = Path(tempfile.gettempdir()) / f"sonic-ad-hoc-stop-{tmp_path.name}.py"
