"""Tests for the cross-Sonic-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Sonic profile's
...
Reference: May 2026 incident — a sonic-security profile session
accidentally edited skills under both ~/.sonic/profiles/sonic-security/skills/
AND ~/.sonic/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Sonic root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_sonic(tmp_path, monkeypatch):
    """Build a fake Sonic layout:
    ...
    sec_home = root / "profiles" / "sonic-security"
    ...
    monkeypatch.setattr(fs, "_sonic_root_path", lambda: root)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "sonic-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_sonic, monkeypatch):
        """If SONIC_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_sonic_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_sonic["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_sonic, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_sonic["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "sonic-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_sonic, monkeypatch):
        ...
        assert result["target_profile"] == "sonic-security"

    def test_named_to_named_cross_profile(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_sonic["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_sonic, monkeypatch, area):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_sonic["default_home"] / area / "foo.txt"
    ...
    def test_hermes_config_not_classified_as_cross_profile(self, fake_sonic, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_sonic["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "sonic-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_sonic, monkeypatch):
        _set_active_home(monkeypatch, fake_sonic["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_sonic["default_home"] / "skills" / "foo" / "SKILL.md")
