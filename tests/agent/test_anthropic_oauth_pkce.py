...
History:
  - PR #1775 first fixed this on ``run_sonic_oauth_login()``.
  - PR #2647 (b17e5c10) added ``run_sonic_oauth_login_pure()`` and silently
...
def _patch_oauth_flow(
    ...
    *,
    ...
    """Wire up monkeypatches that let ``run_sonic_oauth_login_pure()`` run
    ...
    monkeypatch.setattr(
        "sonic_cli.auth._can_open_graphical_browser", lambda: True
    )
