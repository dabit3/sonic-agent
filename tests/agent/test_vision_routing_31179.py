...
def isolated_home(monkeypatch):
    """Temp SONIC_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="sonic_test_31179_")
    sonic_home = os.path.join(test_home, ".sonic")
    os.makedirs(sonic_home)
...
def _fresh_modules():
    """Drop cached sonic modules so each test reloads against current env."""
