...
to a generic "Your API key was rejected... run sonic setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "sonic setup".
    assert "sonic auth add nous --type oauth" in source
