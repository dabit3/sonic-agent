...
""",
        ...
        from sonic_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        (sonic_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        from sonic_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from sonic_cli.plugins import PluginContext, PluginManifest, PluginManager
