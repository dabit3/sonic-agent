...
""",
        ...
        from lightning_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        lightning_home = tmp_path / ".lightning"
        lightning_home.mkdir()
        (lightning_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
        from lightning_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from lightning_cli.plugins import PluginContext, PluginManifest, PluginManager
