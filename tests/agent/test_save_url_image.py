...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SONIC_HOME under tmp_path."""
    monkeypatch.setenv("SONIC_HOME", str(tmp_path / ".sonic"))
    (tmp_path / ".sonic").mkdir()

    # Force the constants/image cache helpers to re-read SONIC_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("sonic_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_sonic_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SONIC_HOME — gateway cleanup
