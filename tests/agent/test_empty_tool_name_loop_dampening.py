...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SONIC_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="sonic_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".sonic"))
    prev_home = os.environ.get("SONIC_HOME")
    os.environ["SONIC_HOME"] = os.path.join(test_home, ".sonic")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("sonic_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SONIC_HOME"] = prev_home
