...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_sonic_env(self):
        ...
        # even ~/.sonic/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.sonic/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Sonic cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        sonic_home = tmp_path / ".sonic"
        cache = sonic_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._sonic_home_path", return_value=sonic_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Sonic cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        sonic_home = tmp_path / ".sonic"
        hub = sonic_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_sonic_home(self, tmp_path):
        """The env basename guard does not depend on SONIC_HOME resolution."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        sonic_home = tmp_path / ".sonic"
        cache = sonic_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
