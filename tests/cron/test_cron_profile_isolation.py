...
profile's SONIC_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.sonic/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active SONIC_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever SONIC_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    SONIC_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import sonic_constants

    monkeypatch.setattr(
        sonic_constants, "_get_platform_default_sonic_home", lambda: root
    )
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    # cron/jobs.py computes SONIC_DIR from get_sonic_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.SONIC_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_sonic_home", None, raising=False)
    assert scheduler._get_sonic_home().resolve() == profile_home.resolve()
    assert scheduler._get_sonic_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (SONIC_HOME == root), the store is the root's cron dir
