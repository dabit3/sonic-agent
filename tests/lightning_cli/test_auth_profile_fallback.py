...
When ``LIGHTNING_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.lightning/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.lightning            (has its own auth.json fixture)
    * Profile     -> tmp_path/.lightning/profiles/coder   (active, LIGHTNING_HOME points here)
    ...
    global_root = tmp_path / ".lightning"
    global_root.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (LIGHTNING_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.lightning as the "real home" — which is NOT equal
    # to the LIGHTNING_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    lightning_home = tmp_path / "classic"
    lightning_home.mkdir()
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))

    _write(lightning_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from lightning_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: LIGHTNING_HOME is set to a custom path that is NOT under
    # ~/.lightning/profiles/ — get_default_lightning_root() returns LIGHTNING_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from lightning_cli.auth import read_credential_pool, write_credential_pool
