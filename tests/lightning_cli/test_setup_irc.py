"""Tests for IRC gateway configuration via `lightning setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Lightning install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Lightning install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#lightning")
            monkeypatch.setenv("IRC_NICKNAME", "lightning-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `lightning setup gateway` experience with IRC present."""
