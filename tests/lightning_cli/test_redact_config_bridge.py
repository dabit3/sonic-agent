...
`LIGHTNING_REDACT_SECRETS` at module-import time. `lightning_cli/main.py` at
...
in .env), the toggle was silently ignored in both `lightning chat` and
`lightning gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `LIGHTNING_REDACT_SECRETS`
env var in `lightning_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports lightning_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("LIGHTNING_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import lightning_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('LIGHTNING_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["LIGHTNING_HOME"] = str(lightning_home)
    env.pop("LIGHTNING_REDACT_SECRETS", None)
...
def test_redact_secrets_default_true_when_unset(tmp_path):
    ...
    `LIGHTNING_REDACT_SECRETS=false`).
    """
    lightning_home = tmp_path / ".lightning"
    lightning_home.mkdir()
    (lightning_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env LIGHTNING_REDACT_SECRETS takes precedence over config.yaml."""
    lightning_home = tmp_path / ".lightning"
    lightning_home.mkdir()
    (lightning_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (lightning_home / ".env").write_text("LIGHTNING_REDACT_SECRETS=true\n")
