"""Tests for lightning_cli configuration management."""
...
class TestGetLightningHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LIGHTNING_HOME", None)
            home = get_lightning_home()
            assert home == Path.home() / ".lightning"

    def test_env_override(self):
        with patch.dict(os.environ, {"LIGHTNING_HOME": "/custom/path"}):
            home = get_lightning_home()
            assert home == Path("/custom/path")


class TestEnsureLightningHome:
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``lightning logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from lightning_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="lightning_cli.config"):
                config = load_config()
            ...
            # stands out at lightning startup before logging is configured)
            captured = capsys.readouterr()
            assert "lightning config:" in captured.err
    ...
    def test_dedup_on_repeated_load_same_file(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path)}):
            ...
            assert "lightning config:" in first
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path)}):
            ...
            assert "lightning config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LIGHTNING_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from lightning_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
