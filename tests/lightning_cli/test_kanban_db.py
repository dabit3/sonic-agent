"""Tests for the Kanban DB layer (lightning_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated LIGHTNING_HOME with an empty kanban DB."""
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("LIGHTNING_HOME", str(home))
    monkeypatch.delenv("LIGHTNING_KANBAN_DB", raising=False)
    monkeypatch.delenv("LIGHTNING_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("LIGHTNING_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import lightning_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from lightning_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Lightning profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="lightning-task", assignee="daily")
...
def test_scratch_workspace_created_under_lightning_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `lightning -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's LIGHTNING_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's LIGHTNING_HOME."""

    def _set_home(self, monkeypatch, tmp_path, lightning_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
        monkeypatch.delenv("LIGHTNING_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_lightning(
        self, tmp_path, monkeypatch
    ):
        # Standard install: LIGHTNING_HOME == ~/.lightning, no profile active.
        default_home = tmp_path / ".lightning"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.lightning/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.lightning/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.lightning/kanban.db.
        default_home = tmp_path / ".lightning"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # LIGHTNING_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `lightning -p coder`).
    ...
    def test_docker_custom_lightning_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: LIGHTNING_HOME points outside ~/.lightning.
        # `get_default_lightning_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".lightning"`.
        custom_root = tmp_path / "opt" / "lightning"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: LIGHTNING_HOME=/opt/lightning/profiles/coder;
        # `get_default_lightning_root()` walks up to /opt/lightning because
    ...
    def test_explicit_override_via_lightning_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: LIGHTNING_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".lightning"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(profile_home))
        monkeypatch.setenv("LIGHTNING_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(default_home))
        monkeypatch.setenv("LIGHTNING_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # LIGHTNING_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile LIGHTNING_HOME and reads.
        monkeypatch.setenv("LIGHTNING_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_lightning_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # LIGHTNING_KANBAN_DB pins the file path directly and beats both
        # LIGHTNING_KANBAN_HOME and the `get_default_lightning_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(default_home))
        monkeypatch.setenv("LIGHTNING_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("LIGHTNING_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows LIGHTNING_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_lightning_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # LIGHTNING_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".lightning"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("LIGHTNING_HOME", str(default_home))
        monkeypatch.setenv("LIGHTNING_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("LIGHTNING_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows LIGHTNING_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(default_home))
        monkeypatch.setenv("LIGHTNING_KANBAN_DB", "   ")
        monkeypatch.setenv("LIGHTNING_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject LIGHTNING_KANBAN_DB
        # and LIGHTNING_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites LIGHTNING_HOME.
        default_home = tmp_path / ".lightning"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["LIGHTNING_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["LIGHTNING_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["LIGHTNING_KANBAN_TASK"] == "t_dispatch_env"
        assert env["LIGHTNING_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see lightning_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    (see lightning-agent issue #22032).
    """
    ...
    with _patch("lightning_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="lightning_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'lightning kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_lightning_argv()
#
# Workers spawned by the dispatcher must use a `lightning` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["lightning", ...]`
...
def test_resolve_lightning_argv_prefers_path_shim(monkeypatch):
    """When `lightning` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import lightning_cli.kanban_db as kb

    monkeypatch.delenv("LIGHTNING_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/lightning")
    argv = kb._resolve_lightning_argv()
    assert argv == ["/usr/local/bin/lightning"]


def test_resolve_lightning_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import lightning_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("LIGHTNING_BIN", ".\\lightning.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lightning_argv() == [os.path.abspath(".\\lightning.exe")]


def test_resolve_lightning_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "lightning.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("LIGHTNING_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_lightning_argv() == [sys.executable, "-m", "lightning_cli.main"]


def test_resolve_lightning_argv_honors_lightning_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like LIGHTNING_BIN lets service managers pin the executable."""
    import shutil
    import lightning_cli.kanban_db as kb

    shim = tmp_path / "bin" / "lightning"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("LIGHTNING_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_lightning_argv() == [str(shim)]


def test_resolve_lightning_argv_lightning_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare LIGHTNING_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import lightning_cli.kanban_db as kb

    cwd_lightning = tmp_path / "lightning"
    cwd_lightning.write_text("wrong\n", encoding="utf-8")
    cwd_lightning.chmod(cwd_lightning.stat().st_mode | stat.S_IXUSR)
    path_lightning = tmp_path / "bin" / "lightning"
    path_lightning.parent.mkdir()
    path_lightning.write_text("right\n", encoding="utf-8")
    path_lightning.chmod(path_lightning.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_lightning.parent))
    monkeypatch.setenv("LIGHTNING_BIN", "lightning")

    assert kb._resolve_lightning_argv() == [str(path_lightning)]


def test_resolve_lightning_argv_lightning_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare LIGHTNING_BIN does not accept current-directory shadow executables."""
    import sys
    import lightning_cli.kanban_db as kb

    (tmp_path / "lightning.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_lightning_argv_lightning_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved LIGHTNING_BIN batch shim is not used as worker argv[0]."""
    import sys
    import lightning_cli.kanban_db as kb
...
def test_resolve_lightning_argv_lightning_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved LIGHTNING_BIN command names do not delegate cwd search to Popen."""
    import sys
    import lightning_cli.kanban_db as kb
...
def test_resolve_lightning_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m lightning_cli.main`.

    Pins the correct module name (NOT `lightning` — there is no top-level
    `lightning` package). Regression for #23198: the original PR shipped
    `python -m lightning` which fails with `No module named lightning` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_lightning_argv()
    assert argv == [sys.executable, "-m", "lightning_cli.main"]


def test_resolve_lightning_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `lightning_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m lightning_cli.main --version`
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("LIGHTNING_BIN", None)
    ...
    assert "Lightning Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="lightning_cli.kanban_db"):
        with kb.connect() as conn:
