"""Unit tests for lightning_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf lightning-ok"])
        try:
            output = _read_until(bridge, b"lightning-ok")
            assert b"lightning-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$LIGHTNING_PTY_TEST\""],
            env={**os.environ, "LIGHTNING_PTY_TEST": "pty-env-works"},
        )
        try:
