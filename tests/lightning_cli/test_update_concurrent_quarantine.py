"""Tests for issue #26670 — concurrent lightning.exe detection and improved
quarantine retry / reboot-deferred fallback during `lightning update` on Windows.
...
from lightning_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_lightning_instances
# helper (and need the autouse stub in tests/lightning_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_lightning_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "lightning.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "lightning.exe").write_bytes(b"")
    (scripts_dir / "lightning-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_lightning_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "lightning.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_lightning_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "lightning.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "lightning.exe"),
    ...
    assert result == [(other_pid, "lightning.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning lightning.EXE (uppercase ext) from same path
    upper = str(shim).replace("lightning.exe", "LIGHTNING.EXE")
    procs = [_make_proc(9999, upper, "LIGHTNING.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_lightning_instances(scripts_dir)

    assert result == [(9999, "LIGHTNING.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_lightning_instances(tmp_path) == []
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "lightning.exe"), (5678, "lightning.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "lightning.exe" in msg
    assert "Lightning Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "lightning.exe") in msg


# ---------------------------------------------------------------------------
# _quarantine_running_lightning_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "lightning.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_lightning_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("lightning.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_lightning_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Lightning Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another lightning.exe is running, the update bails out before
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_lightning_instances",
        return_value=[(4242, "lightning.exe")],
    ), patch.object(
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "lightning.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_lightning_instances", detect
    ), patch.object(
