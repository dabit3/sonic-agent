"""Tests for the ``lightning send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``lightning_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from lightning_cli import send_cmd
...
def test_load_lightning_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``lightning send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``lightning config set``) lives
    ...
    (lightning_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_lightning_home() to re-resolve under the patched env.
    from importlib import reload

    import lightning_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_lightning_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_lightning_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    lightning_home = tmp_path / ".lightning"
    lightning_home.mkdir()
    (lightning_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_lightning_env_handles_missing_files(tmp_path, monkeypatch):
