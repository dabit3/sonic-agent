...
Before the fix, ``lightning whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``lightning gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    lightning = home / ".lightning"
    lightning.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning))
...
def _env_value(lightning_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("lightning_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``lightning whatsapp`` with an existing paired session and
