...
``lightning_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``lightning --help``, ``lightning version``,
``lightning logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``lightning --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from lightning_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["lightning", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["lightning", "-w", "--tui"], None),
        (["lightning", "version"], "version"),
        (["lightning", "--tui", "chat"], "chat"),
        (["lightning", "-w", "logs"], "logs"),
        (["lightning", "chat", "hello world"], "chat"),
        (["lightning", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["lightning", "-m", "gpt5", "chat"], "chat"),
        (["lightning", "--model", "gpt5", "chat", "hi"], "chat"),
        (["lightning", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["lightning", "-z", "hello world"], None),
        (["lightning", "-z", "hello", "chat"], "chat"),
        (["lightning", "--model=gpt5", "chat"], "chat"),     # inline form
        (["lightning", "--", "chat"], "chat"),               # -- terminator
        (["lightning", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["lightning", "some-plugin-cmd"], "some-plugin-cmd"),
        (["lightning", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["lightning"],                          # bare → chat
        ["lightning", "--help"],                # top-level help
        ["lightning", "-h"],
        ["lightning", "version"],               # known built-in
        ["lightning", "logs"],
        ["lightning", "gateway", "run"],
        ["lightning", "--tui"],
        ["lightning", "-w", "--tui"],
        ["lightning", "chat", "hi"],
        ["lightning", "help"],                  # accepted built-in-ish
        ["lightning", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["lightning", "meet", "join"],          # potential google_meet plugin
        ["lightning", "honcho", "status"],      # potential memory plugin
        ["lightning", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``lightning help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"lightning_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
