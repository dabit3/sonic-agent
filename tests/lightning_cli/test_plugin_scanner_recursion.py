...
from lightning_cli.plugins import PluginManager, PluginManifest
...
def _enable(lightning_home: Path, name: str) -> None:
    """Append ``name`` to ``plugins.enabled`` in ``<lightning_home>/config.yaml``."""
...
class TestCategoryNamespaceRecursion:
    def test_category_namespace_discovered(self, tmp_path, monkeypatch):
        ...
        import os
        lightning_home = Path(os.environ["LIGHTNING_HOME"])  # set by hermetic conftest fixture
        user_plugins = lightning_home / "plugins"

        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(lightning_home, "image_gen/openai")
    ...
    def test_flat_plugin_key_matches_name(self, tmp_path, monkeypatch):
        ...
        _enable(lightning_home, "my-plugin")
    ...
    def test_category_dir_with_manifest_is_leaf(self, tmp_path, monkeypatch):
        ...
        # at the parent.
        _write_plugin(user_plugins, ["image_gen", "openai"])
        _enable(lightning_home, "image_gen")
        _enable(lightning_home, "image_gen/openai")
...
class TestKindField:
    def test_default_kind_is_standalone(self, tmp_path, monkeypatch):
        import os
        lightning_home = Path(os.environ["LIGHTNING_HOME"])  # set by hermetic conftest fixture
        _write_plugin(lightning_home / "plugins", ["p1"])
        _enable(lightning_home, "p1")
...
class TestBackendGate:
    ...
    def test_user_backend_loads_when_enabled(self, tmp_path, monkeypatch):
        ...
        _enable(lightning_home, "image_gen/fancy")
    ...
    def test_exclusive_kind_skipped(self, tmp_path, monkeypatch):
        ...
        _enable(lightning_home, "some-backend")
...
class TestRegisterImageGenProvider:
    def test_accepts_valid_provider(self, tmp_path, monkeypatch):
        ...
        _enable(lightning_home, "my-img-plugin")
    ...
    def test_rejects_non_provider(self, tmp_path, monkeypatch, caplog):
        ...
        _enable(lightning_home, "bad-img-plugin")
