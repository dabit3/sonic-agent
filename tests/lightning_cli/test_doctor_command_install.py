"""Tests for the Command Installation check in lightning doctor."""
...
import lightning_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal LIGHTNING_HOME + PROJECT_ROOT for doctor tests."""
    ...
    lightning_bin = venv_bin_dir / "lightning"
    lightning_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    lightning_bin.chmod(0o755)
    ...
    return home, project, lightning_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, lightning_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "lightning"
        cmd_link.symlink_to(lightning_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "lightning doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "lightning"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == lightning_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "lightning"
        wrong_target = tmp_path / "wrong_lightning"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        lightning_bin = project / ".venv" / "bin" / "lightning"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "lightning"
        cmd_link.symlink_to(lightning_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/lightning" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/lightning is a regular file (not symlink), accept it."""
        home, project, lightning_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "lightning"
        cmd_link.write_text("#!/bin/sh\nexec python -m lightning_cli.main \"$@\"\n")
