...
The notice prints the most recent curator run summary on `lightning update`,
...
session start) so users normally never see the rename map. `lightning update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    importlib.reload(lightning_main)
