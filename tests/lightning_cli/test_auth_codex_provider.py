"""Tests for Codex auth — tokens stored in Lightning auth store (~/.lightning/auth.json)."""
...
def _setup_lightning_auth(lightning_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Lightning auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_lightning_auth(lightning_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_lightning_auth(lightning_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
    ...
    monkeypatch.setattr("lightning_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    lightning_home = tmp_path / "lightning"
    _setup_lightning_auth(lightning_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Lightning auth store, not ~/.codex/."""
    lightning_home = tmp_path / "lightning"
    codex_home = tmp_path / "codex-cli"
    lightning_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "lightning-at", "refresh_token": "lightning-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Lightning store
    assert not (codex_home / "auth.json").exists()

    # Lightning auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "lightning-at"


def test_resolve_returns_lightning_auth_store_source(tmp_path, monkeypatch):
    lightning_home = tmp_path / "lightning"
    _setup_lightning_auth(lightning_home)
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "lightning_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("lightning_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("lightning_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
