...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LIGHTNING_HOME with .container-mode file."""
    lightning_home = tmp_path / ".lightning"
    lightning_home.mkdir()
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
    monkeypatch.delenv("LIGHTNING_DEV", raising=False)

    container_mode = lightning_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=lightning-agent\n"
        "exec_user=lightning\n"
        "lightning_bin=/data/current-package/bin/lightning\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("lightning_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "lightning-agent"
    assert info["exec_user"] == "lightning"
    assert info["lightning_bin"] == "/data/current-package/bin/lightning"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("lightning_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_lightning_dev(container_env, monkeypatch):
    """Returns None when LIGHTNING_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("LIGHTNING_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_lightning_dev_zero(container_env, monkeypatch):
    """LIGHTNING_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("LIGHTNING_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        lightning_home = Path(tmpdir) / ".lightning"
        lightning_home.mkdir()
        (lightning_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("lightning_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_lightning_home": lambda: lightning_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LIGHTNING_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=lightning-custom\n"
        "exec_user=myuser\n"
        "lightning_bin=/opt/lightning/bin/lightning\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "lightning-custom"
    assert info["exec_user"] == "myuser"
    assert info["lightning_bin"] == "/opt/lightning/bin/lightning"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from lightning_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "lightning"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "lightning-agent" in cmd
    assert "/data/current-package/bin/lightning" in cmd
    assert "chat" in cmd
