"""Tests for the top-level `./lightning` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./lightning` should use `lightning_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "lightning"
    called = []

    fake_main_module = types.ModuleType("lightning_cli.main")

    def fake_main():
        called.append("lightning_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "lightning_cli.main", fake_main_module)
    ...
    assert called == ["lightning_cli.main"]
