...
def test_tui_finds_bundled_entry_js(tmp_path):
    ...
    tui_dist = tmp_path / "lightning_cli" / "tui_dist"
    tui_dist.mkdir(parents=True)
    entry = tui_dist / "entry.js"
    entry.write_text("// bundled TUI", encoding="utf-8")

    from lightning_cli.main import _find_bundled_tui
    result = _find_bundled_tui(lightning_cli_dir=tmp_path / "lightning_cli")
