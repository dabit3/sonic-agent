"""Tests for the `lightning memory reset` CLI command.
...
- Profile-scoped reset (uses LIGHTNING_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake LIGHTNING_HOME with memory files."""
    lightning_home = tmp_path / ".lightning"
    memories = lightning_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nLightning repo is at ~/.lightning/lightning-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
    ...
    return lightning_home, memories
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `lightning memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        lightning_home = tmp_path / ".lightning"
        (lightning_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's LIGHTNING_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_lightning_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
