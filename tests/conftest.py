...
_SONIC_BEHAVIORAL_VARS = frozenset({
    ...
    "SONIC_DASHBOARD_OAUTH_CLIENT_ID",
...
def _live_system_guard(request, monkeypatch):
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `sonic update` (or the
        # equivalent `python -m sonic_cli.main update`).  These commands
        ...
        if "update" in low and (
            # sonic update / sonic update --gateway / setsid bash -c ... sonic update
            ("sonic" in low and "update" in low.split())
            or
            # python -m sonic_cli.main update --gateway
            ("sonic_cli" in low and "update" in low.split())
            or
            # venv/bin/sonic update  (absolute path variant used in tests)
            (".venv/bin/sonic" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`sonic update` against the real checkout, fetching "
