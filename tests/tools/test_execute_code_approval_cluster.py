...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SONIC_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SONIC_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("SONIC_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("SONIC_CRON_SESSION", "1")
    monkeypatch.delenv("SONIC_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_sonic_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SONIC_HOME": "/h", "SONIC_PROFILE": "p",
        "SONIC_CONFIG": "/c.yaml", "SONIC_ENV": "/e",
        # other SONIC_* → dropped (broad prefix removed)
        "SONIC_BASE_URL": "https://x", "SONIC_INTERACTIVE": "1",
        "SONIC_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("SONIC_HOME", "SONIC_PROFILE", "SONIC_CONFIG", "SONIC_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SONIC_BASE_URL", "SONIC_INTERACTIVE", "SONIC_KANBAN_DB",
...
def test_env_scrub_logs_dropped_sonic_vars(caplog):
    """Dropping a non-allowlisted, non-secret SONIC_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset SONIC_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "SONIC_HOME": "/h",          # allowlisted → kept, not logged
        "SONIC_BASE_URL": "https://x",   # dropped → logged
        "SONIC_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SONIC_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SONIC_HOME" in out and "PATH" in out
    assert "SONIC_BASE_URL" not in out and "SONIC_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SONIC_BASE_URL" in msgs and "SONIC_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SONIC_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SONIC_* vars."""
    import logging
