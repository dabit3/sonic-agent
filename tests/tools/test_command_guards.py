...
def _clean_state():
    ...
    saved = {}
    for k in ("SONIC_INTERACTIVE", "SONIC_GATEWAY_SESSION", "SONIC_EXEC_ASK", "SONIC_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SONIC_INTERACTIVE"] = "1"
        session_key = os.getenv("SONIC_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SONIC_INTERACTIVE or SONIC_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
