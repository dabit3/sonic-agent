...
def _clean_state():
    ...
    saved = {}
    for k in ("LIGHTNING_INTERACTIVE", "LIGHTNING_GATEWAY_SESSION", "LIGHTNING_EXEC_ASK", "LIGHTNING_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LIGHTNING_INTERACTIVE"] = "1"
        session_key = os.getenv("LIGHTNING_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LIGHTNING_INTERACTIVE or LIGHTNING_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
