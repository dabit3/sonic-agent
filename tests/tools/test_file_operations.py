...
class TestIsWriteDenied:
    ...
    def test_sonic_control_files_oauth_and_mcp_tokens_denied(self, path):
        """Sonic control files, PKCE creds, mcp-tokens, and pairing entries must be write-denied."""
    ...
    def test_sonic_control_files_and_oauth_traversal_denied(self, path):
        """Path traversal attempts to protected Sonic files must be blocked."""
        from sonic_constants import get_sonic_home
        sonic_home = get_sonic_home()
        full_path = str(sonic_home / path)
        assert _is_write_denied(full_path) is True
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.sonic/pairing/{platform}-approved.json as the
