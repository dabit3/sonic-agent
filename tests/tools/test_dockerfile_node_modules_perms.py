...
Older images made /opt/sonic/ui-tui and /opt/sonic/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SONIC_TUI_DIR=/opt/sonic/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R sonic:sonic /opt/sonic/ui-tui" not in text
    assert "chown -R sonic:sonic /opt/sonic/node_modules" not in text
