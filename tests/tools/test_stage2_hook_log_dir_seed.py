"""Contract test: the s6-overlay stage2 hook seeds $SONIC_HOME/logs/gateways
as the sonic user.
...
sonic user and s6-log crash-loops on `mkdir: Permission denied`.

Seeding `logs/gateways` in stage2 (cont-init runs before any service starts)
guarantees the parent already exists sonic-owned by the time the first
...
def _seed_mkdir_block(text: str) -> str:
    """Extract the `as_sonic mkdir -p \\ ...` seed block."""
    m = re.search(r"as_sonic mkdir -p \\\n(?:[^\n]*\\\n)*[^\n]*\n", text)
    assert m, "stage2-hook.sh must contain the as_sonic mkdir -p seed block"
    return m.group(0)


def test_logs_gateways_is_seeded(stage2_text: str) -> None:
    block = _seed_mkdir_block(stage2_text)
    assert '"$SONIC_HOME/logs/gateways"' in block, (
        "logs/gateways must be seeded sonic-owned in stage2 so profiles "
    ...
    assert '"$SONIC_HOME/logs"' in block
