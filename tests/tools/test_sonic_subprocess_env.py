"""Tests for sonic_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_SONIC_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
_TIER1_SAMPLE = {
    ...
    "SONIC_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return sonic_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestStripByDefault:
    ...
    def test_force_prefix_hints_stripped(self):
        result = _build({f"{_SONIC_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-x"})
        assert f"{_SONIC_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result
