"""Contract tests for the Docker image's immutable /opt/sonic install tree."""
...
def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    ...
    assert "ENV SONIC_HOME=/opt/data" in text
    assert "ENV SONIC_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "SONIC_TUI_DIR=/opt/sonic/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_sonic() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+sonic:sonic\s+/opt/sonic/\.venv",
        r"chown\s+-R\s+sonic:sonic\s+/opt/sonic/ui-tui",
        r"chown\s+-R\s+sonic:sonic\s+/opt/sonic/gateway",
        r"chown\s+-R\s+sonic:sonic\s+/opt/sonic/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/sonic must stay immutable; "
