...
class TestGatingWithTarget:
    """``SONIC_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_disable_env_blocks_without_target(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "sonic_cli.config.load_config", lambda: {}, raising=False
        )
    ...
    def test_normal_mode_unaffected(self, monkeypatch):
        ...
        monkeypatch.delenv("SONIC_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr(
            "sonic_cli.config.load_config", lambda: {}, raising=False
        )
...
@pytest.mark.skipif(
    os.environ.get("SONIC_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set SONIC_RUN_NETWORK_TESTS=1); CI runs "
