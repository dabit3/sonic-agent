"""Regression tests for SONIC_HOME override propagation onto the MCP loop.
...
get_sonic_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("SONIC_HOME", str(process_home))
    ...
    token = set_sonic_home_override(str(profile_home))
    try:
...
def test_oauth_token_paths_follow_override(tmp_path, monkeypatch, mcp_loop):
    """The actual symptom path: SonicTokenStorage resolving inside the
    ...
    async def token_path():
        ...
        return str(SonicTokenStorage("probe-srv")._tokens_path())
