...
and the shared slash-command handlers (sonic_cli/write_approval_commands.py).
...
@pytest.fixture
def sonic_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="sonic_wa_test_")
    home = os.path.join(d, ".sonic")
    os.makedirs(home)
...
def _set_approval(subsystem, enabled):
    import sonic_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(sonic_home):
...
def test_invalid_subsystem_is_off(sonic_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(sonic_home):
...
def test_memory_gate_on_no_interactive_stages(sonic_home):
...
def test_memory_gate_on_then_apply(sonic_home):
...
def test_skill_gate_off_allows_create(sonic_home):
...
def test_skill_gate_on_always_stages(sonic_home):
...
def test_skill_gate_on_then_apply_writes_file(sonic_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's SONIC_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(sonic_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(sonic_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(sonic_home):
...
def test_handle_approve_all(sonic_home):
    from sonic_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(sonic_home):
    from sonic_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(sonic_home):
    from sonic_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(sonic_home):
    from sonic_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(sonic_home):
...
def test_handle_approval_invalid(sonic_home):
    from sonic_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(sonic_home):
    from sonic_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(sonic_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(sonic_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(sonic_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(sonic_home, monkeypatch):
...
def test_skills_never_prompt_inline_even_with_callback(sonic_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(sonic_home):
