...
When SONIC_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the sonic user's home directory ($SONIC_HOME == /opt/data).
Runtime-writable trees under ``/opt/sonic`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/sonic/gateway`` is one such
