...
Hosted/container Sonic keeps user-writable state under SONIC_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/sonic must remain root-owned/non-writable by the runtime sonic user so
...
def test_stage2_does_not_chown_install_tree_to_sonic(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to sonic; "
            "the Dockerfile keeps /opt/sonic immutable and writable state "
            "belongs under SONIC_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "SONIC_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/sonic" in stage2_text
