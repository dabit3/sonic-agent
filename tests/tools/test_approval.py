...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("sonic_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("sonic_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"SONIC_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_sonic_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.sonic/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_sonic_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $SONIC_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_sonic_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$SONIC_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSonicConfigWriteProtection:
    ...
    def test_sed_in_place_absolute_sonic_home_config(self):
    ...
    def test_sed_in_place_absolute_sonic_home_env(self):
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_sonic_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $SONIC_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.sonic/sonic-agent && source venv/bin/activate && python -m sonic_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m sonic_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m sonic_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "sonic_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart sonic-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_sonic_gateway_stop_detected(self):
        cmd = "sonic gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_sonic_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `sonic` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "sonic -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_sonic_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "sonic --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_sonic_gateway_multiple_flags_detected(self):
        cmd = "sonic -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_sonic_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "sonic -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_sonic_gateway_start_not_flagged(self):
        cmd = "sonic gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_sonic_detected(self):
        """pkill targeting sonic/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_sonic_detected(self):
        cmd = "killall sonic"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep sonic) bypasses the pkill/killall name-matching
    ...
    def test_pkill_sonic_still_detected(self):
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("sonic_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"SONIC_INTERACTIVE": "1"}, clear=False):
