"""Tests for sonic_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import sonic_logging
# Use whatever RotatingFileHandler class sonic_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. sonic_logging
...
from sonic_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    sonic_logging._install_session_record_factory()
    yield
...
def sonic_home(tmp_path, monkeypatch):
    """Provide an isolated SONIC_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_sonic_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["SONIC_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, sonic_home):
        log_dir = sonic_logging.setup_logging(sonic_home=sonic_home)
        assert log_dir == sonic_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
        sonic_logging.setup_logging(sonic_home=sonic_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        sonic_logging.setup_logging(sonic_home=sonic_home, force=True)
    ...
    def test_custom_log_level(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, sonic_home):
        sonic_logging.setup_logging(
            sonic_home=sonic_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
    ...
    def test_writes_to_agent_log(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)

        test_logger = logging.getLogger("test_sonic_logging.write_test")
        ...
        agent_log = sonic_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)

        test_logger = logging.getLogger("test_sonic_logging.warning_test")
        ...
        agent_log = sonic_home / "logs" / "agent.log"
        errors_log = sonic_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)

        test_logger = logging.getLogger("test_sonic_logging.info_test")
    ...
    def test_explicit_params_override_config(self, sonic_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (sonic_home / "config.yaml").write_text(yaml.dump(config))

        sonic_logging.setup_logging(sonic_home=sonic_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, sonic_home):
        """The custom record factory injects session_tag on all records."""
        sonic_logging.setup_logging(sonic_home=sonic_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_sonic_session_injector", False), (
            "Record factory should have _sonic_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, sonic_home):
    ...
    def test_gateway_log_not_created_in_cli_mode(self, sonic_home):
    ...
    def test_gateway_log_created_after_cli_init(self, sonic_home):
        ...
        gw_log = sonic_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, sonic_home):
    ...
    def test_gateway_log_receives_gateway_records(self, sonic_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, sonic_home):
    ...
    def test_agent_log_still_receives_all(self, sonic_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, sonic_home):
        """When session context is set, log lines include [session_id]."""
        sonic_logging.setup_logging(sonic_home=sonic_home)
        sonic_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, sonic_home):
    ...
    def test_clear_session_context(self, sonic_home):
        """After clearing, session tag disappears."""
        sonic_logging.setup_logging(sonic_home=sonic_home)
        sonic_logging.set_session_context("xyz789")
        sonic_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, sonic_home):
        ...
        def thread_a():
            sonic_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            sonic_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        sonic_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        sonic_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = sonic_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = sonic_logging._ComponentFilter(
            sonic_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = sonic_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in sonic_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the sonic_plugins
        ...
        # break this test.
        gateway_prefixes = sonic_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "sonic_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = sonic_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == sonic_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = sonic_logging.COMPONENT_PREFIXES["cli"]
        assert "sonic_cli" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
        sonic_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_sonic_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, sonic_home):
        sonic_logging.setup_logging(sonic_home=sonic_home)
        sonic_logging.setup_verbose_logging()
        sonic_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        sonic_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = sonic_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        sonic_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("sonic_cli.config.is_managed", return_value=True):
                sonic_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, sonic_home):
        level, max_size, backup = sonic_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, sonic_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, sonic_home):
        import yaml
        config = {"model": "test"}
        (sonic_home / "config.yaml").write_text(yaml.dump(config))
