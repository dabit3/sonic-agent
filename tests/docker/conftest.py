...
Override the image with ``SONIC_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SONIC_TEST_IMAGE", "sonic-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SONIC_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SONIC_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged sonic user
# ---------------------------------------------------------------------------
#
# Background: every Sonic runtime path inside the container drops to UID
# 10000 (the ``sonic`` user) via ``s6-setuidgid sonic``. ``docker exec``
...
# fails (PermissionError swallowed) for sonic, so a test that ran as root
...
# helpers below run every probe under ``-u sonic`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: sonic).
