...
Before the s6 migration, ``docker run nousresearch/sonic-agent gateway
...
crash and a supervised dashboard alongside (when ``SONIC_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``SONIC_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``SONIC_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``sonic`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``sonic gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "SONIC_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The SONIC_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``sonic gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``sonic gateway
    ...
    # (main-wrapper.sh → python → sonic gateway run) can take well
    ...
    # started either — so we'd see the CMD's `sonic gateway run`
    ...
    # `sonic gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*sonic.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `sonic gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-sonic`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``SONIC_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${SONIC_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Sonic Gateway Starting…"
    ...
    assert "⚕" in combined or "Sonic Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Sonic Gateway Starting" in file_contents, (
