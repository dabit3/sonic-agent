...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "SONIC_GATEWAY_NO_SUPERVISE=1",
...
def test_supervised_gateway_does_not_recurse(
    ...
    # Count python processes running `sonic gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*sonic.*gateway run' | wc -l")
    assert r.returncode == 0
