...
Build the real image and verify: a config.yaml present in $SONIC_HOME
is migrated by docker_config_migrate.py on boot, running as the sonic
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $SONIC_HOME must be migrated on boot by
    docker_config_migrate.py, running as the sonic user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $SONIC_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/sonic/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by sonic (migration ran as sonic)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "sonic", (
        f"config.yaml not owned by sonic (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
...
def test_config_migration_opt_out_env_var_respected(
    ...
    """SONIC_SKIP_CONFIG_MIGRATION=1 must skip the migration."""
    start_container(
        built_image, container_name, "SONIC_SKIP_CONFIG_MIGRATION=1",
    )
