"""Runtime smoke test for Docker $SONIC_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the sonic user after container boot.
...
registered later runs its log service as the dropped sonic user and
...
def test_logs_gateways_seeded_and_sonic_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by sonic after boot."""
    ...
    assert "logs=sonic" in r.stdout, (
        f"logs/ not owned by sonic: {r.stdout}"
    )
    assert "gateways=sonic" in r.stdout, (
        f"logs/gateways/ not owned by sonic: {r.stdout}"
    )
