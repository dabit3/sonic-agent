"""Harness: dashboard opt-in via SONIC_DASHBOARD.

Today (tini): dashboard starts once when SONIC_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``sonic`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without SONIC_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "sonic dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without SONIC_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without SONIC_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `sonic doctor` and any other health check).
    ...
    SONIC_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without SONIC_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with SONIC_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # SONIC_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "SONIC_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With SONIC_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'sonic dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with SONIC_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """SONIC_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "sonic dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as sonic, so the
    # sonic user can kill it (same UID).
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    start_container(
        built_image, container_name,
        "SONIC_DASHBOARD=1",
        "SONIC_DASHBOARD_HOST=0.0.0.0",
        "SONIC_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and SONIC_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``SONIC_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
