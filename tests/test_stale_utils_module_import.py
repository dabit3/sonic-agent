...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.sonic/sonic-agent/utils.py)
...
2. ``sonic update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``sonic_cli/main.py`` (the ``sonic update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``sonic_constants`` after the
