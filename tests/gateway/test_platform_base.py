...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.sonic/media/'
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_sonic_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        sonic_dir = fake_home / ".sonic"
        sonic_dir.mkdir(parents=True)
        config_file = sonic_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_sonic_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared Sonic root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".sonic" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        sonic_root = fake_home / ".sonic"
        config_file = sonic_root / "config.yaml"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._SONIC_ROOT",
            sonic_root,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the SONIC_HOME root (google_token.json)
        ...
        sonic_dir = fake_home / ".sonic"
        sonic_dir.mkdir(parents=True)
        token = sonic_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._SONIC_HOME", sonic_dir)
        monkeypatch.setattr("gateway.platforms.base._SONIC_ROOT", sonic_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.sonic/pairing/ (platform pairing tokens) are
    ...
    def test_sonic_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        sonic_dir = fake_home / ".sonic"
        cache_dir = sonic_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_sonic_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.sonic
        ...
        sonic_dir = fake_home / ".sonic"
        sonic_dir.mkdir(parents=True)
        artifact = sonic_dir / "adhoc_report.pdf"
    ...
    def test_root_home_sonic_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.sonic/.env`` stays blocked under the $HOME exception — it is a
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"SONIC_MEDIA_DELIVERY_STRICT": "1",
                                     "SONIC_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SONIC_MEDIA_DELIVERY_STRICT", "0")
