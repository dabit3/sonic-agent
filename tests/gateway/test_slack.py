...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.sonic."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /sonic, /btw, /stop,
        ...
        for expected in ("/sonic", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@sonic`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@sonic"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@sonic")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /sonic subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /sonic btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_sonic_prefix_still_works(self, adapter):
        """Backward compat: /sonic btw foo must still route to /btw foo.

        Old workspace manifests only declared /sonic as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_sonic_freeform_question(self, adapter):
        """/sonic <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/sonic",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_sonic_slash_stashes_context(self, adapter):
        """Legacy /sonic <subcommand> also stashes context."""
        command = {
            "command": "/sonic",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_sonic_question_does_not_stash_context(self, adapter):
        """Free-form /sonic <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/sonic",
