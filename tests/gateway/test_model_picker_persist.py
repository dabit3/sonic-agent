...
closure the PR changed, against a real temp ``SONIC_HOME``.
"""
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # (`from sonic_cli.model_switch import switch_model as _switch_model`),
    # so patching the source-module attribute takes effect at call time.
    monkeypatch.setattr(
        "sonic_cli.model_switch.switch_model",
    ...
    monkeypatch.setattr(
        "sonic_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_sonic_home() / config.yaml`` — point it here.
    monkeypatch.setattr("sonic_constants.get_sonic_home", lambda: sonic_home)
    monkeypatch.setattr("sonic_cli.config.get_sonic_home", lambda: sonic_home)
    return cfg_path
