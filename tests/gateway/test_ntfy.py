...
class TestNtfyRequirements:
    ...
    def test_returns_true_when_topic_set_via_env(self, monkeypatch):
        monkeypatch.setattr("gateway.platforms.ntfy.HTTPX_AVAILABLE", True)
        monkeypatch.setenv("NTFY_TOPIC", "sonic-cfg")
        from gateway.platforms.ntfy import check_ntfy_requirements
        assert check_ntfy_requirements() is True
...
class TestNtfyConfigLoading:
    ...
    def test_ntfy_topic_enables_platform(self, monkeypatch):
        ...
        assert pc.extra["topic"] == "sonic-in"
    ...
    def test_ntfy_publish_topic_stored_in_extra(self, monkeypatch):
        ...
        config = load_gateway_config()
        pc = config.platforms[Platform.NTFY]
        assert pc.extra.get("publish_topic") == "sonic-out"

    def test_ntfy_home_channel_set(self, monkeypatch):
        from gateway.config import load_gateway_config

        monkeypatch.setenv("NTFY_TOPIC", "sonic-in")
        monkeypatch.setenv("NTFY_HOME_CHANNEL", "sonic-home")
        config = load_gateway_config()
        pc = config.platforms[Platform.NTFY]
        assert pc.home_channel is not None
        assert pc.home_channel.chat_id == "sonic-home"
...
class TestSend:
    ...
    def _make_adapter(self, topic="sonic-in", publish_topic="", token=""):
        from gateway.platforms.ntfy import NtfyAdapter
...
class TestOnMessage:
    ...
    def _make_adapter(self):
        ...
        adapter = NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "sonic-in"}))
        return adapter
...
class TestToolsets:
    ...
    def test_sonic_ntfy_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("sonic-ntfy")
        assert ts is not None
        assert "tools" in ts

    def test_sonic_ntfy_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("sonic-gateway")
        assert "sonic-ntfy" in gw["includes"]

    def test_sonic_ntfy_resolves_tools(self):
        from toolsets import resolve_toolset

        tools = resolve_toolset("sonic-ntfy")
        assert len(tools) > 0

    def test_sonic_ntfy_description_mentions_ntfy(self):
        from toolsets import get_toolset
