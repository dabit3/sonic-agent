...
SONIC_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SONIC_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    sonic_home = tmp_path / ".sonic"
    sonic_home.mkdir()
    (sonic_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (sonic_home / ".env").write_text("SONIC_MAX_ITERATIONS=123\n", encoding="utf-8")
    ...
    assert os.environ["SONIC_MAX_ITERATIONS"] == "123"


def test_current_max_iterations_reloads_before_reading(monkeypatch) -> None:
    monkeypatch.setenv("SONIC_MAX_ITERATIONS", "90")

    def _fake_reload() -> None:
        os.environ["SONIC_MAX_ITERATIONS"] = "200"
