...
def adapter(tmp_path):
    ...
    ~/.lightning/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.lightning/...)
...
class TestOnPubsubMessage:
    ...
    def test_relay_flat_bot_sender_is_filtered_end_to_end(self, adapter):
        ...
        envelope = {
            ...
            "sender_display_name": "LightningBot",
            "sender_type": "BOT",
            "text": "reply from bot",
...
class TestExtractMessagePayload:
    ...
    def test_relay_flat_format_synthesizes_chat_api_shape(self):
        ...
        GCP credentials off the Lightning host. The relay flattens Chat
...
class TestTypingLifecycle:
    ...
    async def test_send_typing_concurrent_calls_create_only_one_card(self, adapter):
        ...
        'Lightning is thinking…' stuck in chat. Race fix via
    ...
    async def test_orphan_typing_cards_reaped_on_completion(self, adapter):
        ...
        'Lightning is thinking…' messages."""
        from plugins.platforms.google_chat.adapter import _TYPING_CONSUMED_SENTINEL
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/lightning-chat"
            ),
        ...
        monkeypatch.setattr("lightning_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("lightning_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("lightning_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "lightning_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "lightning_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "lightning_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "lightning_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
