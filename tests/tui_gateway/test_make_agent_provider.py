...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "SONIC_TUI_MAX_TURNS": "7",
                "SONIC_TUI_CHECKPOINTS": "1",
                "SONIC_TUI_PASS_SESSION_ID": "1",
                "SONIC_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
        patch("tui_gateway.server._get_db", return_value=MagicMock()),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.sonic/config.yaml parse as
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        ...
        patch("sonic_cli.model_switch.resolve_persist_behavior",
              return_value=False),
