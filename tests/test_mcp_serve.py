"""
Tests for mcp_serve — Sonic MCP server.
...
def _isolate_sonic_home(tmp_path, monkeypatch):
    """Redirect SONIC_HOME to a temp directory."""
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    try:
        import sonic_constants
        monkeypatch.setattr(sonic_constants, "get_sonic_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking sonic_state schema."""
