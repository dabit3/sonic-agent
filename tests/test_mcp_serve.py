"""
Tests for mcp_serve — Lightning MCP server.
...
def _isolate_lightning_home(tmp_path, monkeypatch):
    """Redirect LIGHTNING_HOME to a temp directory."""
    monkeypatch.setenv("LIGHTNING_HOME", str(tmp_path))
    try:
        import lightning_constants
        monkeypatch.setattr(lightning_constants, "get_lightning_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking lightning_state schema."""
