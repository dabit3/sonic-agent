"""Tests for lightning_constants module."""
...
class TestGetDefaultLightningRoot:
    """Tests for get_default_lightning_root() — Docker/custom deployment awareness."""

    def test_no_lightning_home_returns_native(self, tmp_path, monkeypatch):
        """When LIGHTNING_HOME is not set, returns ~/.lightning."""
        monkeypatch.delenv("LIGHTNING_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_lightning_root() == tmp_path / ".lightning"

    def test_lightning_home_is_native(self, tmp_path, monkeypatch):
        """When LIGHTNING_HOME = ~/.lightning, returns ~/.lightning."""
        native = tmp_path / ".lightning"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(native))
        assert get_default_lightning_root() == native

    def test_lightning_home_is_profile(self, tmp_path, monkeypatch):
        """When LIGHTNING_HOME is a profile under ~/.lightning, returns ~/.lightning."""
        native = tmp_path / ".lightning"
    ...
    def test_lightning_home_is_docker(self, tmp_path, monkeypatch):
        """When LIGHTNING_HOME points outside ~/.lightning (Docker), returns LIGHTNING_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(docker_home))
        assert get_default_lightning_root() == docker_home

    def test_lightning_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any LIGHTNING_HOME outside ~/.lightning is treated as the root."""
        custom = tmp_path / "my-lightning-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(custom))
        assert get_default_lightning_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (LIGHTNING_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LIGHTNING_HOME", str(profile))
        assert get_default_lightning_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(lightning_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(lightning_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".lightning"
