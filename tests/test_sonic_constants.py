"""Tests for sonic_constants module."""
...
from sonic_constants import (
    ...
    find_sonic_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_sonic_root,
    get_sonic_home,
    heal_sonic_managed_node,
    sonic_managed_node_tree_present,
    iter_sonic_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultSonicRoot:
    """Tests for get_default_sonic_root() — Docker/custom deployment awareness."""

    def test_no_sonic_home_returns_native(self, tmp_path, monkeypatch):
        """When SONIC_HOME is not set, returns ~/.sonic."""
        monkeypatch.delenv("SONIC_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_sonic_root() == tmp_path / ".sonic"

    def test_sonic_home_is_native(self, tmp_path, monkeypatch):
        """When SONIC_HOME = ~/.sonic, returns ~/.sonic."""
        native = tmp_path / ".sonic"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(native))
        assert get_default_sonic_root() == native

    def test_sonic_home_is_profile(self, tmp_path, monkeypatch):
        """When SONIC_HOME is a profile under ~/.sonic, returns ~/.sonic."""
        native = tmp_path / ".sonic"
    ...
    def test_sonic_home_is_docker(self, tmp_path, monkeypatch):
        """When SONIC_HOME points outside ~/.sonic (Docker), returns SONIC_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(docker_home))
        assert get_default_sonic_root() == docker_home

    def test_sonic_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SONIC_HOME outside ~/.sonic is treated as the root."""
        custom = tmp_path / "my-sonic-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(custom))
        assert get_default_sonic_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SONIC_HOME=<root>/profiles/<name>),
...
class TestSonicManagedNode:
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sonic_constants.sys, "platform", "win32")
        monkeypatch.setenv("SONIC_HOME", str(home))
        monkeypatch.setattr(sonic_constants, "node_tool_runnable", lambda path: True)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(sonic_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(sonic_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".sonic"
