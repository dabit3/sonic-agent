...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSonicSessionId"] == "root1"
    assert prov["rootSonicSessionId"] == "root1"
    assert prov["parentSonicSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_sonic_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSonicSessionId"] == "old"
    assert prov["rootSonicSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSonicSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootSonicSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSonicSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_sonic_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousSonicSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"sonic"}
    assert "sessionProvenance" in meta["sonic"]
    assert meta["sonic"]["sessionProvenance"]["currentSonicSessionId"] == "root1"
