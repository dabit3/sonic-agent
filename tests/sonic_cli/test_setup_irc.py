"""Tests for IRC gateway configuration via `sonic setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Sonic install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Sonic install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#sonic")
            monkeypatch.setenv("IRC_NICKNAME", "sonic-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `sonic setup gateway` experience with IRC present."""
