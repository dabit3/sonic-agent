"""Tests for sonic_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $SONIC_HOME under tmp_path; no real
...
from sonic_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running sonic containers, those
    ...
    """
    monkeypatch.setattr(
        "sonic_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under sonic_home/profiles/<name>/."""
    p = sonic_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `sonic profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    SONIC_HOME root (the implicit default profile)."""
    if state is not None:
        (sonic_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (sonic_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (sonic_home / "processes.json").write_text("[]")
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from sonic_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        sonic_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $SONIC_HOME/profiles doesn't exist (fresh install), the
    ...
    so `sonic gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        sonic_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare SONIC_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $SONIC_HOME/profiles/default/ instead of
    the root profile. It must call `sonic gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the SONIC_HOME root (left
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Through s6 /init + the main-wrapper that re-execs `sonic`.
        ("/init", "/opt/sonic/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
            "/opt/sonic/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "127.0.0.1",
            "--no-open",
        ),
        # Wrapper that kept the explicit `sonic` argv0.
        ("/init", "/opt/sonic/docker/main-wrapper.sh", "sonic", "dashboard"),
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from sonic_cli.container_boot import _is_dashboard_container
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/sonic/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/sonic/docker/main-wrapper.sh", "sonic", "gateway", "run"),
        ("chat",),
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/sonic/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/sonic/docker/main-wrapper.sh", "gateway", "run"),
    )
...
def test_main_ignores_removed_skip_reconcile_env_var(
    ...
    """The legacy SONIC_SKIP_PROFILE_RECONCILE flag is gone: setting it on a
    ...
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    monkeypatch.setenv("S6_PROFILE_GATEWAY_SCANDIR", str(scandir))
    monkeypatch.setenv("SONIC_SKIP_PROFILE_RECONCILE", "1")
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/sonic/docker/main-wrapper.sh", "gateway", "run"),
