...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    token = _jwt(
        {
            ...
            "org_id": "org_123",
            "client_id": "sonic-cli",
            "product_id": "nous-sonic-agent",
            "nous_client": "sonic-agent",
    ...
    assert info.product_id == "nous-sonic-agent"
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Log in with `sonic model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `sonic model`" in message
