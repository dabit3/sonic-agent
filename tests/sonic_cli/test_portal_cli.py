"""Tests for `sonic portal` dispatch.

`sonic portal` (no subcommand) is the human-readable alias for the Nous Portal
one-shot onboarding (`sonic auth add nous --type oauth` / `sonic setup
--portal`). The prior status default moved to `sonic portal info`, with
...
from sonic_cli import portal_cli
...
def test_bare_portal_and_login_run_one_shot(monkeypatch, sub):
    """`sonic portal`, `sonic portal login` -> one-shot onboarding."""
    ...
    monkeypatch.setattr(
        "sonic_cli.setup._run_portal_one_shot", fake_one_shot
    )
...
def test_info_and_status_alias_run_status(monkeypatch, sub):
    """`sonic portal info` and the `status` back-compat alias -> status."""
    calls = {"login": 0, "status": 0}

    monkeypatch.setattr(
        "sonic_cli.setup._run_portal_one_shot",
...
def test_login_cancelled_returns_one(monkeypatch):
    ...
    monkeypatch.setattr("sonic_cli.setup._run_portal_one_shot", boom)
    monkeypatch.setattr(portal_cli, "load_config", lambda: {})
