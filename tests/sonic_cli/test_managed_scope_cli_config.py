...
sonic_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `sonic config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_sonic_home = get_sonic_home()`` at import time (module
    singleton), so monkeypatching SONIC_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SONIC_HOME.)
    """
    import cli

    cli._sonic_home = home
