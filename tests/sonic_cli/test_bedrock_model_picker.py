...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SONIC_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SONIC_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SONIC_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from sonic_cli.providers import SONIC_OVERLAYS
        assert "bedrock" in SONIC_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from sonic_cli.providers import SONIC_OVERLAYS
        assert SONIC_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from sonic_cli.providers import SONIC_OVERLAYS
        assert SONIC_OVERLAYS["bedrock"].auth_type == "aws_sdk"
