...
from sonic_cli.psutil_android import (
    MARKER,
    REPLACEMENT,
    PSUTIL_URL,
...
def test_install_psutil_android_compat_uses_patched_tree(tmp_path):
    ...
    with patch("urllib.request.urlretrieve", side_effect=fake_urlretrieve), \
         patch.object(sonic_main, "_run_install_with_heartbeat", side_effect=fake_run_install):
        sonic_main._install_psutil_android_compat(
            ["uv", "pip"],
            env={"SONIC_TEST": "1"},
        )

    assert captured["cmd"][:4] == ["uv", "pip", "install", "--no-build-isolation"]
    assert captured["env"] == {"SONIC_TEST": "1"}
