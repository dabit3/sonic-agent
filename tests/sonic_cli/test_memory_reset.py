"""Tests for the `sonic memory reset` CLI command.
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SONIC_HOME with memory files."""
    sonic_home = tmp_path / ".sonic"
    memories = sonic_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SONIC_HOME", str(sonic_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSonic repo is at ~/.sonic/sonic-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        sonic_home = tmp_path / ".sonic"
        (sonic_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SONIC_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_sonic_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
