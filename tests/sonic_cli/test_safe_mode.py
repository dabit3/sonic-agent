"""Tests for `sonic chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("SONIC_SAFE_MODE", "SONIC_IGNORE_USER_CONFIG", "SONIC_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in sonic_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("SONIC_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under SONIC_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("SONIC_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("SONIC_SAFE_MODE", raising=False)
        from sonic_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under SONIC_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `sonic chat`."""
