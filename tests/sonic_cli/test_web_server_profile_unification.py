...
profile switcher can target any profile's SONIC_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via SONIC_HOME.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_sonic_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_sonic_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import sonic_cli.mcp_config as mcp_config
        from sonic_constants import get_sonic_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same SONIC_HOME the rest
        ...
        monkeypatch.setattr(
            "sonic_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_sonic_home(self, isolated_profiles, monkeypatch):
        import sonic_cli.web_server as web_server

        monkeypatch.setattr(
            "sonic_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["SONIC_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "SONIC_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("SONIC_HOME") != str(isolated_profiles["worker_beta"])
