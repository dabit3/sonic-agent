...
the SAME provider universe as ``sonic model`` (the CLI/TUI picker).
...
def test_every_sonic_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `sonic model` universe."""
    ...
    assert not missing, (
        "providers shown in `sonic model` but not configurable in the desktop "
