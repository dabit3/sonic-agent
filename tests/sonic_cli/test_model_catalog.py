"""Tests for sonic_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate SONIC_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".sonic"
    home.mkdir()
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from sonic_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://lightning-agent.nousresearch.com/docs/api/model-catalog.json"
    FALLBACK = (
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in sonic_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from sonic_cli import model_catalog
        from sonic_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``sonic model`` CLI — not the live
        ...
        # ``SONIC_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` SONIC_HOME directly instead.
        import importlib
        from sonic_cli import model_catalog
        from sonic_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from sonic_cli.model_switch import list_picker_providers
