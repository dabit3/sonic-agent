...
def test_loopback_protected_route_accepts_session_token(client_loopback):
    ...
    r = client_loopback.get(
        "/api/sessions",
        headers={"X-Sonic-Session-Token": web_server._SESSION_TOKEN},
    )
...
def test_loopback_index_injects_session_token(client_loopback):
    ...
    assert "__SONIC_SESSION_TOKEN__" in r.text
...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    ("sonic-agent-prod-abc.fly.dev", False, True),
])
...
def test_start_server_surfaces_nous_skip_reason_when_unconfigured(monkeypatch):
    ...
    'set SONIC_DASHBOARD_OAUTH_CLIENT_ID', not 'install a plugin'."""
    from sonic_cli.dashboard_auth import clear_providers
    ...
    assert "SONIC_DASHBOARD_OAUTH_CLIENT_ID" in msg
    assert "nous:" in msg
