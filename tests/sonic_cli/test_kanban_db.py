"""Tests for the Kanban DB layer (sonic_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated SONIC_HOME with an empty kanban DB."""
    home = tmp_path / ".sonic"
    home.mkdir()
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("SONIC_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("SONIC_HOME", str(home))
    monkeypatch.delenv("SONIC_KANBAN_DB", raising=False)
    monkeypatch.delenv("SONIC_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("SONIC_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from sonic_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Sonic profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="sonic-task", assignee="daily")
...
def test_scratch_workspace_created_under_sonic_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    ...
    ``shutil.rmtree`` Sonic's DB/metadata/logs on task completion.
    """
...
# spawned with `sonic -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's SONIC_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's SONIC_HOME."""

    def _set_home(self, monkeypatch, tmp_path, sonic_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.delenv("SONIC_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_sonic(
        self, tmp_path, monkeypatch
    ):
        # Standard install: SONIC_HOME == ~/.sonic, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.sonic/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.sonic/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.sonic/kanban.db.
        default_home = tmp_path / ".sonic"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # SONIC_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `sonic -p coder`).
    ...
    def test_docker_custom_sonic_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: SONIC_HOME points outside ~/.sonic.
        # `get_default_sonic_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".sonic"`.
        custom_root = tmp_path / "opt" / "sonic"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: SONIC_HOME=/opt/sonic/profiles/coder;
        # `get_default_sonic_root()` walks up to /opt/sonic because
    ...
    def test_explicit_override_via_sonic_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: SONIC_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".sonic"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(profile_home))
        monkeypatch.setenv("SONIC_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(default_home))
        monkeypatch.setenv("SONIC_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # SONIC_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile SONIC_HOME and reads.
        monkeypatch.setenv("SONIC_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_sonic_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # SONIC_KANBAN_DB pins the file path directly and beats both
        # SONIC_KANBAN_HOME and the `get_default_sonic_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(default_home))
        monkeypatch.setenv("SONIC_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SONIC_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows SONIC_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_sonic_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # SONIC_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".sonic"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("SONIC_HOME", str(default_home))
        monkeypatch.setenv("SONIC_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SONIC_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows SONIC_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SONIC_HOME", str(default_home))
        monkeypatch.setenv("SONIC_KANBAN_DB", "   ")
        monkeypatch.setenv("SONIC_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject SONIC_KANBAN_DB
        # and SONIC_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites SONIC_HOME.
        default_home = tmp_path / ".sonic"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["SONIC_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SONIC_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SONIC_KANBAN_TASK"] == "t_dispatch_env"
        assert env["SONIC_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see sonic_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("sonic_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="sonic_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'sonic kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_sonic_argv()
#
# Workers spawned by the dispatcher must use a `sonic` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["sonic", ...]`
...
def test_resolve_sonic_argv_prefers_path_shim(monkeypatch):
    """When `sonic` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import sonic_cli.kanban_db as kb

    monkeypatch.delenv("SONIC_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/sonic")
    argv = kb._resolve_sonic_argv()
    assert argv == ["/usr/local/bin/sonic"]


def test_resolve_sonic_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import sonic_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("SONIC_BIN", ".\\sonic.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_sonic_argv() == [os.path.abspath(".\\sonic.exe")]


def test_resolve_sonic_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "sonic.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("SONIC_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_sonic_argv() == [sys.executable, "-m", "sonic_cli.main"]


def test_resolve_sonic_argv_honors_sonic_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like SONIC_BIN lets service managers pin the executable."""
    import shutil
    import sonic_cli.kanban_db as kb

    shim = tmp_path / "bin" / "sonic"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("SONIC_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_sonic_argv() == [str(shim)]


def test_resolve_sonic_argv_sonic_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare SONIC_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import sonic_cli.kanban_db as kb

    cwd_sonic = tmp_path / "sonic"
    cwd_sonic.write_text("wrong\n", encoding="utf-8")
    cwd_sonic.chmod(cwd_sonic.stat().st_mode | stat.S_IXUSR)
    path_sonic = tmp_path / "bin" / "sonic"
    path_sonic.parent.mkdir()
    path_sonic.write_text("right\n", encoding="utf-8")
    path_sonic.chmod(path_sonic.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_sonic.parent))
    monkeypatch.setenv("SONIC_BIN", "sonic")

    assert kb._resolve_sonic_argv() == [str(path_sonic)]


def test_resolve_sonic_argv_sonic_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare SONIC_BIN does not accept current-directory shadow executables."""
    import sys
    import sonic_cli.kanban_db as kb

    (tmp_path / "sonic.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_sonic_argv_sonic_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved SONIC_BIN batch shim is not used as worker argv[0]."""
    import sys
    import sonic_cli.kanban_db as kb
...
def test_resolve_sonic_argv_sonic_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved SONIC_BIN command names do not delegate cwd search to Popen."""
    import sys
    import sonic_cli.kanban_db as kb
...
def test_resolve_sonic_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m sonic_cli.main`.

    Pins the correct module name (NOT `sonic` — there is no top-level
    `sonic` package). Regression for #23198: the original PR shipped
    `python -m sonic` which fails with `No module named sonic` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_sonic_argv()
    assert argv == [sys.executable, "-m", "sonic_cli.main"]


def test_resolve_sonic_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `sonic_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m sonic_cli.main --version`
    ...
    assert "Sonic Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="sonic_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from sonic_cli.kanban_db import connect, _check_file_length_invariant
