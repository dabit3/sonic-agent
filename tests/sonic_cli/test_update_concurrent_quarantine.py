"""Tests for issue #26670 — concurrent sonic.exe detection and improved
quarantine retry / reboot-deferred fallback during `sonic update` on Windows.
...
from sonic_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_sonic_instances
# helper (and need the autouse stub in tests/sonic_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "sonic.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "sonic.exe").write_bytes(b"")
    (scripts_dir / "sonic-gateway.exe").write_bytes(b"")
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "sonic.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_sonic_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "sonic.exe"
    shim.write_bytes(b"")
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning sonic.EXE (uppercase ext) from same path
    upper = str(shim).replace("sonic.exe", "SONIC.EXE")
    procs = [_make_proc(9999, upper, "SONIC.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_sonic_instances(scripts_dir)

    assert result == [(9999, "SONIC.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_sonic_instances(tmp_path) == []
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "sonic.exe"), (5678, "sonic.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "sonic.exe" in msg
    assert "Sonic Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "sonic.exe") in msg


# ---------------------------------------------------------------------------
# _quarantine_running_sonic_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "sonic.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_sonic_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("sonic.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_sonic_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Sonic Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another sonic.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "sonic.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_sonic_instances", detect
    ), patch.object(
