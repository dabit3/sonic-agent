"""Tests for Codex auth — tokens stored in Sonic auth store (~/.sonic/auth.json)."""
...
def _setup_sonic_auth(sonic_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Sonic auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_sonic_auth(sonic_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("SONIC_HOME", str(sonic_home))
    ...
    monkeypatch.setattr("sonic_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    sonic_home = tmp_path / "sonic"
    _setup_sonic_auth(sonic_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("SONIC_HOME", str(sonic_home))
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``sonic auth add openai-codex`` as a
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Sonic auth store, not ~/.codex/."""
    sonic_home = tmp_path / "sonic"
    codex_home = tmp_path / "codex-cli"
    sonic_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "sonic-at", "refresh_token": "sonic-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Sonic store
    assert not (codex_home / "auth.json").exists()

    # Sonic auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "sonic-at"


def test_resolve_returns_sonic_auth_store_source(tmp_path, monkeypatch):
    sonic_home = tmp_path / "sonic"
    _setup_sonic_auth(sonic_home)
    monkeypatch.setenv("SONIC_HOME", str(sonic_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "sonic_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("sonic_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("sonic_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
