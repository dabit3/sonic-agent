...
``sonic_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``sonic --help``, ``sonic version``,
``sonic logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``sonic --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from sonic_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["sonic", "--tui", "chat"], "chat"),
        (["sonic", "-w", "logs"], "logs"),
        (["sonic", "chat", "hello world"], "chat"),
        (["sonic", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["sonic", "-m", "gpt5", "chat"], "chat"),
        (["sonic", "--model", "gpt5", "chat", "hi"], "chat"),
        (["sonic", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["sonic", "-z", "hello world"], None),
        (["sonic", "-z", "hello", "chat"], "chat"),
        (["sonic", "--model=gpt5", "chat"], "chat"),     # inline form
        (["sonic", "--", "chat"], "chat"),               # -- terminator
        (["sonic", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["sonic", "some-plugin-cmd"], "some-plugin-cmd"),
        (["sonic", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["sonic"],                          # bare → chat
        ["sonic", "--help"],                # top-level help
        ["sonic", "-h"],
        ["sonic", "version"],               # known built-in
        ["sonic", "logs"],
        ["sonic", "gateway", "run"],
        ["sonic", "--tui"],
        ["sonic", "-w", "--tui"],
        ["sonic", "chat", "hi"],
        ["sonic", "help"],                  # accepted built-in-ish
        ["sonic", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["sonic", "meet", "join"],          # potential google_meet plugin
        ["sonic", "honcho", "status"],      # potential memory plugin
        ["sonic", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``sonic help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"sonic_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
