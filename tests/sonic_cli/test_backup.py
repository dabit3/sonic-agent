"""Tests for sonic backup and import commands."""
...
def _make_sonic_tree(root: Path) -> None:
    """Create a realistic ~/.sonic directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "sonic_state.db").write_bytes(b"fake-state")
    ...
    # sonic-agent repo (should be EXCLUDED)
    (root / "sonic-agent").mkdir(exist_ok=True)
    (root / "sonic-agent" / "run_agent.py").write_text("# big file\n")
    (root / "sonic-agent" / ".git").mkdir()
    (root / "sonic-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_sonic_agent(self):
        from sonic_cli.backup import _should_exclude
        assert _should_exclude(Path("sonic-agent/run_agent.py"))
        assert _should_exclude(Path("sonic-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        # get_default_sonic_root needs this
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/sonic-backup-*.zip."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        (sonic_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("sonic-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Sonic backup."""
        from sonic_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only sonic_state.db (old wrong name) is rejected."""
        from sonic_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["sonic_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into sonic home."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (sonic_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (sonic_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (sonic_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (sonic_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_sonic_prefix(self, tmp_path, monkeypatch):
        """Import strips .sonic/ prefix if all entries share it."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".sonic/config.yaml": "model: test\n",
            ".sonic/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (sonic_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_sonic_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a sonic backup."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside sonic home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (sonic_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (sonic_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (sonic_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (sonic_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".sonic"
        src_home.mkdir(parents=True)
        _make_sonic_tree(src_home)

        monkeypatch.setenv("SONIC_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from sonic_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".sonic"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("SONIC_HOME", str(dst_home))
        ...
        # sonic-agent should NOT be present
        assert not (dst_home / "sonic-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from sonic_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without sonic markers fails validation."""
        import io
        from sonic_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_sonic(self):
        """Detects .sonic/ prefix wrapping all entries."""
        import io
        from sonic_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".sonic/config.yaml", "test")
            zf.writestr(".sonic/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".sonic/"
...
class TestBackupEdgeCases:
    def test_nonexistent_sonic_home(self, tmp_path, monkeypatch):
        """Backup exits when sonic home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".sonic"
        monkeypatch.setenv("SONIC_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("sonic-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_sonic_home(self, tmp_path, monkeypatch):
        """Backup handles empty sonic home (no files to back up)."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        # Only excluded dirs, no actual files
        (sonic_home / "__pycache__").mkdir()
        (sonic_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = sonic_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_sonic(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside sonic root."""
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        (sonic_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside sonic home
        out_zip = sonic_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        (sonic_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        sonic_home = tmp_path / ".sonic"
        sonic_home.mkdir()
        (sonic_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("SONIC_HOME", str(sonic_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (sonic_home / "config.yaml").exists()
        assert (sonic_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (sonic_home / "profiles" / "coder" / "config.yaml").exists()
        assert (sonic_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "sonic -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from sonic_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def sonic_home(self, tmp_path):
        """Create a fake SONIC_HOME with critical state files."""
        home = tmp_path / ".sonic"
        home.mkdir()
    ...
    def test_creates_snapshot(self, sonic_home):
    ...
    def test_label_in_id(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", sonic_home=sonic_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
        db_copy = sonic_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
    ...
    def test_missing_files_skipped(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
        with open(sonic_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(sonic_home=empty) is None

    def test_list_snapshots(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", sonic_home=sonic_home)
        id2 = create_quick_snapshot(label="second", sonic_home=sonic_home)

        snaps = list_quick_snapshots(sonic_home=sonic_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", sonic_home=sonic_home)
        snaps = list_quick_snapshots(limit=3, sonic_home=sonic_home)
        assert len(snaps) == 3

    def test_restore_config(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)

        (sonic_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (sonic_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, sonic_home=sonic_home)
        assert result is True
        assert "openrouter" in (sonic_home / "config.yaml").read_text()

    def test_restore_state_db(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)

        conn = sqlite3.connect(str(sonic_home / "state.db"))
    ...
    def test_restore_nonexistent(self, sonic_home):
        from sonic_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", sonic_home=sonic_home) is False

    def test_auto_prune(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", sonic_home=sonic_home)
        snaps = list_quick_snapshots(limit=100, sonic_home=sonic_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", sonic_home=sonic_home)
        deleted = prune_quick_snapshots(keep=3, sonic_home=sonic_home)
        assert deleted == 7
        assert len(list_quick_snapshots(sonic_home=sonic_home)) == 3

    def test_snapshot_includes_pairing_directories(self, sonic_home):
        ...
        (sonic_home / "platforms" / "pairing").mkdir(parents=True)
        (sonic_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (sonic_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (sonic_home / "pairing").mkdir()
        (sonic_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (sonic_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, sonic_home):
        ...
        assert restore_quick_snapshot(snap_id, sonic_home=sonic_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, sonic_home):
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from sonic_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
        copy = sonic_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
        copy = sonic_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, sonic_home):
        from sonic_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(sonic_home=sonic_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(sonic_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, sonic_home):
        ...
        board_dir = sonic_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
        copy = (
            sonic_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, sonic_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(sonic_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, sonic_home):
        ...
        board = sonic_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(sonic_home=sonic_home)
        snap = sonic_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, sonic_home, monkeypatch):
        ...
        copy = sonic_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def sonic_home(self, tmp_path):
        root = tmp_path / ".sonic"
        root.mkdir()
        _make_sonic_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, sonic_home):
        from sonic_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(sonic_home=sonic_home)
        assert out is not None
        assert out.exists()
        assert out.parent == sonic_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, sonic_home):
        """Pre-update backup should include the same user data that
        ``sonic backup`` would, and should exclude the same directories."""
        from sonic_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(sonic_home=sonic_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # sonic-agent repo excluded
        assert not any(n.startswith("sonic-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, sonic_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from sonic_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(sonic_home=sonic_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(sonic_home=sonic_home)
    ...
    def test_rotation_keeps_only_n(self, sonic_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(sonic_home=sonic_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (sonic_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, sonic_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from sonic_cli.backup import create_pre_update_backup

        (sonic_home / "backups").mkdir(exist_ok=True)
        manual = sonic_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(sonic_home=sonic_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from sonic_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(sonic_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, sonic_home):
        ...
        """
        from sonic_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(sonic_home=sonic_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, sonic_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, sonic_home):
        ...
        first = create_pre_update_backup(sonic_home=sonic_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(sonic_home=sonic_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(sonic_home=sonic_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def sonic_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".sonic"
        root.mkdir()
        _make_sonic_tree(root)
        # Point SONIC_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("SONIC_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for sonic_cli.config + sonic_constants so they pick up SONIC_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("sonic_cli.config") or mod == "sonic_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, sonic_home, capsys):
    ...
    def test_no_backup_flag_skips(self, sonic_home, capsys):
        ...
        # No backup written
        assert not (sonic_home / "backups").exists() or not list(
            (sonic_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, sonic_home, capsys):
        ...
        import yaml
        (sonic_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("sonic_cli.config"):
    ...
    def test_config_disabled_is_silent(self, sonic_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, sonic_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (sonic claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``sonic claw migrate`` runs before mutating ~/.sonic/."""
    ...
    def test_creates_backup_under_backups_dir(self, sonic_home):
        from sonic_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(sonic_home=sonic_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `sonic
        # import` and the update-backup listing both pick them up.
        assert out.parent == sonic_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, sonic_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``sonic backup`` / ``create_pre_update_backup`` — no drift."""
        from sonic_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(sonic_home=sonic_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_sonic_import(self, sonic_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Sonic
        backup — `sonic import` should accept it."""
        from sonic_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(sonic_home=sonic_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, sonic_home):
        from sonic_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(sonic_home=sonic_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(sonic_home=sonic_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, sonic_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(sonic_home=sonic_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((sonic_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_sonic_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.sonic yet — nothing to back up."""
        from sonic_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(sonic_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, sonic_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from sonic_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(sonic_home=sonic_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(sonic_home=sonic_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`sonic update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, sonic_home: Path, label="pre-update"):
        from sonic_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, sonic_home=sonic_home, keep=5)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = sonic_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, sonic_home=sonic_home) is None
        assert restore_cron_jobs_if_emptied("", sonic_home=sonic_home) is None
