...
def temp_pyproject(tmp_path, monkeypatch):
    ...
    pyproject.write_text(
        ...
        [project.scripts]
        sonic = "sonic_cli.main:main"
        sonic-agent = "run_agent:main"
        sonic-acp = "acp_adapter.entry:main"
    """
        )
    )
...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("sonic", "sonic-agent", "sonic-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with patch("sonic_cli.main._is_windows", return_value=True), \
             patch("sonic_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("sonic_cli.main._run_quarantined_install") as mock_install:
            from sonic_cli.main import _verify_console_scripts_installed

            _verify_console_scripts_installed(["uv", "pip"], env={})

        mock_install.assert_not_called()

    def test_triggers_reinstall_when_sonic_exe_missing(
        self, temp_pyproject, fake_scripts_dir
    ):
        (fake_scripts_dir / "sonic-agent.exe").write_bytes(b"fake")
        (fake_scripts_dir / "sonic-acp.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("sonic_cli.main._is_windows", return_value=False), \
             patch("sonic_cli.main._run_quarantined_install") as mock_install:
            from sonic_cli.main import _verify_console_scripts_installed
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from sonic_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["sonic", "sonic-agent", "sonic-acp"]
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("sonic_cli.main._is_windows", return_value=False), \
             patch("sonic_cli.main._run_quarantined_install") as mock_install, \
             patch("sonic_cli.main._verify_console_scripts_installed") as mock_verify:
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("sonic_cli.main._is_windows", return_value=True):
            names = {path.name for path in main_mod._sonic_exe_shims(fake_scripts_dir)}

        assert {"sonic.exe", "sonic-agent.exe", "sonic-acp.exe"} <= names
        assert "sonic-gateway.exe" in names
