...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("sonic_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("sonic_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "sonic_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "sonic_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("sonic_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("sonic_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("sonic_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
    ...
    from sonic_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sonic_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("sonic_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("sonic_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "sonic_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_prompt_yes_no_returns_default_when_noninteractive_env_set(monkeypatch):
    """SONIC_NONINTERACTIVE=1 (set by dashboard/desktop spawns) must make
    prompt_yes_no fall back to its default instead of reading stdin."""
    monkeypatch.setenv("SONIC_NONINTERACTIVE", "1")
...
def test_prompt_yes_no_eof_returns_default_instead_of_exiting(monkeypatch):
    ...
    monkeypatch.delenv("SONIC_NONINTERACTIVE", raising=False)
