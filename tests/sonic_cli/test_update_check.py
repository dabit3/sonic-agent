"""Tests for the update check mechanism in sonic_cli.banner."""
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    with patch("sonic_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:dabit3/sonic-agent.git\n")
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    with patch("sonic_cli.banner.subprocess.run") as mock_run:
        with patch("sonic_cli.banner.check_via_pypi", return_value=0):
            result = banner.check_for_updates()
    assert result == 0
    mock_run.assert_not_called()
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when SONIC_HOME has no git repo."""
    import sonic_cli.banner as banner
    ...
    # Point SONIC_HOME at a temp dir with no sonic-agent/.git
    monkeypatch.setenv("SONIC_HOME", str(tmp_path))
    with patch("sonic_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    SONIC_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `sonic update` correctly refusing to run there. The guard
    ...
    with patch("sonic_cli.config.detect_install_method", return_value="docker"), \
         patch("sonic_cli.banner.subprocess.run") as mock_run, \
         patch("sonic_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_check_for_updates_non_docker_still_checks(tmp_path, monkeypatch):
    ...
    with patch("sonic_cli.config.detect_install_method", return_value="pip"), \
         patch("sonic_cli.banner.subprocess.run") as mock_run, \
         patch("sonic_cli.banner.check_via_pypi", return_value=1) as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from sonic_cli.main import _invalidate_update_cache

    # Build a fake ~/.sonic with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"SONIC_HOME": str(default_home)}):
