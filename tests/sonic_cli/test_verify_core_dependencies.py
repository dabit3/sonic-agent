...
base deps) during ``sonic update``, leaving the venv in a broken state
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("sonic_cli.main._resolve_install_target_python", return_value=py), \
             ...
             patch("sonic_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("sonic_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("sonic_cli.main._run_install_with_heartbeat") as mock_install:
            from sonic_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `sonic update`.
        ...
        with patch("sonic_cli.main._is_windows", return_value=True):
            from sonic_cli.main import _resolve_install_target_python
