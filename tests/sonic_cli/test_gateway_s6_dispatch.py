"""Tests for the Phase 4 s6 dispatch helper in sonic_cli.gateway.

`_dispatch_via_service_manager_if_s6` decides whether a
`sonic gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_returns_false_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "sonic_cli.service_manager.detect_service_manager", lambda: "systemd",
    )
    # Should not even attempt to construct a manager.
    monkeypatch.setattr(
        "sonic_cli.service_manager.get_service_manager",
...
def test_dispatch_defaults_profile_to_default(
    ...
    monkeypatch.setattr(
        "sonic_cli.gateway._profile_suffix", lambda: "",
    )
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`sonic -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from sonic_cli import gateway as gw
    from sonic_cli.service_manager import GatewayNotRegisteredError
    ...
    monkeypatch.setattr(
        "sonic_cli.service_manager.get_service_manager", lambda: _RaisesMissing(),
    )
    ...
    assert "sonic profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    monkeypatch.setattr(
        "sonic_cli.service_manager.get_service_manager", lambda: _RaisesS6Error(),
    )
