"""Tests for sonic_cli/goals.py — persistent cross-turn goals."""
...
def sonic_home(tmp_path, monkeypatch):
    """Isolated SONIC_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves SONIC_HOME.
...
class TestGoalManager:
    def test_no_goal_initial(self, sonic_home):
    ...
    def test_set_then_status(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_clear(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, sonic_home):
    ...
    def test_evaluate_after_turn_done(self, sonic_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, sonic_home):
        from sonic_cli import goals
        from sonic_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, sonic_home):
    ...
    def test_evaluate_after_turn_inactive(self, sonic_home):
    ...
    def test_continuation_prompt_shape(self, sonic_home):
        ...
        mgr.set("port goal command to sonic")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to sonic" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from sonic_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, sonic_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from sonic_cli import goals
        from sonic_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, sonic_home):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, sonic_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from sonic_cli import goals
        from sonic_cli.goals import GoalManager, load_goal
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_add_subgoal_requires_active_goal(self, sonic_home):
        import pytest
        from sonic_cli.goals import GoalManager
    ...
    def test_add_empty_subgoal_rejected(self, sonic_home):
        import pytest
        from sonic_cli.goals import GoalManager
    ...
    def test_remove_subgoal(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, sonic_home):
        import pytest
        from sonic_cli.goals import GoalManager
    ...
    def test_clear_subgoals(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_subgoals_persist_across_reloads(self, sonic_home):
...
class TestContinuationPromptWithSubgoals:
    def test_empty_subgoals_uses_original_template(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_with_subgoals_includes_them(self, sonic_home):
        from sonic_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, sonic_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, sonic_home):
        from unittest.mock import patch
        from sonic_cli import goals
...
class TestStatusLineSubgoalCount:
    def test_status_line_no_subgoals(self, sonic_home):
        from sonic_cli.goals import GoalManager
    ...
    def test_status_line_with_subgoals(self, sonic_home):
        from sonic_cli.goals import GoalManager
