...
def sonic_home(tmp_path, monkeypatch):
    ...
    # /etc/sonic on the dev/CI box can't influence the test.
    monkeypatch.setenv("SONIC_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(sonic_home, monkeypatch):
    from sonic_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(sonic_home, monkeypatch):
    from sonic_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(sonic_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_sonic_dotenv(sonic_home=str(home))
