...
When ``SONIC_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.sonic/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.sonic            (has its own auth.json fixture)
    * Profile     -> tmp_path/.sonic/profiles/coder   (active, SONIC_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SONIC_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.sonic as the "real home" — which is NOT equal
    # to the SONIC_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    ...
    _write(sonic_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from sonic_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SONIC_HOME is set to a custom path that is NOT under
    # ~/.sonic/profiles/ — get_default_sonic_root() returns SONIC_HOME
