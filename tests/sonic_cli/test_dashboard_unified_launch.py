...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # SONIC_HOME.  For a standard install (SONIC_HOME unset) that root is
        # the platform-native default (~/.sonic), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from sonic_constants import get_default_sonic_root
        assert env.get("SONIC_HOME") == str(get_default_sonic_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (SONIC_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop SONIC_HOME.

        Dropping it makes the child fall back to $HOME/.sonic
        (= /opt/data/.sonic), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("SONIC_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "sonic_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
