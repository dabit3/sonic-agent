"""Surfacing tests — managed scope shown in `config show` and `sonic doctor`."""
import pytest
...
def test_config_show_flags_managed(homes, capsys):
    from sonic_cli.config import show_config
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("SONIC_MANAGED_DIR", str(tmp_path / "nope"))
    from sonic_cli import managed_scope, doctor
