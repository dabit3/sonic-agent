"""Behavior tests for sonic_cli.inventory.
...
from sonic_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    """
    rows = [{"slug": "nous", "name": "Nous", "models": ["sonic-4-405b"],
             "total_models": 1, "is_current": False, "is_user_defined": False,
             "source": "built-in"}]
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("sonic_cli.models.provider_model_ids") as mock_pm:
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # sonic model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `sonic model`")
        )
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("sonic_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from sonic_cli import model_switch

    with patch("sonic_cli.models.clear_provider_models_cache") as clear:
