"""Tests for sonic_cli.managed_uv — one path, no guessing."""
...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("sonic_cli.managed_uv.get_sonic_home", return_value=tmp_path), \
            ...
            from sonic_cli.managed_uv import managed_uv_path
...
class TestRebuildVenv:
    def test_removes_old_venv_and_creates_new(self, tmp_path):
        ...
        with patch("sonic_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch("sonic_cli.managed_uv.shutil.rmtree") as mock_rmtree:
    ...
    def test_rebuild_failure_returns_false(self, tmp_path):
        ...
        with patch("sonic_cli.managed_uv.subprocess.run") as mock_run, \
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("sonic_cli.managed_uv.get_sonic_home", return_value=tmp_path):
            from sonic_cli.managed_uv import update_managed_uv
            assert update_managed_uv() is None

    def test_self_update_success(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("sonic_cli.managed_uv.get_sonic_home", return_value=tmp_path), \
             patch("sonic_cli.managed_uv.subprocess.run") as mock_run:
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("sonic_cli.managed_uv._install_uv_posix") as mock_posix:
            from sonic_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("sonic_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("sonic_cli.managed_uv._install_uv_windows") as mock_windows:
            from sonic_cli.managed_uv import _install_uv
            _install_uv(target)
