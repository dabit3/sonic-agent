"""Tests for sonic_cli configuration management."""
...
class TestGetSonicHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_sonic_home()
            assert home == Path.home() / ".sonic"

    def test_env_override(self):
        with patch.dict(os.environ, {"SONIC_HOME": "/custom/path"}):
            home = get_sonic_home()
            assert home == Path("/custom/path")


class TestEnsureSonicHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from sonic_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from sonic_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``sonic logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="sonic_cli.config"):
                config = load_config()
            ...
            # stands out at sonic startup before logging is configured)
            captured = capsys.readouterr()
            assert "sonic config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path)}):
            ...
            assert "sonic config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SONIC_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from sonic_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """SONIC_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `sonic setup`) lets a
        ...
        via config.yaml; SONIC_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from sonic_cli.config import OPTIONAL_ENV_VARS
        assert "SONIC_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `sonic setup` surface it (previously only Honcho was listed, leaving
