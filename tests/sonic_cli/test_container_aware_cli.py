...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SONIC_HOME with .container-mode file."""
    sonic_home = tmp_path / ".sonic"
    sonic_home.mkdir()
    monkeypatch.setenv("SONIC_HOME", str(sonic_home))
    monkeypatch.delenv("SONIC_DEV", raising=False)

    container_mode = sonic_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=sonic-agent\n"
        "exec_user=sonic\n"
        "sonic_bin=/data/current-package/bin/sonic\n"
    )
    return sonic_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("sonic_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "sonic-agent"
    assert info["exec_user"] == "sonic"
    assert info["sonic_bin"] == "/data/current-package/bin/sonic"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("sonic_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_sonic_dev(container_env, monkeypatch):
    """Returns None when SONIC_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SONIC_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_sonic_dev_zero(container_env, monkeypatch):
    """SONIC_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SONIC_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        sonic_home = Path(tmpdir) / ".sonic"
        sonic_home.mkdir()
        (sonic_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("sonic_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_sonic_home": lambda: sonic_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SONIC_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=sonic-custom\n"
        "exec_user=myuser\n"
        "sonic_bin=/opt/sonic/bin/sonic\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "sonic-custom"
    assert info["exec_user"] == "myuser"
    assert info["sonic_bin"] == "/opt/sonic/bin/sonic"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from sonic_cli.main import _exec_in_container
