"""Tests that provider selection via `sonic model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated SONIC_HOME with a minimal string-format config."""
    home = tmp_path / "sonic"
    home.mkdir()
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from sonic_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from sonic_cli.auth import _update_config_for_provider
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        with patch("sonic_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from sonic_cli.main import _model_flow_copilot
        from sonic_cli.config import load_config
        ...
        with patch(
            ...
            "sonic_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from sonic_cli.main import _model_flow_copilot_acp
        from sonic_cli.config import load_config

        with patch(
            "sonic_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "sonic_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("sonic_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("sonic_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("sonic_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("sonic_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("sonic_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("sonic_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        with patch("sonic_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("sonic_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.sonic/.env"):
...
class TestZaiEndpointPicker:
    ...
    def test_select_global_endpoint(self, config_home, monkeypatch):
        ...
        with patch("sonic_cli.main._prompt_provider_choice", return_value=0), \
