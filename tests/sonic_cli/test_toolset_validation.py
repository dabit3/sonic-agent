"""Unit tests for sonic_cli.toolset_validation (see #38798).

Pure logic — the validity predicate is injected, so these tests need neither the
tool registry nor a running Sonic.
"""
...
# A representative set of real toolset names. `sonic` is deliberately absent —
# that is the corruption #38798 reported (`sonic-cli` rewritten to `sonic`).
_KNOWN = {
    "sonic-cli",
    "sonic-telegram",
    "sonic-discord",
...
def test_valid_config_produces_no_warnings():
    cfg = {"cli": ["sonic-cli"], "telegram": ["sonic-telegram"]}
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_38798_corruption_warns_and_suggests_correct_name():
    # The exact reported shape: cli holds 'sonic' instead of 'sonic-cli'.
    warnings = validate_platform_toolsets({"cli": ["sonic"]}, _is_valid)
    unknown = [w for w in warnings if "unknown toolset 'sonic'" in w]
    assert len(unknown) == 1
    # Actionable: points at the valid name the entry should have been.
    assert "did you mean 'sonic-cli'?" in unknown[0]
    # And the zero-valid-toolsets safety net fires.
    assert any("zero valid toolsets" in w for w in warnings)


def test_mixed_valid_and_invalid_flags_only_the_invalid():
    cfg = {"cli": ["sonic-cli"], "discord": ["bogus"]}
...
def test_unknown_without_valid_platform_default_omits_suggestion():
    # sonic-mystery is not a known toolset, so no "did you mean" hint.
...
@pytest.mark.parametrize("value", [None, {}, [], "sonic-cli", 42])
...
def test_scalar_toolset_value_is_accepted():
    ...
    assert validate_platform_toolsets({"cli": "sonic-cli"}, _is_valid) == []


def test_non_string_entries_are_skipped_not_counted_invalid():
    cfg = {"cli": [None, 123, "sonic-cli"]}
    # The junk entries are ignored; the valid one keeps it from being "zero".
    assert validate_platform_toolsets(cfg, _is_valid) == []


def test_all_invalid_reports_each_and_the_zero_state():
    cfg = {"cli": ["sonic"], "discord": ["sonic"]}
    warnings = validate_platform_toolsets(cfg, _is_valid)
    assert sum("unknown toolset" in w for w in warnings) == 2
    assert any("zero valid toolsets" in w for w in warnings)


def test_real_validate_toolset_treats_sonic_cli_valid_and_sonic_invalid():
    # Ties the helper to reality: the canonical registry check agrees that
    # `sonic-cli` is the real toolset and `sonic` is not (the #38798 crux).
    from toolsets import validate_toolset

    assert validate_toolset("sonic-cli") is True
    assert validate_toolset("sonic") is False
    warnings = validate_platform_toolsets({"cli": ["sonic"]}, validate_toolset)
    assert any("did you mean 'sonic-cli'?" in w for w in warnings)
