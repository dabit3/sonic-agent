"""Tests for ``sonic gui`` desktop launcher wiring."""
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Sonic.app" / "Contents" / "MacOS" / "Sonic"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Sonic.exe"
    else:
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("sonic_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sonic_cli.main._desktop_build_needed", return_value=True), \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    sonic_root = tmp_path / "custom-sonic"
    ...
    with patch("sonic_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("sonic_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("sonic_cli.main._desktop_build_needed", return_value=True), \
         patch("sonic_cli.main._write_desktop_build_stamp"), \
         patch("sonic_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("sonic_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
    ...
    assert launch_env["SONIC_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["SONIC_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["SONIC_DESKTOP_SONIC_ROOT"] == str(sonic_root)
    assert launch_env["SONIC_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("sonic_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("sonic_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sonic_cli.main._desktop_build_needed", return_value=True), \
         patch("sonic_cli.main._write_desktop_build_stamp"), \
         patch("sonic_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("sonic_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sonic_cli.main._redownload_electron_dist", return_value=True), \
         patch("sonic_cli.main.subprocess.run", side_effect=run_side_effect) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("sonic_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("sonic_cli.main._electron_dist_ok", return_value=False), \
         patch("sonic_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
