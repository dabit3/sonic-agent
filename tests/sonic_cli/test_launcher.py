"""Tests for the top-level `./sonic` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./sonic` should use `sonic_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "sonic"
    called = []

    fake_main_module = types.ModuleType("sonic_cli.main")

    def fake_main():
        called.append("sonic_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "sonic_cli.main", fake_main_module)
