"""Tests for Bug #12905 fix — stale OAuth token detection in sonic model flow.

Bug 3: `sonic model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.sonic/.env but no valid
...
from sonic_cli.config import load_env, save_env_value
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("sonic_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from sonic_cli.main import _model_flow_anthropic
        cfg = {}
