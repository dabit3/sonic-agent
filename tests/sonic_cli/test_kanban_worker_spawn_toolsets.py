...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    SONIC_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_sonic_argv", lambda: ["sonic"])
    ...
    assert pid == 4242
    assert captured["env"]["SONIC_HOME"] == str(profile)
    assert captured["env"]["SONIC_KANBAN_TASK"] == "t_spawn_tools"
