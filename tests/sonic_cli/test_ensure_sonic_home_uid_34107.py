"""Regression tests for #34107 — Docker UID/GID handling in ensure_sonic_home.

When Sonic runs in Docker with ``SONIC_UID=1000`` / ``SONIC_GID=911``,
the entrypoint chowns the top-level ``SONIC_HOME`` once at startup. But
...
The fix is a ``_chown_to_sonic_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_sonic_uid_gid
# ---------------------------------------------------------------------------


class TestResolveSonicUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SONIC_UID", "1000")
        monkeypatch.setenv("SONIC_GID", "911")
        from sonic_cli.config import _resolve_sonic_uid_gid
        uid, gid = _resolve_sonic_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("SONIC_UID", raising=False)
        monkeypatch.delenv("SONIC_GID", raising=False)
        from sonic_cli.config import _resolve_sonic_uid_gid
        uid, gid = _resolve_sonic_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("SONIC_UID", "not-a-number")
        monkeypatch.setenv("SONIC_GID", "911")
        from sonic_cli.config import _resolve_sonic_uid_gid
        uid, gid = _resolve_sonic_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("SONIC_UID", "")
        monkeypatch.setenv("SONIC_GID", "")
        from sonic_cli.config import _resolve_sonic_uid_gid
        uid, gid = _resolve_sonic_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToSonicUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_sonic_uid(d)  # must not raise
