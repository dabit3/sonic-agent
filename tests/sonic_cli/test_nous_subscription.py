...
from sonic_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from sonic_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "sonic_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `sonic tools`
# ---------------------------------------------------------------------------
