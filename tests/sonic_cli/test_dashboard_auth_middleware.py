...
from sonic_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def test_full_login_round_trip_unlocks_api_status(gated_app):
    ...
    pkce = next(
        (c for c in r1.headers.get_list("set-cookie")
         if c.startswith("sonic_session_pkce=")),
        None,
    )
    ...
    assert any(c.startswith("sonic_session_at=") for c in set_cookies)
    assert any(c.startswith("sonic_session_rt=") for c in set_cookies)
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("sonic_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("sonic_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
