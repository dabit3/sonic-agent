"""Tests for the Sonic plugin system (sonic_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<sonic_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<sonic_home>/plugins/``; we derive
    ``<sonic_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SONIC_HOME>/config.yaml.
        # Config is always read from SONIC_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        sonic_home_str = os.environ.get("SONIC_HOME")
        if sonic_home_str:
            sonic_home = Path(sonic_home_str)
        else:
            sonic_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.sonic/plugins/ are discovered."""
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SONIC_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".sonic" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under sonic_plugins.<name>."""
        plugins_dir = tmp_path / "sonic_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("SONIC_HOME", str(tmp_path / "sonic_test"))

        # Clean up any prior namespace module
        sys.modules.pop("sonic_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "sonic_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="sonic_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("sonic_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        sonic_home = tmp_path / "sonic_test"
        plugins_dir = sonic_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "sonic_test" / "plugins"
        # Set SONIC_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("sonic_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("sonic_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("sonic_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("sonic_cli.plugins.PluginContext.dispatch_tool.__module__", "sonic_cli.plugins"):
...
class TestPluginDebugLogging:
    """SONIC_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("SONIC_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With SONIC_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("SONIC_PLUGINS_DEBUG", "1")
        from sonic_cli import plugins as plugins_mod
