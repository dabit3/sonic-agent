"""Tests for the per-profile Projects store (sonic_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Sonic Agent", folders=["/tmp/sonic"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "sonic-agent"
    assert proj.name == "Sonic Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/sonic"
    assert [f.path for f in proj.folders] == ["/tmp/sonic"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "sonic-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Sonic Agent")
    pdb.create_project(conn, name="Sonic Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["sonic-agent", "sonic-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SONIC_HOME.
...
def test_db_path_under_sonic_home():
    # Resolves under SONIC_HOME (set by the autouse isolation fixture).
