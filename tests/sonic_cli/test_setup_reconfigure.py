...
- Bare `sonic setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `sonic setup --quick` runs the narrower "fill in missing items" flow.
- `sonic setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("sonic_cli.setup.ensure_sonic_home", {}),
        ("sonic_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("sonic_cli.config.is_managed", {"return_value": False}),
        ("sonic_cli.setup.load_config", {"return_value": {}}),
        ("sonic_cli.setup.save_config", {}),
        ("sonic_cli.setup.get_env_value", {"return_value": None}),
        ("sonic_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("sonic_cli.setup._print_setup_summary", {}),
        ("sonic_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("sonic_cli.setup.load_config", {"return_value": {}}),
        ("sonic_cli.setup.save_config", {}),
        ("sonic_cli.auth.get_active_provider", {"return_value": None}),
        ("sonic_cli.setup.get_env_value", {"return_value": None}),
        ("sonic_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `sonic setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="sonic_cli.setup.prompt_choice",
                quick="sonic_cli.setup._run_quick_setup",
                model="sonic_cli.setup.setup_model_provider",
                terminal="sonic_cli.setup.setup_terminal_backend",
                agent="sonic_cli.setup.setup_agent_settings",
                gateway="sonic_cli.setup.setup_gateway",
                tools="sonic_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`sonic setup --reconfigure` behaves the same as bare `sonic setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sonic_cli.setup.prompt_choice", {"return_value": 0}),
                first="sonic_cli.setup._run_first_time_quick_setup",
            )
            from sonic_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from sonic_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "sonic_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["sonic", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["sonic", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["sonic", "setup"])
        try:
            main()
        except SystemExit:
            pass
