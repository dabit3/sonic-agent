...
from sonic_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "sonic_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "sonic_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("sonic_cli.config.check_config_version", return_value=(1, 2)), patch(
            ...
            from sonic_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.sonic"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.sonic/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.sonic/profiles/work"))
        ...
        with (
            patch("sonic_cli.profiles.list_profiles", return_value=all_profiles),
            patch("sonic_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("sonic_cli.profiles.list_profiles", return_value=[default_p]),
            patch("sonic_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
