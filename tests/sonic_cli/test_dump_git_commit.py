"""Tests for sonic_cli.dump._get_git_commit — git SHA resolution for ``sonic dump``.

``sonic dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``SONIC_GIT_SHA`` build-arg.
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    with patch("sonic_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("sonic_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_falls_back_when_git_returns_empty_stdout(tmp_path):
    ...
    with patch("sonic_cli.dump.subprocess.run", return_value=empty), \
...
def test_get_git_commit_falls_back_when_git_raises(tmp_path):
    ...
    with patch("sonic_cli.dump.subprocess.run", side_effect=FileNotFoundError("git")), \
         patch("sonic_cli.build_info.get_build_sha", return_value="feedface"):
        commit = dump._get_git_commit(repo_dir)
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``sonic dump`` doesn't have to special-case
    ...
    failed = MagicMock(returncode=128, stdout="")
    with patch("sonic_cli.dump.subprocess.run", return_value=failed), \
         patch("sonic_cli.build_info.get_build_sha", return_value="b2f477a3"):
