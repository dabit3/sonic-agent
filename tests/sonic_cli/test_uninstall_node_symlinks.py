"""Tests for sonic_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SONIC_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Sonic-managed node dir.
"""
...
def _make_sonic_node(sonic_home: Path) -> Path:
    """Create a fake $SONIC_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = sonic_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_sonic_node(fake_home):
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(sonic_home) == []


def test_removes_dangling_symlink_into_sonic_node(fake_home):
    """A link into the Sonic node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Sonic links that exist; ignores the ones that don't."""
    sonic_home = fake_home / ".sonic"
    node_bin = _make_sonic_node(sonic_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Sonic-managed; node is a real user binary.
