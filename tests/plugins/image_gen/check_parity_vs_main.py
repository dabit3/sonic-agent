...
# worktree), ``MAIN_DIR`` falls back to a sibling ``lightning-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "lightning-agent-main"
...
# Isolated LIGHTNING_HOME so the config write is hermetic.
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'lightning-agent-main' checkout pinned to "
