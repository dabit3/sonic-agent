"""Tests for lightning_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Lightning entry point
(lightning, lightning-agent, lightning-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Lightning entry point imports lightning_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported lightning_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("lightning_bootstrap", None)
    import lightning_bootstrap  # noqa: WPS433
    return lightning_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Lightning CLI
...
class TestEntryPointsImportBootstrap:
    """Every Lightning entry point must import lightning_bootstrap as its
    ...
    # Entry points that invoke Lightning as a process.  Each one must
    # import lightning_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "lightning_cli/main.py",   # lightning CLI (console_script)
        "run_agent.py",          # lightning-agent (console_script)
        "acp_adapter/entry.py",  # lightning-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import lightning_bootstrap' and that
        ...
        half-finished ``lightning update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``lightning_bootstrap`` as a top-level module) leaves lightning
        ...
        """
        # Resolve relative to the lightning-agent repo root.  Tests live
        # at tests/test_lightning_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lightning start even when lightning_bootstrap hasn't been
        ...
        assert first_import_name == "lightning_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'lightning_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import lightning_bootstrap' line to be the first import."
        )
