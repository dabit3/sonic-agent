...
See: https://github.com/NousResearch/lightning-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for lightning_constants.get_subprocess_home()."""

    def test_returns_none_when_lightning_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        lightning_home = tmp_path / ".lightning"
        lightning_home.mkdir()
        profile_home = lightning_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("LIGHTNING_HOME", str(lightning_home))
        from lightning_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".lightning" / "profiles"
        ...
        monkeypatch.setenv("LIGHTNING_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("LIGHTNING_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from lightning_constants import (
            get_lightning_home,
            reset_lightning_home_override,
            set_lightning_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_lightning_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_lightning_home_override(profile)
        try:
            assert get_lightning_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_lightning_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_lightning_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(lightning_home / "home")
    ...
    def test_no_injection_when_lightning_home_unset(self, monkeypatch):
        monkeypatch.delenv("LIGHTNING_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["LIGHTNING_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from lightning_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from lightning_cli.profiles import create_profile
