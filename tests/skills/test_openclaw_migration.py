...
"""
    ...
    assert "Tyler Williams > Active Projects: Lightning Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `lightning cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # lightning_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.lightning/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "lightning_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Lightning``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Lightning prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Lightning to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Lightning config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Lightning uses tools well"
    # All-lowercase matches → lowercase ``lightning``; this preserves the
    # real filesystem path ``~/.lightning`` (Lightning home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "lightning should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Lightning remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "lightning prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Lightning was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "lightning uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Lightning said to ask Lightning about Lightning settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.lightning`` (the real Lightning home), not the broken
    ``.Lightning``.
    ...
    rewritten to ``~/.Lightning/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.lightning/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .lightning directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.lightning'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "lightning config path: ~/.lightning/"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    target_root = tmp_path / "lightning"
    target_root.mkdir()
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Lightning" in result
