"""Tests for sonic_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Sonic entry point
(sonic, sonic-agent, sonic-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Sonic entry point imports sonic_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported sonic_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("sonic_bootstrap", None)
    import sonic_bootstrap  # noqa: WPS433
    return sonic_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Sonic CLI
...
class TestEntryPointsImportBootstrap:
    """Every Sonic entry point must import sonic_bootstrap as its
    ...
    # Entry points that invoke Sonic as a process.  Each one must
    # import sonic_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "sonic_cli/main.py",   # sonic CLI (console_script)
        "run_agent.py",          # sonic-agent (console_script)
        "acp_adapter/entry.py",  # sonic-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sonic_bootstrap' and that
        ...
        half-finished ``sonic update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``sonic_bootstrap`` as a top-level module) leaves sonic
        ...
        """
        # Resolve relative to the sonic-agent repo root.  Tests live
        # at tests/test_sonic_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # sonic start even when sonic_bootstrap hasn't been
        ...
        assert first_import_name == "sonic_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sonic_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sonic_bootstrap' line to be the first import."
        )
