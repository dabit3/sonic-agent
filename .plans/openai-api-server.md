# OpenAI-Compatible API Server for Sonic Agent
...
SSE streaming. By exposing this endpoint, sonic-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  sonic-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.sonic/.env`
2. Run `sonic gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with sonic-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns sonic-agent as a model) |
...
Authorization: Bearer sonic-api-key-here
...
But sonic-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "sonic-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured sonic-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of sonic-agent's core prompt
...
Once implemented, sonic-agent works as a drop-in backend for:
