# OpenAI-Compatible API Server for Lightning Agent
...
SSE streaming. By exposing this endpoint, lightning-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  lightning-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.lightning/.env`
2. Run `lightning gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with lightning-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns lightning-agent as a model) |
...
Authorization: Bearer lightning-api-key-here
...
But lightning-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "lightning-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured lightning-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of lightning-agent's core prompt
...
Once implemented, lightning-agent works as a drop-in backend for:
