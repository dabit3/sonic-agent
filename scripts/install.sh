...
check_node() {
    ...
    if [ -x "$SONIC_HOME/node/bin/node" ]; then
        ...
        local found_ver=$("$SONIC_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Sonic-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_desktop() {
    ...
    # Try the Sonic-managed Node first (check_node adds $SONIC_HOME/node/bin
