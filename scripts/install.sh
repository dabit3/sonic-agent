...
# Python-driven tool session (e.g. Sonic terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:dabit3/sonic-agent.git"
REPO_URL_HTTPS="https://github.com/dabit3/sonic-agent.git"
SONIC_HOME="${SONIC_HOME:-$HOME/.sonic}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SONIC_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SONIC_INSTALL_DIR"
...
#   code at /usr/local/lib/sonic-agent, command at /usr/local/bin/sonic,
#   data still at /root/.sonic (SONIC_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SONIC_HOME/.no-bundled-skills so future"
            echo "                   'sonic update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Sonic.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.sonic/sonic-agent"
            echo "                   default (root, Linux): /usr/local/lib/sonic-agent"
            echo "  --sonic-home PATH  Data directory (default: ~/.sonic, or \$SONIC_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Sonic installs the code under"
            echo "  /usr/local/lib/sonic-agent and links the command into"
            echo "  /usr/local/bin/sonic (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SONIC_HOME"
            echo "  (default /root/.sonic).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SONIC_HOME/sonic-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs sonic setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Sonic Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent maintained by Nader Dabit.     │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `sonic update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Sonic-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Sonic Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install sonic command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `sonic` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SONIC_HOME/sonic-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SONIC_HOME/sonic-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/sonic-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SONIC_HOME/sonic-agent — then preserve it)
#
# Always no-op when the user set --dir or $SONIC_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SONIC_HOME.
    if is_termux; then
        INSTALL_DIR="$SONIC_HOME/sonic-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SONIC_HOME/sonic-agent/.git" ]; then
            INSTALL_DIR="$SONIC_HOME/sonic-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/sonic-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/sonic-agent"
        ...
        # /usr/local/bin/sonic wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/sonic"
        log_info "  Data:    $SONIC_HOME (unchanged)"
...
# Point a Sonic-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SONIC_HOME/node/bin — which is NOT on
...
# Sonic's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Sonic-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SONIC_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SONIC_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SONIC_HOME/node/etc/npmrc"
}

get_sonic_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/sonic" ]; then
        echo "$link_dir/sonic"
    else
        echo "sonic"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/dabit3/sonic-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Sonic owns its own uv at $SONIC_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sonic_cli/managed_uv.py) looks in the same
    # place, so install.sh and `sonic update` stay in sync.
    local _managed_uv="$SONIC_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SONIC_HOME/bin ..."
    mkdir -p "$SONIC_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/sonic-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sonic-uv-installer.$$.sh")"
    ...
    # directly into $SONIC_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SONIC_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Sonic-
...
check_node() {
    ...
    # Repair pre-existing Sonic-managed installs where `npm install -g` lands
    ...
    # Prefer a Sonic-managed Node from a previous run over a too-old system one.
    if [ -x "$SONIC_HOME/node/bin/node" ] && node_satisfies_build "$("$SONIC_HOME/node/bin/node" --version)"; then
        export PATH="$SONIC_HOME/node/bin:$PATH"
        log_success "Node.js $("$SONIC_HOME/node/bin/node" --version) found (Sonic-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Sonic-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.sonic/node/..."
    ...
    local installed_ver
    installed_ver=$("$SONIC_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.sonic/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Sonic install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Sonic Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `sonic update` path
                # (#4735).
                ...
                local stash_name
                stash_name="sonic-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Sonic behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up sonic command..."

    if [ "$USE_VENV" = true ]; then
        SONIC_BIN="$INSTALL_DIR/venv/bin/sonic"
    else
        SONIC_BIN="$(which sonic 2>/dev/null || echo "")"
        if [ -z "$SONIC_BIN" ]; then
            log_warn "sonic not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$SONIC_BIN" ]; then
        log_warn "sonic entry point not found at $SONIC_BIN"
    ...
    # Create a user-facing shim for the sonic command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SONIC_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SONIC_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/sonic"
    cat > "$command_link_dir/sonic" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SONIC_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/sonic"
    log_success "Installed sonic launcher → $command_link_display_dir/sonic"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "sonic command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v sonic' \
                >/dev/null 2>&1; then
        ...
        log_info "sonic not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Sonic Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Sonic Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Sonic Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so sonic works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.sonic directory structure (config at top level, code in subdir)
    mkdir -p "$SONIC_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.sonic/.env (top level, easy to find)
    if [ ! -f "$SONIC_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SONIC_HOME/.env"
            log_success "Created ~/.sonic/.env from template"
        else
            touch "$SONIC_HOME/.env"
            log_success "Created ~/.sonic/.env"
        fi
    else
        log_info "~/.sonic/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SONIC_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.sonic/config.yaml (top level, easy to find)
    if [ ! -f "$SONIC_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SONIC_HOME/config.yaml"
            log_success "Created ~/.sonic/config.yaml from template"
        fi
    else
        log_info "~/.sonic/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in sonic_cli/default_soul.py — the
    ...
    if [ ! -f "$SONIC_HOME/SOUL.md" ]; then
        cat > "$SONIC_HOME/SOUL.md" << 'SOUL_EOF'
You are Sonic Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.sonic/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.sonic/"
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `sonic update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'sonic update'." \
            > "$SONIC_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SONIC_HOME/.no-bundled-skills"
        log_info "  Future 'sonic update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.sonic/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.sonic/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SONIC_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SONIC_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.sonic/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Sonic Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Sonic will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Sonic Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (sonic --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `sonic update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'sonic setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m sonic_cli.main setup < /dev/tty
    else
        python -m sonic_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Sonic to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SONIC_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'sonic whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SONIC_CMD="$(get_sonic_command_path)"
                $SONIC_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'sonic whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'sonic gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SONIC_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SONIC_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: sonic gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: sonic gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SONIC_CMD gateway > "$SONIC_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.sonic/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: sonic gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: sonic gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SONIC_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SONIC_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SONIC_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}sonic${NC}              Start chatting"
    echo -e "   ${GREEN}sonic setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}sonic config${NC}       View/edit configuration"
    echo -e "   ${GREEN}sonic config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}sonic gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}sonic update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'sonic' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'sonic' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'sonic' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SONIC_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SONIC_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SONIC_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SONIC_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$SONIC_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Sonic for pip install"
    ...
    SONIC_CMD="$(command -v sonic 2>/dev/null || echo "")"
    if [ -n "$SONIC_CMD" ]; then
        log_info "Running sonic setup..."
        "$SONIC_CMD" setup
    else
        log_warn "sonic command not found on PATH"
        log_info "Try: python -m sonic_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Sonic-managed Node to PATH, so
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Sonic" ]; then
            app="$desktop_dir/release/linux-unpacked/Sonic"
        elif [ -x "$desktop_dir/release/linux-unpacked/sonic" ]; then
            app="$desktop_dir/release/linux-unpacked/sonic"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Sonic.app" \
            "$desktop_dir/release/mac/Sonic.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Sonic is damaged and can't be opened").
    ...
    # checkout stays clean for the next `sonic update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Sonic-managed Node
            # provisioned during prerequisites/node-deps (at $SONIC_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SONIC_HOME.
    # $SONIC_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'sonic update' on this host install.
