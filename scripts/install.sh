...
# Python-driven tool session (e.g. Lightning terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/lightning-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/lightning-agent.git"
LIGHTNING_HOME="${LIGHTNING_HOME:-$HOME/.lightning}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${LIGHTNING_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$LIGHTNING_INSTALL_DIR"
...
#   code at /usr/local/lib/lightning-agent, command at /usr/local/bin/lightning,
#   data still at /root/.lightning (LIGHTNING_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --postinstall)
            ...
            ;;
        -h|--help)
            echo "Lightning Agent Installer"
            echo ""
            ...
            echo "                   default (non-root):  ~/.lightning/lightning-agent"
            echo "                   default (root, Linux): /usr/local/lib/lightning-agent"
            echo "  --lightning-home PATH  Data directory (default: ~/.lightning, or \$LIGHTNING_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Lightning installs the code under"
            echo "  /usr/local/lib/lightning-agent and links the command into"
            echo "  /usr/local/bin/lightning (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$LIGHTNING_HOME"
            echo "  (default /root/.lightning).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$LIGHTNING_HOME/lightning-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs lightning setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `lightning` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $LIGHTNING_HOME/lightning-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $LIGHTNING_HOME/lightning-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/lightning-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $LIGHTNING_HOME/lightning-agent — then preserve it)
#
# Always no-op when the user set --dir or $LIGHTNING_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in LIGHTNING_HOME.
    if is_termux; then
        INSTALL_DIR="$LIGHTNING_HOME/lightning-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$LIGHTNING_HOME/lightning-agent/.git" ]; then
            INSTALL_DIR="$LIGHTNING_HOME/lightning-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/lightning-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/lightning-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/lightning"
        log_info "  Data:    $LIGHTNING_HOME (unchanged)"
        return 0
    fi
...
get_lightning_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/lightning" ]; then
        echo "$link_dir/lightning"
    else
        echo "lightning"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/lightning-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/lightning-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/lightning-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$LIGHTNING_HOME/node/bin/node" ]; then
        ...
        local found_ver=$("$LIGHTNING_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Lightning-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.lightning/node/..."
    ...
    # Place into ~/.lightning/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$LIGHTNING_HOME/node"
    mkdir -p "$LIGHTNING_HOME"
    mv "$extracted_dir" "$LIGHTNING_HOME/node"
    rm -rf "$tmp_dir"
    ...
    local installed_ver
    installed_ver=$("$LIGHTNING_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.lightning/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Lightning install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Lightning Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="lightning-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Lightning behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up lightning command..."

    if [ "$USE_VENV" = true ]; then
        LIGHTNING_BIN="$INSTALL_DIR/venv/bin/lightning"
    else
        LIGHTNING_BIN="$(which lightning 2>/dev/null || echo "")"
        if [ -z "$LIGHTNING_BIN" ]; then
            log_warn "lightning not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$LIGHTNING_BIN" ]; then
        log_warn "lightning entry point not found at $LIGHTNING_BIN"
    ...
    # Create a user-facing shim for the lightning command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $LIGHTNING_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$LIGHTNING_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/lightning"
    cat > "$command_link_dir/lightning" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$LIGHTNING_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/lightning"
    log_success "Installed lightning launcher → $command_link_display_dir/lightning"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "lightning command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v lightning' \
                >/dev/null 2>&1; then
        ...
        log_info "lightning not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Lightning Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Lightning Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Lightning Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so lightning works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.lightning directory structure (config at top level, code in subdir)
    mkdir -p "$LIGHTNING_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.lightning/.env (top level, easy to find)
    if [ ! -f "$LIGHTNING_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$LIGHTNING_HOME/.env"
            log_success "Created ~/.lightning/.env from template"
        else
            touch "$LIGHTNING_HOME/.env"
            log_success "Created ~/.lightning/.env"
        fi
    else
        log_info "~/.lightning/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$LIGHTNING_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.lightning/config.yaml (top level, easy to find)
    if [ ! -f "$LIGHTNING_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$LIGHTNING_HOME/config.yaml"
            log_success "Created ~/.lightning/config.yaml from template"
        fi
    else
        log_info "~/.lightning/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$LIGHTNING_HOME/SOUL.md" ]; then
        cat > "$LIGHTNING_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.lightning/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.lightning/"

    # Seed bundled skills into ~/.lightning/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.lightning/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.lightning/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$LIGHTNING_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$LIGHTNING_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.lightning/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$LIGHTNING_HOME/.env"
    ...
    {
        echo ""
        echo "# Lightning Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Lightning will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (lightning --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'lightning setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m lightning_cli.main setup < /dev/tty
    else
        python -m lightning_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Lightning to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$LIGHTNING_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'lightning whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                LIGHTNING_CMD="$(get_lightning_command_path)"
                $LIGHTNING_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'lightning whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'lightning gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $LIGHTNING_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $LIGHTNING_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: lightning gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: lightning gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $LIGHTNING_CMD gateway > "$LIGHTNING_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.lightning/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: lightning gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: lightning gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $LIGHTNING_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $LIGHTNING_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $LIGHTNING_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}lightning${NC}              Start chatting"
    echo -e "   ${GREEN}lightning setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}lightning config${NC}       View/edit configuration"
    echo -e "   ${GREEN}lightning config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}lightning gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}lightning update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'lightning' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'lightning' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'lightning' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$LIGHTNING_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$LIGHTNING_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$LIGHTNING_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$LIGHTNING_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$LIGHTNING_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Lightning for pip install"
    ...
    LIGHTNING_CMD="$(command -v lightning 2>/dev/null || echo "")"
    if [ -n "$LIGHTNING_CMD" ]; then
        log_info "Running lightning setup..."
        "$LIGHTNING_CMD" setup
    else
        log_warn "lightning command not found on PATH"
        log_info "Try: python -m lightning_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$LIGHTNING_HOME/.install_method"
}
