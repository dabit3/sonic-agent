...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$SonicHome = $(if ($env:SONIC_HOME) { $env:SONIC_HOME } else { "$env:LOCALAPPDATA\sonic" }),
    [string]$InstallDir = $(if ($env:SONIC_HOME) { "$env:SONIC_HOME\sonic-agent" } else { "$env:LOCALAPPDATA\sonic\sonic-agent" }),
    ...
    # builds apps/desktop into a launchable Sonic.exe.
    #
    # Why opt-in:
    #   * Sonic-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Sonic.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Sonic.exe
    ...
    #     `sonic desktop` already builds on demand.
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent maintained by Nader Dabit.     |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Sonic-Setup.exe
...
function Test-Node {
    ...
    Write-Info "Installing Sonic-managed Node.js $NodeVersion LTS..."
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\sonic-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `sonic
                    # update` path (#4735).
                    ...
                    $stashName = "sonic-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Sonic behaves unexpectedly."
                        } else {
    ...
    # next `sonic update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Sonic-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running sonic process.
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            # The launcher CLI (sonic.exe) plus its child tree.
            & taskkill /F /T /IM sonic.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM sonic.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m sonic_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not sonic.exe.
            ...
            # user (see sonic_cli/gateway_windows.py), so the installer always
...
"@ 2>$null
    ...
    if (-not $NoVenv) {
        # uv on Windows can register sonic.exe in dist-info/RECORD but fail to
        ...
        # `sonic` command while sonic-agent.exe / sonic-acp.exe exist
            ...
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m sonic_cli.main <command>"
                    } else {
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.sonic-bootstrap-complete which tells the Sonic
    ...
    $markerPath = Join-Path $InstallDir ".sonic-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in sonic_cli/default_soul.py. The runtime
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Sonic-Setup.exe runs each -Stage NAME
...
function Install-Desktop {
    # Build apps/desktop into a launchable Sonic.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_sonic_desktop command
    # resolves apps/desktop/release/win-unpacked/Sonic.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Sonic-managed
    ...
    # The --dir mode produces an unpacked Sonic.exe in
    ...
    # are NOT trying to work around). The Sonic icon + product name are
    # stamped onto Sonic.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\sonic-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Sonic.exe",
        "$desktopDir\release\win-arm64-unpacked\Sonic.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Sonic.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Sonic icon + identity are stamped onto Sonic.exe by the
    ...
    #    Sonic.exe. We deliberately do NOT point them at `sonic desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Sonic.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Sonic.lnk')
        )
        ...
        # with the Sonic icon, but without this the shortcut can keep drawing
...
if ($IncludeDesktop) {
    ...
    # (Sonic-Setup.exe), never via the irm|iex CLI one-liner.
