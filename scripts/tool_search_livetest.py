#!/usr/bin/env python3
"""Live test harness for Sonic Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any sonic imports.
ORIGINAL_HOME = os.environ.get("SONIC_HOME")
ORIGINAL_AUTH = Path.home() / ".sonic" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.sonic/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.sonic/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="sonic_ts_live_"))
    sonic_home = home_dir / ".sonic"
    sonic_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, sonic_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".sonic" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, sonic_home / ".env")
        ...
        load_sonic_dotenv(sonic_home=str(Path.home() / ".sonic"))
    ...
    (sonic_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return sonic_home
...
def reset_module_state():
    """Drop cached modules so the new SONIC_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "sonic_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SONIC_HOME"] = str(home)
...
def main():
    ...
    # Restore original SONIC_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SONIC_HOME"] = ORIGINAL_HOME
    else:
