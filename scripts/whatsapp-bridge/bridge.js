 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.sonic/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.sonic', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (SONIC_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.SONIC_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sonic', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.SONIC_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sonic', 'document_cache');
const AUDIO_CACHE_DIR = process.env.SONIC_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sonic', 'audio_cache');
...
// `sonic update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Sonic Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Sonic Agent', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Sonic and trigger
      ...
      // Ignore Sonic' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `sonic_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
