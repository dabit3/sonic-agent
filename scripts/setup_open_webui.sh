...
# Bootstrap Open WebUI against Sonic Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.sonic/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-sonic.sh
...
#   SONIC_API_PORT=8642
#   SONIC_API_HOST=127.0.0.1
#   SONIC_API_MODEL_NAME='Sonic Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Sonic Agent WebUI}"
...
SONIC_ENV_FILE="${SONIC_ENV_FILE:-$HOME/.sonic/.env}"
SONIC_API_PORT="${SONIC_API_PORT:-8642}"
SONIC_API_HOST="${SONIC_API_HOST:-127.0.0.1}"
SONIC_API_CONNECT_HOST="${SONIC_API_CONNECT_HOST:-127.0.0.1}"
SONIC_API_MODEL_NAME="${SONIC_API_MODEL_NAME:-Sonic Agent}"
SONIC_API_BASE_URL="http://${SONIC_API_CONNECT_HOST}:${SONIC_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-sonic.sh"
LOG_DIR="$HOME/.sonic/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$SONIC_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$SONIC_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.sonic.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.sonic</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.sonic"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.sonic"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-sonic.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Sonic Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-sonic.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.sonic/logs/openwebui.log
StandardError=append:%h/.sonic/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-sonic.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$SONIC_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Sonic API server is configured...'
  upsert_env API_SERVER_ENABLED true "$SONIC_ENV_FILE"
  upsert_env API_SERVER_HOST "$SONIC_API_HOST" "$SONIC_ENV_FILE"
  upsert_env API_SERVER_PORT "$SONIC_API_PORT" "$SONIC_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$SONIC_API_MODEL_NAME" "$SONIC_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$SONIC_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Sonic gateway so API server settings take effect...'
  sonic gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${SONIC_API_CONNECT_HOST}:${SONIC_API_PORT}/health" >/dev/null; then
    log 'Sonic API server did not answer on the first check. Trying to start gateway in the background...'
    nohup sonic gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${SONIC_API_CONNECT_HOST}:${SONIC_API_PORT}/health" >/dev/null
  ...
  log "Sonic API endpoint: ${SONIC_API_BASE_URL}"
