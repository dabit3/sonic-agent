...
# Bootstrap Open WebUI against Lightning Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.lightning/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-lightning.sh
...
#   OPEN_WEBUI_NAME='Johnny Lightning'
...
#   LIGHTNING_API_PORT=8642
#   LIGHTNING_API_HOST=127.0.0.1
#   LIGHTNING_API_MODEL_NAME='Lightning Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Lightning Agent WebUI}"
...
LIGHTNING_ENV_FILE="${LIGHTNING_ENV_FILE:-$HOME/.lightning/.env}"
LIGHTNING_API_PORT="${LIGHTNING_API_PORT:-8642}"
LIGHTNING_API_HOST="${LIGHTNING_API_HOST:-127.0.0.1}"
LIGHTNING_API_CONNECT_HOST="${LIGHTNING_API_CONNECT_HOST:-127.0.0.1}"
LIGHTNING_API_MODEL_NAME="${LIGHTNING_API_MODEL_NAME:-Lightning Agent}"
LIGHTNING_API_BASE_URL="http://${LIGHTNING_API_CONNECT_HOST}:${LIGHTNING_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-lightning.sh"
LOG_DIR="$HOME/.lightning/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$LIGHTNING_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$LIGHTNING_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.lightning.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.lightning</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.lightning"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.lightning"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-lightning.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Lightning Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-lightning.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.lightning/logs/openwebui.log
StandardError=append:%h/.lightning/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-lightning.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$LIGHTNING_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Lightning API server is configured...'
  upsert_env API_SERVER_ENABLED true "$LIGHTNING_ENV_FILE"
  upsert_env API_SERVER_HOST "$LIGHTNING_API_HOST" "$LIGHTNING_ENV_FILE"
  upsert_env API_SERVER_PORT "$LIGHTNING_API_PORT" "$LIGHTNING_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$LIGHTNING_API_MODEL_NAME" "$LIGHTNING_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$LIGHTNING_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Lightning gateway so API server settings take effect...'
  lightning gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${LIGHTNING_API_CONNECT_HOST}:${LIGHTNING_API_PORT}/health" >/dev/null; then
    log 'Lightning API server did not answer on the first check. Trying to start gateway in the background...'
    nohup lightning gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${LIGHTNING_API_CONNECT_HOST}:${LIGHTNING_API_PORT}/health" >/dev/null
  ...
  log "Lightning API endpoint: ${LIGHTNING_API_BASE_URL}"
