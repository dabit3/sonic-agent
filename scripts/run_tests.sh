#!/usr/bin/env bash
# Canonical test runner for sonic-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.sonic/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.sonic/sonic-agent/venv"; do
...
if [ -f "$HOME/.sonic/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.sonic"
...
exec env -i \
  ...
  ${SONIC_RUN_SLOW_PET_TESTS:+SONIC_RUN_SLOW_PET_TESTS="$SONIC_RUN_SLOW_PET_TESTS"} \
