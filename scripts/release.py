...
VERSION_FILE = REPO_ROOT / "sonic_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows sonic-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+sonic-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "sonic@nousresearch.com": "NousResearch",
    ...
    "sonic@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (SONIC_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SONIC_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: sonic-eval community link)
    ...
    "sonicagent26@gmail.com": "sonicagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+sonicagent26@users.noreply.github.com": "sonicagent26",
    ...
    "vigo@sonic": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level SONIC_HOME state files)
    ...
    "258443621+clovericbot@users.noreply.github.com": "clovericbot",  # upstream contributor
    "6590895+kaishi00@users.noreply.github.com": "kaishi00",  # PR #55203 salvage (skip_preflight opt-out)
    "loongfay@foxmail.com": "loongfay",  # PR #43508 (Yuanbao wechat forward msg)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Sonic
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"sonic-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "sonic@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/dabit3/sonic-agent",
    ...
    lines.append(f"# Sonic Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Sonic Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Sonic Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Sonic Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Sonic Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Sonic Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Sonic Agent v{new_version} ({calver_date})' "
