#!/usr/bin/env python3
"""Lightning Agent Release Script
...
VERSION_FILE = REPO_ROOT / "lightning_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+lightning-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "lightning@nousresearch.com": "NousResearch",
    ...
    "lightning@noushq.ai": "benbarclay",
    ...
    "lightning@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@lightning.local": "jacdevos",
    ...
    "teknium@lightning-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (LIGHTNING_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (LIGHTNING_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: lightning-eval community link)
    ...
    "lightningagent26@gmail.com": "lightningagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+lightningagent26@users.noreply.github.com": "lightningagent26",
    ...
    "vigo@lightning": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.lightning@agentmail.to": "oseftg",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"lightning-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "lightning@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/lightning-agent",
    ...
    lines.append(f"# Lightning Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Lightning Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Lightning Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Lightning Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Lightning Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Lightning Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Lightning Agent v{new_version} ({calver_date})' "
